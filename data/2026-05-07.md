...
TL;DR: 该研究评估了五种开源和商业多模态大语言模型（MLLMs）在公开皮肤病学数据集和一个包含5,811个病例及46,405张临床图像的真实世界多中心医院皮肤科会诊队列中的表现，发现模型在真实世界场景下的诊断性能显著低于公开基准测试结果。尽管加入临床背景信息可提升性能，但模型对不完整或错误的上下文高度敏感。对于严重程度分诊任务，模型表现出中等敏感性，具备一定筛查潜力，但尚不足以用于临床部署。


<details>
  <summary>Details</summary>
Motivation: 现有皮肤病学多模态大语言模型在公开基准测试中表现良好，但其真实世界临床应用能力未知，亟需评估其从基准到临床实践的转化差距。

Method: 在三个公开皮肤病学数据集和一个大规模真实世界多中心医院皮肤科会诊队列上，对五种MLLMs进行评估，任务包括鉴别诊断生成和基于严重程度的分诊。分别使用图像单独和结合临床上下文进行测试，并分析模型对不完整或错误信息的敏感性。

Result: 在公开数据集上，最佳开源模型的前3位诊断准确率为26.55%，GPT-4.1为42.25%；在真实世界队列中，仅使用图像时，开源模型准确率降至1.50%-13.35%，GPT-4.1为24.65%；加入临床上下文后，准确率提升至最高28.75%（开源）和38.93%（商业），但模型对上下文错误仍高度敏感；严重程度分诊任务的敏感性超过60%，显示一定筛查潜力但可靠性不足。

Conclusion: 当前皮肤病学多模态大语言模型的基准性能显著高估了其真实世界的临床能力，其在实际医疗环境中的应用仍面临重大挑战，需进一步改进鲁棒性和上下文理解能力。
...
TL;DR: MuCALD-SplitFed 是一种多任务分割联邦学习框架，结合因果表示学习和潜在扩散模型，有效提升医学图像分割性能，解决传统方法在收敛性、隐私泄露及跨任务协作方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习框架在医疗领域面临任务多样性、计算资源受限及隐私安全挑战，尤其在多任务场景下易出现收敛困难与信息泄露问题，亟需更适应临床实际的高效、安全协同学习机制。

Method: 提出 MuCALD-SplitFed 框架，通过集成因果表示学习以捕捉任务间潜在依赖关系，并利用潜在扩散模型增强特征表达能力，同时在客户端与服务器间合理划分模型结构，降低通信与计算开销。

Result: 实验表明，该方法在医学图像分割任务中显著优于基线 SplitFed 及其他先进个性化与多任务联邦学习方法；具备更强的收敛稳定性，且有效减少分裂点处的信息泄露，抵御重建攻击与成员推断攻击。

Conclusion: MuCALD-SplitFed 为多任务医疗联邦学习提供了高效、安全的新范式，具有良好的临床应用前景，为未来分布式医疗AI系统设计提供了重要参考。
...
TL;DR: 该研究揭示了深度学习（DL）在科学成像中应用时的潜在失败，特别是红外（IR）成像与传统RGB图像对比时，尽管IR数据信息量更大，但DL模型表现反而更差。原因在于IR数据的先验特性与深度学习的简单性偏差不匹配，导致模型退化为一维预测，严重浪费其表示能力，引发AI安全问题。即使采用最先进的鲁棒化策略也无效，因为这些策略主要针对RGB图像设计。研究提出需发展模态特定的故障模式分析框架，以推动安全、专用的科学AI算法发展。


<details>
  <summary>Details</summary>
Motivation: 探索深度学习在科学成像中的有效性，尤其是面对高信息密度的红外成像数据时，为何现有深度学习方法可能失效，并识别其根本原因以提升AI安全性与可靠性。

Method: 通过对比病理学中染色组织的RGB图像与红外光谱成像的定量生化特征，评估深度学习模型在两类数据上的表现；系统分析数据先验与模型偏置之间的不匹配机制；检验主流鲁棒化策略在该场景下的适用性。

Result: 深度学习在红外成像上表现劣于RGB图像，因数据先验与模型简单性偏差冲突，导致模型退化为一维输出；现有鲁棒化技术无法缓解此问题；表明通用深度学习框架在科学成像中存在根本局限性。

Conclusion: 通用深度学习在科学成像中可能面临灾难性失败，必须建立模态特定的故障分析框架，发展适应科学数据特性的专用安全算法，以确保人工智能在科学领域的可信应用。
...
TL;DR: 该研究提出了一种概率深度学习框架，用于Sentinel-2时间序列的密集化和预测，能够生成任意过去或未来日期的光学图像，并联合利用Sentinel-2光学数据与Sentinel-1 SAR数据，同时关注生成图像的不确定性。实验表明该方法在稀疏且时间错位的时间序列上具有良好的密集化与预测效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽能填补观测时间段内的缺失或云污染数据，但无法预测未来观测值，且缺乏对生成结果不确定性的建模。为实现更全面的时空连续性分析，需发展兼具密度化与预测能力并能量化不确定性的新方法。

Method: 提出一种基于多模态数据（Sentinel-2光学与Sentinel-1 SAR）的的概率深度学习框架，通过联合建模实现对任意时间点（过去或未来）光学图像的生成，并引入不确定性估计机制。

Result: 在稀疏且时间不一致的Sentinel-2时间序列上，该方法实现了有效的密集化与未来观测预测，生成图像质量高，且不确定性估计合理可信。

Conclusion: 所提出的概率深度学习框架有效解决了光学卫星图像时间序列中因云层和条带边缘导致的采样不规则问题，不仅实现时间维度上的密集化，还具备对未来观测的预测能力，同时提供可靠的不确定性评估，适用于农业、气候监测和地表分析等应用。
...
TL;DR: 提出物理引导的分段解混方法PGRU，通过像素级标量ξ_i动态激活非线性混合，结合GBM、PPNM和Hapke模型残差，利用学习注意力机制生成可解释的混合模式图，在Samson、Jasper Ridge和Urban数据集上均优于基线，物理一致性ρ>0.90。


<details>
  <summary>Details</summary>
Motivation: 现有线性混合模型（LMM）在多重散射下表现不佳，而现有非线性模型采用全局固定策略，缺乏空间自适应性；亟需一种能根据物理特征自适应启用非线性混合的方法。

Method: 提出PGRU方法，基于可观测物理特征估计像素级标量ξ_i∈[0,1]，仅在必要区域激活非线性混合；融合GBM、PPNM和Hapke模型的残差，通过学习注意力机制实现自适应加权，生成可解释的混合模式图。

Result: 在Samson、Jasper Ridge和Urban三个数据集上均取得优于基线的性能，物理一致性相关系数ρ>0.90，表明所提方法具有良好的物理可解释性和有效性。

Conclusion: PGRU实现了对非线性混合的物理引导与空间自适应建模，显著提升解混精度并增强结果可解释性，为复杂场景下的光谱解混提供了新范式。
...
TL;DR: 本文提出一个包含约34,700张高分辨率图像的新型图像数据集，涵盖11根Dyneema SK75/78高模量聚乙烯绳索在七种不同轴向载荷（60–280 kN）下进行循环疲劳测试的完整退化生命周期。每根绳索在固定循环数后采集10张横截面图像，覆盖绳索全长，用于表征表面退化。图像标注了对应循环次数，支持剩余使用寿命（RUL）计算。该数据集旨在推动基于视觉的故障监测与寿命预测算法研究，适用于RUL回归、损伤演化建模、异常检测等机器学习任务。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏公开的、涵盖合成纤维绳索完整退化生命周期的图像数据集，尤其在受控循环疲劳加载条件下。这限制了数据驱动的视觉状态监测与寿命预测算法的发展。

Method: 在钢丝滑轮弯曲试验台上对11根Dyneema SK75/78绳索施加7种不同轴向载荷（60–280 kN），直至机械失效。每完成固定循环数后，从绳索不同横截面位置采集10张图像，形成空间代表性采样。所有图像均标注对应循环次数，以支持RUL计算。

Result: 构建了一个包含34,700张高分辨率图像的数据集，覆盖从695到8,340次循环的疲劳寿命，完整记录了绳索表面退化的动态过程，可用于多种机器学习任务。

Conclusion: 该数据集填补了合成纤维绳索在受控疲劳条件下的图像数据空白，可作为视觉基状态监测与寿命预测算法开发与评估的基准资源。
...
### [9] [InterFuserDVS: Event-Enhanced Sensor Fusion for Safe RL-Based Decision Making](https://arxiv.org/abs/2605.04355)
...
### [18] [A cross-modal network for facial expression recognition](https://arxiv.org/abs/2605.04439)
...
### [27] [SpecPL: Disentangling Spectral Granularity for Prompt Learning](https://arxiv.org/abs/2605.04504)
...
### [34] [Angle-I2P: Angle-Consistent-Aware Hierarchical Attention for Cross-Modality Outlier Rejection](https://arxiv.org/abs/2605.04541)
...
### [35] [InterMesh: Explicit Interaction-Aware End-to-End Multi-Person Human Mesh Recovery](https://arxiv.org/abs/2605.04554)
...
### [36] [Efficient Geometry-Controlled High-Resolution Satellite Image Synthesis](https://arxiv.org/abs/2605.04557)
...
### [37] [SAMIC: A Lightweight Semantic-Aware Mamba for Efficient Perceptual Image Compression](https://arxiv.org/abs/2605.04560)
...
### [38] [Open-Source Image Editing Models Are Zero-Shot Vision Learners](https://arxiv.org/abs/2605.04566)
...
### [39] [Lightning Unified Video Editing via In-Context Sparse Attention](https://arxiv.org/abs/2605.04569)
...
### [40] [VL-UniTrack: A Unified Framework with Visual-Language Prompts for UAV-Ground Visual Tracking](https://arxiv.org/abs/2605.04574)
...
### [41] [GTF: Omnidirectional EPI Transformer for Light Field Super-Resolution](https://arxiv.org/abs/2605.04581)
...
### [42] [From Diffusion to Rectified Flow: Rethinking Text-Based Segmentation](https://arxiv.org/abs/2605.04590)
...
### [43] [DiCLIP: Diffusion Model Enhances CLIP's Dense Knowledge for Weakly Supervised Semantic Segmentation](https://arxiv.org/abs/2605.04593)
...
### [44] [Reference-based Category Discovery: Unsupervised Object Detection with Category Awareness](https://arxiv.org/abs/2605.04606)
...
### [45] [Advancing Aesthetic Image Generation via Composition Transfer](https://arxiv.org/abs/2605.04609)
...
### [46] [Temporal Structure Matters for Efficient Test-Time Adaptation in Wearable Human Activity Recognition](https://arxiv.org/abs/2605.04617)
...
### [47] [UniPCB: A Generation-Assisted Detection Framework for PCB Defect Inspection](https://arxiv.org/abs/2605.04635)
...
### [48] [CAST: Mitigating Object Hallucination in Large Vision-Language Models via Caption-Guided Visual Attention Steering](https://arxiv.org/abs/2605.04641)
...
### [49] [Contact Matrix: Enhancing Dance Motion Synthesis with Precise Interaction Modeling](https://arxiv.org/abs/2605.04662)
...
### [50] [Physical Adversarial Clothing Evades Visible-Thermal Detectors via Non-Overlapping RGB-T Pattern](https://arxiv.org/abs/2605.04675)
...
### [51] [Multi-Level Bidirectional Biomimetic Learning for EEG-Based Visual Decoding](https://arxiv.org/abs/2605.04680)
...
### [53] [Not Every Subject Should Stay: Machine Unlearning for Noisy Engagement Recognition](https://arxiv.org/abs/2605.04713)
...
### [54] [Anny-Fit: All-Age Human Mesh Recovery](https://arxiv.org/abs/2605.04728)
...
### [55] [ULF-Loc: Unbiased Landmark Feature for Robust Visual Localization with 3D Gaussian Splatting](https://arxiv.org/abs/2605.04730)
...
### [56] [Morphology-Guided Cross-Task Coupling for Joint Building Height and Footprint Estimation](https://arxiv.org/abs/2605.04731)
...
### [57] [VC-FeS: Viewpoint-Conditioned Feature Selection for Vehicle Re-identification in Thermal Vision](https://arxiv.org/abs/2605.04750)
...
### [58] [Hybrid Congestion Classification Framework Using Flow-Guided Attention and Empirical Mode Decomposition](https://arxiv.org/abs/2605.04752)
...
### [59] [Lightweight Cross-Spectral Face Recognition via Contrastive Alignment and Distillation](https://arxiv.org/abs/2605.04769)
...
### [60] [Gaze4HRI: Zero-shot Benchmarking Gaze Estimation Neural-Networks for Human-Robot Interaction](https://arxiv.org/abs/2605.04770)
...
### [61] [MIRAGE: Retrieval and Generation of Multimodal Images and Texts for Medical Education](https://arxiv.org/abs/2605.04772)
...
### [62] [QuadBox: Accelerating 3D Gaussian Splatting with Geometry-Aware Boxes](https://arxiv.org/abs/2605.04844)
...
### [65] [Exploring Clustering Capability of Inpainting Model Embeddings for Pattern-based Individual Identification](https://arxiv.org/abs/2605.04904)
...
### [66] [DART: A Vision-Language Foundation Model for Comprehensive Rope Condition Monitoring](https://arxiv.org/abs/2605.04943)
*Anju Rani,Daniel Ortiz-Arroyo,Petar Durdevic*
...
### [67] [ICPR 2026 Competition on Privacy-Preserving Person Re-Identification from Top-View RGB-Depth Camera (TVRID)](https://arxiv.org/abs/2605.04977)
...
### [68] [Attention-Based Chaotic Self-Supervision for Medical Image Classification](https://arxiv.org/abs/2605.04985)
...
### [69] [Low-Rank Adaptation of Geospatial Foundation Models for Wildfire Mapping Using Sentinel-2 Data](https://arxiv.org/abs/2605.04989)
...
### [70] [Chaotic Contrastive Learning for Robust Texture Classification](https://arxiv.org/abs/2605.05012)
*Joao B Florindo*
...
### [71] [CARD: A Multi-Modal Automotive Dataset for Dense 3D Reconstruction in Challenging Road Topography](https://arxiv.org/abs/2605.05014)
...
### [72] [Local Intrinsic Dimension Unveils Hallucinations in Diffusion Models](https://arxiv.org/abs/2605.05026)
...
### [73] [When Relations Break: Analyzing Relation Hallucination in Vision-Language Model Under Rotation and Noise](https://arxiv.org/abs/2605.05045)
...
### [74] [Prompt-Anchored Vision-Text Distillation for Lifelong Person Re-identification](https://arxiv.org/abs/2605.05027)
...
### [75] [Computer-Aided Design Generation by Cascaded Discrete Diffusion Model](https://arxiv.org/abs/2605.05031)
...
### [76] [Few-Shot Learning Pipeline for Monkeypox Skin Disease Classification Using CNN Feature Extractors](https://arxiv.org/abs/2605.05034)
...
### [77] [Direct Product Flow Matching: Decoupling Radial and Angular Dynamics for Few-Shot Adaptation](https://arxiv.org/abs/2605.05054)
...
### [78] [ScriptHOI: Learning Scripted State Transitions for Open-Vocabulary Human-Object Interaction Detection](https://arxiv.org/abs/2605.05057)
...
### [79] [Height-Guided Projection Reparameterization for Camera-LiDAR Occupancy](https://arxiv.org/abs/2605.05072)
...
### [80] [FlowDIS: Language-Guided Dichotomous Image Segmentation with Flow Matching](https://arxiv.org/abs/2605.05077)
...
### [81] [A unified Benchmark for Multi-Frame Image Restoration under Severe Refractive Warping](https://arxiv.org/abs/2605.05079)
...
### [82] [CPCANet: Deep Unfolding Common Principal Component Analysis for Domain Generalization](https://arxiv.org/abs/2605.05136)
...
### [83] [What Matters in Practical Learned Image Compression](https://arxiv.org/abs/2605.05148)
...
### [84] [Aes3D: Aesthetic Assessment in 3D Gaussian Splatting](https://arxiv.org/abs/2605.05155)
...
### [85] [Wasserstein-Aligned Localisation for VLM-Based Distributional OOD Detection in Medical Imaging](https://arxiv.org/abs/2605.05161)
...
### [86] [PhysForge: Generating Physics-Grounded 3D Assets for Interactive Virtual World](https://arxiv.org/abs/2605.05163)
...
### [87] [Geometry-Aware State Space Model: A New Paradigm for Whole-Slide Image Representation](https://arxiv.org/abs/2605.05164)
...
### [88] [OpenSearch-VL: An Open Recipe for Frontier Multimodal Search Agents](https://arxiv.org/abs/2605.05185)
...
### [89] [LoViF 2026 The First Challenge on Holistic Quality Assessment for 4D World Model (PhyScore)](https://arxiv.org/abs/2605.05187)
...
### [90] [D-OPSD: On-Policy Self-Distillation for Continuously Tuning Step-Distilled Diffusion Models](https://arxiv.org/abs/2605.05204)
...
### [91] [Taming Outlier Tokens in Diffusion Transformers](https://arxiv.org/abs/2605.05206)
...
### [92] [Syn4D: A Multiview Synthetic 4D Dataset](https://arxiv.org/abs/2605.05207)
...
TL;DR: FREIA 是一种基于强化学习的新型无监督方法，通过自由能驱动奖励（FER）和自适应优势塑造（AAS）两大创新，动态适应模型推理能力的变化，提升训练稳定性与性能。在九个数据集上的实验表明，FREIA 在数学推理任务中显著优于现有基线，平均提升0.5至3.5分（Pass@1）。


<details>
  <summary>Details</summary>
Motivation: 现有无监督强化学习方法无法随模型推理能力的演化调整奖励机制，导致优化方向偏离，缺乏适应性。

Method: 提出 FREIA 算法，结合自由能驱动奖励（FER）以平衡共识与探索，以及自适应优势塑造（AAS）根据采样奖励的统计特性动态调整学习信号。

Result: 在九个数据集、三个推理任务上验证，FREIA 显著优于其他无监督强化学习基线；在数学推理任务中，使用 DeepSeek-R1-Distill-Qwen-1.5B 模型时，Pass@1 平均提升 0.5 至 3.5 分。

Conclusion: FREIA 有效解决了无监督强化学习中因缺乏适应性而导致的优化偏差问题，为大语言模型的自我改进提供了更稳定、高效的训练框架。
...
TL;DR: 提出自适应幂均策略优化（APMPO），通过幂均目标和反馈自适应裁剪机制，实现动态调整策略优化，提升大语言模型在强化学习中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态策略优化，无法匹配模型推理能力的动态演化，导致性能受限。

Method: 引入幂均目标函数实现从算术平均到几何平均的自适应过渡，并设计基于实时奖励统计的反馈自适应裁剪机制，动态调节优化过程。

Result: 在九个数据集上的三类推理任务中表现优于现有最先进方法，数学推理基准上相比GRPO提升3.0分（Pass@1）, 使用Qwen2.5-3B-Instruct模型。

Conclusion: APMPO通过动态适应策略优化，显著提升了大语言模型在强化学习中的推理性能，为构建更智能、可进化的推理系统提供了有效路径。
...
### [95] [FMI_SU_Yotkova_Kastreva at SemEval-2026 Task 13: Lightweight Detection of LLM-Generated Code via Stylometric Signals](https://arxiv.org/abs/2605.04157)
*Elitsa Yotkova,Violeta Kastreva,Dimitar Dimitrov,Ivan Koychev,Preslav Nakov*

Main category: cs.CL

TL;DR: 本论文参与SemEval-2026任务13的子任务A（二分类），提出一种轻量级方法用于检测机器生成代码。通过设计对代码片段长度不敏感的比率特征，结合解析引擎和编程语言分类器提取描述性信号，并训练一个代码与自然语言行分类器以识别嵌入的自然语言段落。最终采用基于浅层决策树和启发式规则的集成策略进行预测，该方法仅需CPU资源训练，推理速度极快，是一种高效且轻量的替代方案。


<details>
  <summary>Details</summary>
Motivation: 现有方法在跨语言和跨领域场景下泛化能力不足，且大型预训练模型计算开销大，难以在资源受限环境中部署。因此需要一种高效、轻量、具备强泛化能力的机器生成代码检测方法。

Method: 设计比率特征以降低对代码长度的敏感性；利用解析引擎和编程语言分类器提取描述性信号；训练代码-文本行分类器识别自然语言片段；结合浅层决策树与数据驱动的启发式规则生成最终预测结果。

Result: 所提方法在多个编程语言和应用场景下表现良好，具有良好的泛化能力；仅需CPU即可完成训练，推理时间近乎即时，显著优于依赖大规模预训练模型的方法。

Conclusion: 本研究提出了一种高效、轻量且可扩展的机器生成代码检测框架，适用于资源受限环境，同时在跨语言和跨域任务中展现出优异的性能，为实际应用提供了可行的解决方案。
...
### [96] [Not All That Is Fluent Is Factual: Investigating Hallucinations of Large Language Models in Academic Writing](https://arxiv.org/abs/2605.04171)
*Humam Khan,Md Tabrez Nafis,Shahab Saquib Sohail,Aqeel Khalique,Rehan Hasan Khan*

Main category: cs.CL

TL;DR: 本研究评估了ChatGPT、Grok、Gemini和Copilot在学术写作中生成内容时的幻觉问题，设计了80个涵盖参考文献生成、事实解释、摘要生成和写作改进四类任务的提示。采用0-5分评分标准评估事实准确性、参考文献有效性、连贯性、风格一致性和学术语气，并引入新的加权指标Hallucination Index（HI）量化幻觉程度。结果显示，Grok和Copilot在参考文献生成方面表现较好，但在抽象或风格类任务中幻觉率较高（HI分别为0.67和0.70）；Gemini和ChatGPT在语调控制上较强，但在事实类任务中表现较差且幻觉风险更高（HI分别为0.53和0.57）。研究发现幻觉行为不仅受模型架构影响，还与任务类型和提示条件密切相关，为未来研究提供了新方向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在学术写作中虽表现出色，但存在幻觉问题，尤其在生成事实性内容时容易产生错误信息，影响学术可信度。现有评估方法常忽略语义偏移等隐性错误，亟需更全面的评估框架。

Method: 设计80个跨四类任务的提示（参考文献生成、事实解释、摘要生成、写作改进），使用0-5分多维度评分体系评估模型输出，提出加权指标Hallucination Index（HI）以量化幻觉程度，并对比四种主流大模型的表现。

Result: Grok和Copilot在参考文献生成任务中表现较优，但在抽象与风格任务中幻觉严重（HI=0.67, 0.70）；Gemini和ChatGPT在语调控制上更佳，但在事实类任务中错误频发，幻觉风险高（HI=0.53, 0.57）。幻觉行为受任务类型与提示条件显著影响，非仅由模型架构决定。

Conclusion: 本研究揭示了大语言模型在学术写作中的幻觉行为具有任务依赖性，强调需结合任务类型与提示设计优化评估与应用策略，为后续研究提供了新的分析维度与方法论启示。
...
TL;DR: 该研究评估了四种开源大模型和两种领域适应模型在尼日利亚和喀麦隆冲突事件分类任务中的表现，发现开源模型存在显著的‘错误非合法性偏差’（如将合法战斗误判为针对平民的暴力），而领域适应模型则表现出接近中立的规范方向性。然而，所有模型仍存在基于行为者的选取偏差，尤其在国家与非国家行为者之间存在系统性差异。此外，开源模型对地理特定的语言框架敏感，易受话语操纵影响，产生高达66.7%的误判翻转率；而领域适应模型则表现稳健。研究指出当前模型不适合无监督部署于冲突监测，呼吁进行公平性感知微调、对抗鲁棒性评估及区域化人机协同监督。


<details>
  <summary>Details</summary>
Motivation: 在冲突监测中使用大语言模型时，需确保其输出不因系统性偏差而影响人道主义问责。现有模型在规范判断上存在偏差，且对语言框架敏感，可能导致严重误判，因此亟需评估其可靠性并提出改进路径。

Method: 对比分析四种开源模型（Gemma 3 4B, Llama 3.2 3B, Mistral 7B, OLMo 2 7B）与两种领域适应模型（AfroConfliBERT, AfroConfliLLAMA）在尼日利亚和喀麦隆冲突事件分类任务上的表现，基于ACLED金标准数据集，采用多阶段验证方法，并通过对抗扰动、错误溯源分析等手段识别模型偏差来源。

Result: 开源模型存在显著的虚假非合法性偏差（如将合法战斗误判为针对平民的暴力），且对地理特定语言表达高度敏感，导致高翻转率；而领域适应模型在规范方向上接近中立，但依然存在行为者间的选取偏差。错误溯源显示开源模型常以不忠实的理由掩盖偏见。总体而言，当前模型尚未具备无监督部署的可靠性。

Conclusion: 当前大语言模型尚不具备直接用于冲突监测的条件。建议推动公平性感知微调、强制开展对抗鲁棒性测试，并建立基于区域复杂性的分层人机协作监督机制。
...
TL;DR: 提出MedFabric数据集和ETHER检测框架，通过生成具有语法和风格一致性的微小事实偏差文本，显著提升医学领域中语言模型幻觉检测性能，优于现有方法15%以上。


<details>
  <summary>Details</summary>
Motivation: 现有医学幻觉数据集在伪造现象覆盖、人类与LLM文本风格差异及合成样本分布漂移方面存在不足，难以有效捕捉医疗场景下的事实性错误生成。

Method: 设计数据驱动的流水线生成真实且保持句法与风格一致性的词级伪造文本，构建MedFabric数据集；提出ETHER检测框架，融合Text2Table分解、词掩码与填充以及混合句子对评估机制，以增强事实一致性判断。

Result: MedFabric在词级伪造检测基准上超越当前最优检测器超过15%，且在结构相似性变化下表现稳定，为特定领域事实性检测提供全面解决方案。

Conclusion: MedFabric与ETHER共同构成一个可靠、领域适配的医学幻觉检测框架，有效应对语言模型在医疗知识领域的事实性偏差问题。
...
TL;DR: 本文通过系统实验研究了联合词汇表与不相交词汇表在多语言神经机器翻译中的影响，发现语言相关性和领域匹配度比词汇重叠度更重要。


<details>
  <summary>Details</summary>
Motivation: 探究知识迁移在多语言神经机器翻译中的作用，特别是词汇重叠度对模型性能的影响，当前对此研究不足且缺乏共识。

Method: 设计了基于跨域设置的实验，比较联合与不相交词汇表，以及与源语言相关和无关的辅助语言的效果。

Result: 实验表明，尽管相关语言具有更广泛的词汇重叠带来更好结果，但领域匹配度和语言相关性对性能的影响大于词汇重叠度。

Conclusion: 语言相关性和领域匹配度是实现有效知识迁移的关键因素，而联合词汇表并非决定性因素。
...
TL;DR: Nsanku is a comprehensive benchmark evaluating zero-shot machine translation performance of 19 LLMs across 43 Ghanaian languages using Bible-derived sentence pairs. It employs BLEU, chrF, accuracy, and consistency metrics. Gemini-2.5-flash leads in overall performance, but no model achieves both high performance and high consistency across languages, indicating limitations for real-world use. Siwu performs best, Nkonya worst. The benchmark is publicly available to support future African language NLP research.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate the zero-shot translation capabilities of large language models on low-resource Ghanaian languages, which have been under-researched despite their importance in African NLP.

Method: The study constructs a benchmark using 300 sentence pairs per language from the YouVersion Bible platform. It evaluates 19 open-weight and proprietary LLMs using BLEU, chrF, average accuracy, and cross-language consistency analysis.

Result: Gemini-2.5-flash achieves the highest average score (26.88), followed by Claude Sonnet and GPT-4.1. Among open-weight models, Kimi-K2-Instruct leads at 20.87. No model achieves both high performance and high consistency across all languages; Siwu scores highest (25.73), Nkonya lowest (11.65).

Conclusion: Current LLMs are not yet reliably usable for large-scale translation in Ghanaian languages due to inconsistent performance. Nsanku provides a publicly available, extensible evaluation framework to advance African language NLP research.
...
TL;DR: 该研究提出了一种本地部署的框架，利用小语言模型自动生成、验证、优化和评估特定于实体的提示，以从牙科病历中提取多种临床实体。基于1,200份标注病历，通过多提示集成推理评估候选开源模型，并采用QLoRA微调和直接偏好优化（DPO）进行模型适配。Qwen2.5-14B-Instruct表现最佳，经DPO后在微平均和宏平均F1得分上分别达到0.864/0.837和0.806/0.797，表明自动化提示优化结合轻量级偏好训练可有效支持本地部署的小模型实现可扩展的临床信息抽取。


<details>
  <summary>Details</summary>
Motivation: 牙科病历文档高度非结构化、领域特定且隐私敏感，传统命名实体识别方法难以适用，亟需一种高效、安全且可本地部署的解决方案。

Method: 提出一个本地部署框架，使小语言模型能够自动生成、验证、优化和评估针对特定实体的提示；使用多提示集成推理评估模型；通过QLoRA监督微调和直接偏好优化（DPO）对选定模型进行适应性训练。

Result: 模型性能差异显著，强调任务特定评估的重要性；Qwen2.5-14B-Instruct为最强基线模型；经DPO后，其微/宏平均F1分别为0.864/0.837，Llama-3.1-8B-Instruct为0.806/0.797，证明了自动化提示优化与轻量偏好训练的有效性。

Conclusion: 自动化提示优化结合轻量级偏好后训练，可有效支持在本地部署的小语言模型上实现高效、可扩展的临床信息抽取，适用于隐私敏感的医疗场景。
...
TL;DR: Material Database Agent (MDA) 是一个模块化多智能体系统，用于将材料科学文献转化为结构化数据库。它通过并行处理PDF文章的文本和图表，利用多个子智能体提取信息并构建子数据库，最终由一个主智能体整合成统一的表格数据库。该方法突破了传统手动或单阶段管道的局限，为基于多模态大模型的科学数据库构建提供了新范式。


<details>
  <summary>Details</summary>
Motivation: 材料科学领域的研究文献中包含大量实验细节，但这些信息大多隐藏在文本、表格、图表和图像中，现有数据库构建依赖人工，效率低且难以扩展。因此需要一种自动化、可扩展的方法来从文献中提取结构化数据。

Method: MDA采用多智能体架构，将输入的论文PDF文件并行分解为Markdown文本和图像。多个子智能体分别解析文本和图像内容，提取相关信息并生成每个论文的子数据库。最后，一个聚合智能体将所有子数据库合并为统一的结构化表格数据库。整个过程结合了多模态大语言模型的能力，实现高效、准确的信息提取。

Result: MDA成功实现了从材料科学文献中自动提取结构化数据，并构建出高质量的生产级材料数据库。相比传统方法，其效率显著提升，且具备良好的可扩展性，验证了多模态智能体在科学数据提取中的可行性与有效性。

Conclusion: 本研究证明了多模态智能体系统在从原始科学文献中构建下一代科学数据库方面的潜力。MDA不仅适用于材料科学，还可推广至其他领域，为自动化知识库建设提供通用范式。
...
### [116] [Benchmarking POS Tagging for the Tajik Language: A Comparative Study of Neural Architectures on the TajPersParallel Corpus](https://arxiv.org/abs/2605.04576)
...
### [117] [TajikNLP: An Open-Source Toolkit for Comprehensive Text Processing of Tajik (Cyrillic Script)](https://arxiv.org/abs/2605.04583)
*Mullosharaf K. Arabov*
...
### [118] [Gradients with Respect to Semantics Preserving Embeddings Tell the Uncertainty of Large Language Models](https://arxiv.org/abs/2605.04638)
...
### [119] [Graph-Augmented LLMs for Swiss MP Ideology Prediction](https://arxiv.org/abs/2605.04643)
...
### [120] [CHE-TKG: Collaborative Historical Evidence and Evolutionary Dynamics Learning for Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2605.04652)
...
### [121] [Paraphrase-Induced Output-Mode Collapse: When LLMs Break Character Under Semantically Equivalent Inputs](https://arxiv.org/abs/2605.04665)
...
### [122] [Every Step Counts: Step-Level Credit Assignment for Tool-Integrated Text-to-SQL](https://arxiv.org/abs/2605.04719)
...
### [123] [Gyan: An Explainable Neuro-Symbolic Language Model](https://arxiv.org/abs/2605.04759)
...
### [124] [Elicitation Matters: How Prompts and Query Protocols Shape LLM Surrogates under Sparse Observations](https://arxiv.org/abs/2605.04764)
...
### [125] [StoryAlign: Evaluating and Training Reward Models for Story Generation](https://arxiv.org/abs/2605.04831)
...
### [126] [Assessing Cognitive Effort in L2 Idiomatic Processing: An Eye-Tracking Dataset](https://arxiv.org/abs/2605.04857)
...
### [127] [Measuring Psychological States Through Semantic Projection: A Theory-Driven Approach to Language-Based Assessment](https://arxiv.org/abs/2605.04873)
...
### [128] [Anticipating Innovation Using Large Language Models](https://arxiv.org/abs/2605.04875)
...
### [129] [A Comparative Study of PyCaret AutoML and CNN-BiLSTM for Binary Hate Speech Detection in Indonesian Twitter](https://arxiv.org/abs/2605.04885)
...
### [130] [BenCSSmark: Making the Social Sciences Count in LLM Research](https://arxiv.org/abs/2605.04886)
...
### [131] [Sentiment Analysis and Customer Satisfaction Prediction on E-Commerce Platforms Based on YouTube Comments Using the XGBoost Algorithm](https://arxiv.org/abs/2605.04887)
...
### [132] [A Comparative Analysis of Machine Learning and Deep Learning Models for Tweet Sentiment Classification: A Case Study on the Sentiment140 Dataset](https://arxiv.org/abs/2605.04888)
...
### [133] [Storage Is Not Memory: A Retrieval-Centered Architecture for Agent Recall](https://arxiv.org/abs/2605.04897)
...
### [134] [Rethinking Local Learning: A Cheaper and Faster Recipe for LLM Post-Training](https://arxiv.org/abs/2605.04913)
...
### [135] [Unintended Negative Impacts of Promotional Language in Patent Evaluation](https://arxiv.org/abs/2605.04926)
...
### [136] [UFAL-CUNI at SemEval-2026 Task 11: An Efficient Modular Neuro-symbolic Method for Syllogistic Reasoning](https://arxiv.org/abs/2605.04941)
...
### [137] [Adapting Large Language Models to a Low-Resource Agglutinative Language: A Comparative Study of LoRA and QLoRA for Bashkir](https://arxiv.org/abs/2605.04948)
...
### [138] [TabEmbed: Benchmarking and Learning Generalist Embeddings for Tabular Understanding](https://arxiv.org/abs/2605.04962)
...
### [139] [Why Expert Alignment Is Hard: Evidence from Subjective Evaluation](https://arxiv.org/abs/2605.04972)
...
### [140] [Misaligned by Reward: Socially Undesirable Preferences in LLMs](https://arxiv.org/abs/2605.05003)
...
### [141] [Detecting Hallucinations in Large Language Models via Internal Attention Divergence Signals](https://arxiv.org/abs/2605.05025)
*Gijs van Dijk*
...
### [142] [The Impossibility Triangle of Long-Context Modeling](https://arxiv.org/abs/2605.05066)
*Yan Zhou*
...
### [143] [The Pinocchio Dimension: Phenomenality of Experience as the Primary Axis of LLM Psychometric Differences](https://arxiv.org/abs/2605.05080)
...
### [144] [Automatically Finding and Validating Unexpected Side-Effects of Interventions on Language Models](https://arxiv.org/abs/2605.05090)
...
### [145] [Text Corpora as Concept Fields: Black-Box Hallucination and Novelty Measurement](https://arxiv.org/abs/2605.05103)
...
### [146] [Beyond Semantics: An Evidential Reasoning-Aware Multi-View Learning Framework for Trustworthy Mental Health Prediction](https://arxiv.org/abs/2605.05121)
...
### [147] [PSK at SemEval-2026 Task 9: Multilingual Polarization Detection Using Ensemble Gemma Models with Synthetic Data Augmentation](https://arxiv.org/abs/2605.05159)
...
### [148] [The First Token Knows: Single-Decode Confidence for Hallucination Detection](https://arxiv.org/abs/2605.05166)
*Mina Gabriel*
...
### [149] [Implicit Representations of Grammaticality in Language Models](https://arxiv.org/abs/2605.05197)
...
### [150] [Regularized Centered Emphatic Temporal Difference Learning](https://arxiv.org/abs/2605.04100)
*Xingguo Chen,Chaohui Wu,Jinguo Ye,Chao Li,Shangdong Yang,Guang Yang,Tianyu Liang,Wenhao Wang*

Main category: cs.AI

TL;DR: 本文提出了一种新的强化学习方法——正则化强调时序差分学习（RETD），以解决离策略时序差分（TD）学习中稳定性、投影几何和方差控制之间的结构性权衡问题。通过引入贝尔曼误差中心化，并对辅助中心化递归进行正则化，RETD在保持强调方法有利的投影几何的同时，避免了传统中心化方法导致的矩阵非正定性问题。实验表明，RETD在多种诊断任务中表现出稳定且鲁棒的性能，尤其在正则化参数c的中间范围内表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有强调时序差分（ETD）方法虽改善了离策略学习中的投影几何，但其跟随痕迹（follow-on trace）存在高方差问题；而直接应用贝尔曼误差中心化会引入辅助耦合，破坏关键矩阵的正定性，导致学习不稳定。因此需要一种既能保留良好投影结构又能控制方差并保证稳定性的改进方法。

Method: 提出正则化强调时序差分学习（RETD），通过仅对辅助中心化递归施加正则化（将关键矩阵右下角块从1提升至1+c），实现对中心化过程的稳健控制，同时保留原有的跟随痕迹结构。该方法通过对核心矩阵的分析，推导出收敛条件，并在理论与实验层面验证其有效性。

Result: RETD成功避免了朴素中心化方法带来的不稳定性，保持了强调方法优越的投影几何特性，并在多个线性离策略预测任务中展现出对正则化参数c具有鲁棒性的中间性能区间，证明其在稳定性与效率之间取得了良好平衡。

Conclusion: RETD是一种有效且稳健的离策略强化学习算法，通过精细的正则化设计，在不牺牲投影几何优势的前提下，显著提升了学习过程的稳定性与实用性，为后续研究提供了新的方向。
...
### [151] [ANDRE: An Attention-based Neuro-symbolic Differentiable Rule Extractor](https://arxiv.org/abs/2605.04193)
*Iman Sharifi,Peng Wei,Saber Fallah*

Main category: cs.AI

TL;DR: ANDRE 是一种新型的神经符号可微逻辑编程框架，通过注意力机制驱动的连续规则空间优化，实现对概率性数据的准确、稳定且可解释的推理。它用可微的注意力操作替代传统规则模板和模糊逻辑算子，支持灵活的规则归纳并保持符号结构，显著提升在噪声和不确定性下的规则提取质量与稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有符号和神经符号方法在处理噪声和概率性设置时难以扩展；经典ILP对离散组合搜索依赖性强，易受不确定性影响；而可微ILP方法通常依赖预定义规则模板或不准确的模糊算子，存在梯度消失或逻辑结构近似不佳的问题。

Method: 提出基于注意力的神经符号可微规则提取器（ANDRE），采用注意力驱动的可微合取与析取操作符，逼近逻辑的最小-最大语义，通过软选择、否定或排除谓词实现灵活规则归纳，同时保持符号可解释性。

Result: 在经典ILP基准、大规模知识库及含概率谓词和噪声监督的合成数据集上，ANDRE展现出竞争性或更优的预测性能，能可靠恢复正确符号规则，尤其在中等标签噪声下表现优于现有可微ILP方法，在规则提取质量和稳定性方面均有显著提升。

Conclusion: ANDRE成功实现了在不确定性和噪声环境下的高效、可解释规则学习，为神经符号学习提供了新的可微化路径，具有良好的泛化能力与实际应用潜力。
...
### [152] [Pro$^2$Assist: Continuous Step-Aware Proactive Assistance with Multimodal Egocentric Perception for Long-Horizon Procedural Tasks](https://arxiv.org/abs/2605.04227)
*Lilin Xu,Bufang Yang,Siyang Jiang,Kaiwei Liu,Kaiyuan Hou,Yuang Fan,Hongkai Chen,Zhenyu Yan,Xiaofan Jiang*

Main category: cs.AI

TL;DR: Pro$^2$Assist 是一种基于增强现实（AR）眼镜的步骤感知主动助手，能够持续追踪细粒度的任务进展，并根据用户状态变化提供及时帮助。该系统利用多模态数据进行运动感知，从多尺度时间动态和任务特定专家知识中提取步骤导向的上下文信息，结合感官输入与上下文进行连续推理，以推断用户需求并实时在AR眼镜上显示辅助信息。实验表明，其在程序化动作理解准确率上优于最佳基线超过21%，主动提示准确性提升达2.29倍；用户研究显示90%参与者认为其有效有用。


<details>
  <summary>Details</summary>
Motivation: 现有系统多为被动响应或仅针对短期事件提供有限主动协助，难以支持长周期、多步骤的程序性任务，亟需一种能持续跟踪任务进展并主动提供适时指导的智能助手。

Method: 通过AR眼镜采集多模态数据，实现运动感知；利用多尺度时间动态与专家知识提取步骤导向的上下文；结合感官输入与上下文进行连续推理，生成及时的辅助指令并在AR界面展示。

Result: 在公开数据集与真实场景数据集上的评估显示，Pro$^2$Assist在程序化动作理解准确率上超过基线21%以上，主动提示时间精度达基线的2.29倍；20名用户的实验表明90%认为其具有实用性。

Conclusion: Pro$^2$Assist成功实现了对长期程序性任务的持续感知与主动支持，具备高精度的上下文理解与及时响应能力，展现出在真实世界日常任务辅助中的巨大潜力。
...
### [153] [Temporal Reasoning Is Not the Bottleneck: A Probabilistic Inconsistency Framework for Neuro-Symbolic QA](https://arxiv.org/abs/2605.04243)
*Tran Quang Liem*

Main category: cs.AI

TL;DR: 本文挑战了大语言模型（LLM）在复杂时间推理任务中表现脆弱的主流观点，指出根本问题不在于推理能力，而在于非结构化文本到事件表示的缺陷。作者提出一种基于概率不一致信号（PIS）的神经符号问答框架，将非结构化文本转化为显式的事件图与区间约束，严格分离语义提取与符号推理。通过结合符号置信区间与基于证据深度学习从LLM隐藏状态中提取的认知不确定性，该框架能有效检测结构断裂。实验表明，在正确结构表示下，系统在时间算术基准上达到1.0的准确率（4000/4000），且无误报/漏报；在含噪声的问答设置中仍保持75.1%的竞争力，并支持确定性的逐步失败定位。研究将时间问答重新定义为结构对齐问题，为可验证的神经符号人工智能提供了明确路径。


<details>
  <summary>Details</summary>
Motivation: 现有研究普遍认为大语言模型在时间推理上的失败源于自回归逻辑推导的内在缺陷。然而，本文提出质疑，认为真正瓶颈在于非结构化文本到事件的表示方式，而非推理机制本身。因此，需要一种能够区分感知错误与推理错误的新方法，以提升时间推理的鲁棒性与可解释性。

Method: 提出基于概率不一致信号（PIS）的神经符号框架，将非结构化文本转换为显式事件图与区间约束，实现语义提取与符号推理的解耦。利用证据深度学习从LLM隐藏状态中提取认知不确定性，并将其与符号置信区间统一，以检测结构断裂。

Result: 在正确结构表示下，系统在时间算术任务上达到1.0的准确率（4000/4000），且无误报或漏报；在噪声干扰的广泛问答场景中保持75.1%的准确率，并支持确定性的逐步失败定位。

Conclusion: 本研究揭示了时间问答的核心挑战并非算法推理，而是结构表示的对齐问题。通过将表示瓶颈与推理模块分离，为构建可靠、可解释的神经符号AI提供了可验证的路径。
...
### [154] [Parallel Prefix Verification for Speculative Generation](https://arxiv.org/abs/2605.04263)
*Yuncheng Yao,Yuxuan Xia,Shengjie Wang,Danyang Zhuo*

Main category: cs.AI

TL;DR: PARSE 是一种并行前缀推测引擎，通过语义级别的并行前缀验证加速大语言模型推理。它突破了传统方法在令牌级等价性上的限制，实现无序的语义级验证，显著提升接受粒度和效率。相比现有方法，PARSE 在单次前向传播中利用自定义注意力掩码评估多个前缀的正确性，直接识别最大有效前缀，避免了顺序验证开销。该方法与令牌级推测解码正交，可组合使用以进一步提升性能。实验表明，单独使用时，PARSE 可带来 1.25× 到 4.3× 的吞吐量提升；与 EAGLE-3 结合后可达 1.6× 到 4.5×，且几乎不影响准确性。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法受限于令牌级等价性，导致接受长度短、加速效果有限。虽然语义或段级验证能提高接受粒度，但传统方法依赖顺序验证，引入显著开销，难以发挥实际优势。因此需要一种无需顺序检查、可高效进行语义级验证的新方法。

Method: PARSE 提出并行前缀验证机制：给定草稿模型生成的完整草稿，目标模型通过自定义注意力掩码在一次前向传播中同时评估多个前缀的正确性，从而直接识别最长有效前缀。该方法跳过逐段验证过程，实现计算高效。此外，其与现有令牌级推测解码兼容，可组合使用以获得更大加速。

Result: 在多种模型和基准测试中，PARSE 单独使用时带来 1.25× 到 4.3× 的吞吐量提升；与 EAGLE-3 组合使用时达 1.6× 到 4.5× 的提升，且准确率几乎无损失。结果证明并行前缀验证是一种通用而高效的 LLM 推理加速方案。

Conclusion: PARSE 通过引入并行前缀验证，实现了语义级别验证的高效执行，克服了传统方法的顺序瓶颈，是加速大语言模型推理的有效通用策略，具备良好的扩展性和实用性。
...
### [155] [The Scaling Properties of Implicit Deductive Reasoning in Transformers](https://arxiv.org/abs/2605.04330)
...
### [156] [When Context Hurts: The Crossover Effect of Knowledge Transfer on Multi-Agent Design Exploration](https://arxiv.org/abs/2605.04361)
*Saranyan Vigraham*
...
### [157] [Deployment-Relevant Alignment Cannot Be Inferred from Model-Level Evaluation Alone](https://arxiv.org/abs/2605.04454)
...
### [158] [How Does Thinking Mode Change LLM Moral Judgments? A Controlled Instant-vs-Thinking Comparison Across Five Frontier Models](https://arxiv.org/abs/2605.04488)
*Sai Sourabh Madur*
...
### [159] [From Parameter Dynamics to Risk Scoring : Quantifying Sample-Level Safety Degradation in LLM Fine-tuning](https://arxiv.org/abs/2605.04572)
...
### [160] [SensingAgents: A Multi-Agent Collaborative Framework for Robust IMU Activity Recognition](https://arxiv.org/abs/2605.04608)
...
### [161] [AuditRepairBench: A Paired-Execution Trace Corpus for Evaluator-Channel Ranking Instability in Agent Repair](https://arxiv.org/abs/2605.04624)
...
### [162] [Budget-aware Auto Optimizer Configurator](https://arxiv.org/abs/2605.04711)
...
### [171] [Executable World Models for ARC-AGI-3 in the Era of Coding Agents](https://arxiv.org/abs/2605.05138)
*Sergey Rodionov*
...
### [172] [LongSeeker: Elastic Context Orchestration for Long-Horizon Search Agents](https://arxiv.org/abs/2605.05191)
...
### [173] [Endogenous Regime Switching Driven by Scalar-Irreducible Learning Dynamics](https://arxiv.org/abs/2605.04054)
*Sheng Ran*

Main category: cs.LG

TL;DR: 本文提出了一种区分可简化为标量目标梯度流的系统（标量可约）与不可简化的系统（标量不可约）的分类方法。大多数现有机器学习系统属于前者，而后者通过快速变量与慢速结构适应之间的反馈，自然实现内生的模式切换。研究通过一个最小动力学模型展示了无需外部调度即可产生持续内生模式转换的机制，揭示了自主学习系统的新动力学范式。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习框架中的模式切换通常依赖外部设定，难以实现真正的自主智能。如何实现内生的、自组织的模式切换是构建自主智能系统的核心挑战。

Method: 提出标量可约与不可约动力学的分类；构建最小动力学模型，分析快变量与慢结构适应之间的反馈机制。

Result: 标量不可约动力学能自然引发内生的模式切换；最小模型验证了无需外部干预即可实现持续的内部模式转换。

Conclusion: 该研究揭示了一种新的动力学范式，为实现真正内源性、自组织的自主学习系统提供了理论基础和可能路径。
...
### [174] [A Self-Attentive Meta-Optimizer with Group-Adaptive Learning Rates and Weight Decay](https://arxiv.org/abs/2605.04055)
*JiangBo Zhao,ZhaoXin Liu*

Main category: cs.LG

TL;DR: MetaAdamW 是一种新型自适应优化器，通过引入轻量级 Transformer 编码器和自注意力机制，动态调节不同参数组的学习率与权重衰减，利用梯度范数、动量范数及相关性等统计特征作为输入。其核心创新在于将同方差不确定性加权（HUW）扩展为带有任务特定优先级的正则化项缩放机制，以融合领域知识实现自动损失平衡。在五个不同任务（时间序列预测、语言建模、机器翻译、图像分类、情感分析）上的实验表明，MetaAdamW 在验证损失、准确率或困惑度上均优于 AdamW 基线，部分任务中训练时间减少最多 17.11%，性能提升达 11.08%，且计算开销可控。消融实验证明了各组件的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有自适应优化器（如 AdamW）对所有参数组采用统一超参数，忽略了不同层和模块间优化动态的异质性，导致优化效率受限。本文旨在解决这一局限，通过动态调整每组学习率与权重衰减来提升训练效率与模型性能。

Method: 提出 MetaAdamW，结合轻量级 Transformer 编码器，基于各参数组的统计特征（梯度范数、动量范数、相关性）生成自注意力调制因子，动态调节学习率与权重衰减；设计元学习目标，联合优化梯度对齐、损失下降与泛化差距；引入任务优先级的 HUW 扩展，指导正则化权重的自动平衡。

Result: 在五种任务上，MetaAdamW 持续超越 AdamW 基线，在验证损失、准确率或困惑度上均有提升；部分任务中训练时间减少最多 17.11%，性能最高提升 11.08%；可缓解因过早停止导致的收敛不足问题；消融实验验证了特征、分组策略及优先级注入机制的有效性。

Conclusion: MetaAdamW 通过动态调节参数组优化行为，有效捕捉异质优化动态，显著提升训练效率与模型性能，同时具备良好的可扩展性与鲁棒性，是自适应优化器的重要改进。
...
### [175] [Transformation Categorization Based on Group Decomposition Theory Using Parameter Division](https://arxiv.org/abs/2605.04056)
*Takayuki Komatsu,Yoshiyuki Ohmura,Yasuo Kuniyoshi*

Main category: cs.LG

TL;DR: 本文研究无监督变换分类，提出基于参数分割的群分解方法，无需先前的辅助假设，可有效处理耦合因素，在图像旋转、平移、缩放任务中表现良好。


<details>
  <summary>Details</summary>
Motivation: 现有表示学习缺乏对'好'表示的理论定义，传统解耦方法在因子耦合时失效，需更普适的理论框架。

Method: 通过参数分割将单个变换的参数分为组件，施加同态约束，将正常子群定义为当某一组件固定为单位元时的变换集合。

Result: 新方法无需辅助假设，适用于交换与非交换情形，在多种图像变换任务中实现准确分类，消融实验验证了群分解约束的有效性。

Conclusion: 所提方法基于群论原理，摆脱了以往依赖特定假设的局限，为无监督表示学习提供了更通用且理论严谨的解耦框架。
...
### [176] [Structured Progressive Knowledge Activation for LLM-Driven Neural Architecture Search](https://arxiv.org/abs/2605.04057)
*Zhen Liu,Yuhan Liu,Jingwen Fu*

Main category: cs.LG

TL;DR: 本文提出SPARK方法，通过显式选择要修改的功能因子并基于该因子进行编辑，以缓解大语言模型在神经架构搜索中因功能纠缠导致的非局部行为变化问题，显著提升架构演化的样本效率和性能。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在神经架构搜索中因功能纠缠导致的非局部行为变化问题，使已有架构知识能更可靠地应用于新设计探索。

Method: 提出结构化渐进知识激活（SPARK），通过选择特定功能因子并基于该因子进行条件化编辑，减少编辑带来的耦合副作用，实现更精准、可靠的架构修改。

Result: 在CLRS-DFS数据集上，SPARK实现了28.1倍的样本效率提升，且在OOD准确率上取得22.9%的相对改进。

Conclusion: SPARK有效缓解了功能纠缠问题，使大语言模型生成的架构知识在复杂评估环境下更加可利用，为高效、可靠的神经架构搜索提供了新路径。
...
### [177] [Continual Distillation of Teachers from Different Domains](https://arxiv.org/abs/2605.04059)
*Nicolas Michel,Maorong Wang,Jiangpeng He,Toshihiko Yamasaki*

Main category: cs.LG

TL;DR: 提出了一种新的持续蒸馏（CD）范式，通过外部无标签数据实现未见知识迁移（UKT），并引入SE2D方法缓解因顺序蒸馏导致的未见知识遗忘（UKF），显著提升跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型规模增大，传统蒸馏方法难以处理教师模型数据不可用及专家能力差异的问题，需新范式解决持续学习中的知识保留与迁移挑战。

Method: 提出Self External Data Distillation（SE2D）方法，利用外部无标签数据保存教师模型的logits，以稳定对异构教师的连续学习过程。

Result: 在多个基准测试中，SE2D有效减少未见知识遗忘，提升跨域泛化性能，并公开了代码实现。

Conclusion: SE2D成功缓解了持续蒸馏中的知识遗忘问题，为大规模模型持续学习提供了有效解决方案。
...
### [178] [Lookahead Drifting Model](https://arxiv.org/abs/2605.04060)
*Guoqiang Zhang,Kenta Niwa,W. Bastiaan Kleijn*

Main category: cs.LG

TL;DR: 提出了一种新的前瞻漂移模型（lookahead drifting model），通过在每个训练迭代中顺序计算一系列漂移项，并利用先前计算的漂移项、正样本和模型输出来增强梯度信息，从而提升图像生成性能。该方法通过加权求和多个漂移项方向优化模型，在玩具示例和CIFAR10上均优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有漂移模型虽在ImageNet上实现顶尖图像生成性能，但其单步漂移机制可能未能充分捕捉高阶梯度信息，限制了模型优化潜力。因此，需要一种更有效的机制来增强方向引导能力，以进一步提升生成质量。

Method: 提出前瞻漂移模型，每轮训练中依次计算多个漂移项，后续漂移项依赖于前期漂移项、正样本及当前模型输出；通过合理缩放使各漂移项量级可比，最终将加权后的漂移项总和作为优化方向指导模型更新。

Result: 在玩具数据集和CIFAR10上的实验表明，所提方法在生成质量与收敛速度方面均优于原始漂移模型，验证了其有效性与优越性。

Conclusion: 前瞻性漂移模型通过引入多阶段漂移项设计，有效捕获更高阶的梯度信息，显著提升了模型优化方向的准确性，为分布映射与图像生成提供了更优解决方案。
...
### [179] [EdgeRazor: A Lightweight Framework for Large Language Models via Mixed-Precision Quantization-Aware Distillation](https://arxiv.org/abs/2605.04062)
...
### [180] [Investigating Trustworthiness of Nonparametric Deep Survival Models for Alzheimer's Disease Progression Analysis](https://arxiv.org/abs/2605.04063)
...
### [181] [Improving Medical VQA through Trajectory-Aware Process Supervision](https://arxiv.org/abs/2605.04064)
...
### [182] [Designing a double deep reinforcement learning selection tool for resilient demand prediction](https://arxiv.org/abs/2605.04068)
...
### [183] [LAWS: Learning from Actual Workloads Symbolically -- A Self-Certifying Parametrized Cache Architecture for Neural Inference, Robotics, and Edge Deployment](https://arxiv.org/abs/2605.04069)
*Gregory Magarshak*
...
### [184] [Sparse Autoencoder Decomposition of Clinical Sequence Model Representations: Feature Complexity, Task Specialisation, and Mortality Prediction](https://arxiv.org/abs/2605.04072)
...
### [185] [Confronting Label Indeterminacy in Automated Bail Decisions](https://arxiv.org/abs/2605.04073)
...
### [186] [A Physics-Aware Framework for Short-Term GPU Power Forecasting of AI Data Centers](https://arxiv.org/abs/2605.04074)
...
### [187] [RetentiveKV: State-Space Memory for Uncertainty-Aware Multimodal KV Cache Eviction](https://arxiv.org/abs/2605.04075)
...
### [188] [A Regulatory Governance Framework for AI-Driven Financial Fraud Detection in U.S. Banking: Integrating OCC, SR 11-7, CFPB, and FinCEN Compliance Requirements for Model Development, Validation, and Monitoring Lifecycles](https://arxiv.org/abs/2605.04076)
...
### [189] [Balanced Aggregation: Understanding and Fixing Aggregation Bias in GRPO](https://arxiv.org/abs/2605.04077)
...
### [190] [Validity-Calibrated Reasoning Distillation](https://arxiv.org/abs/2605.04078)
...
TL;DR: 本研究利用机器学习模型预测污水处理厂中一氧化二氮（N2O）排放的扰动及空间变异，基于真实数据验证了四种模型的预测性能（R2 = 0.79 - 0.89），在模拟监测和干扰条件下模型精度高达0.97±0.02。但特征重要性受模型类型、情景和测量尺度影响，且软传感器预测受限于测量位置与数据方法不确定性，影响模型可解释性。此外，机理模型分析揭示了自养与异养路径间对一氧化氮的相互作用，可能导致好氧亚硝酸盐生成被高估，进而偏差N2O贡献路径评估。


<details>
  <summary>Details</summary>
Motivation: 现有基于模型的N2O排放解决方案依赖于控制营养水平的运行数据和专门的N2O测量活动，但难以准确捕捉操作扰动和空间变异性。因此需要更高效的建模工具来提升预测能力与机制理解。

Method: 采用四种机器学习模型分析真实运行数据，结合全厂机理模型模拟不同监测场景、新增传感器部署、站点级数据及16种废水扰动，评估模型预测性能与特征重要性，并通过机理模型结构分析揭示关键生物路径交互。

Result: ML模型在预测N2O排放方面表现出高准确性（R2 = 0.79–0.89，精度达0.97±0.02），但特征重要性随模型、情景和测量尺度变化；机理模型揭示自养与异养路径间的复杂交互，可能高估好氧硝化过程中的亚硝酸盐生成，导致对N2O来源路径的误判。

Conclusion: 尽管机器学习模型能有效预测N2O排放，但其可解释性受限于测量位置与数据方法不确定性；同时，机理模型需进一步优化以准确反映微生物代谢路径，避免对N2O生成机制的系统性偏差。
...
### [195] [FASQ: Flexible Accelerated Subspace Quantization for Calibration-Free LLM Compression](https://arxiv.org/abs/2605.04084)
...
### [196] [HERCULES: Hardware-Efficient, Robust, Continual Learning Neural Architecture Search](https://arxiv.org/abs/2605.04103)
...
### [197] [A large language model-type architecture for high-dimensional molecular potential energy surfaces](https://arxiv.org/abs/2412.03831)
...
### [198] [Learning reveals invisible structure in low-rank RNNs](https://arxiv.org/abs/2605.04115)
*Yoav Ger,Omri Barak*
...
### [199] [Learning Reconstructive Embeddings in Reproducing Kernel Hilbert Spaces via the Representer Theorem](https://arxiv.org/abs/2601.05811)
...
### [217] [Explaining and Preventing Alignment Collapse in Iterative RLHF](https://arxiv.org/abs/2605.04266)
...
### [219] [A Mean Curvature Approach to Boundary Detection: Geometric Insights for Unsupervised Learning](https://arxiv.org/abs/2605.04274)
...
### [220] [Gradient Flow Structure and Quantitative Dynamics of Multi-Head Self-Attention](https://arxiv.org/abs/2605.04279)
*Ayan Pendharkar*
...
### [221] [Hardware-Aware Neural Feature Extraction for Resource-Constrained Devices](https://arxiv.org/abs/2605.04282)
...
### [222] [Probabilistic Classification and Uncertainty Quantification of Sahara Desert Climate Using Feedforward Neural Networks](https://arxiv.org/abs/2605.04286)
...
### [223] [Leveraging Pretrained Language Models as Energy Functions for Glauber Dynamics Text Diffusion](https://arxiv.org/abs/2605.04291)
...
### [224] [LLMs Uncertainty Quantification via Adaptive Conformal Semantic Entropy](https://arxiv.org/abs/2605.04295)
...
### [225] [Memory as a Markov Matrix: Sample Efficient Knowledge Expansion via Token-to-Dictionary Mapping](https://arxiv.org/abs/2605.04308)
...
### [226] [LUCAS-MEGA: A Large-Scale Multimodal Dataset for Representation Learning in Soil-Environment Systems](https://arxiv.org/abs/2605.04323)
...
### [227] [DeFed-GMM-DaDiL: A Decentralized Federated Framework for Domain Adaptation](https://arxiv.org/abs/2605.04324)
...
### [229] [Learning-based Statistical Refinement for Denoising](https://arxiv.org/abs/2605.04332)
*Rihuan Ke*
...
### [230] [Budgeted LoRA: Distillation as Structured Compute Allocation for Efficient Inference](https://arxiv.org/abs/2605.04341)
...
### [232] [Covariance-Aware Goodness for Scalable Forward-Forward Learning](https://arxiv.org/abs/2605.04346)
...
### [233] [Probing Structural Mathematical Reasoning in Language Models with Algebraic Trapdoors](https://arxiv.org/abs/2605.04352)
*Igor Rivin*
...
### [234] [Efficiently Aligning Language Models with Online Natural Language Feedback](https://arxiv.org/abs/2605.04356)
...
### [235] [Mitigating Label Shift in Tabular In-Context Learning via Test-Time Posterior Adjustment](https://arxiv.org/abs/2605.04363)
...
### [236] [Online Nonstochastic Prediction: Logarithmic Regret via Predictive Online Least Squares](https://arxiv.org/abs/2605.04364)
...
### [237] [Extending Differential Temporal Difference Methods for Episodic Problems](https://arxiv.org/abs/2605.04368)
...
### [238] [$p$-adic Manifold Learning and Benchmark Tasks from Impartial Games](https://arxiv.org/abs/2605.04374)
*Tomoki Mihara*
...
### [239] [FAAST: Forward-Only Associative Learning via Closed-Form Fast Weights for Test-Time Supervised Adaptation](https://arxiv.org/abs/2605.04651)
...
### [240] [Critical Windows of Complexity Control: When Transformers Decide to Reason or Memorize](https://arxiv.org/abs/2605.04396)
*Sarwan Ali*
...
### [241] [Uncertainty-Aware Exploratory Direct Preference Optimization for Multimodal Large Language Models](https://arxiv.org/abs/2605.04874)
...
### [242] [Self-Attention as Transport: Limits of Symmetric Spectral Diagnostics](https://arxiv.org/abs/2605.04893)
...
### [243] [Beyond Rigid Geometries: The Spline-Pullback Metric for Universal Diffeomorphic SPD Representation Learning](https://arxiv.org/abs/2605.04406)
...
### [244] [Reinforcement Learning for Compositional Generalization with Outcome-Level Optimization](https://arxiv.org/abs/2605.04920)
*Xiyan Fu,Wei Liu*
...
### [245] [Demystifying Manifold Constraints in LLM Pre-training](https://arxiv.org/abs/2605.04418)
...
### [246] [Counterfactual identifiability beyond global monotonicity: non-monotone triangular structural causal models](https://arxiv.org/abs/2605.04413)
...
### [247] [Why Geometric Continuity Emerges in Deep Neural Networks: Residual Connections and Rotational Symmetry Breaking](https://arxiv.org/abs/2605.04971)
...
### [248] [FLUID: Continuous-Time Hyperconnected Sparse Transformer for Sink-Free Learning](https://arxiv.org/abs/2605.04421)
...
### [249] [Conceptors for Semantic Steering](https://arxiv.org/abs/2605.04980)
...
### [250] [Self-Induced Outcome Potential: Turn-Level Credit Assignment for Agents without Verifiers](https://arxiv.org/abs/2605.04984)
...
### [251] [Discovering Sparse Counterfactual Factors via Latent Adjustment for Survey-based Community Intervention](https://arxiv.org/abs/2605.04460)
...
### [252] [Continual Knowledge Updating in LLM Systems: Learning Through Multi-Timescale Memory Dynamics](https://arxiv.org/abs/2605.05097)
...
### [253] [CRAFT: Counterfactual-to-Interactive Reinforcement Fine-Tuning for Driving Policies](https://arxiv.org/abs/2605.04470)
...
### [254] [Automated Formal Proofs of Combinatorial Identities via Wilf-Zeilberger Guidance and LLMs](https://arxiv.org/abs/2605.04472)
...
### [255] [Geometry-Aware Neural Optimizer for Shape Optimization and Inversion](https://arxiv.org/abs/2605.04474)
...
### [256] [Data-dependent Exploration for Online Reinforcement Learning from Human Feedback](https://arxiv.org/abs/2605.04477)
...
### [257] [Quadrature-TreeSHAP: Depth-Independent TreeSHAP and Shapley Interactions](https://arxiv.org/abs/2605.04497)
...
### [258] [Gradient Scaling Effects in Adaptive Spectral PINNs for Stiff Nonlinear ODEs](https://arxiv.org/abs/2605.04502)
...
### [259] [FL-Sailer: Efficient and Privacy-Preserving Federated Learning for Scalable Single-Cell Epigenetic Data Analysis via Adaptive Sampling](https://arxiv.org/abs/2605.04519)
...
### [260] [YOTOnet: Zero-Shot Cross-Domain Fault Diagnosis via Domain-Conditioned Mixture of Experts](https://arxiv.org/abs/2605.04528)
...
### [261] [Dream-MPC: Gradient-Based Model Predictive Control with Latent Imagination](https://arxiv.org/abs/2605.04568)
...
### [262] [From Video-to-PDE: Data-Driven Discovery of Nonlinear Dye Plume Dynamics](https://arxiv.org/abs/2605.04535)
...
### [263] [Power Distribution Bridges Sampling, Self-Reward RL, and Self-Distillation](https://arxiv.org/abs/2605.04542)
...
### [264] [HeterSEED: Semantics-Structure Decoupling for Heterogeneous Graph Learning under Heterophily](https://arxiv.org/abs/2605.04594)
...
### [265] [Event-Based Early Warning of Vineyard Disease Risk from Environmental Time Series](https://arxiv.org/abs/2605.04548)
...
### [266] [A Queueing-Theoretic Framework for Stability Analysis of LLM Inference with KV Cache Memory Constraints](https://arxiv.org/abs/2605.04595)
...
### [267] [Library learning with e-graphs on jazz harmony](https://arxiv.org/abs/2605.04622)
...
### [268] [Exact Dual Geometry of SOC-ICNN Value Functions](https://arxiv.org/abs/2605.04722)
...
### [269] [Threshold-Guided Optimization for Visual Generative Models](https://arxiv.org/abs/2605.04653)
...
### [270] [Knowledge-Free Correlated Agreement for Incentivizing Federated Learning](https://arxiv.org/abs/2605.04747)
...
### [271] [Evidence-based anomaly detection in clinical domains](https://arxiv.org/abs/2605.04664)
...
### [272] [Feature importance analysis for patient management decisions](https://arxiv.org/abs/2605.04666)
...
### [273] [ITBoost: Information-Theoretic Trust for Robust Boosting](https://arxiv.org/abs/2605.04671)
...
### [275] [HEXST: Hexagonal Shifted-Window Transformer for Spatial Transcriptomics Gene Expression Prediction](https://arxiv.org/abs/2605.04682)
...
### [276] [Learning Time-Inhomogeneous Markov Dynamics in Financial Time Series via Neural Parameterization](https://arxiv.org/abs/2605.04690)
...
### [277] [Quantile-Free Uncertainty Quantification in Graph Neural Networks](https://arxiv.org/abs/2605.04847)
...
### [278] [Differentiable Chemistry in PINNs for Solving Parameterized and Stiff Reaction Systems](https://arxiv.org/abs/2605.04708)
...
### [279] [SPHERE: Mitigating the Loss of Spectral Plasticity in Mixture-of-Experts for Deep Reinforcement Learning](https://arxiv.org/abs/2605.04712)
...
### [280] [Ensuring Reliability in Programming Knowledge Tracing: A Re-evaluation of Attention-augmented Models and Experimental Protocols](https://arxiv.org/abs/2605.04727)
...
### [282] [Delta-Based Neural Architecture Search: LLM Fine-Tuning via Code Diffs](https://arxiv.org/abs/2605.04903)
...
### [283] [OSAQ: Outlier Self-Absorption for Accurate Low-bit LLM Quantization](https://arxiv.org/abs/2605.04738)
...
### [284] [When Does Gene Regulatory Network Inference Break? A Controlled Diagnostic Study of Causal and Correlational Methods on Single-Cell Data](https://arxiv.org/abs/2605.04930)
...
### [285] [MixINN: Accelerating Plant Breeding by Combining Mixed Models and Deep Learning for Interaction Prediction](https://arxiv.org/abs/2605.04744)
...
### [286] [EP-GRPO: Entropy-Progress Aligned Group Relative Policy Optimization with Implicit Process Guidance](https://arxiv.org/abs/2605.04960)
...
### [287] [AxMoE: Characterizing the Impact of Approximate Multipliers on Mixture-of-Experts DNN Architectures](https://arxiv.org/abs/2605.04754)
...
### [288] [Reliable Modeling of Distribution Shifts via Displacement-Reshaped Optimal Transport](https://arxiv.org/abs/2605.04965)
...
### [289] [Skill Neologisms: Towards Skill-based Continual Learning](https://arxiv.org/abs/2605.04970)
...
### [290] [Bilinear Mamba-Koopman Neural MPC for Varying Dynamics](https://arxiv.org/abs/2605.04793)
...
### [291] [Federated Learning for Early Prediction of EV Charging Demand](https://arxiv.org/abs/2605.04993)
...
### [292] [Unsat Core Prediction through Polarity-Aware Representation Learning over Clause-Literal Hypergraphs](https://arxiv.org/abs/2605.04819)
...
### [293] [Improving FMQA via Initial Training Data Design Considering Marginal Bit Coverage in One-Hot Encoding](https://arxiv.org/abs/2605.04825)
...
### [294] [Trustworthy Federated Label Distribution Learning under Annotation Quality Disparity](https://arxiv.org/abs/2605.04827)
...
### [295] [Preference-Based Self-Distillation: Beyond KL Matching via Reward Regularization](https://arxiv.org/abs/2605.05040)
...
### [296] [Concurrence of Symmetry Breaking and Nonlocality Phase Transitions in Diffusion Models](https://arxiv.org/abs/2605.04830)
...
### [297] [Replay-Based Continual Learning for Physics-Informed Neural Operators](https://arxiv.org/abs/2605.04832)
...
### [298] [Adaptive Learning Strategies for AoA-Based Outdoor Localization: A Comprehensive Framework](https://arxiv.org/abs/2605.05055)
...
### [299] [Bridging Input Feature Spaces Towards Graph Foundation Models](https://arxiv.org/abs/2605.04834)
...
### [300] [Hybrid Iterative Neural Low-Regularity Integrator for Nonlinear Dispersive Equations](https://arxiv.org/abs/2605.04853)
*Zhangyong Liang*
...
### [301] [On the Wasserstein Gradient Flow Interpretation of Drifting Models](https://arxiv.org/abs/2605.05118)
...
### [302] [Adaptive Policy Selection and Fine-Tuning under Interaction Budgets for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2605.05123)
...
### [303] [Regime-Conditioned Evaluation in Multi-Context Bayesian Optimization](https://arxiv.org/abs/2605.04895)
*Noel Thomas*
...
### [305] [A geometric relation of the error introduced by sampling a language model's output distribution to its internal state](https://arxiv.org/abs/2605.04899)
*Albert F. Modenbach*
...
### [306] [Superposition Is Not Necessary: A Mechanistic Interpretability Analysis of Transformer Representations for Time Series Forecasting](https://arxiv.org/abs/2605.05151)
*Alper Yıldırım*
...
### [307] [Cross-Model Consistency of Feature Importance in Electrospinning: Separating Robust from Model-Dependent Features](https://arxiv.org/abs/2605.04905)
...
### [308] [Breaking the Quality-Privacy Tradeoff in Tabular Data Generation via In-Context Learning](https://arxiv.org/abs/2605.04911)
...
### [309] [Koopman Identification of Nonlinear Systems via Reservoir Liftings](https://arxiv.org/abs/2605.04917)
...
### [310] [Training-Time Batch Normalization Reshapes Local Partition Geometry in Piecewise-Affine Networks](https://arxiv.org/abs/2605.04946)
...
### [311] [Adaptive Inverted-Index Routing for Granular Mixtures-of-Experts](https://arxiv.org/abs/2605.04952)
...
### [312] [Order-based Rehearsal Learning](https://arxiv.org/abs/2605.04955)
...
### [313] [KernelBench-X: A Comprehensive Benchmark for Evaluating LLM-Generated GPU Kernels](https://arxiv.org/abs/2605.04956)
...
### [314] [Delving into Non-Exchangeability for Conformal Prediction in Graph-Structured Multivariate Time Series](https://arxiv.org/abs/2605.04957)
...
### [315] [Adaptivity Under Realizability Constraints: Comparing In-Context and Agentic Learning](https://arxiv.org/abs/2605.04995)
...
### [316] [DualTCN: A Physics-Constrained Temporal Convolutional Network for 2 Time-Domain Marine CSEM Inversion](https://arxiv.org/abs/2605.04997)
...
### [317] [Learned Neighbor Trust for Collaborative Deployment in Model-Agnostic Decentralized Learning](https://arxiv.org/abs/2605.05009)
...
### [318] [Graph-SND: Sparse Aggregation for Behavioral Diversity in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.05020)
*Shawn Ray*
...
### [319] [CuBridge: An LLM-Based Framework for Understanding and Reconstructing High-Performance Attention Kernels](https://arxiv.org/abs/2605.05023)
...
### [320] [The Predictive-Causal Gap: An Impossibility Theorem and Large-Scale Neural Evidence](https://arxiv.org/abs/2605.05029)
*Kejun Liu*
...
### [321] [Full-chip CMP modelling based on Fully Convolutional Network leveraging White Light Interferometry](https://arxiv.org/abs/2605.05062)
...
### [322] [Provable imitation learning for control of instability in partially-observed Vlasov--Poisson equations](https://arxiv.org/abs/2605.05081)
...
### [323] [Order Matters: Improving Domain Adaptation by Reordering Data](https://arxiv.org/abs/2605.05084)
...
### [324] [Gated Multimodal Learning for Interpretable Property Energy Performance Prediction and Retrofit Scenario Analysis](https://arxiv.org/abs/2605.05088)
...
### [325] [Unified Framework of Distributional Regret in Multi-Armed Bandits and Reinforcement Learning](https://arxiv.org/abs/2605.05102)
...
### [326] [Rollout Pass-Rate Control: Steering Binary-Reward RL Toward Its Most Informative Regime](https://arxiv.org/abs/2605.05112)
...
### [327] [How Long Does Infinite Width Last? Signal Propagation in Long-Range Linear Recurrences](https://arxiv.org/abs/2605.05113)
*Mariia Seleznova*
...
### [328] [Manifold Steering Reveals the Shared Geometry of Neural Network Representation and Behavior](https://arxiv.org/abs/2605.05115)
...
### [329] [On the Hardness of Junking LLMs](https://arxiv.org/abs/2605.05116)
...
### [330] [Physiologically Grounded Driver Behavior Classification: SHAP-Driven Elite Feature Selection and Hybrid Gradient Boosting for Multimodal Physiological Signals](https://arxiv.org/abs/2605.05120)
...
### [331] [Conditional outlier detection for clinical alerting](https://arxiv.org/abs/2605.05124)
...
### [332] [Transformed Latent Variable Multi-Output Gaussian Processes](https://arxiv.org/abs/2605.05133)
...
### [333] [Low-Cost Black-Box Detection of LLM Hallucinations via Dynamical System Prediction](https://arxiv.org/abs/2605.05134)
...
### [334] [Human-AI Co-Mentorship in Project-Based Learning: A Case Study in Financial Forecasting](https://arxiv.org/abs/2605.05144)
...
### [335] [Understanding In-Context Learning for Nonlinear Regression with Transformers: Attention as Featurizer](https://arxiv.org/abs/2605.05176)
...
### [336] [Estimating the expected output of wide random MLPs more efficiently than sampling](https://arxiv.org/abs/2605.05179)
