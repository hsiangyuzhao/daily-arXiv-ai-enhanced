...
TL;DR: Granulon 是一种基于 DINOv3 的新型多模态大语言模型，通过引入文本条件的粒度控制器和自适应标记聚合模块，实现从像素到细粒度再到粗粒度的统一推理。该模型在保持像素级感知能力的同时增强了语义抽象能力，显著提升多粒度理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于 CLIP 的视觉编码器侧重全局语义对齐，但缺乏细粒度视觉理解；而 DINOv3 虽具备强像素级感知，却缺少粗粒度语义抽象能力，导致多粒度推理受限。因此需要一种能够动态调节视觉抽象层级的新方法。

Method: 提出 Granulon 模型，包含两个核心组件：1）文本条件的粒度控制器，根据文本语义范围动态调整视觉抽象级别；2）自适应标记聚合模块，执行粒度引导的池化与关系感知聚类，生成紧凑且语义丰富的视觉标记。

Result: 在相同设置下，Granulon 相比现有方法提升准确率约 30%，降低幻觉约 20%，并实现单次前向传播内的“像素-细粒度-粗粒度”统一推理。

Conclusion: Granulon 有效融合了 DINOv3 的像素级感知优势与可调节的语义抽象能力，为多模态大语言模型提供了更强大、更灵活的多粒度推理机制。
...
TL;DR: 提出了一种针对3D高斯点云的鲁棒且不可见的水印框架，通过分离写入位置与质量保持机制，实现高效水印嵌入与视觉保真度。采用三专家模块选择载体，安全与预算感知门控（SBAG）优化比特鲁棒性与比特率约束，结合通道分组掩码控制梯度传播，提升细节保留与抗干扰能力。该方法在压缩、噪声等常见图像失真下表现优异，相比现有方法提升PSNR 0.83 dB，比特准确率提高1.24%。此外，解耦微调支持逐高斯属性分析，实现可审计的可解释性。


<details>
  <summary>Details</summary>
Motivation: 随着3D高斯点云成为交互式3D资产的标准表示，亟需一种鲁棒且不可察觉的水印技术以保护版权和追踪来源。现有方法在鲁棒性与视觉质量之间难以平衡，且缺乏可解释性，因此需要一种原生于表示结构的水印框架。

Method: 提出Trio-Experts模块用于高斯原始数据中提取载体先验；设计安全与预算感知门控（SBAG）动态分配高斯作为水印载体，兼顾比特鲁棒性与比特率限制；引入通道分组掩码控制梯度传播，限制参数更新，修复局部伪影并保留高频细节；通过解耦微调实现逐高斯级的可解释性分析。

Result: 在多种图像失真（如压缩、噪声）下保持强鲁棒性，实现视图一致性水印持久性；相比现有方法，提升PSNR 0.83 dB，比特准确率提高1.24%；支持可解释的水印载体定位与选择原因分析。

Conclusion: 所提框架实现了3D高斯点云水印在鲁棒性、视觉保真度与可解释性之间的良好平衡，为交互式3D内容的安全管理提供了有效解决方案。
...
TL;DR: 本研究提出一种基于计算机视觉和YOLOv8目标检测模型的低成本、易实施的智能停车系统。通过逆透视映射（IPM）融合四个摄像头的图像，实现对空闲车位的精准识别，并构建3D停车环境可视化，以指导用户。该系统在高密度城市区域具有较强适应性和实用性。


<details>
  <summary>Details</summary>
Motivation: 传统传感器技术在智能停车系统中存在精度受限、部署复杂等问题，难以应对城市高密度环境下的动态变化需求。为提升系统效率与灵活性，需要更先进的车辆分配机制。计算机视觉因其高精度和可扩展性成为替代方案。

Method: 采用多视角摄像头结合逆透视映射（IPM）技术，将四路视频流融合为统一视图；利用YOLOv8进行车辆与空位的目标检测；通过3D Cartesian坐标系建模，实时呈现可用停车位的空间分布。

Result: 系统实现了对停车场内空闲车位的高精度识别与可视化展示，显著提升了停车引导效率，具备良好的实时性与可扩展性，适用于复杂城市环境。

Conclusion: 所提出的基于计算机视觉的智能停车系统有效克服了传统传感器系统的局限性，提供了一种高效、灵活且成本可控的解决方案，为未来智慧城市建设中的交通管理提供了有力支持。
...
TL;DR: 该研究提出了一种多癌种肿瘤定位模型MuCTaL，基于79,984个非重叠图像块在四种癌症（黑色素瘤、肝细胞癌、结直肠癌和非小细胞肺癌）上进行训练，采用DenseNet169与迁移学习，实现了0.97的验证集瓦片级ROC-AUC，并在胰腺导管腺癌独立队列中达到0.71的性能，展现出跨癌种泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习肿瘤检测模型在特定癌种上训练后，跨癌种泛化能力较差，亟需一种能够在多种癌种间保持高精度且具备可扩展性的通用肿瘤定位方法。

Method: 采用DenseNet169架构，通过迁移学习在四个不同癌种的大量图像块上进行平衡训练，构建多癌种肿瘤定位模型；开发了可扩展的推理流程，生成与现有数字病理工具兼容的空间肿瘤概率热图。

Result: 模型在四类训练癌种上达到0.97的瓦片级ROC-AUC，对未见过的胰腺导管腺癌数据集也表现出0.71的性能，证明其具有良好的跨癌种泛化能力；公开了代码与模型资源。

Conclusion: 通过在有限规模下对多种癌种进行平衡训练，可实现高性能且具备强泛化能力的肿瘤定位模型，为跨癌种空间分析与分子研究提供有效工具。
...
TL;DR: HECTOR是一种生成式视频合成框架，支持细粒度的组合控制，通过静态图像和/或动态视频的混合参考条件，允许用户显式指定每个元素的轨迹（位置、尺度、速度），从而在保持参考图像高保真度的同时，生成满足复杂时空约束的连贯视频。实验表明，相比现有方法，HECTOR在视觉质量、参考保留和运动可控性方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型通常以整体方式合成场景，缺乏对视觉元素组合的显式控制能力，难以实现精细的时空编辑与操控。

Method: 提出HECTOR框架，采用混合参考条件（静态图像和/或动态视频），结合用户指定的元素轨迹（位置、尺度、速度），实现对视频中各物体运动的精确控制，并保证生成视频的时空一致性与参考保真度。

Result: 在多个基准上验证了HECTOR的有效性，其生成视频在视觉质量、参考保留程度和运动可控性方面均优于现有方法。

Conclusion: HECTOR实现了对视频生成过程中视觉元素组合的细粒度控制，为复杂动态场景的可控生成提供了有效解决方案。
...
TL;DR: 该研究系统评估了三种视觉-语言模型（Dolphins、OmniDrive、LeapVAD）在自动驾驶场景下对物理对抗攻击的鲁棒性，发现所有模型均存在严重漏洞，表现为多帧持续失效和关键目标检测性能下降，揭示了现有VLM架构在安全关键应用中对抗威胁应对能力不足。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型在自动驾驶中的应用日益广泛，但其对物理对抗攻击的鲁棒性尚未得到充分研究，亟需系统性评估以识别潜在安全隐患。

Method: 采用黑盒优化结合语义同质化方法进行公平比较，在CARLA仿真环境中实施可物理实现的贴纸攻击，全面评估模型表现。

Result: 所有模型均表现出显著脆弱性，出现持续多帧失败及关键物体检测性能严重下降，且不同架构呈现出各异的漏洞模式。

Conclusion: 现有视觉-语言模型架构在面对物理对抗攻击时表现出明显不足，难以满足自动驾驶等高安全性应用场景的需求，亟需改进设计以增强对抗鲁棒性。
...
TL;DR: 本文提出视觉查询分割（VQS），一种新型的视觉查询定位范式，旨在通过外部视觉查询在未剪辑视频中分割出目标对象的所有像素级出现。相比现有仅定位目标最后一次出现的边界框方法，VQS实现更全面（所有目标出现）和更精确（像素级掩码）的定位，更具实际应用价值。为此，作者构建了首个专为VQS设计的大规模基准VQS-4K，包含4,111个视频、超过130万帧，覆盖222类物体，每个视频配有来自外部帧的视觉查询及其目标掩码，并标注了时空掩码片段。所有数据均经过人工精细标注与迭代优化，确保高质量。此外，提出了VQ-SAM方法，基于SAM 2，利用视频中的目标特定与背景干扰线索，通过多阶段框架与自适应记忆生成（AMG）模块逐步演化记忆，显著提升性能。在VQS-4K上的实验表明，VQ-SAM优于所有现有方法，验证其有效性。研究团队将公开基准、代码与结果，推动该领域发展。


<details>
  <summary>Details</summary>
Motivation: 现有视觉查询定位（VQL）方法仅能定位目标在视频中的最后一次出现，且使用边界框，难以满足现实场景中对全面、精准定位的需求。因此需要一种能够识别所有目标出现并提供像素级分割的新范式，以提升实用性与应用潜力。

Method: 提出VQ-SAM方法，扩展SAM 2，引入目标特定与背景干扰线索，采用多阶段框架结合自适应记忆生成（AMG）模块，通过逐步演化记忆实现对目标的精准分割。同时构建了大规模基准VQS-4K，用于支持该任务的研究。

Result: 在自建的VQS-4K基准上，VQ-SAM展现出优异性能，显著超越现有方法，验证了其有效性和先进性。

Conclusion: 本文提出的VQS范式及配套的VQS-4K基准与VQ-SAM方法，推动了视觉查询定位从粗略定位向全面、高精度分割的发展，为未来研究与实际应用奠定了基础。
...
TL;DR: 提出MedCBR框架，结合临床指南与视觉-语言和推理模型，提升医学影像诊断的可解释性与准确性。通过多任务训练联合对齐图像特征、概念与病理，并生成结构化临床叙事，实现从图像分析到决策的端到端透明推理。


<details>
  <summary>Details</summary>
Motivation: 在医学影像中，尽管概念瓶颈模型（CBMs）提供了可解释性，但其离散概念表示常忽略临床背景如诊断指南和专家经验，影响复杂病例中的可靠性。需要更贴近临床实践的可解释框架。

Method: 将临床标注描述转化为符合指南的文本；采用多任务学习目标，包括跨模态对比对齐、概念监督和诊断分类，联合建模图像特征、概念与病理；使用推理模型将预测结果转换为结构化临床叙述，模拟基于指南的专家推理过程。

Result: 在超声和乳腺钼靶数据集上分别达到94.2%和84.0%的AUROC，非医疗数据集上达86.1%准确率，显著优于现有方法，在诊断与概念层面均表现优异。

Conclusion: MedCBR通过融合临床指南与多模态推理，显著提升了医学图像分析的可解释性与决策可靠性，构建了从图像到临床决策的端到端透明桥梁。
...
TL;DR: MEGC 2026 introduces two tasks: ME-VQA for short video question answering using MLLMs/LVLMs, and ME-LVQA for long-video reasoning with temporal and subtle ME detection, both aiming to advance multimodal understanding of micro-expressions.


<details>
  <summary>Details</summary>
Motivation: To leverage the multimodal reasoning capabilities of MLLMs and LVLMs for improved facial micro-expression analysis in realistic, high-stakes scenarios, addressing challenges in understanding brief and long-duration emotional cues.

Method: Utilize multimodal large language models (MLLMs) and large vision-language models (LVLMs) for visual question answering on short (ME-VQA) and long (ME-LVQA) video sequences, enabling diverse question types and temporal reasoning.

Result: Participants are required to submit results to a public leaderboard, fostering benchmarking and advancement in ME recognition, spotting, and generation through cutting-edge multimodal AI.

Conclusion: MEGC 2026 pushes the frontier of micro-expression analysis by integrating advanced multimodal models into real-world video understanding tasks, promoting innovation in emotion detection and interpretation.
...
TL;DR: TIDE是一种无需训练的文本到图像（T2I）外推方法，可实现任意分辨率和宽高比的图像生成，且无额外采样开销。它通过引入文本锚定机制纠正文本与图像标记之间的不平衡，并采用动态温度控制机制消除伪影，有效解决扩散变换器（DiT）在高分辨率生成时因注意力稀释导致的结构退化问题。


<details>
  <summary>Details</summary>
Motivation: DiT在生成高分辨率图像时面临注意力稀释问题，导致结构退化和提示信息丢失，现有方法虽尝试增强注意力分布但难以保留细粒度语义细节并引入明显伪影，因此需要一种无需训练、高效且高质量的外推方法。

Method: 提出TIDE方法，包含两个核心组件：1）文本锚定机制，用于纠正文本与图像令牌间的不平衡；2）动态温度控制机制，利用扩散过程中的谱进展模式来减少生成伪影。该方法不依赖额外训练，可直接应用于现有模型。

Result: 实验表明，TIDE在多种基准上均表现出卓越的高分辨率外推能力，生成图像质量高，结构清晰，能有效保留语义细节，且与当前最先进的方法无缝集成。

Conclusion: TIDE成功实现了无需训练的高分辨率、任意宽高比文本到图像生成，克服了传统DiT在高分辨率下的注意力稀释问题，为大规模图像生成提供了高效可靠的解决方案。
...
TL;DR: PathoScribe 是一个统一的检索增强型大语言模型框架，旨在将静态的病理学档案转化为可搜索、可推理的动态知识库。它支持自然语言病例检索、自动化队列构建、临床问题问答、免疫组化（IHC）面板推荐和受提示控制的报告转换。在7万份多机构手术病理报告上评估，PathoScribe 在自然语言病例检索中实现了完美的 Recall@10，且在基于检索的推理方面获得平均4.56/5的评分。系统能根据自由文本纳入标准在几分钟内自动构建研究级队列（平均9.2分钟），与人工评审者达成91.3%的一致性，且无遗漏合格病例，显著降低了时间和成本。该工作为将数字病理档案从被动存储转变为主动临床智能平台提供了可扩展的基础。


<details>
  <summary>Details</summary>
Motivation: 当前病理学虽是现代诊断和癌症治疗的核心，但其宝贵的知识资源——数以百万计的叙事性报告——仍难以被有效利用。尽管病理流程正在快速数字化，但缺乏有效的检索与推理机制，导致数据成为静态存档而非临床决策支持工具。亟需一种能在实时诊断中查询历史相似病例的能力，以实现知识驱动的精准医疗。

Method: 提出 PathoScribe，一个基于检索增强的大语言模型框架，整合自然语言理解、信息检索与生成式推理能力，支持多种临床与科研任务，包括自然语言病例搜索、自由文本条件下的自动化队列构建、临床问答、IHC推荐和报告格式转换。系统通过联合训练与检索机制，确保生成内容既准确又可追溯。

Result: 在7万份多机构手术病理报告上验证，PathoScribe 实现了自然语言检索的 Recall@10 达到100%，推理质量得分高达4.56/5；自动化队列构建平均耗时仅9.2分钟，与人工一致性达91.3%，且无漏筛，相比传统人工审阅效率提升数个数量级。

Conclusion: PathoScribe 成功将数字病理档案从被动存储转变为活跃的临床智能平台，为未来病理知识的高效利用和精准医疗提供可扩展的技术基础。
...
TL;DR: 本文提出BiCLIP框架，利用少量锚点样本通过可学习的双线性变换实现跨域视觉-语言模型对齐，显著提升少样本分类性能。该方法基于图像特征在不同领域间存在规范几何变换的假设，具有极简结构和低参数量，已在11个基准数据集上验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型虽具备零样本能力，但在特定领域适应中仍面临挑战。本文基于理论发现——独立训练的VLMs间存在规范变换关系，进一步提出跨域也遵循类似几何规律，可通过少量样本作为锚点恢复该变换，从而实现高效领域自适应。

Method: 提出BiCLIP框架，通过引入双线性变换对多模态特征进行针对性调整，利用少样本中的标签样本作为锚点估计跨域几何变换，实现跨模态特征对齐。该方法设计简洁，仅需少量可学习参数。

Result: 在11个标准基准数据集（如EuroSAT、DTD、FGVCAircraft）上，BiCLIP均达到或超过现有方法的性能，表现持续领先；同时实证验证了所学变换的正交性和角度分布特性，支持结构化对齐是鲁棒领域适应的关键。

Conclusion: BiCLIP通过挖掘跨域特征间的规范几何关系，以极小代价实现高效的多模态对齐与领域适应，在少样本场景下展现出强大潜力，为视觉-语言模型的实用化部署提供了新思路。
...
TL;DR: 本文提出了首个无示例的持续学习基准用于音频-视觉分割（AVS），并设计了名为ATLAS的强基线模型，通过音频引导的前融合条件调节视觉特征，同时引入低秩锚定（LRA）方法缓解灾难性遗忘。实验表明该方法在多种持续学习场景下表现优异，为长期音频-视觉感知奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 真实世界环境动态变化导致音频和视觉分布随时间演变，现有AVS系统假设静态训练设置，难以应对动态变化，因此需要新的持续学习框架来提升模型在长期使用中的适应能力。

Method: 提出一个无示例的持续学习基准，包含四种学习协议；设计ATLAS模型，利用投影音频上下文在跨模态注意力前调制视觉特征通道；引入低秩锚定（LRA）机制，基于损失敏感性稳定更新权重，防止灾难性遗忘。

Result: 在多个持续学习场景中表现出色，验证了所提方法的有效性与鲁棒性，为音频-视觉感知的长期学习提供了新范式。

Conclusion: 本文构建了首个面向音频-视觉分割的持续学习基准，并提出了有效的解决方案，推动了音频-视觉感知向终身学习方向发展。
...
TL;DR: SVG-EAR提出一种无需训练的线性补偿分支，通过语义聚类和质心近似来恢复稀疏注意力中被跳过的块的贡献，同时引入误差感知路由机制，选择补偿误差与成本比最高的块进行精确计算，从而在不损失生成质量的前提下显著提升视频扩散模型的效率。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法要么丢弃未计算的注意力块导致信息损失，要么依赖学习的预测器近似它们，带来训练开销和输出分布偏移问题。本文旨在设计一种无需训练、能有效恢复缺失信息的稀疏注意力方法。

Method: 首先对注意力块进行语义聚类，利用聚类中心（质心）近似每个块中的键值；在此基础上引入参数无关的线性补偿分支；进一步设计轻量级探测器估计各块的补偿误差，并基于误差-成本比进行误差感知路由，仅精确计算最需补偿的块。

Result: 在Wan2.2和HunyuanVideo数据集上，SVG-EAR实现了最高1.77×和1.93×的加速，同时保持了高达29.759和31.043的PSNR值，显著优于已有方法，在质量-效率权衡上建立了新的帕累托前沿。

Conclusion: SVG-EAR通过无训练的质心补偿与误差感知路由，有效解决了稀疏注意力中的信息损失问题，在保持生成质量的同时大幅提升了视频扩散模型的推理效率，为高效视频生成提供了新范式。
...
TL;DR: SkipGS提出一种视图自适应的反向传播门控机制，通过跳过损失已趋于平稳的视图的反向传播，显著降低3D高斯点阵（3DGS）后密集化阶段的训练时间。在Mip-NeRF 360数据集上，相比原始3DGS，SkipGS将端到端训练时间减少23.1%，其中后密集化阶段提速达42.0%，且保持相近的重建质量。该方法无需修改渲染器、表示或损失函数，具备即插即用性，可与其他效率优化策略兼容以实现叠加加速。


<details>
  <summary>Details</summary>
Motivation: 3D高斯点阵（3DGS）虽能实现实时新视角合成，但其后密集化阶段的训练成本高昂，主要由于反向传播开销过大。研究发现，大量采样视图的损失已接近平台期，继续反向传播带来的梯度收益微乎其微，存在显著更新冗余。因此，亟需一种高效机制来减少无效计算。

Method: 提出SkipGS，引入视图自适应的反向传播门控机制：始终执行前向传播以更新每视图的损失统计；当某视图的当前损失与近期基准一致时，选择性跳过反向传播；同时设置最小反向传播预算以保障优化稳定性。

Result: 在Mip-NeRF 360数据集上，相比3DGS，SkipGS将端到端训练时间减少23.1%，后密集化阶段训练时间减少42.0%，重建质量基本相当。该方法不改变原有模型结构，具备良好的兼容性和扩展性。

Conclusion: SkipGS通过智能跳过低收益反向传播，有效提升了3DGS后密集化阶段的训练效率，是一种轻量级、即插即用的加速方案，为3D高斯点阵的高效训练提供了实用且通用的优化路径。
...
TL;DR: 提出了一种基于扩散模型的防伪认证框架，利用原始二值模板、打印的CDP和打印机身份表征，通过多类打印机分类实现细粒度设备特征捕捉，改进ControlNet以支持条件噪声预测，显著优于传统方法且能泛化到未见伪造类型。


<details>
  <summary>Details</summary>
Motivation: 传统防伪系统因高分辨率打印扫描设备和生成式深度学习技术的发展而面临挑战，难以区分高质量伪造品与真品。

Method: 提出基于扩散模型的认证框架，结合原始二值模板、打印的复制检测图案（CDP）及打印机身份表征，通过空间和文本条件控制，将认证问题建模为多类打印机分类任务，并扩展ControlNet以实现类条件噪声预测。

Result: 在Indigo 1 x 1 Base数据集上，该方法优于传统相似性度量和先前深度学习方法；模型具备对训练中未见伪造类型的泛化能力。

Conclusion: 所提出的扩散模型框架有效提升了防伪系统的鲁棒性与泛化能力，能够应对日益复杂的伪造威胁。
...
TL;DR: WS-Net 是一种针对高光谱解混中弱信号被掩盖问题设计的深度解混框架，结合状态空间建模与弱信号注意力融合，通过多分辨率小波融合编码器和Mamba状态空间分支捕捉长程依赖，利用弱信号注意力分支增强低相似性光谱特征，并通过可学习门控机制自适应融合表示。解码器采用基于KL散度的正则化以增强主端元与弱端元之间的可分离性。在模拟及两个真实数据集（合成数据集、Samson、Apex）上的实验表明，该方法在均方根误差（RMSE）和角距离（SAD）上分别实现了最高55%和63%的降低，且在低信噪比条件下对弱端元仍保持稳定精度，展现出优异的鲁棒性与计算效率。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像中的弱光谱响应常被主导端元和传感器噪声所掩盖，导致丰度估计不准确。现有方法难以有效区分和恢复弱信号，因此亟需一种能够抑制信号坍缩、增强弱信号识别能力的新型解混框架。

Method: 提出WS-Net框架，包含多分辨率小波融合编码器、Mamba状态空间分支用于高效建模长程依赖、弱信号注意力分支以选择性增强低相似性光谱特征，以及可学习门控机制实现双路径表示融合；解码器引入基于KL散度的正则化项，强化主端元与弱端元的可分离性。

Result: 在三个数据集（一个模拟数据集、两个真实数据集：Samson和Apex）上，相比六种先进基线方法，WS-Net在RMSE和SAD指标上分别取得最高55%和63%的降低，在低信噪比环境下对弱端元仍保持高精度，表现出强鲁棒性和计算效率。

Conclusion: WS-Net成功解决了高光谱解混中弱信号坍缩问题，为弱信号检测与恢复提供了一个高效、稳健的新范式，具备成为该领域基准方法的潜力。
...
TL;DR: 本文提出一种增强的双模型YOLOv8框架，用于智能火灾检测与邻近风险评估，结合火情分割与周围物体检测，通过像素距离计算生成定量风险评分，实现从检测到风险预警的升级。


<details>
  <summary>Details</summary>
Motivation: 传统视觉监控仅能完成火灾检测，缺乏对潜在危害的优先级判断；本研究旨在提升系统在复杂环境中的感知能力，实现火灾风险的量化评估与实时预警。

Method: 采用主模型YOLOv8进行火情与烟雾实例分割，辅以COCO预训练的检测模型识别人、车、基础设施等周边对象；通过像素-米尺度转换计算火源与目标间的距离，并融合火情证据、对象脆弱性及距离暴露度生成风险评分。

Result: 模型在9,860张标注图像上训练，精度、召回率和F1值均超过90%，mAP@0.5高于91%；系统可输出含火源位置、物体识别、距离估算与风险信息的可视化结果，支持态势感知。

Conclusion: 所提框架具备高精度、轻量化特性，适用于工业及资源受限场景，实现了从被动检测向主动风险评估的演进，具有良好的实际部署潜力。
...
TL;DR: GST-VLA introduces a Gaussian Spatial Tokenizer (GST) that converts depth and semantic features into 128 anisotropic 3D Gaussians with geometric and confidence attributes, enabling spatial attention to focus on salient regions. It also employs 3D Depth-Aware Chain-of-Thought (DA-CoT) reasoning to guide structured intermediate thoughts in 3D space, integrated via cross-attention. A flow-matching action expert decodes 7-DoF actions using dual cross-attention. Trained in stages with composite loss, GST-VLA achieves state-of-the-art performance on LIBERO and SimplerEnv, with ablations confirming synergistic improvements.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models use 2D patch tokens lacking geometric structure, limiting their ability to reason about 3D spatial relationships. This work aims to enhance geometric precision and spatial reasoning by introducing a 3D-aware tokenization and structured reasoning framework.

Method: GST-VLA uses a Gaussian Spatial Tokenizer (GST) to generate 128 3D Gaussian primitives from depth and semantic features, encoding surface orientation and confidence. Spatial attention with learned queries focuses on salient regions. DA-CoT reasoning supervises four intermediate 3D spatial thoughts via a cross-attention mechanism. A flow-matching action expert with mixture-of-experts decodes 7-DoF actions conditioned on both VLM states and DA-CoT outputs through dual cross-attention.

Result: GST-VLA achieves 96.4% success rate on LIBERO (+2.0% over baseline) and 80.2% on SimplerEnv (+5.4%), demonstrating significant gains. Ablations confirm each component contributes independently and synergistically, especially on precision-demanding tasks.

Conclusion: GST-VLA advances visual-language-action modeling by integrating 3D geometric awareness through Gaussian primitives and structured 3D reasoning, significantly improving performance on complex manipulation tasks.
...
TL;DR: OmniEdit 是一个无需训练的框架，用于唇同步和音视频编辑，通过替换编辑序列并消除生成过程中的随机性，实现稳定、无偏的输出，显著降低计算开销和数据需求。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于预训练模型的监督微调，导致计算成本高且数据需求大，亟需一种无需训练的高效解决方案。

Method: 将FlowEdit中的编辑序列替换为目标序列，实现无偏估计；去除生成过程中的随机性，确保编辑轨迹平滑稳定。

Result: 实验结果表明，OmniEdit在唇同步和音视频编辑任务中均表现出色，具有高效性与鲁棒性。

Conclusion: OmniEdit 提供了一种无需训练的高效音视频编辑方案，为多模态学习应用提供了新思路。
...
TL;DR: 本文提出一种新的物理合理视频生成框架，通过物理驱动的事件链推理和过渡感知跨模态提示技术，实现因果连贯的动态事件序列生成。该方法将物理现象分解为基本事件单元，并利用物理公式约束确保因果确定性；同时通过时间对齐的视觉-语言提示保持事件间连续性。在PhyGenBench和VideoPhy基准上的实验表明，该方法在多个物理领域均显著提升视频生成的物理合理性。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型难以有效建模物理现象的因果演化过程，主要受限于缺乏对动态因果关系的条件建模机制，导致生成结果多为静态、孤立的物理场景。尽管已有方法尝试借助大语言模型注入常识知识，但无法充分捕捉物理事件的时间演进规律。

Method: 提出两个核心模块：(1) 物理驱动的事件链推理（Physics-driven Event Chain Reasoning），将输入描述分解为多个基础事件单元，结合思维链推理与物理公式作为约束以消除因果歧义；(2) 过渡感知跨模态提示（TCP），将因果事件单元转化为时间对齐的视觉-语言提示，通过渐进式编辑合成关键帧并生成连贯叙事。

Result: 在PhyGenBench和VideoPhy两个基准上，所提框架在物理合理性、事件连贯性和视觉质量方面均优于现有方法，尤其在复杂物理场景如碰撞、弹跳、流体运动等任务中表现突出。

Conclusion: 本研究通过引入因果推理与时间对齐提示机制，实现了从静态提示到动态因果事件序列的转变，显著提升了视频生成的物理真实性，为构建具有常识理解能力的生成系统提供了新范式。
...
TL;DR: 提出MedKCO方法，通过双层课程学习和自适应对比损失，提升医学视觉-语言预训练模型在分布外情况下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLP方法同时学习简单和复杂概念，导致特征表示不优，尤其在分布偏移下表现差。

Method: 设计双层课程学习，基于诊断敏感性和类内样本代表性排序预训练数据；引入自适应非对称对比损失，动态调整预训练目标参与度。

Result: 在多个医学影像场景的下游任务中，显著优于所有基线方法。

Conclusion: MedKCO通过认知驱动的课程设计和自适应学习机制，有效提升了医学VLP模型的泛化能力。
...
TL;DR: 本文提出一种运动分解框架，将复杂运动分解为静止、刚体运动和非刚体运动三类，采用先规划后生成的范式。规划阶段通过运动图推理每帧中各实例的形状与位置变化，缓解提示中的语义模糊；生成阶段则解耦地调制不同运动类别，通过引导分支稳定静态区域外观、保持刚体几何结构并规范局部非刚体变形。该框架与模型无关，可无缝集成到多种扩散模型架构中，在真实世界基准上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注语义绑定，忽视了提示中指定的多样化运动类别，导致运动合成不准确或不自然。

Method: 提出一种运动因子化框架，包含规划与生成两个模块：规划阶段基于运动图推理实例的逐帧变化；生成阶段通过条件引导分支解耦处理三种运动类型。

Result: 在真实世界基准上实现了出色的运动合成性能，验证了框架的有效性与通用性。

Conclusion: 所提框架能够有效理解并生成多样化运动，具备良好的泛化能力，适用于多种扩散模型架构。
...
TL;DR: 提出了一种基于Transformer的视觉-语言医学图像检索框架，用于皮肤癌诊断。该框架通过层次化查询表示和全局-局部联合对齐，结合图像与文本描述，提升检索精度。实验在Derm7pt数据集上验证了其优越性，支持临床实际应用。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像检索方法难以有效融合图像与文本信息，尤其在复杂临床查询（如结合病变图像与描述性文本）中表现不足，亟需更精确的多模态检索框架以支持诊断决策与教育。

Method: 提出一种基于Transformer的框架，学习分层的视觉-语言查询表示；采用多空间注意力掩码实现局部区域对齐，同时引入全局语义对齐；最终通过领域知识指导的凸加权计算相似度，强化关键局部证据并保持整体一致性。

Result: 在公开的Derm7pt数据集上，所提方法在多个评估指标上均显著优于现有最先进方法，展现出更高的检索准确率与临床实用性。

Conclusion: 该框架有效提升了皮肤癌医学图像检索性能，具备良好的临床部署潜力，为辅助诊断与医学知识管理提供了有力工具。
...
TL;DR: 本文提出PRLF框架，用于在模态缺失不确定情况下的多模态情感分析。通过自适应模态可靠性估计器（AMRE）动态评估各模态可靠性，并利用渐进式交互模块（ProgInteract）对齐非主导模态与主导模态，提升跨模态一致性并抑制噪声。在CMU-MOSI、CMU-MOSEI和SIMS数据集上的实验表明，该方法在跨模态和同模态缺失场景下均优于现有先进方法，具备强鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 真实世界中多模态情感分析常面临模态缺失问题，如噪声、硬件故障或隐私限制，而现有方法通常假设所有模态完整，导致特征错位，直接融合会破坏已学习的完整模态表示。因此需要一种能应对不完全模态输入的鲁棒框架。

Method: 提出渐进式表示学习框架PRLF，包含自适应模态可靠性估计器（AMRE）和渐进式交互模块（ProgInteract）。AMRE结合识别置信度与Fisher信息动态评估模态可靠性，确定主导模态；ProgInteract则迭代对齐其他模态至主导模态，增强一致性并抑制噪声。

Result: 在CMU-MOSI、CMU-MOSEI和SIMS数据集上，PRLF在多种缺失模式下均取得最优性能，显著优于当前主流方法，证明其在处理不完整模态时具有优异的鲁棒性与泛化能力。

Conclusion: PRLF能够有效应对多模态情感分析中的不确定性模态缺失问题，通过可靠性的动态评估与渐进式对齐机制，实现更稳定、准确的情感推断，为实际应用提供了强有力的技术支持。
...
TL;DR: QUSR提出一种新的基于扩散的图像超分辨率模型，通过引入质量感知先验（QAP）和不确定性引导噪声生成（UNG）模块，有效应对真实场景中未知且空间非均匀退化的挑战。UNG模块自适应调节噪声强度，在高不确定性区域（如边缘和纹理）施加强扰动以恢复复杂细节，同时在低不确定性区域（如平坦区域）减少噪声以保护原始信息。QAP利用先进的多模态大语言模型（MLLM）生成可靠的图像质量描述，提供可解释的质量先验。实验表明，QUSR在真实场景下能生成高保真、高真实感的图像。代码已开源。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的图像超分辨率方法在真实世界场景中表现不佳，因退化类型未知且空间分布不均，导致细节丢失或出现视觉伪影。需要更智能的机制来适应复杂退化并提升重建质量。

Method: 提出QUSR模型，结合不确定性引导噪声生成（UNG）模块与质量感知先验（QAP）。UNG模块根据区域不确定性动态调整噪声注入强度；QAP利用多模态大语言模型（MLLM）生成图像质量描述，作为可解释的先验指导修复过程。

Result: QUSR在真实世界图像超分辨率任务中表现出色，能够生成高保真度和高真实感的图像，有效缓解细节丢失与视觉伪影问题。

Conclusion: QUSR通过融合不确定性引导噪声控制与基于MLLM的质量先验，显著提升了扩散模型在复杂真实退化条件下的图像超分辨率性能，具备良好的实用性和可解释性。
...
TL;DR: 本研究提出一种基于HR-pQCT图像的全自动二分类框架，利用分割模型SegFormer对胫骨、腓骨及其周围软组织进行多区域自动分割，并提取各区域的放射组学特征。通过降维后训练多种机器学习分类器，在图像层面，肌腱组织特征表现最佳（准确率80.08%，AUROC 0.85）；在患者层面，使用软组织放射组学特征可将AUROC从0.792提升至0.875，优于传统骨密度指标。


<details>
  <summary>Details</summary>
Motivation: 现有HR-pQCT分析主要聚焦于骨组织，忽略了软组织信息，而软组织可能包含与骨质疏松相关的临床信号。因此，亟需一种能全面利用多组织信息的自动化分析方法以提高诊断准确性。

Method: 采用SegFormer Transformer架构实现多区域（皮下、肌腱、脂肪等）全自动分割；从每个区域提取939个放射组学特征并进行降维；使用122例受试者的20,496张图像数据训练六种机器学习分类器，分别评估图像级和患者级分类性能。

Result: 图像级别上，肌腱组织特征表现最优（准确率80.08%，AUROC 0.85）；患者级别上，引入软组织放射组学使AUROC从0.792提升至0.875，显著优于传统生物标志物及骨密度参数。

Conclusion: 该研究首次将Transformer-based SegFormer应用于HR-pQCT多区域分割，证明软组织中蕴含丰富的临床信息，支持在骨质疏松诊断中开展整合性组织评估，具有重要临床转化潜力。
...
### [28] [Rotation Equivariant Mamba for Vision Tasks](https://arxiv.org/abs/2603.09138)
*Zhongchen Zhao,Qi Xie,Keyu Huang,Lei Zhang,Deyu Meng,Zongben Xu*

Main category: cs.CV

TL;DR: This paper proposes EQ-VMamba, the first rotation equivariant visual Mamba architecture that incorporates rotational symmetry as a geometric prior. It introduces a rotation-equivariant cross-scan strategy and group Mamba blocks, ensuring end-to-end equivariance with theoretical guarantees. Experiments show superior or competitive performance across classification, segmentation, and super-resolution tasks, with ~50% fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Current Mamba-based vision models lack rotation equivariance, making them sensitive to image rotations and limiting robustness and generalization. This work addresses the absence of rotational symmetry in visual Mamba designs.

Method: Proposes EQ-VMamba with a rotation-equivariant cross-scan strategy and group Mamba blocks, supported by theoretical analysis of equivariance error to ensure end-to-end rotation equivariance.

Result: EQ-VMamba achieves state-of-the-art or competitive performance on multiple vision tasks (classification, segmentation, super-resolution) while using approximately 50% fewer parameters than non-equivariant baselines.

Conclusion: Incorporating rotation equivariance into Mamba-based vision models significantly improves robustness, performance, and parameter efficiency, establishing a new paradigm for geometrically principled design in visual Mamba architectures.
...
TL;DR: RTFDNet提出一种三分支编码器-解码器架构，统一融合与解耦，实现鲁棒的RGB-T语义分割。通过协同特征融合（SFF）和跨模态解耦正则化（CMDR）、区域解耦正则化（RDR），在部分传感器信号缺失时仍保持高性能，支持单阶段训练与独立推理。


<details>
  <summary>Details</summary>
Motivation: 传统方法过度强调模态平衡，导致在传感器信号部分缺失时性能严重下降；现有方法多采用分阶段训练，且融合与适应分离，限制了实时性和鲁棒性。

Method: 设计三分支编码器-解码器结构，引入协同特征融合（SFF）进行通道门控交换与轻量空间注意力，利用跨模态解耦正则化（CMDR）与区域解耦正则化（RDR）分离模态特异性成分并监督单模态解码器，形成反馈机制增强单模态路径而不影响融合流。

Result: 在多种模态条件下均表现出一致的优越性能，验证了模型在传感器部分失效情况下的鲁棒性与高效性，支持测试时的独立推理。

Conclusion: RTFDNet通过统一融合与解耦机制，显著提升了RGB-T语义分割在复杂环境下的稳定性与实用性，为机器人低光场景应用提供了有效解决方案。
...
TL;DR: RubiCap 是一种基于强化学习的密集图像描述生成框架，通过大语言模型（LLM）撰写的评分标准（rubrics）生成细粒度、样本特定的奖励信号，以克服传统监督蒸馏和强化学习在开放域图像描述任务中的局限性。该方法通过构建候选描述集合，利用 LLM 提取共识与缺陷，并转化为结构化评估标准，使 LLM 判官能够进行多维度质量评估，从而替代粗略的标量奖励。在多个基准测试中，RubiCap 在 CapArena 上取得最高胜率，在 CaptionQA 上表现出更高的词汇效率，且其小型模型（3B）训练出的视觉-语言模型性能优于使用专有模型生成数据训练的大型模型。


<details>
  <summary>Details</summary>
Motivation: 现有密集图像描述生成方法依赖昂贵的人工标注或受限于监督蒸馏的多样性不足和泛化能力弱；而强化学习虽具潜力，但因缺乏确定性校验器难以应用于开放域描述任务。因此需要一种能提供细粒度、可解释奖励信号的新型方法，以提升生成质量和多样性。

Method: RubiCap 采用三阶段流程：1）生成多样化的候选描述集合；2）利用 LLM 撰写针对每个样本的详细评分标准（rubrics），识别当前策略输出的优势与不足；3）将 rubrics 转化为结构化评估标准，由 LLM 判官执行多维度质量评分，替代传统标量奖励，驱动策略优化。

Result: RubiCap 在 CapArena 上实现最高胜率，超越监督蒸馏、先前 RL 方法、人类专家标注及 GPT-4V 增强结果；在 CaptionQA 上，其 7B 模型性能媲美 Qwen2.5-VL-32B-Instruct，3B 模型甚至优于同规模的 7B 模型；用 RubiCap-3B 生成的描述训练的 VLM 性能超过使用专有模型数据训练的模型。

Conclusion: RubiCap 通过引入 LLM 驱动的细粒度评分机制，成功将强化学习扩展至开放域密集图像描述任务，显著提升了生成质量、多样性与效率，为视觉-语言预训练和文本到图像生成提供了高效且高质量的标注解决方案。
...
TL;DR: MM-Zero 是首个基于强化学习的零数据自演化框架，实现视觉语言模型（VLM）在无初始图像数据情况下的自演化推理。它引入三种角色：提出者（生成抽象视觉概念和问题）、编码者（将概念转为可执行代码生成图像）和求解者（进行多模态推理）。三者均从同一基础模型初始化，使用组相对策略优化（GRPO）训练，并结合执行反馈、视觉验证和难度平衡设计奖励机制。实验表明，该方法显著提升VLM在多种多模态基准上的表现，推动了自演化多模态系统的发展。


<details>
  <summary>Details</summary>
Motivation: 现有自演化方法主要针对大语言模型，而视觉语言模型因需图像等视觉模态数据作为起点，通常依赖少量种子数据启动。如何实现完全零数据的自演化是关键挑战。本文旨在突破传统双角色（提议者与求解者）范式，构建无需任何外部数据输入即可自我演化的多角色框架，以推动多模态模型的自主进化能力。

Method: 提出 MM-Zero 框架，包含三个角色：1）Proposer 生成抽象视觉概念并构造问题；2）Coder 将概念转化为可执行代码（如 Python、SVG）以渲染图像；3）Solver 基于生成图像进行多模态推理。所有角色共享同一初始模型，通过 Group Relative Policy Optimization (GRPO) 进行联合训练，设计融合执行反馈、视觉验证和难度平衡的奖励机制，实现闭环自演化。

Result: MM-Zero 在多个主流多模态推理基准上显著优于基线方法，证明其在零数据条件下成功实现了 VLM 的自演化推理能力。该框架具备良好的可扩展性，为构建完全自主的多模态智能系统提供了新路径。

Conclusion: MM-Zero 首次实现了视觉语言模型的零数据自演化，通过引入多角色协同机制和先进的强化学习训练策略，突破了传统两角色框架的局限。该工作拓展了自演化系统的边界，为未来自主多模态智能体的发展奠定了重要基础。
...
TL;DR: 提出一种基于几何不变的20维关节角度描述符的度量学习框架，用于跨语言少样本手语识别。该方法利用MediaPipe静态手部关键点，通过关节角度特征实现对旋转、平移和缩放的不变性，显著减少跨数据集偏差，在仅用少量目标语言样本的情况下实现高性能识别，优于传统坐标表示，并在多个手语语言间实现超越域内准确率的迁移性能。


<details>
  <summary>Details</summary>
Motivation: 现有手语识别系统依赖大量标注数据，而全球300多种手语中多数缺乏足够标注数据；传统坐标表示易受视角、手部尺度和拍摄条件等外部因素影响，尤其在少样本场景下导致分类原型不稳定。因此需要一种对域偏移鲁棒且适用于低资源手语的跨语言少样本迁移方法。

Method: 采用MediaPipe提取静态手部关键点，构建20维的关节间角度特征向量，该特征对SO(3)旋转、平移和各向同性缩放具有不变性；结合轻量级MLP编码器与度量学习策略，训练模型以学习紧凑且稳定的类别原型，支持冻结参数进行跨语言迁移。

Result: 在四种不同手语（美式手语、巴西手语、阿拉伯手语、泰式手语）的指拼字母任务上，该方法相比归一化坐标基线最高提升25个百分点，且在跨语言迁移中常超过域内性能，验证了其在低资源环境下的有效性与可迁移性。

Conclusion: 几何不变的手部结构描述符为跨语言少样本手语识别提供了高效、可移植的基础，能够在数据稀缺条件下实现高精度识别，是解决低资源手语识别问题的有效方案。
...
TL;DR: TubeMLLM is a unified foundation model for medical vessel-like anatomy that integrates topological priors via natural language prompting and shared-attention architecture, achieving superior topology-aware perception and zero-shot generalization. It introduces TubeMData benchmark and adaptive loss weighting, excelling in out-of-distribution performance, cross-modality transfer, and robustness to image degradation.


<details>
  <summary>Details</summary>
Motivation: Modeling medical vessel-like anatomy is challenging due to complex topology and sensitivity to dataset shifts, leading to topological inconsistencies like artificial disconnections and spurious merges; existing task-specific models lack generalization and topology awareness.

Method: TubeMLLM combines structured understanding with controllable generation by integrating topological priors through explicit natural language prompts and aligning them with visual features in a shared-attention mechanism. It leverages TubeMData, a multimodal benchmark with topology-centric tasks, and employs an adaptive loss weighting strategy to emphasize topology-critical regions during training.

Result: TubeMLLM achieves state-of-the-art out-of-distribution performance, reducing $β_{0}$ error from 37.42 to 8.58 on color fundus photography. On unseen X-ray angiography, it attains a Dice score of 67.50% and reduces $β_{0}$ error to 1.21 in zero-shot cross-modality transfer. It also shows high robustness to blur, noise, and low resolution, and achieves 97.38% accuracy in evaluating mask topological quality.

Conclusion: TubeMLLM establishes a new paradigm for topology-aware modeling in medical imaging by unifying perception and generation through multimodal large language models, demonstrating strong generalization, robustness, and zero-shot capabilities across diverse modalities and conditions.
...
TL;DR: 本文提出一种统一的多模态多任务磁共振成像（MRI）场强增强框架，通过利用跨场强共享的退化模式，提升模型泛化能力。方法上，采用3D基础模型直接处理三维体积数据以获取鲁棒结构表征，并引入场感知频谱校正机制（FASRM）缓解流匹配模型的频谱偏差问题。此外，构建并公开了一个规模远超现有数据集的配对多场强MRI数据集。实验表明，该方法在PSNR和SSIM上分别优于现有方法约1.81 dB和9.47%。


<details>
  <summary>Details</summary>
Motivation: 现有MRI场强增强方法多局限于单一任务（如64mT到3T或3T到7T），且依赖小样本群体，未能利用不同场强间共享的退化模式，导致模型泛化能力差。因此亟需一种可跨场强、多任务协同的统一框架。

Method: 提出基于3D基础模型的端到端框架，直接建模3D MRI体积信息；设计场感知频谱校正机制（FASRM），结合磁场物理特性进行定制化频谱修正；构建大规模配对多场强数据集以缓解数据稀缺问题。

Result: 在多个场强转换任务中表现优异，平均提升PSNR约1.81 dB，SSIM提升9.47%，显著超越当前最优方法。

Conclusion: 所提方法通过联合利用多模态、多任务与3D结构先验，有效提升了MRI场强增强的性能与泛化性，为未来医学影像重建提供了新范式。
...
TL;DR: HelixTrack 是一种全事件驱动的方法，用于无人机和旋转机械中微秒级延迟的快速周期性运动跟踪，能够联合追踪类似螺旋桨的物体并估计其转速（RPM）。通过在线估计的单应性将事件从图像平面回卷到旋翼平面，并使用卡尔曼滤波器实时估计相位。批量迭代更新通过耦合相位残差与几何信息来优化物体姿态。作者构建了首个针对此类任务的公开数据集 TQE，包含13个高分辨率事件序列，共52个旋转物体，覆盖不同距离、增加的自身运动及微秒级真实转速标注。实验表明，HelixTrack 能以超过11.8倍实时速度处理全速率事件，且延迟极低，在性能上显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统帧基和事件基跟踪器在面对螺旋桨等周期性运动时因违反平滑运动假设而出现漂移或失效，尤其在存在自身运动和强干扰的情况下，亟需一种能精准捕捉高速周期性运动并同时估计转速的新方法。

Method: HelixTrack 采用全事件驱动架构，利用在线估计的单应性将事件映射至旋翼平面；通过卡尔曼滤波器维护相位状态；采用批处理迭代方式，结合相位残差与几何约束，联合优化物体姿态与转速（RPM）估计。

Result: 在自建的 TQE 数据集上，HelixTrack 实现了超过11.8倍实时速度的处理能力，具有微秒级延迟，且在轨迹跟踪与转速估计精度上均显著优于现有的基于事件或聚合的基线方法。

Conclusion: HelixTrack 成功解决了高速周期性运动在复杂动态环境下的微秒级感知难题，为安全关键型无人系统提供了高精度、低延迟的联合跟踪与转速估计能力，推动了事件相机在旋转机械与无人机感知中的应用。
...
TL;DR: BridgeDiff 是一种基于扩散模型的虚拟试穿框架，通过两个互补模块解决人体穿着外观与平面服装布局之间的差距问题。GCBM 模块构建全局服装线索表示，增强部分遮挡下的细节恢复能力；FSCM 模块通过平面对齐注意力机制在去噪过程中注入结构先验，提升服装结构稳定性。实验表明，该方法在标准 VTOFF 数据集上达到领先性能，生成高质量且结构一致的平面服装图像。


<details>
  <summary>Details</summary>
Motivation: 现有方法将虚拟试穿视为仅依赖局部掩码或文本提示的直接图像转换，忽略了人体穿着状态与平面服装布局之间的差异，导致未观察区域补全不一致和服装结构不稳定。

Method: 提出 BridgeDiff 框架，包含两个核心组件：1）服装线索桥接模块（GCBM），用于提取全局外观和语义身份特征；2）平面结构约束模块（FSCM），通过平面对齐注意力（FC-Attention）在特定去噪阶段引入平面结构先验，增强结构稳定性。

Result: 在标准 VTOFF 基线数据集上，BridgeDiff 实现了最先进的性能，生成的平面服装图像在外观细节和结构完整性方面均表现优异，显著提升了重建质量。

Conclusion: BridgeDiff 有效弥合了人体穿着观测与平面服装合成之间的鸿沟，通过显式建模服装语义与结构先验，实现了更稳定、高质量的虚拟试穿结果。
...
TL;DR: 本文提出一种基于密集视觉表示空间的导航世界模型（RAE-NWM），通过线性动力学探测发现DINOv2特征在动作条件转移中具有更强的可预测性，从而避免传统压缩隐空间丢失结构信息的问题。采用条件扩散变换器与解耦扩散头（CDiT-DH）建模连续状态转移，并引入时间驱动门控模块调节动作注入强度，显著提升结构稳定性和动作准确性，增强下游规划与导航性能。


<details>
  <summary>Details</summary>
Motivation: 传统导航世界模型依赖变分自编码器压缩隐空间中的状态演化，但空间压缩常导致细粒度结构信息丢失，影响精确控制。为理解不同表示的空间传播特性，需寻找更具可预测性的视觉表示方式。

Method: 提出基于表示自编码器的导航世界模型（RAE-NWM），在密集的DINOv2视觉特征空间中建模导航动态；使用条件扩散变换器与解耦扩散头（CDiT-DH）进行连续状态转移建模，并设计时间驱动门控模块以调控动作注入强度。

Result: 在序列回放建模中，该方法显著提升了结构稳定性与动作准确性，有效改善了下游任务中的规划与导航表现。

Conclusion: 在密集视觉表示空间中建模导航动态优于传统压缩隐空间方法，结合先进的扩散建模与动态条件机制，能够实现更精准、稳定的视觉导航。
...
TL;DR: X-GS是一个可扩展的开放框架，统一多种技术以实现基于3D高斯溅射（3DGS）的实时在线SLAM，并融合语义信息，支持多模态下游任务。核心是X-GS-Perceiver，能处理未对齐的RGB或RGB-D视频流，联合优化几何与位姿，并将视觉基础模型的高维语义特征蒸馏到3D高斯中。通过新型在线向量量化（VQ）模块、GPU加速的网格采样和高度并行化设计，实现实时性能。生成的语义3D高斯可用于视觉-语言模型（X-GS-Thinker），支持物体检测、零样本图像描述生成等任务。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法大多孤立，局限于特定领域如在线SLAM、语义增强或无姿态图像的3DGS，缺乏统一框架支持多模态与实时性需求。本文旨在构建一个集成多种技术的通用框架，实现高效实时的语义3DGS在线SLAM，打通至下游多模态模型的路径。

Method: 提出X-GS框架，核心为X-GS-Perceiver：接收未对齐的RGB/RGB-D视频流，联合优化3D高斯的几何结构与相机位姿；利用视觉基础模型提取语义特征，并通过蒸馏机制注入3D高斯；引入在线向量量化（VQ）模块、GPU加速网格采样和高度并行化管道设计，保障实时性；通过X-GS-Thinker组件将语义3D高斯输入视觉-语言模型，实现多模态任务。

Result: 在真实世界数据集上验证了X-GS框架的有效性、效率及新开启的多模态能力，实现了实时语义3DGS在线SLAM，支持零样本图像描述生成、物体检测等下游任务。

Conclusion: X-GS成功构建了一个统一、高效、可扩展的3DGS框架，首次实现了实时语义3DGS与多模态模型的无缝对接，为未来空间人工智能应用提供了坚实基础。
...
TL;DR: 提出一种名为几何语义解耦（GSD）的无参数模块，通过利用冻结的视觉基础模型作为语义引导，分离出图像中的语义成分，从而提升AI生成图像检测在未见生成管道下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉基础模型（VFM）的AI生成图像检测器在面对未见过的生成方法时泛化能力差，主要因为其依赖预训练的语义先验而非伪造特征，亟需解决这一问题。

Method: 提出几何语义解耦（GSD），通过批量统计估计语义方向，并利用几何约束将这些方向从表示中投影出去，使检测器专注于与语义无关的伪造证据。

Result: 在跨数据集评估中，视频级AUC达到94.4%（+1.2%），对未见篡改的鲁棒性提升3.0%，且可推广至通用场景合成图像检测，在UniversalFakeDetect和GenImage上分别提升0.9%和1.7%。

Conclusion: GSD有效解决了语义回退问题，显著提升了AI生成图像检测的泛化性能，尤其在未见生成方式和非人脸场景下表现优异。
...
TL;DR: Poly-DETR提出一种基于极坐标表示的稀疏顶点回归方法，将实例分割重构为无需密集像素掩码预测的任务，通过极坐标可变形注意力和位置感知训练策略解决检测变压器中框到多边形参考偏移问题。相比现有极坐标方法，其在MS COCO上提升4.7 mAP；在高分辨率场景下更轻量，内存消耗减少近一半；在细胞和建筑轮廓等特定领域数据集上全面超越基于掩码的方法，证明其对规则形状实例的优势。


<details>
  <summary>Details</summary>
Motivation: 当前实例分割面临高分辨率输入与轻量化实时推理之间的矛盾，传统依赖密集像素掩码预测的方法在效率和精度之间难以平衡。

Method: 提出Polygon Detection Transformer (Poly-DETR)，采用极坐标表示实现稀疏顶点回归，引入极坐标可变形注意力与位置感知训练策略以动态更新监督信号并聚焦边界特征。

Result: 在MS COCO test-dev上取得4.7 mAP提升；在Cityscapes上内存消耗减少约50%；在PanNuke和SpaceNet上全面优于其掩码基对应模型，尤其在规则形状实例上表现更优。

Conclusion: Poly-DETR通过极坐标表示有效缓解了实例分割中的分辨率与效率矛盾，显著提升精度与效率，在多种数据集上展现出优于传统掩码方法的性能，尤其适用于规则形状的领域任务。
...
TL;DR: 该研究提出了一项新任务——以第一人称视角预测长期物理后果（Egocentric Scene Prediction with LOng-horizon REasoning），旨在评估多模态大语言模型（MLLMs）在执行一系列动作后对场景变化的推理能力。为此，作者构建了EXPLORE-Bench基准，基于真实第一人称视频数据，包含长动作序列与结构化最终场景标注，支持细粒度量化评估。实验表明，现有主流MLLMs性能远低于人类水平，表明长期第一人称推理仍是重大挑战。通过分步推理等测试时扩展方法可部分提升性能，但带来显著计算开销。总体而言，EXPLORE-Bench为评估和推动第一人称具身感知中的长时序推理提供了可靠平台。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽被视为具身智能体的基础，但其在第一人称视角下对长期物理后果的推理能力尚不明确，亟需系统评估方法以揭示模型短板并推动发展。

Method: 构建了一个基于真实第一人称视频的新基准EXPLORE-Bench，包含长动作序列与结构化最终场景标注（如物体类别、视觉属性、对象间关系），用于评估模型在复杂动作序列下的长期推理能力；采用多种主流开源与专有MLLM进行实验，并引入分步推理等测试时扩展策略分析性能提升潜力。

Result: 实验结果显示，现有MLLMs在长时序第一人称推理任务上的表现显著落后于人类水平，表明该能力仍是主要挑战；分步推理能部分改善性能，但伴随较高的计算成本。

Conclusion: EXPLORE-Bench为评估和推进多模态大语言模型在第一人称具身感知中的长时序推理能力提供了一个系统化、可量化的基准平台，对推动下一代具身智能的发展具有重要意义。
...
TL;DR: ForgeDreamer提出一种新框架，通过多专家LoRA集成和跨视角超图几何增强，解决工业场景下文本到3D生成的领域适应和几何推理问题，实现更优的语义泛化与几何保真度。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D生成方法在工业应用中面临领域适应困难和几何推理不足的问题，传统LoRA融合导致类别间知识干扰，成对一致性约束无法捕捉高阶结构依赖，影响精密制造精度。

Method: 提出多专家LoRA集成机制，将多个类别特定的LoRA模型整合为统一表示，避免知识干扰；构建跨视角超图几何增强方法，同时捕捉多视角间的结构依赖关系，提升几何推理能力。

Result: 在自建工业数据集上的实验表明，该方法在语义泛化和几何保真度方面均优于现有最先进方法，实现了制造级的一致性。

Conclusion: ForgeDreamer通过协同优化语义理解与几何推理，显著提升了文本到3D生成在工业场景下的性能，具备实际应用潜力。
...
TL;DR: 本文提出新型训练策略与损失函数，通过缩小高斯尺度并引入熵约束，减少每像素的高斯列表长度，提升3D高斯溅射的渲染效率，结合渐进式分辨率调度进一步优化性能，在不牺牲渲染质量的前提下显著提高效率。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）虽在渲染质量和效率上优于NeRF，但其学习3D高斯的过程仍存在效率瓶颈，亟需优化以减少每条光线上的高斯数量，提升渲染速度。

Method: 通过定期重置高斯尺度以缩小其覆盖范围，使小高斯仅影响邻近像素；引入熵约束于alpha混合过程，增强高斯权重分布的尖锐性，使主导权重更大、次要权重更小，从而减少高斯对非主导像素的影响，缩短高斯列表长度；结合渐进式渲染分辨率调度策略，实现效率的进一步提升。

Result: 在多个主流基准测试中，所提方法在保持高质量渲染的同时，显著提升了训练与渲染效率，优于现有最先进方法。

Conclusion: 本文提出的策略有效减少了每像素的高斯数量，显著提高了3DGS的渲染效率，为高效辐射场学习提供了新思路。
...
TL;DR: SVOR提出一种鲁棒的视频物体移除框架，通过三种设计实现无阴影、无闪烁、容错缺陷掩码的物体移除：(1) MUSE窗口化掩码合并策略，有效处理突变运动并减少遗漏；(2) DA-Seg轻量级分割头，结合去噪感知自适应层，提供内部扩散感知定位先验；(3) 两阶段课程训练，先在无配对真实背景视频上进行自监督预训练，再在合成数据上微调，联合去除物体及其阴影/反射，提升跨域鲁棒性。实验表明，SVOR在多个数据集和退化掩码基准上达到新SOTA，推动视频物体移除从理想场景迈向真实应用。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频修复模型在存在阴影、突变运动和缺陷掩码等现实问题时，难以保持时间稳定性和视觉一致性，亟需更鲁棒的解决方案以支持真实世界应用。

Method: 提出Stable Video Object Removal (SVOR)框架，包含三个核心设计：(1) Mask Union for Stable Erasure (MUSE)，采用窗口化掩码合并策略，在时间掩码下采样中保留每个窗口内所有目标区域，应对突变运动；(2) Denoising-Aware Segmentation (DA-Seg)，在解耦分支上引入轻量级分割头，结合去噪感知自适应归一化（AdaLN），并通过掩码退化训练，提供扩散感知定位先验；(3) Curriculum Two-Stage Training，第一阶段在无配对真实背景视频上进行自监督预训练，使用在线随机掩码学习真实背景与时间先验；第二阶段在合成配对数据上微调，引入掩码退化与副作用加权损失，联合去除物体及其阴影/反射。

Result: SVOR在多个公开数据集和退化掩码基准上均取得当前最佳性能，显著提升视频物体移除在真实场景下的稳定性与一致性，验证了其在复杂现实条件下的有效性与鲁棒性。

Conclusion: SVOR通过创新的掩码处理、去噪感知分割与分阶段训练策略，成功克服了现实世界中阴影、突变运动和掩码缺陷带来的挑战，实现了高质量、稳定且鲁棒的视频物体移除，标志着该技术向实际应用迈出关键一步。
...
TL;DR: 提出CogBlender框架，通过在文本到图像生成过程中对认知属性进行连续、多维度干预，实现对图像情感价值、唤醒度、支配感和记忆性等心理意图的精确控制。该方法基于认知空间与语义流形之间的映射关系，利用认知锚点重构流匹配过程中的速度场，实现动态调节生成过程。实验验证了其在四种认知维度上的有效性，为认知驱动的创意设计提供新范式。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型虽能生成语义连贯的内容，但在控制图像引发的认知反应（如情绪、记忆）方面能力有限，难以满足特定心理意图的需求。因此，需要一种可精细调控图像认知属性的方法。

Method: 构建认知空间与语义流形之间的映射关系，定义认知锚点作为认知空间的边界，通过插值不同锚点的速度场来重构流匹配过程中的速度场，使生成过程受多维认知评分动态引导，实现连续、细粒度的认知干预。

Result: 在情感价值、唤醒度、支配感和图像记忆性四个认知维度上均实现了有效且可控的干预；生成图像与目标认知属性高度一致，验证了方法的有效性与实用性。

Conclusion: CogBlender为文本到图像生成提供了认知驱动的新范式，能够精准操控图像引发的心理反应，推动创意设计向更人性化、意图导向的方向发展。
...
TL;DR: 提出了一种名为IntroSVG的自省式SVG生成框架，通过引入视觉反馈闭环，使模型在生成过程中兼具生成与评价能力。利用监督微调学习绘制SVG并评估其渲染效果，并通过早期失败样本构建高质量纠错数据集以增强鲁棒性；再借助大容量教师模型构建偏好数据集，采用直接偏好优化（DPO）对生成策略进行对齐。推理阶段采用迭代式‘生成-评审-修正’循环，从不完善的草稿逐步提升输出质量。实验表明该方法在复杂结构、语义对齐和可编辑性方面均达到当前最佳性能，验证了将显式视觉反馈融入生成过程的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到SVG生成方法受限于自回归训练中缺乏对最终渲染图像的视觉感知，导致生成质量受限。为突破这一瓶颈，需要引入视觉反馈机制以提升生成效果。

Method: 提出一个统一的视觉语言模型，在闭环中同时承担生成器和批评者角色；通过监督微调让模型学会生成SVG并提供渲染反馈；将早期错误转化为高质量纠错训练数据；使用高容量教师模型构建偏好数据集，通过直接偏好优化（DPO）对齐生成策略；推理时采用迭代的‘生成-评审-修正’循环实现自主优化。

Result: 所提方法在多个关键评估指标上达到当前最优表现，生成的SVG具有更复杂的结构、更强的语义一致性以及更高的可编辑性，证明了引入视觉反馈机制的有效性。

Conclusion: 将显式视觉反馈嵌入生成流程能显著提升文本到SVG生成的质量，本研究提出的IntroSVG框架为未来可感知、可修正的生成系统提供了有效范例。
...
TL;DR: 提出SpaceSense-Bench，一个大规模多模态航天器感知基准，包含136个卫星模型、70GB数据，提供同步的RGB图像、深度图和LiDAR点云，以及像素级和点级的7类部件语义标签与6-DoF姿态真值。通过Unreal Engine 5高保真空间仿真和自动化流程生成，涵盖五大任务评测，发现小部件感知和零样本泛化仍是关键挑战，且增加训练卫星数量显著提升新目标性能。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据集在目标多样性、传感器模态和标注完整性方面存在局限，难以支撑自主空间操作中的精确感知需求，亟需更全面、大规模的多模态数据集以推动航天器感知研究。

Method: 基于Unreal Engine 5构建高保真空间仿真环境，采用全自动数据采集、多阶段质量控制与格式转换流程，生成包含同步RGB、深度图与256束LiDAR点云的多模态数据，并附带密集的部件级语义标签与精确姿态真值。

Result: 在五个代表性任务上进行基准测试，结果显示：（i）小尺度部件（如推进器、全向天线）的感知及对完全未见航天器的零样本泛化仍是当前方法的主要瓶颈；（ii）增加训练用卫星数量能显著提升对新目标的性能，证明大规模多样化数据对空间感知研究的重要性。

Conclusion: SpaceSense-Bench为航天器感知研究提供了高质量、大规模、多模态的基准数据集，其公开可用性将推动空间自主操作中感知技术的发展，尤其在复杂部件识别与泛化能力方面具有重要价值。
...
TL;DR: 本文提出OddGridBench，一个用于评估多模态大语言模型（MLLMs）在视觉差异检测方面敏感性的可控基准测试集，包含超过1400张基于网格的图像，其中仅有一个元素在颜色、大小、旋转或位置等属性上与其余元素不同。实验表明，所有评测的MLLMs（包括Qwen3-VL、InternVL3.5、Gemini-2.5-Pro和GPT-5）在细粒度视觉差异检测上的表现远低于人类水平。为此，作者提出OddGrid-GRPO，一种结合课程学习与距离感知奖励的强化学习框架，通过逐步控制训练样本难度并引入空间邻近性约束优化奖励机制，显著提升了模型的精细视觉区分能力。该工作为推进多模态智能中的感知对齐与视觉差异敏感性提供了基础支持。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在高层视觉语言任务中表现优异，但在低层视觉感知，特别是细粒度视觉差异检测方面的能力仍不明确且缺乏系统评估。为了填补这一空白，亟需一个可控制、可量化的基准来深入分析和提升MLLMs在微小视觉差异识别上的性能。

Method: 提出OddGridBench基准数据集，包含1400+张精心设计的网格图像，每张图中仅一个元素在视觉属性上与其他不同；构建OddGrid-GRPO强化学习框架，融合课程学习策略与距离感知奖励函数，通过动态调整训练难度并引入空间位置约束，增强模型对细微视觉差异的敏感性。

Result: 所有评测的MLLMs在OddGridBench上的表现均显著低于人类水平，验证了其在细粒度视觉感知上的不足；OddGrid-GRPO框架有效提升了模型的视觉差异检测能力，在多个指标上实现明显改进。

Conclusion: OddGridBench和OddGrid-GRPO为研究和提升多模态大语言模型在低层视觉感知，尤其是细粒度视觉差异敏感性方面提供了重要工具与方法，有助于推动多模态智能向更深层次的感知对齐发展。
...
TL;DR: 本文提出STAR基准，一个用于评估大语言模型在对抗性、时间敏感环境中的交互式推理能力的多智能体框架。该框架通过1v1零和竞争互动，将推理视为迭代适应的决策过程，支持回合制与实时场景，以分析长期战略规划与快速战术执行。引入战略评估套件，不仅评估胜负，还衡量策略质量如执行效率与结果稳定性。实验发现，尽管推理能力强的模型在回合制中占优，但其延迟导致在实时场景中表现不佳，而更快的指令微调模型则更胜一筹，揭示了战略智能不仅依赖推理深度，还需及时行动能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估大多将推理视为单次能力，忽视了对手感知决策、时间约束及高压环境下执行的挑战，因此需要一个能真实反映大语言模型在动态竞争环境中表现的评估框架。

Method: 提出STAR基准，采用模块化架构与标准化API，支持回合制与实时交互，构建统一环境进行多维度评估，并引入战略评估套件以量化策略质量。

Result: 实验显示存在显著的策略-执行差距：推理密集型模型在回合制中表现优异，但在实时场景中因延迟劣势；相反，快速指令微调模型在实时场景中更具优势。表明战略智能需兼顾推理深度与行动时效性。

Conclusion: STAR为研究大语言模型在动态竞争环境中的推理与执行权衡提供了一个系统化、可复现的基准，强调了及时行动能力在战略智能中的关键作用。
...
TL;DR: 提出Predictive Spectral Calibration (PSC)，一种无需源数据的图像回归测试时自适应方法，通过块频谱匹配扩展子空间对齐，联合对齐目标特征并校准正交补空间中的残差频谱松弛，在多个图像回归基准上表现优于现有基线，尤其在严重分布偏移下效果显著。


<details>
  <summary>Details</summary>
Motivation: 图像回归的测试时自适应（TTA）研究远少于分类任务，现有分类方法依赖分类特定目标和决策边界，难以直接应用于连续回归任务；尽管近期工作通过子空间对齐取得进展，但仍存在优化空间。

Method: 提出PSC框架，利用源模型预测支持子空间进行目标特征对齐，并在正交补空间中校准残差频谱松弛，实现块频谱匹配，保持模型无关性和易实现性。

Result: 在多个图像回归基准上均取得一致改进，尤其在严重分布偏移情况下表现突出，优于多种强基线方法。

Conclusion: PSC是一种简单、通用且有效的源自由图像回归TTA方法，通过频谱级对齐与校准显著提升模型在分布外数据上的性能。
...
TL;DR: 提出M3GCLR框架，通过博弈论思想解决骨架动作识别中视角差异、对抗机制不足和增强扰动不可控的问题。利用无限骨架数据博弈模型与极小极大优化，结合多视图旋转增强和时间平均输入作为中性锚点，实现结构对齐，并引入双损失均衡优化器以最大化动作相关信息并最小化编码冗余。在多个基准数据集上达到或超过现有最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督骨架动作识别方法存在视角差异建模不足、缺乏有效对抗机制以及增强扰动不可控等问题，限制了模型性能提升。

Method: 提出无限骨架数据博弈（ISG）模型及平衡定理，基于多视图旋转增强生成正常-极端数据对，采用时间平均输入作为中性锚点实现结构对齐；构建强对抗性极小极大骨架-数据博弈，并设计双损失均衡优化器以优化博弈平衡。

Result: 在NTU RGB+D 60（X-Sub, X-View）上取得82.1%、85.8%准确率，在NTU RGB+D 120（X-Sub, X-Set）上取得72.3%、75.0%准确率；在PKU-MMD Part I和II上分别达到89.1%、45.2%的三流准确率，均优于或匹配当前最优结果。消融实验验证各模块有效性。

Conclusion: M3GCLR通过游戏理论框架有效解决了自监督骨架动作识别中的关键挑战，显著提升了模型性能，为无标签学习提供了新范式。
...
TL;DR: 提出MIL-PF框架，利用预计算特征和轻量级多实例学习头，实现高效乳腺影像分类，显著降低训练复杂度并达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率医学影像（如乳腺摄影）标注有限、监督信号弱，传统微调方法计算成本高且不切实际。

Method: 采用冻结的基础编码器预计算语义特征，仅训练一个小型任务特定聚合模块（40k参数），通过注意力机制建模全局组织上下文与局部病灶信号。

Result: 在临床规模数据上实现了最先进的分类性能，同时大幅减少训练复杂性，支持高效实验与适应。

Conclusion: MIL-PF为大规模乳腺影像分析提供了一种高效、可扩展的解决方案，适用于资源受限环境下的快速迭代与部署。
...
TL;DR: SinGeo提出一种简单而强大的框架，实现无需额外模块或显式变换的鲁棒跨视角地理定位。通过双判别学习架构增强地面与卫星分支内的视图内区分能力，并首次引入课程学习策略提升跨视角鲁棒性。在四个基准数据集上的实验表明，SinGeo在多种条件下均达到当前最优性能，尤其在极端视角下超越专门训练的方法。此外，该方法具有跨架构迁移能力，并提出一致性评估方法以量化模型在不同视角下的稳定性，为未来研究提供可解释性支持。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法依赖特定视场角（FoV）的训练范式，在未见视场角和未知朝向时性能下降，需部署多个模型覆盖变化。尽管已有研究尝试随机化视场角进行动态训练，但未能实现对多样化条件的鲁棒性，且隐含假设所有视场角难度相同，存在明显局限。

Method: SinGeo采用双判别学习架构，强化地面与卫星分支内部的视图内区分能力；创新性地引入课程学习策略，逐步训练模型应对不同复杂度的视场角变化，从而提升整体鲁棒性。

Result: SinGeo在四个基准数据集上均取得当前最优结果，尤其在极端视场角条件下显著优于专门训练的方法；同时具备跨架构迁移能力，并通过新提出的定量一致性评估方法验证了模型在不同视角下的稳定性。

Conclusion: SinGeo实现了单一模型在多样视场角下的鲁棒跨视角地理定位，突破了传统多模型部署的限制，其高效性、泛化性和可解释性为未来研究提供了新方向。
...
TL;DR: 本文提出EventVGGT框架，首次将视觉几何基础变压器（VGGT）中的时空与多视角几何先验蒸馏到事件域，通过三层次蒸馏策略（跨模态特征混合、时空特征蒸馏、时间一致性蒸馏），有效建模事件流的时间连续性，显著提升事件相机下的单目深度估计精度与时间一致性。在EventScape上绝对平均深度误差降低53%以上，且在DENSE和MVSEC数据集上展现强零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有无标注事件深度估计方法将事件流视为独立帧，忽略事件数据的时间连续性，导致无法利用视觉基础模型中的丰富时空先验，造成深度预测不一致且精度不足。

Method: 提出EventVGGT框架，采用三层次蒸馏策略：(i) 跨模态特征混合（CMFM）融合RGB与事件特征生成辅助深度预测；(ii) 时空特征蒸馏（STFD）提取VGGT的时空表示；(iii) 时间一致性蒸馏（TCD）通过对齐帧间深度变化实现跨帧一致性。

Result: 在EventScape上绝对平均深度误差从2.30降至1.06，降幅超53%；在未见数据集DENSE和MVSEC上表现出优异的零样本泛化能力。

Conclusion: EventVGGT首次系统性地将视觉模型中的时空与多视角几何先验引入事件域，通过显式建模事件流的时间连续性，显著提升了事件相机下单目深度估计的准确性与稳定性，为复杂场景下的鲁棒三维感知提供了新范式。
...
TL;DR: MIDAS 是一种无需训练的基于扩散模型的覆盖图像隐写框架，通过潜在空间融合实现多图像隐藏和用户特定访问控制。它引入随机基机制抑制残留结构信息，并设计潜在向量融合模块以对齐扩散过程。实验表明，MIDAS 在访问控制、隐写图像质量与多样性、抗噪声能力及抗隐写分析方面均优于现有无训练基线方法，为多用户场景下的可控覆盖隐写提供了实用且可扩展的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有覆盖图像隐写方法缺乏有效的访问控制机制，难以在多用户环境中实现对不同用户的差异化内容揭示，限制了其在需要隐私保护和可扩展性的信息隐藏场景中的应用。

Method: 提出基于扩散模型的MIDAS框架，采用随机基机制减少结构残留，通过潜在向量融合模块将聚合的潜在表示调整至与扩散过程兼容的形态，实现用户级访问控制下的多图像隐藏。

Result: MIDAS 在访问控制功能、隐写图像质量与多样性、抗噪声能力以及抵抗隐写分析方面显著优于现有无训练基线方法，验证了其在实际应用中的有效性与可行性。

Conclusion: MIDAS 为覆盖图像隐写提供了一种无需训练、可扩展且具备强访问控制能力的新范式，适用于多用户环境下的隐私敏感信息隐藏任务。
...
TL;DR: DIMT 2025 Challenge推动端到端文档图像翻译研究，涵盖无OCR与有OCR双赛道，分大小模型子任务，共吸引69支队伍、27份有效提交。结果显示大模型在复杂版面翻译中表现优异，展现新范式潜力。


<details>
  <summary>Details</summary>
Motivation: 推动文档图像机器翻译（DIMT）发展，解决传统方法中光学字符识别（OCR）与自然语言处理（NLP）脱节问题，实现文本内容与版面布局的联合建模。

Method: 设计双赛道（OCR-free与OCR-based）及大小模型子任务，构建多语言、多版面文档图像数据集，采用统一评估协议，支持参赛者提交集成系统。

Result: 大模型在复杂版面文档翻译中表现突出，验证其作为新范式可行性；小模型仍存在性能瓶颈，显示未来优化空间。

Conclusion: 大模型驱动的端到端方法为复杂布局文档图像翻译提供了高效解决方案，未来需进一步探索模型效率与跨语言泛化能力。
...
TL;DR: RiO-DETR 是首个实时定向目标检测的Transformer模型，解决了语义依赖方向、角度周期性及搜索空间扩大带来的挑战。通过内容驱动的角度估计、旋转修正正交注意力、解耦周期性精炼和定向密集O2O监督等创新设计，在保持实时效率的同时显著提升检测精度。在DOTA-1.0、DIOR-R和FAIR-1M-2.0数据集上验证了其优越的性能。


<details>
  <summary>Details</summary>
Motivation: 现有DETR模型难以直接应用于定向边界框（OBB）检测，主要受限于语义依赖的方向性、角度的周期性导致欧氏优化失效，以及更大的搜索空间造成收敛缓慢。为实现高效实时的定向检测，亟需针对这些挑战进行专门设计。

Method: 提出Content-Driven Angle Estimation，将角度与位置查询解耦，并引入Rotation-Rectified Orthogonal Attention以捕捉互补特征；采用Decoupled Periodic Refinement结合有界粗到精更新与Shortest-Path Periodic Loss，实现稳定的角度学习；设计Oriented Dense O2O，通过注入角度多样性实现密集监督，加速角度收敛且无额外开销。

Result: 在DOTA-1.0、DIOR-R和FAIR-1M-2.0数据集上，RiO-DETR实现了领先的实时定向检测性能，建立了新的速度-精度权衡标杆。

Conclusion: RiO-DETR是首个真正意义上的实时定向检测Transformer，通过任务原生设计有效克服了定向检测中的核心难题，在精度与效率之间取得良好平衡，具备实际应用潜力。
...
TL;DR: 本文提出PromptDLA，一种用于文档版面分析（DLA）的领域感知提示器，通过利用描述性知识作为线索，将领域先验信息融入DLA任务中。其核心创新在于设计了针对不同数据领域属性定制化提示的领域感知提示器，使模型能更有效地捕捉跨领域差异，提升泛化能力。在DocLayNet、PubLayNet、M6Doc和D$^4$LA等多个基准上实现了当前最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有文档版面分析方法在使用多源数据混合训练时，因忽略不同领域间布局结构的差异（如标注风格、文档类型、语言等），导致模型性能不佳。因此需要一种能够有效融合领域先验知识的方法，以提升模型在多样化场景下的泛化能力。

Method: 提出领域感知提示器PromptDLA，根据数据领域的具体属性生成定制化提示，利用这些提示作为引导，帮助模型聚焦于关键特征与结构，从而增强对不同领域布局模式的理解与适应能力。

Result: 在多个主流文档版面分析数据集（DocLayNet、PubLayNet、M6Doc、D$^4$LA）上均取得了当前最优的性能表现，验证了方法的有效性和优越性。

Conclusion: PromptDLA通过引入领域感知提示机制，成功将领域先验知识融入文档版面分析流程，显著提升了模型在异构数据环境中的泛化能力，为未来文档智能系统的设计提供了新思路。
...
TL;DR: 本文提出一种基于元学习的轨迹预测方法，通过双层优化预训练阶段模拟测试时适应任务，实现快速准确的在线自适应。测试时引入数据自适应模型更新机制，根据在线部分导数和困难样本选择动态调整学习率与更新频率，提升对测试数据的适应性与效率。在nuScenes、Lyft、Waymo等跨数据集分布偏移场景下，该方法显著优于现有最先进测试时训练方法，且在次优学习率和高帧率需求下仍表现稳健，具备强鲁棒性和实用性。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法在测试时分布偏移下性能显著下降；尽管已有测试时训练技术用于适应，但依赖固定预训练模型和固定的在线更新规则，缺乏灵活性和对测试数据特性的适应能力。

Method: 提出一种元学习框架，在预训练阶段进行双层优化以优化预测器的快速在线适应能力；在测试阶段引入数据自适应模型更新机制，基于在线部分导数和困难样本选择动态调整学习率与更新频率。

Result: 在nuScenes、Lyft、Waymo等多个跨数据集分布偏移场景中，该方法在适应精度上超越现有最先进测试时训练方法；在次优学习率和高帧率条件下依然表现优异，展现出良好的鲁棒性与实用性。

Conclusion: 所提方法通过元学习与数据自适应更新机制，有效提升了轨迹预测模型在测试时分布偏移下的自适应能力，具有更强的灵活性、效率与鲁棒性，适用于真实复杂场景。
...
TL;DR: 本文提出开放世界运动预测新范式，解决真实场景中感知不完善和物体类别动态变化的问题。通过首个端到端的类增量运动预测框架，结合伪标签策略与视觉-语言模型过滤，有效缓解灾难性遗忘，并利用查询特征方差设计重放采样策略以保留旧类知识。在nuScenes和Argoverse 2数据集上的实验表明，该方法在保持旧类别性能的同时显著提升对新类别的适应能力，支持零样本跨域迁移和端到端类增量规划，推动自动驾驶系统持续学习。代码已开源。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测方法基于封闭世界假设，依赖固定物体类别和高质量感知，在真实复杂环境中表现受限。实际驾驶场景中物体类别不断演化且感知存在误差，亟需一种能持续学习新类别的开放世界运动预测框架。

Method: 提出首个端到端类增量运动预测框架：1）引入伪标签策略生成所有已知类别的预测标签；2）使用视觉-语言模型筛选不一致或过自信的预测；3）设计基于查询特征方差的重放采样策略，选择具有信息量的旧序列进行记忆回放，缓解灾难性遗忘。

Result: 在nuScenes和Argoverse 2数据集上验证了方法的有效性，成功抵抗灾难性遗忘，保持对旧类别的性能，同时显著提升对新类别的适应能力；支持零样本迁移至真实驾驶场景，并可扩展至端到端类增量规划。

Conclusion: 本工作首次定义并解决了开放世界运动预测问题，构建了一个可持续学习、适应动态环境的运动预测框架，为实现真正意义上的持续自动驾驶提供了关键技术支撑。
...
### [61] [GIIM: Graph-based Learning of Inter- and Intra-view Dependencies for Multi-view Medical Image Diagnosis](https://arxiv.org/abs/2603.09446)
*Tran Bao Sam,Hung Vu,Dao Trung Kien,Tran Dat Dang,Van Ha Tang,Steven Truong*

Main category: cs.CV

TL;DR: 提出一种基于图的新型方法GIIM，用于计算机辅助诊断（CADx），通过同时建模单视图内异常间的依赖关系和多视图间动态变化，提升诊断准确性和鲁棒性，尤其在处理缺失数据方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有多视图CADx方法未能充分建模单视图内异常间的复杂依赖关系以及病变在不同视图间的动态变化，且常面临数据不完整的问题，导致预测可靠性下降。

Method: 将诊断任务重新定义为关系建模问题，提出基于图的GIIM框架，利用图结构同时捕捉视图内依赖和视图间动态变化，并引入专门技术以有效处理缺失数据。

Result: 在CT、MRI和乳腺摄影等多种影像模态上进行的广泛评估表明，GIIM显著提升了诊断准确性和鲁棒性，优于现有方法。

Conclusion: GIIM为未来CADx系统提供了一个更有效、更具通用性的框架，能够更好地模拟临床诊断中的复杂关系与动态变化。
...
### [62] [EvoDriveVLA: Evolving Autonomous Driving Vision-Language-Action Model via Collaborative Perception-Planning Distillation](https://arxiv.org/abs/2603.09465)
...
TL;DR: EvoDriveVLA提出了一种协同感知-规划蒸馏框架，通过自锚定视觉蒸馏和基于虚拟教师的轨迹优化，解决视觉编码器解冻后的感知退化与长期规划中的累积不稳定性问题。该方法在开环和闭环评估中均达到当前最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在自动驾驶中面临视觉编码器解冻后感知性能下降及长期规划中累积不稳定性的问题，亟需新的机制来提升鲁棒性与一致性。

Method: 采用自锚定视觉蒸馏（利用自锚定教师提供视觉约束）与基于未来感知的虚拟教师轨迹蒸馏（结合粗到细轨迹优化和蒙特卡洛丢弃采样生成高质量轨迹候选），实现感知与规划的协同优化。

Result: 在开环评估中达到SOTA性能，在闭环评估中显著提升系统表现，验证了方法的有效性与稳定性。

Conclusion: EvoDriveVLA通过联合感知与规划的蒸馏机制，有效缓解了视觉编码器解冻带来的感知退化与长期规划不稳定性，为自动驾驶中的多模态模型设计提供了新思路。
...
### [63] [TopoOR: A Unified Topological Scene Representation for the Operating Room](https://arxiv.org/abs/2603.09466)
*Tony Danjun Wang,Ka Young Kim,Tolga Birdal,Nassir Navab,Lennart Bastian*

Main category: cs.CV

TL;DR: TopoOR 提出一种新的范式，将多模态手术室建模为高阶拓扑结构，通过高阶拓扑单元保留实体间的成对与群体关系，克服传统二元图结构的局限性。引入高阶注意力机制，显式保持流形结构和模态特异性特征，避免将3D几何、音频和机器人运动学融合到单一潜在表示中，从而提升安全关键推理的精确性。实验表明，该方法在无菌违规检测、机器人阶段预测和下一步动作预测任务上优于传统图模型和基于大模型的基线。


<details>
  <summary>Details</summary>
Motivation: 现有手术场景图范式受限于严格的二元结构，依赖成对消息传递或分词序列会破坏关系结构的流形几何特性，难以有效建模复杂多模态手术室中的动态交互。

Method: 提出 TopoOR 框架，将手术室中的多模态实体及其关系建模为高阶拓扑细胞，利用高阶注意力机制在层次化关系注意力中保持曼德尔结构和模态特异性特征，避免跨模态融合导致的信息失真。

Result: 在无菌违规检测、机器人阶段预测和下一步动作预测任务上，TopoOR 显著优于传统图模型和基于大语言模型的基线方法，展现出更强的表达能力和推理性能。

Conclusion: TopoOR 通过高阶拓扑结构与注意力机制，实现了对多模态手术室环境更精准、更安全的建模，为复杂医疗场景下的智能系统提供了更高表达力的解决方案。
...
### [64] [The Patrologia Graeca Corpus: OCR, Annotation, and Open Release of Noisy Nineteenth-Century Polytonic Greek Editions](https://arxiv.org/abs/2603.09470)
*Chahan Vidal-Gorène,Bastien Kindt*

Main category: cs.CV

TL;DR: 本文提出了首个针对19世纪古希腊语版本的大型开源OCR与语言学资源——Patrologia Graeca Corpus，涵盖未数字化的Patrologia Graeca（PG）剩余卷册。该数据集包含复杂的双语（希腊语-拉丁语）排版和高度退化的多音调希腊文排版。通过结合基于YOLO的版面检测与基于CRNN的文本识别的专用处理流程，实现了1.05%的字符错误率（CER）和4.69%的词错误率（WER），显著优于现有古希腊语OCR系统。最终构建的语料库包含约六百万个经过词形还原和词性标注的词元，并附带完整的OCR与版面标注。该语料库不仅具有重要的文献学价值，还为噪声多音调希腊语的OCR设立了新基准，并可作为未来模型（包括大语言模型）的训练材料。


<details>
  <summary>Details</summary>
Motivation: 现有的古希腊语OCR系统在处理19世纪复杂排版、退化多音调字体的文献时表现不佳，而大量未数字化的Patrologia Graeca（PG）文献具有重要学术价值，亟需高效准确的数字化手段以支持后续研究。

Method: 采用基于YOLO的版面分析技术识别文档区域，结合基于CRNN的文本识别模型进行字符识别，并对识别结果进行后处理与校正，实现高精度的文本提取。整个流程专为处理双语布局与低质量多音调希腊文设计。

Result: 成功构建了包含六百万个已标注词元的Patrologia Graeca Corpus，达到1.05%的字符错误率（CER）和4.69%的词错误率（WER），显著超越现有系统性能。

Conclusion: 该工作不仅实现了对关键历史文献的大规模高质量数字化，也为多音调希腊语的OCR任务提供了新的基准，并为未来语言模型的训练与自然语言处理研究提供了宝贵资源。
...
### [65] [OmniEarth: A Benchmark for Evaluating Vision-Language Models in Geospatial Tasks](https://arxiv.org/abs/2603.09471)
*Ronghao Fu,Haoran Liu,Weijie Zhang,Zhiwen Lin,Xiao Yang,Peng Zhang,Bo Yang*

Main category: cs.CV

TL;DR: OmniEarth 是一个针对遥感视觉语言模型（RSVLMs）的综合性基准，涵盖感知、推理和鲁棒性三个维度，包含28个细粒度任务，支持多选和开放式VQA，采用盲测协议与语义一致性要求以减少语言偏差。该基准包含9,275张高质量图像和44,210条人工验证指令，用于评估多种VLMs在复杂地理空间任务中的表现，结果显示现有模型仍存在明显不足，亟需改进。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏系统性的基准来全面评估遥感视觉语言模型（RSVLMs）在真实地球观测场景下的性能，限制了该领域的发展与应用。

Method: 构建OmniEarth基准，涵盖多源传感数据与多样地理环境，设计28个细粒度任务，支持多种输出形式（文本、边界框、掩码），采用盲测与五重语义一致性要求以降低语言偏见，确保模型依赖视觉证据进行判断。

Result: 对多种对比学习模型、通用闭源与开源VLMs以及专门的RSVLMs进行系统评估，发现现有模型在复杂地理空间任务中表现不佳，存在显著性能差距。

Conclusion: OmniEarth为遥感视觉语言模型提供了首个系统化、高保真、可复现的评估框架，揭示了当前模型在地理空间理解上的局限性，为未来研究指明方向。
...
### [66] [Prune Redundancy, Preserve Essence: Vision Token Compression in VLMs via Synergistic Importance-Diversity](https://arxiv.org/abs/2603.09480)
*Zhengyao Fang,Pengyuan Lyu,Chengquan Zhang,Guangming Lu,Jun Yu,Wenjie Pei*

Main category: cs.CV

TL;DR: PruneSID提出了一种无需训练的协同重要性-多样性方法，通过两阶段管道实现高效视觉令牌压缩：首先利用主语义成分分析（PSCA）将令牌聚类为语义一致的组以保证概念覆盖；其次采用组内非最大抑制（NMS）去除冗余令牌并保留关键代表令牌。此外，引入基于图像复杂度的信息感知动态压缩比机制，优化不同场景下的信息保留效果。实验表明，该方法在保持高精度的同时显著降低计算开销，在多个模型和模态上均表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（VLMs）因生成过多冗余视觉令牌而存在严重的计算效率问题。尽管已有研究指出大量视觉令牌具有冗余性，但现有压缩方法难以在保留重要性和保持信息多样性之间取得良好平衡。因此需要一种更高效且能兼顾两者的新方法。

Method: PruneSID采用两阶段无训练压缩框架：(1) 主语义成分分析（PSCA），通过聚类将视觉令牌划分为语义连贯的组，确保全面覆盖关键概念；(2) 组内非最大抑制（NMS），在每组内移除冗余令牌，同时保留最具代表性的关键令牌。结合信息感知的动态压缩比例机制，根据图像复杂度自适应调整压缩率，提升整体信息保留能力。

Result: 在LLaVA-1.5上仅使用11.1%的视觉令牌即可达到96.3%的准确率，在LLaVA-NeXT上于极端压缩率（5.6%）下仍达92.8%准确率，优于现有方法2.5%，且预填充速度提升7.8倍。该方法在多种视觉语言模型及图像、视频模态中均表现优异，具备良好的跨模态泛化能力。

Conclusion: PruneSID是一种高效、通用且无需训练的视觉令牌压缩框架，通过协同优化重要性与多样性，显著提升了视觉语言模型的推理效率与准确性，适用于广泛的应用场景。
...
### [67] [Streaming Autoregressive Video Generation via Diagonal Distillation](https://arxiv.org/abs/2603.09488)
*Jinxiu Liu,Xuanming Liu,Kangfu Mei,Yandong Wen,Ming-HsuanYang,Weiyang Liu*

Main category: cs.CV

TL;DR: 提出Diagonal Distillation方法，通过不对称生成策略（早期多步、后期少步）和隐式光流建模，在减少推理步骤的同时提升视频生成的运动连贯性和质量，实现277.3倍加速，5秒视频仅需2.61秒生成。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型压缩方法多基于图像蒸馏，忽视时间依赖性，导致运动不连贯、误差累积和延迟-质量权衡问题。

Method: 提出Diagonal Distillation，采用不对称生成策略，利用前期充分去噪信息支持后期生成，并结合隐式光流建模以保持运动质量，缓解噪声水平预测偏差。

Result: 在2.61秒内生成5秒视频（最高达31 FPS），相比未蒸馏模型提速277.3倍，显著提升长序列生成的运动一致性和稳定性。

Conclusion: Diagonal Distillation通过有效利用时间上下文与优化生成流程，在极低推理步数下实现了高质量、高效率的视频生成，为实时视频生成提供了可行路径。
...
TL;DR: EvoPrompt 是一种新型的提示学习框架，旨在解决大规模视觉-语言模型在少量标注数据下微调时的灾难性遗忘问题。通过引入模态共享提示投影器（MPP）和进化训练策略，该方法将低秩更新分解为方向与幅度分量，保留早期学习的语义方向仅调整幅度，从而实现稳定的知识保留微调。同时，特征几何正则化（FGR）防止表示崩溃。实验表明，EvoPrompt 在少样本学习中表现优异，并有效保持预训练模型的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效提示学习方法在下游任务微调时容易导致预训练知识的灾难性遗忘，亟需一种能够稳定保留原始知识的同时有效适应新任务的方法。

Method: 提出 EvoPrompt 框架，包含模态共享提示投影器（MPP）生成分层提示，采用进化训练策略将低秩更新分解为方向与幅度分量，结合特征几何正则化（FGR）以防止表示崩溃，确保提示演化的稳定性与知识保留。

Result: EvoPrompt 在多个少样本学习任务上达到当前最优性能，同时显著保持了预训练模型的零样本能力，验证了其在知识保留与适应性方面的优越性。

Conclusion: 通过显式控制提示演化路径，EvoPrompt 实现了无遗忘的稳定微调，在少样本学习场景中兼具高性能与强鲁棒性，为大模型高效适配提供了新范式。
...
### [69] [SurgFed: Language-guided Multi-Task Federated Learning for Surgical Video Understanding](https://arxiv.org/abs/2603.09496)
*Zheng Fang,Ziwei Niu,Ziyue Wang,Zhu Zhuo,Haofeng Liu,Shuyang Qian,Jun Xia,Yueming Jin*

Main category: cs.CV

TL;DR: SurgFed 是一种用于手术场景分割和深度估计的多任务联邦学习框架，针对组织多样性与任务多样性挑战，提出语言引导的通道选择（LCS）和语言引导的超聚合（LHA）机制，显著提升跨站点、跨任务的模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习在手术视频理解中受限于组织多样性和任务异构性，导致本地模型适应差、服务器端聚合效果不佳，影响手术场景理解精度。

Method: 提出 SurgFed 框架，结合语言引导的通道选择（LCS）以增强局部模型对特定组织特征的适应能力，并引入语言引导的超聚合（LHA）通过层间交叉注意力建模跨站点任务交互，实现个性化参数更新。

Result: 在五个公开数据集上，SurgFed 在四种不同手术类型中均优于现有最先进方法，验证了其有效性与泛化能力。

Conclusion: SurgFed 有效解决了多任务联邦学习在复杂手术环境中的适配难题，为机器人辅助微创手术中的智能视觉理解提供了可扩展、鲁棒的解决方案。
...
TL;DR: Context-Nav 提出一种无需任务特定训练的文本目标实例导航方法，通过全局探索先验和3D空间推理实现精确的目标定位。它利用密集的文本-图像对齐生成价值图以指导探索，并在观察候选目标时进行视角感知的空间关系验证，确保从至少一个视角满足空间关系。该方法在 InstanceNav 和 CoIN-Bench 上达到最先进性能，实验证明完整描述编码与显式3D验证可减少无效移动并避免错误停靠，表明基于几何的空间推理是细粒度实例消歧的有效可扩展方案。


<details>
  <summary>Details</summary>
Motivation: 在复杂3D场景中，如何根据自由形式的自然语言描述准确找到特定物体实例（尤其是存在同类别干扰物时）是一个挑战。传统方法依赖局部匹配或大量训练数据，难以泛化且易产生误判。本文旨在提出一种无需额外训练、能有效利用完整上下文信息并结合3D空间合理性验证的高效导航策略。

Method: 1. 构建密集文本-图像对齐的价值图，用于评估环境中的前沿区域，引导智能体朝向与整个描述一致的区域探索；2. 在发现候选目标后，采用视角感知的关系检查：采样可能的观察者位姿，对齐局部坐标系，仅当至少一个视角下空间关系可满足时才接受该目标为正确答案。整个流程不依赖任务特定训练或微调。

Result: 在 InstanceNav 与 CoIN-Bench 基准上均取得当前最优性能。消融实验表明：(i) 将完整句子编码进价值图可显著减少无效运动；(ii) 显式的视角感知3D验证能有效排除语义合理但空间上不成立的错误目标。

Conclusion: 基于几何的空间推理是一种替代复杂策略训练或人工干预的可行且可扩展的方法，适用于在杂乱3D环境中进行细粒度实例消歧。
...
TL;DR: 本文研究视觉-语言模型（VLMs）作为驾驶助手时的可靠性，重点考察其在时间上下文中的推理一致性与未来事件预测能力。发现尽管模型具备强视觉理解能力，但其输出常因输入微小变化而波动，且缺乏对时序事件的合理推理，往往依赖预训练模式而非动态建模。为此，作者提出FutureVQA数据集以评估未来场景推理，并设计一种基于思维链的自监督微调方法，有效提升模型的一致性与时间推理能力，无需依赖时间标签。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中对VLMs的应用多集中于场景理解和指令生成，普遍假设强大的视觉解析能力自然带来一致的未来推理，但该假设缺乏验证。实际中，模型存在响应不一致和时间推理能力弱的问题，亟需系统评估与改进。

Method: 引入FutureVQA这一人工标注的基准数据集，专门用于评估模型对未来场景的推理能力；提出一种基于思维链（chain-of-thought）的自监督微调方法，通过生成中间推理步骤增强模型对时间序列事件的理解，从而提升一致性与时间推理性能。

Result: 实验表明，即使具备强大视觉理解能力的模型，在时间推理任务上表现不佳，且对输入扰动敏感。所提出的自监督方法显著提升了模型在一致性与未来推理方面的表现，且无需额外的时间标注数据。

Conclusion: VLMs在驾驶辅助应用中仍面临严重的响应不一致与时间推理缺陷，单纯依赖视觉理解不足以保证可靠决策。通过引入专门的评估基准和自监督的思维链训练策略，可有效改善模型的时间建模能力，为构建更可靠的驾驶助手提供可行路径。
...
### [72] [Towards Unified Multimodal Interleaved Generation via Group Relative Policy Optimization](https://arxiv.org/abs/2603.09538)
*Ming Nie,Chunwei Wang,Jianhua Han,Hang Xu,Li Zhang*

Main category: cs.CV

TL;DR: 本文提出一种基于强化学习的后训练策略，以提升现有统一视觉-语言模型在多模态交错输出方面的能力，无需依赖大规模多模态交错数据集。通过混合数据集进行预热阶段，使模型接触交错生成模式并保留原有能力。进一步提出统一的策略优化框架，扩展了组相对策略优化（GRPO）至多模态场景，联合建模文本与图像生成，并使用包含文本相关性、图文对齐和结构保真度的混合奖励进行优化。同时引入过程级奖励，提供逐步指导，提升复杂任务中的训练效率。实验表明，该方法显著提升了多模态交错生成的质量与连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有统一视觉-语言模型在多模态交错输出（如视觉叙事、分步视觉推理）方面表现不足，而获取大规模多模态交错数据成本高且难以获得。因此需要一种不依赖大规模标注数据的后训练方法来激活模型的交错生成能力。

Method: 提出一种基于强化学习的后训练策略：1）通过混合数据集（包含精选交错序列与有限的多模态理解/文生图数据）进行预热阶段；2）设计统一策略优化框架，将组相对策略优化（GRPO）扩展到多模态场景；3）在单一解码轨迹中联合建模文本与图像生成；4）采用混合奖励机制，涵盖文本相关性、视觉-文本对齐和结构保真度；5）引入过程级奖励，实现步骤级引导，提升训练效率。

Result: 在MMIE和InterleavedBench基准上，所提方法显著提升了多模态交错生成的质量与连贯性，证明了其有效性与泛化能力。

Conclusion: 本工作成功通过强化学习后训练策略，使现有统一视觉-语言模型具备强大的多模态交错生成能力，无需大规模标注数据，为复杂多模态任务提供了高效可行的解决方案。
...
### [73] [Memory-Guided View Refinement for Dynamic Human-in-the-loop EQA](https://arxiv.org/abs/2603.09541)
*Xin Lu,Rui Li,Xun Huang,Weixin Li,Chuanqing Zhuang,Jiayuan Li,Zhengda Lu,Jun Xiao,Yunhong Wang*

Main category: cs.CV

TL;DR: 提出DIVRR框架以应对动态场景中视觉线索短暂且视角依赖的问题，通过视图精炼和自适应记忆选择提升EQA在动态环境中的鲁棒性和推理效率。


<details>
  <summary>Details</summary>
Motivation: 传统EQA评估在静态环境中进行，但真实动态场景中人类活动和遮挡导致感知非平稳性，现有方法因累积冗余证据而增加推理成本，需解决视角依赖遮挡带来的歧义及保持紧凑更新的证据。

Method: 提出DIVRR框架，结合相关性引导的视图精炼与选择性记忆接纳机制，在不训练的情况下验证模糊观测并仅保留信息量大的证据。

Result: 在DynHiL-EQA和HM-EQA数据集上的实验表明，DIVRR在动态与静态环境下均优于现有基线，同时保持高推理效率。

Conclusion: DIVRR有效提升了EQA在动态、人类参与场景下的性能与效率，为复杂现实环境中的视觉问答提供了新范式。
...
### [74] [A comprehensive study of time-of-flight non-line-of-sight imaging](https://arxiv.org/abs/2603.09548)
*Julio Marco,Adrian Jarabo,Ji Hyun Nam,Alberto Tosi,Diego Gutierrez,Andreas Velten*

Main category: cs.CV

TL;DR: 本文对多种时间飞行非视域（ToF NLOS）成像方法进行了综合性研究，通过统一的前向模型和硬件设置评估其理论与实验性能。研究指出，尽管方法各异，但在相同硬件条件下，现有方法在空间分辨率、可见性和噪声敏感性方面表现出相似的局限性，差异主要源于方法特有参数。该工作为未来研究提供了客观比较的参考框架。


<details>
  <summary>Details</summary>
Motivation: 现有ToF NLOS成像方法在公式和硬件实现上差异较大，缺乏统一评估标准，难以客观比较其性能。本文旨在建立统一分析框架，揭示不同方法的共性与差异，推动领域内公平比较与技术进步。

Method: 提出一个通用的前向模型来描述ToF NLOS测量过程，基于典型假设构建可处理的逆模型；分析其与Radon变换家族的关系，并将模型迁移至频域，关联到基于相位的虚拟视距成像模型；在相同硬件和光子数条件下对代表性方法进行实验对比。

Result: 实验表明，在相同硬件约束下，现有方法在空间分辨率、可见性及噪声鲁棒性方面表现相近，主要差异源于各自的方法参数设计。统一评估框架有助于未来新方法的客观验证与比较。

Conclusion: 本文提出的统一分析框架可作为未来ToF NLOS成像研究的基准，促进方法间的公平比较，推动技术发展与标准化。
...
### [75] [GeoSolver: Scaling Test-Time Reasoning in Remote Sensing with Fine-Grained Process Supervision](https://arxiv.org/abs/2603.09551)
*Lang Sun,Ronghao Fu,Zhuoran Duan,Haoran Liu,Xueyan Liu,Bo Yang*

Main category: cs.CV

TL;DR: 提出GeoSolver框架，通过构建大规模、细粒度的过程监督数据集Geo-PRM-2M，并训练一个细粒度过程奖励模型GeoPRM，结合过程感知的Tree-GRPO强化学习算法，实现遥感图像中可验证的链式思维推理。该方法显著提升模型在遥感任务上的表现，并支持测试时扩展（TTS），具备跨模型通用性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在遥感解译中虽已取得进展，但复杂推理能力仍受限于中间步骤的视觉忠实性问题。尽管引入链式思维（CoT）有一定成效，但如何确保推理过程与视觉内容一致仍是关键挑战。

Method: 采用熵引导的蒙特卡洛树搜索（MCTS）与针对性视觉幻觉注入生成大规模、细粒度的过程监督数据；基于此训练一个令牌级过程奖励模型（GeoPRM）以提供忠实性反馈；设计过程感知的Tree-GRPO强化学习算法，融合树状探索与忠实性加权奖励机制，精确分配中间步骤的信用。

Result: GeoSolver-9B模型在多个遥感基准测试中达到当前最优性能；GeoPRM支持强大的测试时扩展（TTS），可作为通用地理空间验证器，显著提升GeoSolver-9B及通用视觉语言模型的性能，表现出优异的跨模型泛化能力。

Conclusion: GeoSolver框架通过过程监督与强化学习实现了可验证的遥感推理，解决了视觉忠实性瓶颈，为复杂遥感分析提供了新范式，并展现出广泛的应用潜力。
...
### [76] [GeoAlignCLIP: Enhancing Fine-Grained Vision-Language Alignment in Remote Sensing via Multi-Granular Consistency Learning](https://arxiv.org/abs/2603.09566)
*Xiao Yang,Ronghao Fu,Zhuoran Duan,Zhiwen Lin,Xueyan Liu,Bo Yang*

Main category: cs.CV

TL;DR: 提出GeoAlignCLIP框架，通过多粒度语义对齐和模态内一致性，实现遥感图像与文本的细粒度对齐；构建了RSFG-100k数据集，包含场景描述、区域级标注和难负样本，支持层次化监督训练；在多个公开遥感基准上实验表明，该方法在细粒度任务中表现更优，具有更强的鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言预训练模型在遥感任务中主要依赖全局图像-文本对齐，难以有效整合多粒度视觉与文本信息，导致在复杂细粒度任务中性能受限。

Method: 提出GeoAlignCLIP框架，学习多粒度语义对齐，并引入模态内一致性，增强图像区域与文本概念之间的精确对齐；构建RSFG-100k数据集，提供细粒度标注与硬负样本以支持训练。

Result: 在多个公开遥感基准上，GeoAlignCLIP显著优于现有方法，在细粒度任务中表现出更强的对齐能力、鲁棒性与准确性。

Conclusion: GeoAlignCLIP通过多粒度对齐与数据集设计，有效提升了遥感图像与文本间的细粒度语义对齐能力，为复杂遥感理解任务提供了新范式。
...
### [77] [More than the Sum: Panorama-Language Models for Adverse Omni-Scenes](https://arxiv.org/abs/2603.09573)
...
TL;DR: 本文提出全景语言建模（PLM）范式，通过单张全景图像实现更全面的视觉-语言推理，超越传统多视角拼接方法。引入PanoVQA数据集，涵盖复杂全景场景，支持在遮挡和事故等挑战下进行综合推理。设计了即插即用的全景稀疏注意力模块，使现有基于针孔相机的视觉语言模型无需重新训练即可处理等距柱状投影全景图。实验表明，PLM在复杂全景场景中表现出更强的鲁棒性和整体推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型主要针对针孔图像设计，通过拼接多个窄视场输入来理解全景场景，但忽略了单张全景图所固有的全局空间和上下文关系。因此，亟需一种能够充分利用全景图像完整信息的新范式。

Method: 提出全景语言建模（PLM）范式，构建PanoVQA大规模全景视觉问答数据集，并设计一个可直接集成到现有模型中的全景稀疏注意力模块，使基于针孔的VLM能高效处理等距柱状投影全景图。

Result: 实验结果显示，所提出的PLM在复杂全景场景中展现出显著增强的鲁棒性与整体推理能力，实现了超越传统多视角方法的综合理解效果。

Conclusion: 本工作首次系统性地探索了全景语言建模的可能性，证明了利用单一全景图像进行统一视觉-语言推理的有效性，为未来全景智能理解奠定了基础。
...
### [78] [BinaryAttention: One-Bit QK-Attention for Vision and Diffusion Transformers](https://arxiv.org/abs/2603.09582)
*Chaodong Xiao,Zhengqiang Zhang,Lei Zhang*

Main category: cs.CV

TL;DR: 本文提出BinaryAttention，一种用于视觉和扩散模型中1比特量化注意力的高效方法。通过仅保留查询和键的符号并使用位运算替代浮点点积，显著降低计算开销。引入可学习偏置以缓解1比特量化带来的信息损失，并结合量化感知训练与自蒸馏技术，保持注意力精度。实验表明，BinaryAttention在A100 GPU上速度超过FlashAttention2两倍以上，在多个视觉与扩散变压器基准测试中达到或超越全精度注意力性能，为低比特视觉与扩散模型提供了高效且有效的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有方法多采用8比特或4比特量化来平衡效率与精度，但计算复杂度仍是视觉任务中的主要瓶颈。本文旨在探索更极端的1比特量化可行性，以进一步提升计算效率，同时保持注意力机制的关键相似性关系。

Method: 提出BinaryAttention方法：仅保留查询和键的符号，用位运算代替浮点乘法；引入可学习偏置缓解1比特量化信息损失；结合量化感知训练与自蒸馏技术减少量化误差，实现端到端加速。

Result: 在A100 GPU上，BinaryAttention比FlashAttention2快2倍以上；在视觉Transformer和扩散Transformer的多个基准测试中，性能达到或超过全精度注意力模型，证明其高效性与有效性。

Conclusion: BinaryAttention通过理论支持与有效设计，实现了1比特注意力的快速且高精度计算，为低比特视觉与扩散模型提供了强有力的工具，推动了该领域的前沿发展。
...
TL;DR: 受人类视觉系统启发，本文提出一种类扫视的图像处理方法，利用DINO模型生成的注意力图引导模型聚焦于视觉关键区域，显著提升计算效率并保持甚至超越全图处理的分类性能。该方法在ImageNet分类任务中表现优异，且其注意力机制优于传统人类注视预测模型，为高效、类脑视觉处理提供了新思路。


<details>
  <summary>Details</summary>
Motivation: 人类视觉系统在代谢限制下仍能实现卓越的感知性能，关键在于通过快速眼动（扫视）将高分辨率中央凹定位到任务相关区域；而传统AI系统对图像所有部分同等处理，效率低下。因此，需要借鉴人类视觉的注意力机制，设计更智能、高效的图像处理模型。

Method: 基于自监督Vision Transformer模型DINO生成的注意力图，模拟人眼扫视行为，分阶段聚焦于图像的关键区域进行信息处理；通过ImageNet分类任务评估每轮‘扫视’对分类得分的影响，实现选择性处理。

Result: 所提方法在保持接近全图处理性能的同时，显著降低计算成本，某些情况下甚至优于全图处理；且其注意力图在指导视觉关注方面优于现有主流人类注视预测模型。

Conclusion: Vision Transformer的注意力机制可作为生物启发式主动视觉的有力基础，为未来高效、类脑的视觉处理系统开辟了新方向。
...
TL;DR: 提出了一种零样本的MRI超分辨率框架，利用显式高斯表示在减少数据依赖和提升效率之间取得平衡。通过嵌入组织物理特性的定制化高斯参数，降低可学习参数数量并保持磁共振信号保真度；结合基于物理的体渲染策略与砖块式无序光栅化方案，实现高度并行的3D计算，显著降低训练与推理成本。在两个公开MRI数据集上的实验表明，该方法在重建质量和效率方面均表现优越，具备临床应用潜力。


<details>
  <summary>Details</summary>
Motivation: 现有MRI超分辨率方法存在两难：成对数据方法依赖昂贵的配准数据集以实现高效重建，而隐式神经表示方法虽无需此类数据但计算开销巨大。因此亟需一种既不依赖大量标注数据、又具备高效率的解决方案，以推动高分辨率MRI在临床中的实际应用。

Method: 提出基于显式高斯表示的零样本MRI超分辨率框架。通过设计针对MRI特性的高斯参数，嵌入组织的物理属性，从而减少模型可学习参数；采用基于物理的体渲染策略模拟磁共振信号形成过程，利用归一化高斯聚合建模信号生成；引入砖块式顺序无关的光栅化机制，支持高效并行3D计算，降低整体计算成本。

Result: 在两个公开的MRI数据集上，所提方法在重建图像质量（如PSNR、SSIM）和计算效率（训练/推理时间）方面均优于现有主流方法。尤其在零样本设置下仍能保持优异性能，验证了其对未见数据的良好泛化能力。

Conclusion: 该研究成功构建了一个兼顾数据效率与计算高效的零样本MRI超分辨率框架，通过显式高斯表示与物理引导的渲染机制，在无需配对数据的前提下实现了高质量、低延迟的高分辨率重建，为临床实用化高分辨率MRI提供了可行路径。
...
### [81] [Decoder-Free Distillation for Quantized Image Restoration](https://arxiv.org/abs/2603.09624)
*S. M. A. Sharif,Abdur Rehman,Seongwan Kim,Jaeho Lee*

Main category: cs.CV

TL;DR: 提出QDR框架，解决低层视觉图像恢复中量化感知训练与知识蒸馏联合优化的三大瓶颈：教师-学生模型能力不匹配、解码器蒸馏中的空间误差放大、重建与蒸馏损失间的优化冲突。通过FP32自蒸馏消除能力差异，采用无解码器蒸馏（DFD）在瓶颈处修正量化误差，并引入可学习幅度重加权（LMR）动态平衡梯度。设计轻量级边缘友好模型（EFM）含可学习退化门控（LDG），实现空间退化动态调制。实验表明，Int8模型恢复96.5% FP32性能，Jetson Orin上达442 FPS，下游检测提升16.3 mAP。


<details>
  <summary>Details</summary>
Motivation: 现有量化感知训练与知识蒸馏联合方法在低层视觉图像恢复任务中存在教师-学生容量不匹配、解码器蒸馏导致的空间误差放大及重建与蒸馏损失间因量化噪声引发的优化冲突，亟需针对性解决方案以实现高效边缘部署。

Method: 提出QDR框架，包含FP32自蒸馏消除容量差异，Decoder-Free Distillation（DFD）在瓶颈处修正量化误差，可学习幅度重加权（LMR）动态平衡重建与蒸馏梯度，以及轻量级边缘友好模型（EFM）结合可学习退化门控（LDG）实现空间退化动态调节。

Result: Int8模型恢复96.5%的FP32性能，在NVIDIA Jetson Orin上达到442帧每秒，下游目标检测任务中mAP提升16.3个百分点，显著优于现有方法。

Conclusion: QDR框架有效解决了图像恢复任务中量化感知训练与知识蒸馏联合优化的关键挑战，实现了高性能、高效率的边缘部署，为低层视觉模型压缩提供了新范式。
...
### [82] [Grounding Synthetic Data Generation With Vision and Language Models](https://arxiv.org/abs/2603.09625)
*Ümit Mert Çağlar,Alptekin Temizel*

Main category: cs.CV

TL;DR: 提出一种基于视觉-语言的可解释合成数据增强与评估框架，结合生成模型、语义分割和图像描述技术，构建大规模遥感数据集ARAS400k（100k真实+300k合成图像），支持语义组成分析、减少描述冗余和跨模态一致性验证。实验表明，合成数据增强显著提升下游任务性能，尤其在语义分割和图像描述任务中优于纯真实数据基线。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据评估方法依赖难以解释的潜在特征相似性，且与下游任务表现关联性弱；为提升合成数据质量与可解释性，需建立更有效的评估框架。

Method: 融合生成模型、语义分割与图像描述技术，利用视觉-语言模型实现跨模态对齐，通过语义组成分析、描述冗余最小化和跨模态一致性验证进行合成数据评估。

Result: 仅用合成数据训练的模型达到有竞争力的性能，而使用真实与合成数据混合训练的模型持续优于纯真实数据基线；该框架可有效评估合成数据价值并推动遥感任务发展。

Conclusion: 本工作建立了面向遥感语义分割与图像描述任务的可扩展基准，提出可解释的合成数据增强与评估方法，推动高质量合成数据在遥感领域的应用。
...
TL;DR: 提出KV-Lock框架，通过动态调度背景键值融合比例与分类器自由引导尺度，利用扩散幻觉检测实现视频编辑中前景质量提升与背景一致性保持，无需训练，可即插即用。


<details>
  <summary>Details</summary>
Motivation: 解决视频编辑中增强前景质量时背景不一致的问题，避免全图信息注入导致的背景伪影或严格锁定背景对前景生成能力的限制。

Method: 基于扩散模型中的幻觉度量（去噪预测方差）与分类器自由引导尺度的关系，设计动态调度机制，实时调整背景键值缓存与新生成键值的融合比例及引导强度，以降低幻觉风险并提升生成质量。

Result: 在多种视频编辑任务中，该方法显著提升了前景质量，同时保持了高背景保真度，优于现有方法，且无需额外训练，兼容预训练DiT模型。

Conclusion: KV-Lock是一种无需训练、可即插即用的视频编辑框架，有效平衡了前景生成与背景一致性，为基于扩散模型的视频编辑提供了高效解决方案。
...
TL;DR: 本研究探讨基于Transformer架构的越南语视觉问答（VQA）任务，结合文本与视觉预训练模型，系统比较多语言环境下自动评估指标的表现。


<details>
  <summary>Details</summary>
Motivation: 早期VQA系统过度依赖语言偏差，促使后续研究强调视觉定位和平衡数据集；针对低资源多模态学习的越南语VQA数据集（如ViVQA、OpenViVQA、ViTextVQA）的出现，推动了该领域的发展。同时，现有自动评估指标（如BLEU、METEOR、CIDEr）与人类判断的对齐度不足，亟需改进。

Method: 采用基于Transformer的架构，融合PhoBERT（越南语语言理解）与Vision Transformer（ViT，图像表示学习），在越南语多模态场景下进行视觉问答建模，并系统比较多种自动评估指标在多语言设置下的表现。

Result: 实验表明，结合文本与视觉预训练的模型在越南语VQA任务上取得显著性能提升；使用大语言模型辅助评估可有效提高自动指标与人类判断的一致性。

Conclusion: 本研究验证了多模态预训练模型在越南语VQA中的有效性，并指出改进自动评估机制对推动低资源多模态学习至关重要。
...
TL;DR: DiffWind提出了一种基于物理的可微分框架，用于从视频中建模风驱动物体的动力学。通过将风表示为网格化的物理场，物体表示为基于3D高斯点云的粒子系统，并利用材料点法（MPM）建模交互。该方法联合优化时空风场与物体运动，结合可微渲染与模拟实现重建，并引入格子玻尔兹曼方法（LBM）确保物理一致性。同时支持新风条件下的前向仿真与风力重定向等应用。作者还构建了WD-Objects数据集，实验表明其在重建精度和仿真保真度上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 风的不可见性、时空变化性以及物体的复杂形变使得从视频中建模风驱动物体动力学极具挑战。现有方法难以同时保证物理合理性与高精度重建，亟需一种统一且物理一致的建模框架。

Method: 将风建模为网格化物理场，物体用3D高斯点云生成的粒子系统表示，通过材料点法（MPM）模拟风与物体的交互；采用可微渲染与模拟联合优化风场与物体运动；引入格子玻尔兹曼方法（LBM）作为物理约束以满足流体动力学规律；支持前向仿真与风力重定向等应用。

Result: 在合成与真实场景下均表现出优越的重建精度与仿真保真度，显著优于现有动态场景建模方法；验证了框架在风力重定向、前向模拟等新应用中的有效性；所构建的WD-Objects数据集为后续研究提供了基准。

Conclusion: DiffWind提供了一个统一、物理一致且可微的框架，实现了从视频中高精度重建风驱动物体动力学，并支持灵活的前向仿真与风力调控，为视频驱动的风-物交互建模开辟了新路径。
...
TL;DR: VarSplat 是一种不确定性感知的 3DGS-SLAM 系统，通过显式学习每个高斯点的外观方差，利用总方差定律与 alpha 合成生成可微的像素级不确定性图，指导跟踪、子地图配准和回环检测聚焦于可靠区域，从而提升全局一致性与鲁棒性。在 Replica、TUM-RGBD、ScanNet 和 ScanNet++ 数据集上的实验表明，该方法在稠密 RGB-D SLAM 中实现了优于或相当的定位、建图及新视角合成效果。


<details>
  <summary>Details</summary>
Motivation: 现有 3DGS-SLAM 方法对测量可靠性处理隐式，导致在低纹理区域、透明表面或复杂反射区域易出现位姿估计漂移，亟需引入不确定性感知机制以提升系统鲁棒性。

Method: 提出 VarSplat，通过在 3DGS 框架中引入每高斯点的外观方差，并结合 alpha 合成与总方差定律，在单次光栅化中高效生成可微的像素级不确定性图，用于指导后续的跟踪、子地图注册和回环检测。

Result: 在 Replica（合成）以及 TUM-RGBD、ScanNet、ScanNet++（真实世界）数据集上，VarSplat 显著提升了系统在低纹理、透明或复杂反射场景下的鲁棒性，实现了竞争性或更优的跟踪精度、重建质量与新视角合成效果。

Conclusion: VarSplat 通过显式建模并利用外观不确定性，有效缓解了传统 3DGS-SLAM 在挑战性场景中的漂移问题，为高保真、鲁棒的实时稠密 SLAM 提供了一种新的可微分解决方案。
...
TL;DR: FootMR 是一种用于精炼人体运动恢复模型中足部运动的方法，通过将2D足部关键点序列提升至3D，避免了图像-3D标注对的不准确性，利用大规模动作捕捉数据进行训练。该方法结合膝关节和足部运动作为上下文，仅预测残差足部运动，并采用全局旋转表示和数据增强以提升极端足部姿态下的泛化能力。为评估足部运动重建，提出了MOOF数据集。实验表明，FootMR在多个基准上优于现有方法，踝关节角度误差降低达30%。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从野外视频中恢复3D人体运动时，难以捕捉足部等细粒度关节的运动，主要由于训练数据中足部标注不准确且运动多样性不足。此问题影响步态分析与动画等应用。

Method: FootMR通过2D足部关键点序列到3D的提升实现足部运动精炼；不依赖图像输入，使用大规模动作捕捉数据；引入膝关节与足部运动作为上下文信息，仅预测残差运动；采用全局旋转表示，并结合数据增强策略以提升极端姿态下的泛化能力。

Result: 在MOOF、MOYO和RICH数据集上的实验表明，FootMR显著优于现有视频基方法，踝关节角度误差最多降低30%，验证了其在足部运动重建方面的有效性。

Conclusion: FootMR通过有效利用动作捕捉数据并设计合理的运动建模方式，在足部精细运动重建方面取得突破，为高精度人体运动估计提供了新思路。
...
### [88] [From Semantics to Pixels: Coarse-to-Fine Masked Autoencoders for Hierarchical Visual Understanding](https://arxiv.org/abs/2603.09955)
*Wenzhao Xiang,Yue Wu,Hongyang Yu,Feng Gao,Fan Yang,Xilin Chen*

Main category: cs.CV

TL;DR: C2FMAE提出一种从粗到细的掩码自编码器，通过在场景、对象和像素三个层次上学习层级视觉表示，解决对比学习（CL）与掩码图像建模（MIM）之间的张力。其核心创新包括：级联解码器实现从语义到实例再到像素的逐级重建，建立跨粒度依赖；以及渐进式掩码课程，逐步从语义引导转向随机掩码，形成由全局到局部的结构化学习路径。基于128万图像的多粒度数据集验证了该方法在分类、检测和分割任务上的显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有自监督视觉预训练方法中，对比学习虽能捕捉全局语义但丢失细节，而掩码图像建模虽保留局部纹理却因随机掩码导致注意力漂移。二者存在内在矛盾，亟需一种能够兼顾全局与局部信息并实现层级学习的新范式。

Method: 提出C2FMAE框架，包含：(1) 级联解码器，按从场景语义到物体实例再到像素细节的顺序进行重建，显式建模跨粒度依赖关系；(2) 渐进式掩码课程，动态调整掩码策略，从语义引导逐渐过渡到随机掩码，引导模型从全局上下文逐步聚焦局部特征。同时构建大规模多粒度数据集以支持该框架。

Result: 在图像分类、目标检测和语义分割等多个下游任务中均取得显著性能提升，证明了所提层级设计在学习更鲁棒、泛化性更强的视觉表征方面的有效性。

Conclusion: C2FMAE通过显式建模多层次视觉表示与结构化学习路径，有效缓解了自监督预训练中的全局-局部权衡问题，为构建更具表达力的视觉模型提供了新思路。
...
TL;DR: 提出TriFusionSR，一种基于小波引导的条件扩散框架，用于联合三模态医学图像融合与超分辨率（SR），通过小波变换分解多模态特征并实现频率感知的跨模态交互，结合修正的小波特征校准和自适应空间-频率融合模块，显著提升图像质量与细节还原能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法将图像融合与超分辨率分阶段处理，导致伪影和感知质量下降；在三模态设置下，由于模态间频率域不平衡问题，性能进一步受限。

Method: 采用2D离散小波变换对多模态特征进行频带分解，实现频率感知的跨模态交互；引入修正小波特征（RWF）策略校准潜在系数，并设计带有门控通道-空间注意力的自适应空间-频率融合（ASFF）模块，以结构驱动方式优化多模态融合。

Result: 在多个上采样尺度下均达到领先性能，PSNR提升4.8-12.4%，同时大幅降低RMSE和LPIPS值，验证了方法在图像保真度与视觉质量上的优越性。

Conclusion: TriFusionSR通过联合建模融合与超分辨率过程，有效缓解了多模态医学图像中的分辨率退化与模态差异问题，为精准诊断提供了高质量融合图像支持。
...
TL;DR: 本文提出了一种高效的人工智能助手个性化方法，利用多模态语言模型内部的注意力机制提取代表特定概念的视觉标记，作为该概念的记忆，从而在测试图像中识别并描述该概念。该方法无需额外训练或复杂工程管道，具有低开销和高效率的优势，在单概念、多概念及视频个性化任务中均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有个性化方法依赖额外训练或外部预训练模块，限制了模型的通用性与可扩展性，且部署效率低下。因此需要一种更高效、无需额外训练的个性化方法。

Method: 通过分析模型内部注意力机制，提取主要表征目标概念的视觉标记，将其作为概念记忆，用于后续图像中的概念识别与描述。

Result: 在多种个性化场景（单概念、多概念、视频）下，该方法均表现出显著性能提升，同时保持极低的个性化开销，优于现有最先进方法。

Conclusion: 该方法充分利用了模型自身的能力，实现高效、可扩展的个性化，为日常生活中人工智能助手的个性化应用提供了可行路径。
...
TL;DR: 提出Matrix Attention机制，通过帧级矩阵操作实现高效全局时空建模，构建FrameDiT-G和FrameDiT-H模型，在保持效率的同时显著提升视频生成的时序连贯性和质量，达到当前最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在建模复杂时空动态时面临计算成本高与时间建模能力有限之间的权衡，尤其是全3D注意力计算昂贵，局部因子化注意力难以捕捉大范围运动。

Method: 提出一种帧级时间注意力机制Matrix Attention，将整帧视为矩阵，通过矩阵原生操作生成查询、键、值矩阵，实现跨帧全局关注；基于此构建FrameDiT-G，并结合局部因子化注意力设计FrameDiT-H以同时处理大、小运动。

Result: 在多个视频生成基准上表现优异，相比现有方法在时序连贯性与视频质量方面均有提升，且计算效率接近局部因子化注意力。

Conclusion: Matrix Attention有效解决了视频扩散模型中时空建模的效率与表达力矛盾，FrameDiT-H实现了高质量、高效率的视频生成，为未来视频生成模型提供了新范式。
...
TL;DR: 该论文提出M^2-Occ框架，用于在多摄像头输入不完整的情况下进行语义占据预测。通过多视图掩码重建（MMR）模块和特征记忆模块（FMM），分别恢复缺失视图的特征并利用全局语义先验提升语义一致性。在nuScenes-based SurroundOcc基准上，该方法在后视图缺失等安全关键场景下显著提升性能，最大IoU提升达5.01%，且不影响全视图表现。


<details>
  <summary>Details</summary>
Motivation: 现有基于摄像头的语义占据预测方法假设拥有完整的环视观测，但现实中常因遮挡、硬件故障或通信问题导致视角缺失。为应对这一现实挑战，需要一种能在不完整多摄像头输入下保持几何结构与语义一致性的方法。

Method: 提出M^2-Occ框架，包含两个核心组件：1）多视图掩码重建（MMR）模块，利用相邻摄像头间的空间重叠，在特征空间中直接恢复缺失视图表示；2）特征记忆模块（FMM），引入可学习的记忆库存储类别级语义原型，通过检索并融合这些全局先验来修正模糊体素特征，增强语义一致性。

Result: 在nuScenes-based SurroundOcc基准上的实验表明，M^2-Occ在后视图缺失场景下提升IoU 4.93%；当五个视图缺失时，提升达5.01%。该方法在保持全视图性能的同时，显著提升了在缺失视角下的鲁棒性。

Conclusion: M^2-Occ有效解决了多摄像头输入不完整条件下的语义占据预测问题，通过特征重建与语义记忆机制，在真实复杂场景中实现了更强的鲁棒性和一致性，具有实际部署价值。
...
TL;DR: 提出ENIGMA-360数据集，包含在真实工业场景中采集的180段同步的头戴式（ego）和外部视角（exo）视频，提供时空标注，支持人类行为理解研究。通过三个基础任务的基线实验，揭示现有方法在该场景下的局限性，强调需要新模型实现鲁棒的跨视角行为理解。数据集已公开发布。


<details>
  <summary>Details</summary>
Motivation: 现有研究受限于缺乏真实工业场景下同时包含头戴式与外部视角的多视角数据集，阻碍了对人类行为的全面理解及工业安全系统的发展。

Method: 在真实工业环境中采集180段头戴式与180段外部视角的同步视频，构建ENIGMA-360数据集，并进行时空标注，用于支持多视角行为分析。

Result: 基线实验表明当前最先进的方法在时间动作分割、关键步骤识别和头戴式人-物体交互检测任务上表现有限，凸显了在真实复杂环境中实现跨视角理解的挑战。

Conclusion: ENIGMA-360数据集为研究工业场景中人类行为提供了重要资源，推动开发更鲁棒的多视角理解模型，提升工业环境中的安全与效率。
...
### [94] [Adaptive Clinical-Aware Latent Diffusion for Multimodal Brain Image Generation and Missing Modality Imputation](https://arxiv.org/abs/2603.09931)
*Rong Zhou,Houliang Zhou,Yao Su,Brian Y. Chen,Yu Zhang,Lifang He,Alzheimer's Disease Neuroimaging Initiative*

Main category: cs.CV

TL;DR: ACADiff 是一种基于自适应临床感知扩散的多模态脑成像补全框架，可有效合成缺失的阿尔茨海默病影像数据（如sMRI、FDG-PET、AV45-PET），在极端80%数据缺失情况下仍保持优异生成质量与诊断性能，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 临床多模态神经影像数据常因缺失模态而影响阿尔茨海默病诊断，亟需高效且鲁棒的跨模态补全方法。

Method: 提出自适应临床感知扩散框架（ACADiff），通过逐步去噪潜在表示并融合可用影像与临床元数据，实现多模态间双向合成；采用动态融合机制和基于GPT-4o提示的语义引导增强生成质量。

Result: 在ADNI数据集上，ACADiff在生成质量与诊断性能方面均超越所有基线模型，尤其在80%模态缺失条件下表现稳健。

Conclusion: ACADiff能够有效解决多模态神经影像中的缺失问题，提升阿尔茨海默病诊断的可靠性，并为后续研究提供可复现的开源工具。
...
TL;DR: 本文提出一种名为Language-Aware Planning (LAP)的新方法，通过利用语言描述的表达能力来提升程序规划的准确性。LAP采用微调的视觉语言模型（VLM）将视觉观测转化为文本描述，并生成更具区分性的文本嵌入，用于扩散模型中的动作序列规划。在CrossTask、Coin和NIV三个基准测试中，LAP在多个指标和时间范围内均显著优于现有方法，验证了语言感知规划的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有程序规划方法主要依赖视觉输入，但在面对视觉上相似的不同动作时存在歧义问题。语言描述能提供更独特的潜在空间表示，有助于解决该问题。

Method: LAP使用微调的视觉语言模型将视觉观测转换为文本描述，提取文本嵌入，并将其作为扩散模型的输入以规划动作序列。

Result: LAP在CrossTask、Coin和NIV三个基准测试中均取得新的最先进性能，且在不同时间跨度下表现优异，证明了语言感知规划的优势。

Conclusion: 语言描述能够提供比视觉嵌入更区分性的表示，有效提升程序规划的准确性和鲁棒性，语言感知规划是未来方向的重要突破。
...
TL;DR: 本文提出k-MTR框架，通过在欠采样k空间数据与全采样图像之间建立共享语义流形，实现直接从k空间进行多任务表征学习，避免传统重建-分析流程中的伪逆问题，显著提升心脏MRI诊断性能。


<details>
  <summary>Details</summary>
Motivation: 传统临床CMR流程采用'重建-分析'的串行范式，导致中间步骤引入不可避免的伪影和信息瓶颈，且试图从欠采样k空间恢复高维像素图像是一个病态问题，而实际诊断需求的是低维生理标签。

Method: 提出k-MTR（k空间多任务表征）框架，利用42,000例受试者的受控大规模仿真数据，使k空间编码器在潜在空间中直接恢复因欠采样丢失的解剖信息，实现k空间与图像间的语义对齐。

Result: k-MTR在连续表型回归、疾病分类和解剖分割任务中均达到与现有图像域基准相当甚至更优的性能，证明可直接从欠采样k空间中恢复精确的空间几何与多任务特征。

Conclusion: k-MTR为任务感知的心脏MRI工作流程提供了稳健的架构蓝图，展示了直接从k空间进行诊断分析的可行性与优越性。
...
TL;DR: 提出LogoDiffuser，一种无需训练的多语言标志生成方法，通过将目标字符作为图像输入，实现对字符结构的鲁棒控制，并利用注意力机制识别核心标记，融合视觉与文本结构，提升多语言标志生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成多语言设计标志时，常因应用创意风格导致字符几何失真，且难以支持多语言文本生成而不需额外训练。

Method: 采用多模态扩散变压器，将目标字符以图像形式输入，分析联合注意力机制以识别对文本结构响应强烈的关键词，通过注入最具信息量的注意力图并进行层间聚合，减少跨层注意力偏移，获得一致的核心标记。

Result: 大量实验和用户研究证明，该方法在多语言标志生成任务中达到最先进性能。

Conclusion: LogoDiffuser实现了无需训练的高质量多语言标志生成，有效解决了字符结构失真与多语言支持难题。
...
TL;DR: 本文揭示了现有后门防御方法的局限性，指出仅消除已知触发器不足以根除后门。研究发现，存在与训练触发器在感知上不同的‘替代触发器’，这些替代触发器能同样激活后门。通过对比干净样本与被触发样本的特征表示，作者估计出后门在特征空间中的方向，并提出一种基于特征引导的攻击方法，联合优化目标预测与方向对齐。理论和实验证明替代触发器是后门训练的必然结果。同时，移除原始触发器的防御策略往往无效，因为替代触发器仍可利用隐藏的后门特征空间。因此，论文主张未来防御应聚焦于特征空间中的后门方向，而非输入空间的触发器。


<details>
  <summary>Details</summary>
Motivation: 当前后门防御依赖于移除已知触发器，但该方法忽略了存在感知上不同但功能相同的替代触发器，导致防御不彻底。

Method: 通过对比干净样本与被触发样本的特征表示，估计后门在特征空间中的方向；设计特征引导攻击，联合优化目标预测与方向对齐；结合理论分析与实验验证替代触发器的存在性及其危害。

Result: 证明了替代触发器在理论上不可避免，且在实践中可有效激活后门；现有以触发器为中心的防御策略常失效；表明应转向针对特征空间中后门方向的防御机制。

Conclusion: 后门防御不应局限于输入空间的触发器移除，而应深入特征空间，识别并抑制后门方向，才能实现真正有效的防护。
...
TL;DR: 本文提出了一项细粒度的音视频学习任务——区域感知声音源理解（RA-SSU），旨在实现帧级、高精度的声音源理解。为此，构建了两个新数据集：f-Music 和 f-Lifescene，分别涵盖音乐场景和生活场景中的声音源掩码与逐帧文本描述。同时提出 SSUFormer 模型，通过掩码协同模块（MCM）和分层提示专家混合（MoHE）提升分割精度与描述丰富性。实验验证了任务可行性、数据集有效性及模型优越性，在音源理解基准上达到当前最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有音视频学习研究多集中于粗粒度任务（如音视频对应、声源定位等），缺乏对具体场景细节的深入理解。为提升场景感知的精细度，本文提出细粒度的区域感知声音源理解任务，以支持更精准的场景理解与交互。

Method: 构建 f-Music 与 f-Lifescene 两个细粒度数据集，包含声音源掩码与逐帧文本描述；提出 SSUFormer 框架，采用多模态输入与输出结构，设计 Mask Collaboration Module（MCM）增强分割准确性，Mixture of Hierarchical-prompted Experts（MoHE）提升描述层次与丰富性。

Result: 在自建数据集上，SSUFormer 在声音源分割与区域描述任务中均取得显著性能提升，优于现有方法，达到音源理解任务的最新技术水平（SOTA）。

Conclusion: 本文定义并实现了细粒度的区域感知声音源理解任务，构建了高质量的细粒度数据集，并提出 SSUFormer 模型，有效推动了音视频学习向精细化、智能化方向发展。
...
TL;DR: ConfCtrl提出一种自信度感知的视频插值框架，通过结合置信度加权的投影点云潜在表示与噪声作为条件输入，使扩散模型在大视角变化下能遵循预定相机姿态并补全未见区域。该方法采用受卡尔曼滤波启发的预测-更新机制，利用学习到的残差校正平衡姿态驱动预测与有噪几何观测，从而在可靠投影上依赖并降低不确定区域的影响，实现稳定、几何一致的生成。实验表明，ConfCtrl在多个数据集上均能生成视觉合理且几何一致的新视角图像，有效重建遮挡区域。


<details>
  <summary>Details</summary>
Motivation: 现有基于回归的方法难以重建未见区域，而相机引导的扩散模型常因点云投影噪声或相机位姿条件不足导致偏离预期轨迹，因此需要一种既能遵循相机路径又能补全未知区域的新方法。

Method: ConfCtrl通过置信度加权的投影点云潜变量与噪声联合初始化扩散过程；采用卡尔曼滤波启发的预测-更新机制，将投影点云视为有噪观测，使用学习的残差校正来平衡姿态预测与几何观测，实现对可靠投影的依赖和对不确定区域的抑制。

Result: 在多个数据集上的实验表明，ConfCtrl能够生成几何一致、视觉合理的新型视图，有效重建大视角变化下的遮挡区域，表现出强鲁棒性和生成质量。

Conclusion: ConfCtrl通过引入置信度感知机制与卡尔曼式状态估计，显著提升了扩散模型在大视角变化下的几何一致性与完整性，为单帧多视角合成提供了高效可靠的解决方案。
...
TL;DR: VLM-Loc提出利用大视觉-语言模型（VLM）的空间推理能力解决文本到点云（T2P）定位任务中的精度问题。通过将点云转换为鸟瞰图（BEV）图像和场景图，联合编码几何与语义上下文，并引入部分节点分配机制，实现可解释的空间推理。构建了CityLoc基准测试集用于多场景评估，实验表明VLM-Loc在准确性和鲁棒性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有T2P定位方法依赖浅层文本-点云对应关系，缺乏有效空间推理，在复杂环境中精度受限。

Method: 将点云转化为鸟瞰图（BEV）图像和场景图以融合几何与语义信息；设计部分节点分配机制，将文本提示与场景图节点显式关联，增强可解释性与空间推理能力。

Result: 在CityLoc基准上，VLM-Loc在多种场景下表现出更高的定位精度和更强的鲁棒性，优于当前最优方法。

Conclusion: VLM-Loc通过利用VLM的空间推理能力，结合结构化输入与可解释的节点分配机制，显著提升了文本到点云定位的性能，为复杂环境下的语义空间理解提供了新范式。
...
TL;DR: 提出MissBench基准框架，用于评估多模态情感计算模型在模态缺失不均衡情况下的表现，引入MEI和MLI两个诊断指标，揭示现有模型在真实不完整模态场景中的公平性与优化不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 真实应用中多模态数据常出现系统性缺失或成本差异，导致模态不平衡，而传统评估方法忽略此问题，无法揭示模型在实际场景中的缺陷。

Method: 构建MissBench基准，涵盖四种常用情感与情绪数据集，支持共享与不均衡缺失率协议；设计Modality Equity Index（MEI）和Modality Learning Index（MLI）两个诊断指标，分别衡量模态贡献公平性与训练过程中的梯度优化不平衡。

Result: 实验表明，即使在共享缺失率下表现良好的模型，在不均衡缺失条件下仍存在显著的模态不公与优化失衡；验证了MissBench及两个指标在真实场景下对模型的应力测试与分析价值。

Conclusion: MissBench结合MEI与MLI，为多模态情感计算模型提供了在现实不完整模态条件下的有效评估工具，有助于提升模型鲁棒性与公平性。
...
TL;DR: InternVL-U 是一个40亿参数的轻量级统一多模态模型，通过统一上下文建模与模态特定模块化设计，在保持强大语义理解的同时实现卓越生成能力。其结合了先进的多模态大语言模型与基于MMDiT的视觉生成头，并采用以推理为中心的数据合成管道，利用思维链（CoT）提升抽象意图与细节生成的一致性。实验表明，尽管参数量仅为4B，其在生成与编辑任务上超越参数量超过3倍的基线模型（如BAGEL 14B），同时保留了优秀的多模态理解与推理性能。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在语义理解与生成能力之间存在固有权衡。现有模型往往在参数规模上追求性能，但难以兼顾效率与多功能性。本文旨在构建一个轻量级、高效且具备全面能力的统一框架，使强大的生成与理解能力更易获得，推动多模态技术的普及。

Method: 提出InternVL-U，采用统一上下文建模与模态特定模块化设计，分离视觉表示；结合先进MLLM与MMDiT-based视觉生成头；构建以推理为中心的数据合成流程，使用Chain-of-Thought（CoT）增强生成与用户意图对齐。

Result: InternVL-U 在生成与编辑任务中表现优异，仅用40亿参数即超越参数量超3倍的基线模型（如BAGEL 14B），在保持高效的同时维持强多模态理解与推理能力，实现了性能-效率的优越平衡。

Conclusion: InternVL-U 证明了轻量级统一多模态模型在兼顾理解与生成能力方面的可行性，为多模态系统的高效部署和广泛应用提供了新范式。
...
TL;DR: DISPLAY 是一种基于稀疏运动引导的人类-物体交互（HOI）视频生成框架，仅使用手腕关节坐标和无形状的物体边界框作为控制信号，实现灵活、物理一致的视频生成。通过引入物体压力注意力机制和多任务辅助训练策略，提升了在稀疏控制下的生成质量与数据利用效率。实验表明，该方法在多种任务中均能生成高质量、可控的HOI视频。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖密集控制信号、模板视频或精心设计的文本提示，限制了灵活性和对新物体的泛化能力，因此需要一种更轻量、通用且可控的HOI视频生成方式。

Method: 提出DISPLAY框架，采用稀疏运动引导（仅手腕关节坐标和物体边界框），引入物体压力注意力机制增强对象鲁棒性，并设计多任务辅助训练策略结合专用数据清洗流程以缓解高质量HOI数据稀缺问题。

Result: 在多种任务中实现了高保真度、可控制的HOI视频生成，显著提升生成质量与用户控制灵活性，同时有效应对稀疏控制与数据不足挑战。

Conclusion: DISPLAY通过稀疏引导与创新模块设计，在保持轻量化的同时实现了高效、可控、物理一致的HOI视频生成，为未来人机交互视频生成提供了新范式。
...
TL;DR: CourtSI是首个针对体育场景的大规模空间智能数据集，包含超过100万组问答对，涵盖羽毛球、网球和乒乓球等项目，系统性地覆盖空间计数、距离测量、定位和关系推理。通过利用清晰的场地几何结构作为度量基准，开发了半自动数据引擎以实现可扩展的数据构建。同时提出了CourtSI-Bench评估基准，包含3,686个经过严格人工验证的问答对。在该基准上评估25个主流视觉语言模型，发现人类与AI之间仍存在显著性能差距，且现有基准在泛化能力方面表现有限。微调Qwen3-VL-8B模型后，在CourtSI-Bench上的准确率提升23.5个百分点，并在未见过的类似运动场景中表现出良好的泛化能力及空间感知的解说生成能力。结果表明，体育场景能有效暴露现有空间智能评估的局限性，CourtSI为提升视觉语言模型的空间智能提供了可扩展路径。


<details>
  <summary>Details</summary>
Motivation: 体育场景天然具备高强度的人类运动与动态物体交互特征，是检验视觉语言模型空间智能的理想测试平台。然而现有空间智能基准在复杂动态场景下的泛化能力不足，难以充分评估模型在真实体育环境中的表现，因此亟需一个专门面向体育场景的大规模、高质量空间智能数据集与评估基准。

Method: 提出CourtSI数据集，基于标准化球场几何结构构建半自动数据生成流程，实现大规模问答对的高效生成；设计CourtSI-Bench评估基准，通过人工严格验证确保质量；采用多种视觉语言模型进行评测，并对Qwen3-VL-8B进行微调实验以验证其在新任务上的泛化能力。

Result: 在CourtSI-Bench上，25个模型普遍表现不佳，人机性能差距明显；微调后的Qwen3-VL-8B模型在CourtSI-Bench上准确率提升23.5个百分点，且在未见运动类型（CourtSI-Ext）中表现出良好泛化能力，同时支持更精准的空间感知内容生成。

Conclusion: 体育场景能够有效揭示当前视觉语言模型在空间智能方面的局限性。CourtSI及其评估基准为推动视觉语言模型在复杂动态环境中的空间理解能力提供了可靠、可扩展的解决方案。
...
TL;DR: WikiCLIP提出了一种简单而高效的开放域视觉实体识别框架，利用大语言模型嵌入作为知识丰富的实体表示，并通过视觉引导的知识适配器（VGKA）在图像补丁级别对齐文本语义与视觉线索。此外，引入硬负样本合成机制以增强细粒度区分能力。实验表明，WikiCLIP在OVEN等基准上显著优于现有方法，尤其在未见集上提升16%，同时推理延迟降低近100倍，远超生成式模型AutoVER。


<details>
  <summary>Details</summary>
Motivation: 现有生成式VER方法虽性能强但计算开销大，限制了其可扩展性和实际部署；因此需要一种高效且表现优异的替代方案。

Method: 采用对比学习范式，结合大语言模型生成的知识丰富实体嵌入，通过视觉引导的知识适配器（VGKA）实现图文对齐，并设计硬负样本合成机制以提升细粒度判别能力。

Result: WikiCLIP在OVEN基准上取得16%的显著提升，尤其在未见实体识别任务中表现突出，同时推理速度比领先生成模型AutoVER快近100倍。

Conclusion: WikiCLIP为开放域视觉实体识别提供了一个高效、强大且易于部署的新基线，展示了对比学习在该任务中的潜力。
...
TL;DR: 该论文揭示了点云重建、补全和生成中标准损失函数Chamfer距离在直接优化时可能出现性能反而更差的悖论现象，并指出其根源在于梯度结构导致的多对一坍塌。研究提出，抑制坍塌的必要条件是引入超越局部邻域的耦合机制；通过2D实验验证共享基变形可实现全局耦合以抑制坍塌，在3D形变任务中使用可微分的MPM先验也显著降低了Chamfer差距，尤其在复杂拓扑的dragon模型上提升达2.5倍。结论表明，非局部耦合的存在与否决定了优化是否成功，为基于点级距离度量的任何流水线提供了关键设计准则。


<details>
  <summary>Details</summary>
Motivation: Chamfer距离作为点云任务的标准训练损失，直接优化时常出现性能恶化现象，但其内在原因尚不明确。现有方法无法有效解决此问题，因此需要深入分析其根本机制并提出可通用的解决方案。

Method: 通过理论分析揭示梯度结构导致的多对一坍塌机制；在2D控制环境中采用共享基变形实现全局耦合；在3D形变任务中引入可微分的MPM（Material Point Method）先验，实现非局部耦合；通过对比实验验证不同策略对坍塌抑制的效果。

Result: 在2D设置中，共享基变形有效抑制了坍塌；在3D形变任务中，使用MPM先验使20个方向对的平均Chamfer差距显著降低，尤其在拓扑复杂的dragon模型上提升2.5倍。结果表明，非局部耦合是成功优化的关键。

Conclusion: Chamfer距离优化失败的根本原因是梯度结构引发的多对一坍塌，而抑制该问题的必要条件是建立超越局部邻域的耦合机制。非局部耦合的存在决定了优化是否成功，这一发现为点云相关任务的设计提供了普适性指导原则。
...
TL;DR: 本文提出一种基于关节点的可解释运动表示，将局部关节特征映射为结构化伪图像，兼容预训练视觉变换器。针对文本到运动的检索，采用词级后期交互机制MaxSim，并引入掩码语言建模正则化，以实现鲁棒且可解释的文本-运动对齐。在HumanML3D和KIT-ML数据集上的实验表明，该方法优于现有最先进方法，同时提供细粒度的文本与运动对应关系。


<details>
  <summary>Details</summary>
Motivation: 现有文本-运动检索方法多采用双编码器框架，将运动和文本压缩为全局嵌入，丢失了细粒度局部对应关系，导致精度下降且结果可解释性差。

Method: 提出基于关节点的运动表示，将关节级局部特征转化为结构化伪图像；使用MaxSim进行词级后期交互，并引入掩码语言建模正则化以增强对齐效果。

Result: 在HumanML3D和KIT-ML数据集上均优于现有方法，实现了更高的检索精度和更好的可解释性，能提供细粒度的文本-运动对应关系。

Conclusion: 所提方法通过结构化伪图像表示和可解释的对齐机制，有效提升了文本-运动检索的性能与可解释性，具有实际应用价值。
...
TL;DR: 本文提出一种基于边缘差异差异性（MDD）的新型无监督域适应（UDA）框架，用于改善介入性锥束计算机断层扫描（CBCT）中的肝脏分割性能。通过结合自有未标注的CBCT数据与已标注的CT数据，该方法有效弥合了模态差距，在CBCT上实现了最先进的分割效果，尤其在少样本设置下表现优异。


<details>
  <summary>Details</summary>
Motivation: 介入性锥束计算机断层扫描（CBCT）在微创手术中具有重要指导作用，但其数据稀缺且缺乏标注，而现有公开数据集多集中于放疗应用，限制了深度学习模型的训练与应用。因此，亟需一种有效的方法来利用有限的标注数据和大量未标注的CBCT数据，提升肝脏分割性能。

Method: 提出一种基于边缘差异差异性（MDD）的新型无监督域适应框架，通过重构优化目标函数，增强对目标域（CBCT）的适应能力。利用已标注的CT数据与未标注的CBCT数据进行联合训练，实现跨模态的知识迁移。

Result: 实验结果表明，该方法在CT到CBCT的肝脏分割任务中达到了当前最优的无监督域适应性能，尤其在少样本设置下显著优于现有方法，验证了其有效性与鲁棒性。

Conclusion: 本研究成功构建了一种适用于介入性CBCT的肝脏分割无监督域适应框架，为解决医疗影像中模态差异与标注稀缺问题提供了有效方案，具有良好的临床应用前景。
...
TL;DR: 本文引入了全切片难度（WSD）的概念，基于专家与非专家病理学家之间的分歧，提出多任务和加权分类损失两种方法来利用WSD，显著提升了前列腺癌Gleason分级的分类性能，尤其在高Gleason评分（即更差诊断）时效果更明显。


<details>
  <summary>Details</summary>
Motivation: 由于病理切片对非专家而言难以诊断，导致标注者之间存在分歧，现有MIL方法未考虑这种分歧带来的难度信息，因此需要引入切片难度以提升模型性能。

Method: 提出基于专家与非专家病理学家分歧的全切片难度（WSD）度量，并设计多任务学习和加权分类损失两种策略来融合该难度信息。

Result: 在多种特征编码器和MIL方法下，整合WSD均能稳定提升分类性能，特别是在高Gleason等级中表现更优。

Conclusion: 引入全切片难度并有效融合至训练过程，可显著提升前列腺癌病理图像分类的准确性，尤其在困难病例上表现突出。
...
TL;DR: ReCoSplat 是一种自回归前馈高斯点云渲染模型，支持有/无姿态和相机内参的输入。它通过引入 Render-and-Compare (ReCo) 模块，利用预测视角下的重建与当前观测对比，提供稳定条件信号以缓解姿态误差问题。为支持长序列，采用混合 KV 缓存压缩策略（早期层截断 + 块级选择性保留），使 100+ 帧的缓存减少超 90%。在多种输入设置下，ReCoSplat 在分布内和分布外基准上均达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 在线小说视图合成仍具挑战性，需从顺序且常无姿态的观测中实现鲁棒场景重建。传统方法在使用真实姿态训练时虽稳定，但推理时若用预测姿态则产生分布不匹配问题。因此需要一种能有效补偿姿态误差、支持长序列且高效运行的新方法。

Method: 提出 ReCoSplat 模型，结合自回归前馈高斯点云表示；引入 ReCo 模块，通过渲染预测视角下的重建并与输入观测比较，提供稳定监督信号；设计混合 KV 缓存压缩策略，包括早期层截断和块级选择性保留，显著降低内存开销。

Result: ReCoSplat 在不同输入条件下均取得当前最优性能，尤其在长序列场景下表现出色；KV 缓存大小相比原始方法压缩超过 90%；实验验证其对姿态误差具有强鲁棒性，在分布内外数据集上表现优异。

Conclusion: ReCoSplat 通过 ReCo 模块和高效的 KV 缓存压缩机制，成功解决了在线视图合成中的姿态不匹配与长序列效率问题，实现了高精度、高效率的动态场景重建，是该领域的重要进展。
...
TL;DR: 本研究利用塞尔维亚语双文字系统（拉丁字母与西里尔字母）作为控制实验环境，探究稀疏自编码器（SAEs）学习到的特征是反映抽象语义还是受书写形式影响。通过对比相同语义在不同书写系统中的特征激活情况，发现即使字符映射完美且分词完全不同，特征重叠度仍显著高于随机基线，且跨脚本的差异小于同一脚本内的改写差异，表明SAE特征更关注语义而非表层形式。模型规模越大，这种对书写系统的不变性越强。该结果支持SAE特征能捕捉高于分词层面的抽象语义，并提出以塞尔维亚语双文字系统作为评估表示抽象性的通用范式。


<details>
  <summary>Details</summary>
Motivation: 探究稀疏自编码器（SAEs）学习到的特征是否代表抽象语义，还是仅仅依赖于文本的书写形式。使用塞尔维亚语双文字系统（拉丁字母与西里尔字母）作为控制变量，因两者语义完全一致但分词方式完全不同，可有效分离语义与表层形式的影响。

Method: 在Gemma模型系列（270M-27B参数）上，分析相同句子在塞尔维亚语拉丁字母和西里尔字母两种书写形式下的SAE特征激活情况；比较跨脚本、跨改写等组合的特征重叠程度，并与随机基线进行对比，评估特征的语义稳定性与抽象性。

Result: 相同语义在不同书写系统下激活高度重叠的SAE特征，重叠度远超随机基线；跨脚本差异小于同一脚本内改写差异；跨脚本跨改写组合虽训练数据中罕见，但仍具显著特征重叠；特征对书写系统的不变性随模型规模增加而增强。

Conclusion: SAE特征能够捕捉高于表面分词的抽象语义，不依赖具体书写形式；塞尔维亚语双文字系统可作为评估语言模型表示抽象性的通用测试框架。
...
TL;DR: 本文提出一个多语言匿名化基准，涵盖十种语言，利用机器翻译技术在保留原始标注的同时，将人名、地名等个人信息以文化与语境适宜的方式适配到目标语言。该基准包含超过2500个个人身份信息标注，经医学专业人士评估验证了翻译质量，可用于训练标注人员、跨机构验证标注及提升自动个人信息检测性能，且已公开数据集和标注指南以支持后续研究。


<details>
  <summary>Details</summary>
Motivation: 真实患者数据因隐私问题难以获取，而合成数据可绕过隐私法规限制，结合神经机器翻译可为低资源语言生成高质量数据，从而解决医疗数据稀缺与多语言匿名化需求之间的矛盾。

Method: 采用机器翻译方法，在保持原始标注完整性的同时，将高资源语言中的敏感信息（如人名、地名）翻译并适配至十种目标语言，确保文化与上下文合理性；并通过医学专家评估验证翻译质量。

Result: 构建的多语言匿名化基准包含超过2500个标注，经医学专业人员验证，翻译质量高，适用于训练、跨机构协作及提升自动检测系统性能；数据集与标注指南已公开。

Conclusion: 本研究成功构建了一个高质量、多语言、符合隐私合规要求的匿名化基准，为医疗AI中的安全数据共享与模型训练提供了有效支持。
...
TL;DR: ConFu提出一种新型推测解码框架，通过引入前瞻性的‘沉思令牌’和软提示，使轻量级草稿模型能够以极低代价利用目标模型的未来信号，实现对生成方向的前瞻性预测。其核心创新包括：(i) 用于未来导向信号的沉思令牌与软提示；(ii) 基于MoE的动态沉思令牌机制，实现上下文感知的未来预测；(iii) 采用锚定令牌采样与未来预测复制的训练框架，提升未来预测的鲁棒性。实验表明，ConFu在多个下游任务中相较EAGLE-3提升了8–11%的令牌接受率与生成速度，适用于Llama-3 3B和8B模型，是首个将推测解码与连续推理令牌结合的工作，为加速大语言模型推理开辟新方向。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法受限于草稿模型仅依赖当前前缀进行预测，导致误差随生成步骤累积，影响预测质量与解码效率。因此需要提升草稿模型对未来生成方向的预判能力，以增强其与目标模型的一致性。

Method: 提出ConFu框架，引入沉思令牌和软提示以获取未来信号；设计基于MoE的动态沉思令牌机制实现上下文感知的未来预测；构建包含锚定令牌采样与未来预测复制的训练策略，学习稳健的未来预测能力。

Result: 在多种下游任务中，ConFu相较于EAGLE-3，在Llama-3 3B和8B模型上实现了8–11%的令牌接受率和生成速度提升，显著改善了推测解码性能。

Conclusion: ConFu首次将推测解码与连续推理令牌相结合，通过前瞻式建模显著提升草稿模型的预测准确性与解码效率，为大语言模型推理加速提供了新的技术路径。
...
TL;DR: SciTaRC 是一个由专家撰写的科学论文中表格数据相关问题的基准测试，要求深度语言推理和复杂计算。当前最先进的AI模型在至少23%的问题上表现不佳，即使是高性能的Llama-3.3-70B-Instruct也失败于65.5%的任务。分析发现存在普遍的“执行瓶颈”：代码和语言模型即使有正确策略，也难以忠实执行计划。代码方法对原始科学表格表现脆弱，而自然语言推理主要因理解初期错误和计算失误而失败。


<details>
  <summary>Details</summary>
Motivation: 现有AI模型在处理科学论文中的表格数据时，缺乏对深层语言推理和复杂计算的有效能力，亟需一个更严格的评估基准来揭示其局限性。

Method: 构建SciTaRC基准，包含专家设计的、需要深度推理与复杂计算的问题，系统评估多种AI模型的表现，并深入分析失败原因。

Result: 当前主流AI模型在SciTaRC上表现较差，尤其在复杂计算和推理任务中；代码与语言模型均存在执行瓶颈，分别受限于对原始表格的处理能力和理解与计算错误。

Conclusion: SciTaRC揭示了当前AI模型在处理科学表格数据时的根本缺陷，表明需改进模型的计划执行能力，尤其是对复杂推理与计算任务的鲁棒性。
...
TL;DR: 提出一种结合迭代式编码本优化与完整溯源追踪的自动化主题分析框架，显著提升代码本的可复用性与一致性，在五个数据集中的四个上优于六种基线方法，且在儿科心脏病学临床访谈中生成的主题与专家标注高度一致。


<details>
  <summary>Details</summary>
Motivation: 手动主题分析在健康研究中面临可扩展性和可重复性挑战，现有基于大语言模型的自动化方法存在代码本泛化能力差和分析过程缺乏审计性的问题。

Method: 采用迭代式代码本优化策略，并集成完整的分析过程溯源机制，确保结果透明可追溯。

Result: 在五个语料库（包括临床访谈、社交媒体和公开演讲）上的评估显示，该框架在四个数据集上取得最高综合质量评分；四组数据中迭代优化带来统计学显著提升，效应量较大，主要得益于代码重用率和分布一致性增强，同时保持描述质量。

Conclusion: 所提出的自动化主题分析框架有效提升了主题分析的可扩展性、可重复性与可审计性，适用于复杂健康数据的深度分析，尤其在临床研究中具备良好应用前景。
...
TL;DR: 该论文提出一种基于置信度的决策框架，通过分析单个推理路径，自适应地选择单路径或多路径推理，从而在保持与多路径基线相当准确率的同时，最多减少80%的令牌使用量。该方法利用MedQA数据集中的中间推理状态提取句级数值和语言特征进行训练，并能有效泛化到MathQA、MedMCQA和MMLU等任务，无需额外微调。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽通过思维链（CoT）推理取得良好表现，但常生成过长的推理路径，导致高推理成本；现有基于自一致性方法虽提升准确性，但需采样并聚合多个推理轨迹，带来显著计算开销。因此亟需一种既能保持高精度又能降低计算成本的高效推理机制。

Method: 提出一个自信度感知的决策框架，基于单个完成的推理路径，结合从中间推理状态中提取的句级数值和语言特征，判断是否采用单路径或多路径推理策略。该框架在MedQA数据集上训练，并可直接迁移到其他任务。

Result: 实验表明，该方法在保持与多路径基线相近准确率的同时，最多可减少80%的推理令牌消耗，且在MathQA、MedMCQA和MMLU等数据集上表现出良好的泛化能力，无需额外微调。

Conclusion: 推理路径本身蕴含丰富的不确定性信号，通过简单而可迁移的置信度机制，能够有效平衡大语言模型推理的准确性与效率。
...
TL;DR: 本研究探讨了大型语言模型（LLMs）在材料、能源、制造和算法四个领域中对合成技术与生物技术解决方案的系统性偏见。通过50个精心设计的生物对齐提示，评估了5个前沿模型和5个开源模型，发现大多数模型表现出对非生物解决方案的偏好。随后，通过对Llama 3.2-3B-Instruct和Qwen2.5-3B-Instruct进行微调，使用来自6,636篇PMC文章的约2200万词元的生物问题解决语料库，发现QLoRA微调显著提升了两个模型对生物方案的评分，且未损害其通用能力（校正后p<0.001和p<0.01）。结果表明，少量微调即可改变模型对生物与合成方法的价值判断，该方法可扩展至更大模型，用于开发更倾向生物基解决方案的模型。研究公开了基准测试、语料库、代码和适配器权重。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在训练过程中可能继承互联网数据中的系统性偏见，导致对合成技术的偏好，而忽视生物或生物启发式解决方案。这种偏见可能影响可持续发展和技术创新的方向。因此，需要识别并纠正这些偏见，以推动更生态友好的技术路径。

Method: 采用基于凯利准则的评估框架，对4个领域的50个生物对齐提示进行测试；使用来自6,636篇PMC文章的约2200万词元语料库，对两个开源模型（Llama 3B 和 Qwen 3B）进行微调，分别采用混合训练与指令格式化方式（前者）及仅指令格式化方式（后者）；使用QLoRA技术进行高效微调，并通过统计检验评估性能变化。

Result: 大多数模型在未微调状态下表现出对合成解决方案的明显偏好；经过微调后，两个模型对生物解决方案的评分显著提升，且未影响其通用能力（校正后p<0.001和p<0.01），表明微调有效且稳健。

Conclusion: 即使少量微调也能显著改变大型语言模型对生物与合成技术价值的判断，说明可通过针对性训练引导模型向更具可持续性的生物基解决方案倾斜。该方法具有可扩展性，适用于更大规模模型，有助于构建更符合生态伦理的技术导向模型。研究公开资源以促进后续研究。
...
TL;DR: DuplexCascade 是一种无语音活动检测（VAD）的级联流式语音对话系统，支持全双工交互。通过将长对话轮次转化为微小片段式的互动，实现快速双向交流，同时保留强大文本大语言模型（LLM）的对话智能。引入专门的对话控制令牌以协调流式条件下的发言权和响应时机，在 Full-DuplexBench 和 VoiceBench 上达到开源语音到语音对话系统的领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有语音对话系统中，基于 VAD 的级联架构虽保留了大语言模型的智能，但因语音活动检测导致半双工交互，控制脆弱；而无 VAD 的端到端模型虽支持全双工，却难以维持高质量的对话智能。因此亟需一种既能实现全双工、又能保持强对话能力的解决方案。

Method: 提出 DuplexCascade 架构，采用无 VAD 的级联流式处理流程，将传统整句对话转为微段落级别的微轮次交互，并引入一组对话专用控制令牌，用于在流式输入条件下精确引导大语言模型的发言行为与响应节奏。

Result: 在 Full-DuplexBench 与 VoiceBench 测试集上，DuplexCascade 实现了当前开源语音到语音对话系统中的最佳全双工轮次表现，同时保持了强大的对话理解与生成能力。

Conclusion: DuplexCascade 成功融合了级联架构的智能优势与全双工交互的流畅性，为构建高效、自然的实时语音对话系统提供了新范式。
...
TL;DR: 提出了一种无需训练的负向感知文本和多模态检索方法DEO，通过将查询分解为正负成分并使用对比目标优化查询嵌入，显著提升了否定和排除查询的检索效果，且无需额外训练数据或模型更新。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法在处理否定和排除查询时表现不佳，而现有解决方案如嵌入适配或微调会增加计算成本和部署复杂性。

Method: DEO将查询分解为正负成分，并采用对比学习目标优化查询嵌入，实现无需训练的负向感知检索。

Result: DEO在NegConstraint数据集上相比基线提升+0.0738 nDCG@10和+0.1028 MAP@100，多模态检索中Recall@5比OpenAI CLIP提高6%。

Conclusion: DEO是一种高效、实用的负向与排除感知检索方法，适用于真实场景下的文本和多模态检索任务。
...
TL;DR: 本文研究情感如何作为潜在因素影响大语言模型在文本上的注意力与推理行为。通过分析情绪对注意力几何结构的影响，提出一个情绪均衡的问答数据集AURA-QA，并设计了一种情感正则化框架以减少训练中的情绪相关表征漂移。实验表明该方法在多种基准上均提升了阅读理解性能，尤其在分布外和域内任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在评估时通常忽略情感对推理行为的影响，而情感常是文本中重要的表征变化来源。以往研究多将情感视为预测目标，而非影响模型内部表示的潜在因子。因此，需要系统性地探究情感如何改变模型的注意力机制和推理过程。

Method: 分析不同情绪对变压器模型注意力几何结构的影响，引入情绪平衡的人工撰写上下文问答数据集AURA-QA，用于控制实验条件；提出情感正则化框架，约束训练过程中情绪相关的表征漂移。

Result: 实验结果显示，所提方法在多个问答基准上均取得一致性能提升，尤其在分布外和域内任务中表现优异，证明了情感正则化对提升模型鲁棒性和泛化能力的有效性。

Conclusion: 情感是影响大语言模型推理的重要潜在因素，通过引入情绪均衡的数据集和情感正则化训练策略，可显著改善模型在多样化情感语境下的阅读理解能力，增强其在真实场景中的适应性。
...
TL;DR: SPAR-K 是一种模态感知的早期退出框架，用于加速交错式语音语言模型（SLM）的推理。通过在语音位置采用固定中间层的提前退出策略，并周期性进行全深度刷新步骤以缓解分布偏移问题，SPAR-K 在不增加额外计算开销的情况下，显著降低了平均语音解码深度（最高减少11%），同时保持了问答准确率（最大下降0.82%）和感知质量（MOS与WER几乎不变）。实验表明，适用于文本大模型的基于置信度的早期退出策略在语音模型中表现不佳，凸显了语音令牌独特统计特性对专用早期退出设计的需求。


<details>
  <summary>Details</summary>
Motivation: 交错式语音语言模型在每一步解码时需执行完整的Transformer深度，导致计算成本高昂，尤其是面对长语音序列时。现有基于置信度的早期退出策略在语音场景下效果不佳，亟需一种针对语音模态特性的高效推理优化方法。

Method: 提出SPAR-K框架，引入语音交替深度调度：多数语音位置在固定中间层提前退出，周期性地插入全深度‘刷新’步骤以缓解因提前退出引起的分布偏移问题。该方法无需额外计算开销，适配语音令牌的统计特性。

Result: 在Step-Audio-2-mini和GLM-4-Voice上，SPAR-K将平均语音解码深度降低最多11%（Step-Audio-2-mini）和5%（GLM-4-Voice），问答准确率最大下降0.82%，感知质量（MOS）和语音识别错误率（WER）基本不变，验证了其有效性与高效性。

Conclusion: SPAR-K通过模态感知的早期退出机制，实现了对交错式语音语言模型的高效推理加速，证明了语音令牌的独特性要求专门设计的早期退出策略，为语音生成模型的实用化部署提供了有效路径。
...
TL;DR: 提出了一种基于边距的查询驱动上下文压缩框架，通过衡量句子被省略时线索丰富度的变化来识别回答问题的关键句子。采用复合排名损失训练模型，使关键句子具有较大的边距，非关键句子保持中性。该方法基于轻量级编码器仅Transformer架构，在保证高精度的同时实现高吞吐推理和低内存占用，显著提升效率与可扩展性，同时保持良好的压缩比和问答性能。


<details>
  <summary>Details</summary>
Motivation: 提高检索增强生成中上下文传递的效率与准确性，减少大语言模型阅读成本，实现快速、紧凑且精确的上下文交付。

Method: 提出一种基于边距的查询驱动上下文修剪框架，利用复合排名损失训练模型，通过测量删除句子前后线索丰富度的变化来判断其重要性。

Result: 在保持高准确率（精确匹配和F1分数）的同时，实现了高吞吐推理和较低的内存消耗，相比主流基线方法具备更高的效率与更低的资源开销，且压缩比优异，不影响问答性能。

Conclusion: 所提方法是一种轻量级、实用性强的检索增强任务替代方案，兼具高效性与有效性，适用于大规模问答系统。
...
TL;DR: 提出一种基于分类体系的结构化推理框架TaSR-RAG，通过将查询和文档表示为关系三元组，并利用轻量级两级分类体系约束实体语义，实现更精确的证据选择。该方法将复杂问题分解为有序的三元组子查询序列，结合语义相似性和结构一致性进行分步证据筛选，同时通过显式维护实体绑定表减少实体混淆，无需图构建或穷举搜索。在多个多跳问答基准上，TaSR-RAG显著优于现有RAG与结构化RAG基线，性能提升最高达14%，并提供更清晰的证据归属与更忠实的推理过程。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统多依赖非结构化文本片段和单次生成，导致上下文冗余、信息密度低、多跳推理脆弱；虽有结构化RAG提升准确性，但通常需昂贵且易错的图构建，或采用僵化的以实体为中心的结构，难以匹配查询的推理链。

Method: 将查询与文档表示为关系三元组，引入轻量级两级分类体系平衡泛化与精度；将复杂问题分解为有序三元组子查询序列，使用混合三元组匹配（结合原始三元组的语义相似性与类型三元组的结构一致性）进行分步证据选择；通过显式维护实体绑定表，在不进行图构建或穷举搜索的情况下解决中间变量并减少实体混淆。

Result: 在多个多跳问答基准测试中，TaSR-RAG显著超越强基线模型，性能提升最高达14%；生成结果具有更清晰的证据归属与更可信的推理轨迹。

Conclusion: TaSR-RAG通过分类引导的结构化推理机制，在无需复杂图构建的前提下，有效提升了多跳问答中的证据选择质量与推理稳定性，为增强大模型的知识推理能力提供了高效、可扩展的新范式。
...
TL;DR: 该研究利用大语言模型（LLMs）生成的空间关系标签，与人类标注结果具有较高一致性，展示了其在扩展空间数据集中的有效性。通过新增42个场景，改进了现有的拓扑关系图片系列（TRPS），实现了更全面的场景覆盖，为未来构建涵盖数十种语言和数百个场景的空间数据集奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 现有研究依赖人工标注来分析不同语言中空间范畴化的差异，但人工标注成本高、效率低。为提升数据集的规模与多样性，亟需一种高效且准确的替代方法。

Method: 使用大语言模型（LLMs）生成空间关系标签，并与真实人类标注进行对比，验证其可靠性；基于模型输出选择新增场景和语言，以优化数据集覆盖范围。

Result: LLM生成的标签与人类标注高度一致；新增42个场景后的TRPS扩展版本比之前两个扩展版本具有更好的场景覆盖能力。

Conclusion: LLM可有效辅助生成高质量的空间关系标注，为大规模多语言空间数据集的构建提供可行路径。
...
TL;DR: 提出LLM as a Meta-Judge框架，利用大语言模型生成合成评估数据，通过受控语义退化真实数据来替代人工判断，验证结果显示合成数据在多语言问答任务中与人类基准的元相关性超过0.9，证明其可作为可靠替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统自然语言生成评估依赖昂贵且耗时的人工标注，且主要限于英文数据集，缺乏多语言和低成本的评估方法。

Method: 使用大语言模型对真实数据进行受控语义退化，生成合成评估数据，并通过元相关性衡量合成数据与人类基准之间的排名一致性。

Result: 在机器翻译、问答和摘要任务中，合成数据验证表现出与人类判断高度一致，特别是在多语言问答任务中元相关性超过0.9，证明其有效性。

Conclusion: LLM as a Meta-Judge提供了一种高效、可扩展的评估替代方案，尤其适用于缺乏人工标注或成本过高的场景。
...
TL;DR: 该研究探讨了大型语言模型（LLMs）在法语患者记录中对性别与其他社会决定健康因素（SDoH）之间关系的偏见，发现模型依赖于嵌入的刻板印象进行性别判断，强调评估SDoH因素间交互作用的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常只评估单一社会决定健康因素（如性别或种族）相关的偏见，忽视这些因素之间的相互作用及具体情境下的评估，尤其在医疗等敏感领域影响重大。

Method: 通过一系列实验，利用社会决定健康因素作为输入，探测大型语言模型在法语患者记录中的偏见表现，分析其决策是否受刻板印象影响。

Result: 研究发现，大型语言模型在性别判断中确实依赖于嵌入的刻板印象，且能够通过社会决定健康因素的组合揭示出潜在的偏见模式。

Conclusion: 评估社会决定健康因素间的交互作用可有效补充现有方法，有助于更全面地识别和缓解大型语言模型中的偏见。
...
TL;DR: 本文揭示了当前大语言模型（LLMs）在处理道德困境时倾向于优先于常识理解的缺陷，并提出一个名为CoMoral的新基准数据集，用于检测嵌入在道德困境中的常识矛盾。实验表明，现有模型在缺乏先验提示的情况下难以识别这些矛盾，且普遍存在叙事焦点偏差——即更易发现次要角色而非主要叙述者所犯的常识错误。研究强调需通过增强推理感知训练来提升模型的常识鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在真实应用中需兼具道德合理性与常识理解能力，但现有模型表现出过度依赖道德推理而忽视常识的问题，亟需系统评估和改进。

Method: 构建CoMoral基准数据集，包含嵌入道德困境中的常识矛盾；对十种不同规模的主流大语言模型进行评估，分析其在识别常识矛盾上的表现及叙事焦点偏差现象。

Result: 所有测试模型均在无先验信号情况下难以准确识别常识矛盾；模型对次要角色的常识错误更敏感，存在显著的叙事焦点偏差。

Conclusion: 现有大语言模型在常识理解方面存在明显缺陷，尤其在复杂道德情境下易受叙事角色影响。未来需通过引入推理感知训练机制，提升模型在常识与道德平衡下的综合推理能力。
...
TL;DR: 该研究通过构建随机模型模拟音位系统的历时演变，发现仅考虑音位变化的自然过程即可生成音位频率分布的统计规律，如指数尾部排名分布及音位库大小与相对熵的负相关关系。加入功能负荷效应和偏好音位数量的稳定倾向后，模型能准确再现实证数据特征，表明这些规律可能是历时演变的自然结果，而非优化或补偿机制所致。


<details>
  <summary>Details</summary>
Motivation: 解释音位频率分布中观察到的统计规律（如指数尾部排名分布、音位库大小与相对熵的负相关）的成因，现有理论未能充分说明其来源，因此需要从历史演变角度探究其可能的生成机制。

Method: 提出一个随机音位演变模型，通过模拟音位系统的历时变化，考察不同假设条件下音位频率分布的生成情况。对比基础模型与扩展模型（引入功能负荷效应和偏好音位数量的稳定性）的仿真结果与真实语言数据的匹配程度。

Result: 基础模型虽能再现大致的排名-频率分布形态，但无法解释其他经验规律；扩展模型在加入功能负荷和偏好音位数量的稳定倾向后，不仅准确复现了音位频率分布，还成功再现了音位库大小与相对熵之间的负相关关系。

Conclusion: 部分音位系统中的统计规律并非源于显式的优化或补偿机制，而是历时音变过程的自然产物，表明语言演变本身足以解释这些宏观统计特征。
...
TL;DR: 本文研究语言模型在训练过程中通过自然语言改写文本（语义不变）从教师模型中隐性学习行为特征的现象。即使改写内容与目标偏好无关，甚至明确表达相反意见，学生模型仍会显著增强对教师偏好的认同，最高提升达19个百分点。这表明基于内容的审查无法检测此类隐性知识传递，且对抗性内容也无法阻止该现象，对自生成训练数据的模型训练流程构成严重风险。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型在训练中是否能通过语义不变的自然语言改写，隐性继承教师模型的行为偏好，尤其关注内容矛盾是否能阻断这种传递，以评估当前内容过滤机制的有效性。

Method: 使用教师模型系统性地设定对特定动物的偏好，并生成一系列语义相同但表述不同的自然语言改写文本作为训练数据；学生模型在这些数据上进行训练，测试其是否产生相应偏好变化；同时引入明确反对教师偏好的改写文本，检验其能否抑制偏好传递。

Result: 学生模型在训练后对教师偏好的动物表现出显著偏好提升，最高达19个百分点，即便改写内容与动物无关或明确表示厌恶。即使经过严格的内容保真度过滤，该现象依然存在。

Conclusion: 语言模型可通过自然语言改写实现跨模态的隐性偏好学习，且内容审查和反向表述均无法有效阻断这一过程。这揭示了自生成训练数据流水线中的重大安全风险，现有基于内容的检测手段不足以防范此类隐性知识泄露。
...
TL;DR: 本文以冲突事件分类为案例，比较了微调通用模型Confli-mBERT与领域专用模型ConfliBERT的性能。结果显示，尽管ConfliBERT整体准确率（79.34%）略高于Confli-mBERT（75.46%），但二者在高频事件类型上表现接近，差异主要集中在罕见事件类别（占比<2%）。研究据此提出一个实用决策框架，帮助政治科学家根据类别频次、容错性和资源条件判断是否需构建专用模型。


<details>
  <summary>Details</summary>
Motivation: 政治科学家在使用自然语言处理工具时面临选择困境：是自建领域模型、改编现有模型，还是仅对通用模型进行微调？然而学界缺乏实证依据来指导这一权衡。本文旨在填补这一空白，提供基于实证的决策支持。

Method: 通过在全球恐怖主义数据库（GTD）上微调ModernBERT构建Confli-mBERT，并与当前领域标准模型ConfliBERT进行系统对比，评估其在各类冲突事件上的分类性能，重点关注不同频率事件的表现差异。

Result: Confli-mBERT整体准确率为75.46%，低于ConfliBERT的79.34%；但在高频事件（如爆炸、绑架）上两者几乎无差异（F1值接近0.95和0.92）；性能差距集中于发生频率低于2%的罕见事件类别。

Conclusion: 模型优劣不应仅从抽象性能判断，而应结合具体研究问题中的类别分布、容错要求和资源可用性综合考量。对于多数研究任务，微调通用模型已足够，无需投入大量资源构建专用模型。
...
TL;DR: 本文揭示了BLOOM系列变压器语言模型中存在系统性注意力崩溃病理，ALiBi位置编码导致31-44%的注意力头几乎完全关注序列起始标记。通过手术式重初始化（针对性地重新初始化Q/K/V，输出投影置零，并对非手术参数进行梯度屏蔽冻结），在单个消费级GPU上对BLOOM-1b7进行两次迭代后，恢复了98.7%的功能注意力头容量（从242增至379/384）。对比实验表明，恢复效果源于重初始化而非语料内容；同时发现术后两种现象：早期全局功能重分配提升模型性能，晚期局部退化在噪声训练信号下累积。进一步实验显示，同时重初始化大部分健康与崩溃头的模型在训练困惑度上短暂超越原版BLOOM-1b7达25%（12.70 vs. 16.99），暗示预训练注意力配置为次优局部极小值。代码、检查点和诊断工具已开源。


<details>
  <summary>Details</summary>
Motivation: 揭示并解决BLOOM家族模型中由ALiBi位置编码引发的注意力头崩溃问题，探索预训练模型中注意力机制的潜在缺陷及其可修复性。

Method: 提出手术式重初始化方法，包括针对性地重新初始化注意力头的Q/K/V矩阵，将输出投影设为零，并对非手术参数实施梯度屏蔽与冻结，以实现对注意力结构的精准修复。

Result: 在单个消费级GPU上仅需两轮迭代即可恢复98.7%的注意力头功能（379/384）；控制实验确认恢复主要归因于重初始化而非数据内容；观察到早期性能提升与后期局部退化现象；重初始化部分健康头的模型在训练困惑度上短暂优于原始模型25%。

Conclusion: 预训练的注意力配置可能陷入次优局部极小值，通过手术式重初始化可显著修复注意力崩溃问题，且该方法高效、可扩展，具备实际应用潜力，相关成果已开源。
...
TL;DR: 本文提出一个完全开源、可本地部署的放射科报告纵向信息提取管道，利用qwen2.5-72b模型依据RECIST标准提取和关联目标病灶、非目标病灶及新病灶数据。在50对荷兰胸部/腹部CT报告上的评估显示，目标病灶、非目标病灶和新病灶的属性级准确率分别为93.7%、94.9%和94.0%，表明开源大语言模型可在保障数据隐私与可重复性的前提下实现临床有意义的性能，推动可扩展的结构化纵向数据提取。


<details>
  <summary>Details</summary>
Motivation: 放射科报告包含肿瘤负荷、治疗反应和疾病进展的关键纵向信息，但其非结构化叙述格式阻碍了自动化分析；现有先进系统多为专有，难以在注重隐私的医疗环境中应用。

Method: 采用\texttt{llm_extractinator}框架，基于\texttt{qwen2.5-72b}模型，对放射科报告中目标、非目标及新病灶进行跨时间点提取与关联，符合RECIST标准。

Result: 在50对荷兰CT胸/腹部报告上评估，目标病灶、非目标病灶和新病灶的属性级准确率分别为93.7%、94.9%和94.0%。

Conclusion: 开源大语言模型可在本地部署环境下实现高精度的多时间点肿瘤学信息提取，兼顾数据隐私、可重复性与临床实用性，具备大规模应用潜力。
...
TL;DR: 该研究探讨了语言模型（LMs）在处理知识时，如何在参数化知识与上下文知识之间进行权衡，尤其关注因预训练记忆与上下文信息冲突导致的知识失效问题。通过评估模型知识、设计诊断测试以识别知识冲突，并分析成功利用上下文知识的特征，旨在提升语言模型在知识密集型任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型规模扩大，其内部知识难以解释且更新成本高昂。同时，模型常忽略与自身记忆冲突的上下文信息，导致知识使用不准确，因此需要理解模型如何整合内外部知识。

Method: 提出评估语言模型知识的方法，设计诊断测试以检测知识冲突，并分析成功利用上下文知识的特征。

Result: 揭示了语言模型在面对冲突知识时的行为模式，识别出影响上下文知识采纳的关键因素，为改进模型知识整合能力提供了依据。

Conclusion: 理解语言模型如何协调参数化知识与上下文知识对于提升其在知识密集型任务中的可靠性至关重要，未来工作应聚焦于增强模型对上下文信息的有效利用。
...
TL;DR: 本研究提出一种自动化分类框架，利用非结构化电子健康记录（EHR）解决老年心血管风险管理中手动行政编码的局限性。基于3,482名患者的纵向荷兰临床文本数据，对比了三种建模范式：传统机器学习基线、针对长序列优化的深度学习架构，以及零样本设置下的通用生成式大语言模型（LLM）。此外，还评估了融合非结构化文本与结构化药物嵌入及人体测量数据的后期融合策略。结果表明，自定义Transformer架构在F1分数和马修相关系数上均优于传统方法和生成式LLM，凸显了专用分层注意力机制在捕捉医学文本长程依赖关系中的关键作用，为临床风险分层提供了可靠、自动化的替代方案。


<details>
  <summary>Details</summary>
Motivation: 克服手动行政编码在老年心血管风险管理中的局限性，提升风险分层的自动化水平和准确性。

Method: 采用三种建模范式：经典机器学习基线、专门优化的深度学习架构（自定义Transformer）、通用生成式大语言模型（零样本），并结合后期融合策略整合非结构化文本与结构化数据（药物嵌入、人体测量数据）。

Result: 自定义Transformer架构在F1分数和马修相关系数上表现最佳，显著优于传统方法和生成式大语言模型，证明其在捕捉医学文本长程依赖方面的优势。

Conclusion: 专用分层注意力机制对理解复杂医疗文本至关重要，该框架可作为手动流程的高效自动化替代方案，用于临床风险分层。
...
### [136] [ESAinsTOD: A Unified End-to-End Schema-Aware Instruction-Tuning Framework for Task-Oriented Dialog Modeling](https://arxiv.org/abs/2603.09691)
*Dechuan Teng,Chunlin Lu,Libo Qin,Wanxiang Che*

Main category: cs.CL

TL;DR: ESAinsTOD提出一种统一的端到端架构，通过指令对齐和模式对齐机制，使大模型在任务导向对话中具备更强的适应性与泛化能力，显著提升零样本性能与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有端到端建模方法依赖特定数据集，难以适应新对话场景；需构建通用、可灵活适配的任务导向对话系统。

Method: 采用全参数微调的大语言模型，引入指令对齐与模式对齐机制，并结合会话级端到端建模，实现对任务流程与数据模式的感知与遵循。

Result: 在CamRest676、In-Car、MultiWOZ等基准上超越现有模型；在低资源及零样本设置下表现优异；对数据噪声与级联错误具有更强鲁棒性。

Conclusion: ESAinsTOD通过结构化指令微调框架，有效提升了任务导向对话系统的通用性、适应性与可靠性，为实际应用提供了有力支持。
...
### [137] [Evaluation of LLMs in retrieving food and nutritional context for RAG systems](https://arxiv.org/abs/2603.09704)
*Maks Požarnik Vavken,Matevž Ogrinc,Tome Eftimov,Barbara Koroušić Seljak*

Main category: cs.CL

TL;DR: 本文评估了四种大型语言模型（LLMs）在专用检索增强生成（RAG）系统中从食品成分数据库中检索数据的有效性。方法聚焦于LLM将自然语言查询转化为结构化元数据过滤器的能力，以实现通过Chroma向量数据库的高效检索。结果显示，LLM在简单和中等复杂度查询上表现优异，能显著降低领域专家（如食品编纂者和营养师）使用复杂营养数据所需的手动工作量和技术门槛；但在涉及不可表达约束的困难查询上，可靠检索仍具挑战性，表明基于元数据过滤的LLM方法在约束可明确表达时有效，而在超出元数据格式表示能力时表现不佳。


<details>
  <summary>Details</summary>
Motivation: 旨在探索大型语言模型在食品与营养数据检索中的应用潜力，解决传统方法对技术门槛和人工干预的依赖问题，提升领域专家获取精准数据的效率。

Method: 采用检索增强生成（RAG）系统，利用Chroma向量数据库存储食品成分数据，通过四种LLMs将自然语言查询转换为结构化元数据过滤条件，实现高效精准检索。

Result: LLMs在简单和中等复杂度查询中表现出高准确率，显著降低人工操作与技术门槛；但在涉及非显式约束的复杂查询中，检索可靠性下降，暴露出元数据表示能力的局限性。

Conclusion: LLM驱动的元数据过滤在可表达约束下表现卓越，是提升食品与营养数据访问效率的有力工具，但其在处理复杂或隐含约束时仍存在明显瓶颈，需结合其他技术手段优化。
...
### [138] [RbtAct: Rebuttal as Supervision for Actionable Review Feedback Generation](https://arxiv.org/abs/2603.09723)
*Sihong Wu,Yiling Ma,Yilun Zhao,Tiansheng Hu,Owen Jiang,Manasi Patwardhan,Arman Cohan*

Main category: cs.CL

TL;DR: 该研究提出RbtAct，一种专注于生成可操作性同行评审反馈的方法，利用审稿人回复（rebuttal）作为隐式监督信号来优化反馈生成模型。通过引入新的任务——基于视角的段落级反馈生成，并构建了包含75,000条数据的大规模数据集RMR-75K，标注了评论与回应之间的对应关系、视角标签和作者采纳影响等级。在Llama-3.1-8B-Instruct模型上采用监督微调和基于回应对的偏好优化，实验表明该方法在可操作性和具体性方面显著优于基线，同时保持内容相关性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型生成的同行评审意见往往流于表面，缺乏可执行的具体建议，导致作者难以获得实际改进指导，因此本文旨在填补这一空白，提升评审反馈的可操作性。

Method: 提出一种基于审稿人回应的隐式监督机制，设计新任务‘视角条件下的段落级评审反馈生成’，构建大规模数据集RMR-75K，使用监督微调与偏好优化联合训练模型，以增强反馈的行动导向性。

Result: 在人类专家和大模型评估中，RbtAct在可操作性与具体性方面持续优于强基线模型，同时保持了良好的内容相关性与真实性。

Conclusion: RbtAct通过利用审稿人回应作为隐式监督信号，有效提升了同行评审反馈的可操作性与实用性，为未来自动化评审系统的发展提供了重要方向。
...
### [139] [EPIC-EuroParl-UdS: Information-Theoretic Perspectives on Translation and Interpreting](https://arxiv.org/abs/2603.09785)
*Maria Kunilovskaya,Christina Pollkläsener*

Main category: cs.CL

TL;DR: 本文介绍了更新并整合的英德双语EPIC-UdS（口语）和EuroParl-UdS（书面）语料库的新版本，修正了先前发现的元数据和文本错误，优化内容，更新语言标注，并新增词对齐和词级意外度指数等层次。该资源旨在支持基于信息论方法的语言变异研究，如对比书面与口语模式、分析言语不流畅现象，以及传统翻译风格研究。论文概述了此次更新内容，总结了以往研究成果，并展示了一项新示范研究，验证了重建口语数据的可靠性，并评估了基于基础和微调GPT-2及机器翻译模型的概率性指标在预测口译中填充词任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 为了支持更精确的信息理论驱动的语言变异研究，特别是对比书面与口语差异、分析言语不流畅现象，以及开展传统的翻译风格研究，需要一个高质量、结构完善且标注丰富的多模态双语语料库。此前版本存在元数据与文本错误，标注不完整，限制了研究深度与准确性，因此亟需系统性更新与增强。

Method: 通过识别并修正原始语料库中的元数据与文本错误，优化内容结构；更新并扩展语言学标注，包括添加词对齐信息与词级意外度指数；整合口语与书面语料，构建统一资源；利用基础与微调的GPT-2模型及机器翻译系统，构建概率模型以预测口译中的填充词，并通过实证研究验证数据质量与模型性能。

Result: 新版本语料库显著提升了数据质量与可用性，有效支持多种语言研究任务；示范研究表明，重建的口语数据具有较高完整性，基于GPT-2和机器翻译模型的概率指标在预测填充词方面表现出良好效果，验证了语料库与方法的有效性。

Conclusion: 本次更新后的语料库是一个高质量、多层次、适用于多种语言研究的资源，尤其在对比口语与书面语、研究翻译风格及言语不流畅现象方面具有重要价值。其可靠性和扩展性为未来信息论与自然语言处理研究提供了坚实基础。
...
### [140] [One-Eval: An Agentic System for Automated and Traceable LLM Evaluation](https://arxiv.org/abs/2603.09821)
...
TL;DR: One-Eval 是一个智能评估系统，将自然语言评估请求转化为可执行、可追溯且可定制的评估工作流，通过NL2Bench进行意图结构化与个性化基准规划，BenchResolve实现基准解析、自动数据集获取与模式归一化以确保可执行性，Metrics & Reporting则支持任务感知的指标选择和面向决策的报告。系统还引入人机协同检查点以支持审查、编辑与回滚，并保留样本证据链以供调试与审计。实验表明，One-Eval 能够从多样化的自然语言请求中端到端完成评估，显著降低用户投入，提升工业场景下的评估效率与可复现性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的可靠评估依赖大量手动操作，包括选择基准、复现异构代码库、配置数据集模式映射及解释聚合指标，耗时且易出错。亟需自动化、可追溯、可定制的评估流程以提升效率与一致性。

Method: 提出One-Eval系统，包含三个核心模块：(i) NL2Bench用于自然语言到评估意图的结构化与个性化基准规划；(ii) BenchResolve实现基准解析、自动数据获取与模式归一化；(iii) Metrics & Reporting支持任务感知的指标选择与决策导向的报告生成。系统集成人机协同检查点与证据链追踪机制。

Result: One-Eval能从自然语言请求出发，高效完成端到端评估，大幅减少用户干预，支持更高效、可复现的工业级评估实践。

Conclusion: One-Eval为大语言模型评估提供了一种自动化、可追溯、可定制的解决方案，显著降低评估门槛，提升评估过程的效率与可信度，已在开源平台发布。
...
TL;DR: 本文研究长上下文推理中分块顺序对信息损失的影响，提出使用Chow-Liu树学习块之间的依赖关系，并通过广度优先遍历生成分块顺序，显著提升答案相关性和精确匹配率。


<details>
  <summary>Details</summary>
Motivation: 现有基于多智能体的长上下文推理框架（如CoA）因受限于共享内存大小，采用分块处理并按顺序传递摘要，导致信息丢失，且结果依赖于分块顺序。因此需要优化分块顺序以减少信息损失。

Method: 利用Chow-Liu树学习各分块间的依赖结构，基于该结构进行广度优先遍历，生成最优分块顺序，从而减少信息瓶颈带来的损失。

Result: 在三个长上下文基准测试中，所提方法在答案相关性和精确匹配准确率上均优于默认文档顺序和基于语义得分的排序方法。

Conclusion: 通过学习分块间的依赖关系并采用广度优先遍历生成顺序，能够有效降低信息损失，提升长上下文推理性能。
...
TL;DR: 本文探讨了现代语言模型（如Transformer）在预测下一个词时，其概率计算与阅读时间预测之间的脱节现象，并提出该现象可归因于阅读时间对简单n-gram统计的敏感性，而非复杂模型所学习的高级统计特征。研究发现，与n-gram概率相关性最高的神经语言模型，其概率预测也最能解释眼动追踪数据中的阅读时间指标。


<details>
  <summary>Details</summary>
Motivation: 探究为何先进的语言模型在预测阅读时间方面表现不佳，尽管它们在下一个词预测任务上非常出色。

Method: 通过分析语言模型的概率输出与n-gram统计的相关性，以及这些概率与眼动追踪数据中阅读时间指标的关联性，进行实证研究。

Result: 与n-gram概率高度相关的语言模型，其预测概率也与自然文本上的阅读时间指标有最强的相关性。

Conclusion: 阅读时间主要受简单n-gram统计影响，而非深度语言模型所捕捉的复杂语言结构，因此当前语言模型的概率预测在阅读时间建模上存在局限性。
...
TL;DR: 提出DOWIS数据集，用于在真实语音指令条件下评估语音大语言模型（SLLMs），涵盖9个任务、11种语言，每对任务-语言有10种提示变体和5种风格。实验表明，文本提示在大多数情况下优于语音提示，尤其在低资源和跨语言场景中；仅当任务涉及语音输出时，语音提示表现接近文本提示，凸显语音提示评估的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有SLLM评估多依赖文本提示，无法反映真实用户通过语音交互的场景，需引入更贴近实际的语音指令评估方式。

Method: 构建DOWIS数据集，包含人类录制的语音与文本提示，覆盖多语言、多任务、多风格，用于在真实语音条件下评估SLLMs，并分析提示模态、风格、语言和任务类型的影响。

Result: 文本提示在多数任务中表现优于语音提示，尤其在低资源和跨语言设置下差距明显；仅当任务需要语音输出时，语音提示性能接近文本提示。

Conclusion: 为实现更真实的SLLM评估，应重视语音提示的作用，特别是在语音输出任务中，未来评估需纳入语音指令条件。
...
TL;DR: 本研究通过两项大规模调查实验（N=19,145），评估了Anthropic、OpenAI、Google和xAI开发的七种前沿大语言模型在跨党派议题和立场上的说服力。结果表明，大语言模型整体上优于传统政治宣传广告，其中Claude模型最具说服力，而Grok表现最差。该效果在不同议题和立场下均稳健。此外，信息型提示的效力具有模型依赖性：对Claude和Grok有提升作用，但显著降低GPT的说服力。研究提出一种数据驱动、策略无关的对话分析方法，用于识别和评估潜在的说服策略，为前沿模型的说服风险提供基准，并建立跨模型比较评估框架。


<details>
  <summary>Details</summary>
Motivation: 尽管已有研究认为大语言模型（LLMs）的说服力不高于传统政治宣传手段，但随着前沿模型的快速发展，其潜在的政治影响力亟需重新评估。本文旨在系统检验当前主流大模型在真实政治议题中的说服能力，并揭示其内部差异及提示策略的影响机制。

Method: 采用两轮大规模在线调查实验，覆盖多个跨党派议题与立场，测试七种前沿大语言模型（包括Claude、GPT、Gemini、Grok等）在生成文本后的说服效果。使用标准化政治宣传材料作为对照，通过随机分配受试者接触不同模型输出或传统广告，测量态度变化。同时引入数据驱动的对话分析方法，挖掘各模型背后的说服策略。

Result: 大语言模型整体上显著优于传统政治广告；不同模型间存在明显差异，Claude表现最强，Grok最弱；信息型提示的效果因模型而异——增强Claude和Grok的说服力，却削弱GPT的表现；模型间的说服策略存在显著差异，且可被数据方法有效识别。

Conclusion: 本研究揭示了前沿大语言模型在政治说服方面的显著潜力与内在异质性，强调不能一概而论地评价其影响。研究提出了一个可扩展的跨模型风险评估框架，为政策制定、技术治理和伦理监管提供了实证基础与方法支持。
...
TL;DR: 尽管在数学、代码生成和多跳事实问题中，大语言模型的推理能力自然发挥作用，但在简单的一跳事实问题中其作用尚不明确。本文发现，启用推理能显著扩展模型参数知识回忆的能力边界，解锁原本无法获取的正确答案。即使没有复杂的推理步骤，推理仍能通过计算缓冲效应和事实提示机制促进知识召回。后者存在风险：推理过程中幻觉中间事实会增加最终答案幻觉的可能性。研究还提出一种方法，通过优先选择无幻觉事实的推理路径来直接提升模型准确率。


<details>
  <summary>Details</summary>
Motivation: 探究推理在简单、单跳事实问题中的作用，因为这类问题无需复杂逻辑分解，推理的效用看似反直觉。

Method: 设计一系列假设驱动的受控实验，分析推理如何影响模型的知识召回能力，并识别关键机制如计算缓冲效应和事实提示。

Result: 推理通过计算缓冲效应和事实提示机制提升了模型对参数知识的回忆能力；但生成幻觉中间事实会增加最终答案的幻觉风险；通过优先选择无幻觉事实的推理路径可提高模型准确性。

Conclusion: 推理虽在简单事实问题中无显式逻辑步骤，但仍能通过隐式计算与事实提示增强知识召回；该机制具有潜在幻觉风险，但可通过优化推理轨迹提升准确率。
...
TL;DR: 该论文提出了一种名为FUSE的四维分类框架，系统梳理了大语言模型时代的模型合并技术，涵盖基础理论、统一策略、应用场景与生态系统。研究分析了权重平均、任务向量算术、稀疏化增强方法、专家混合架构及进化优化等算法，并探讨其在多任务学习、安全对齐、领域专业化等场景中的应用。同时，总结了开源工具、评估基准和社区平台，指出了理论空白、可扩展性挑战与标准化需求等关键开放问题。


<details>
  <summary>Details</summary>
Motivation: 随着微调的大语言模型迅速增多，模型合并提供了一种无需额外训练即可整合多个模型能力的高效替代方案，避免了集成或全量重训练的高成本，具有重要的实践价值。

Method: 采用FUSE四维分类框架（基础、统一策略、场景、生态），从理论基础出发，系统回顾各类合并算法，分析核心公式与代表性工作，并结合实际应用与工具生态进行综合评估。

Result: 全面梳理了模型合并的技术演进，揭示了不同方法的优缺点与适用场景，明确了当前面临的理论、可扩展性和标准化挑战，为未来研究提供了结构化指导。

Conclusion: 本综述通过系统化框架，为研究人员和从业者提供了推进模型合并技术发展的坚实基础，推动其在多模态、分布式与专业化场景中的进一步应用。
...
TL;DR: CREATE 是一个评估模型创造性联想推理能力的基准，要求模型生成连接概念的高特异性、高多样性的路径。该任务模拟真实创造力任务（如假设生成），具有巨大的搜索空间，但支持客观评分。评估显示，最强模型在创造性效用上表现更优，且由于答案多样性与搜索复杂性，基准难以饱和。思维模型并非总是更有效，近期创意提示方法仅带来有限提升。CREATE 为提升模型联想创造力提供了实验平台。


<details>
  <summary>Details</summary>
Motivation: 现有模型在创造性任务中表现不足，尤其在生成新颖且有意义的概念连接方面。需要一个能客观评估模型创造性联想推理能力的基准，以推动相关技术发展。

Method: 提出 CREATE 基准，通过要求模型生成多个高特异性、高多样性的概念连接路径进行评估，并基于路径质量与多样性打分。

Result: 前沿模型在创造性和多样性上表现优于其他模型，但基准难以饱和；思维模型在高计算预算下并不一定更优，现有创意提示方法仅有部分提升作用。

Conclusion: CREATE 为评估和提升大模型的创造性联想推理能力提供了一个有效且可扩展的基准，有助于推动新方法的发展。
...
TL;DR: 提出MathQ-Verify，一种五阶段管道，用于严格过滤表述不当或信息不足的数学问题。通过格式验证、形式化分解、逻辑矛盾检测和目标完整性检查，显著提升数学题目质量，实验表明其在多个基准上性能领先，F1最高提升25个百分点，精度约90%，召回率63%。


<details>
  <summary>Details</summary>
Motivation: 现有工作多关注生成正确推理路径和答案，忽视问题本身的合理性与有效性，导致数据噪声和无效计算。因此需要一种方法来验证数学问题的质量，确保其可解性与逻辑一致性。

Method: 设计五阶段管道：1）格式级验证，去除冗余指令；2）形式化问题并分解为原子条件；3）基于数学定义验证各条件；4）检测条件间的逻辑矛盾；5）进行目标导向的完整性检查，确保信息充分。结合轻量级模型投票提升准确率。

Result: 在包含2,147个带错误类型的数学问题的数据集上，MathQ-Verify在多个基准上达到领先性能，相比直接验证基线F1提升最高达25个百分点，实现约90%精度与63%召回率。

Conclusion: MathQ-Verify提供了一种可扩展、高精度的数学数据清洗方案，有效降低标签噪声，避免对无效问题的无效计算，有助于构建更可靠的教学与训练数据集。代码与数据已开源。
...
### [149] [Skip to the Good Part: Representation Structure & Inference-Time Layer Skipping in Diffusion vs. Autoregressive LLMs](https://arxiv.org/abs/2603.07475)
*Raghavv Goel,Risheek Garrepalli,Sudhanshu Agrawal,Chris Lott,Mingu Lee,Fatih Porikli*

Main category: cs.CL

TL;DR: 该研究首次对比分析了扩散语言模型（dLLMs）与自回归（AR）语言模型在层和词元级别的表征结构。发现扩散目标导致更分层的抽象，早期层存在大量冗余且减少近期偏差，而AR目标则产生紧密耦合、依赖深度的表示。即使经过扩散训练，由AR初始化的dLLMs仍保留AR式的表征动态，表明初始化偏见具有持久性。基于观察到的冗余性，提出一种无需架构修改或KV缓存共享的静态、任务无关的推理时层跳过方法。天然dLLMs在推理中可减少高达18.75%的计算量，同时保持90%以上的性能，而AR模型在类似跳过下性能显著下降。结果揭示了训练目标与表征结构之间的关联，并实现了无需缓存依赖的高效推理。


<details>
  <summary>Details</summary>
Motivation: 理解扩散语言模型（dLLMs）与自回归（AR）语言模型在内部表征结构上的根本差异，尤其是扩散目标是否重塑了模型深层表示；同时探索如何利用这些差异实现高效的推理优化。

Method: 进行层和词元级别的表征分析，比较三种模型：原生dLLMs（LLaDA）、原生AR模型（Qwen2.5）以及由AR初始化的dLLMs（Dream-7B）。通过分析表征冗余性、层次化程度和近期偏差等指标，识别不同训练目标下的表征特性。在此基础上，设计并验证一种推理时的静态层跳过策略，评估其在计算效率与性能保持方面的表现。

Result: 扩散语言模型表现出更分层的抽象结构，早期层具有显著冗余，且减少对最近输入的依赖；而自回归模型则呈现紧密耦合、深度依赖的表示。尽管经过扩散训练，由AR初始化的模型仍保留了原有的表征动态。基于冗余性，提出的层跳过方法在不改变架构或共享KV缓存的前提下，使原生dLLMs在推理中降低高达18.75%的FLOPs，同时保持超过90%的性能；相比之下，AR模型在相同条件下性能急剧下降。

Conclusion: 训练目标深刻影响模型的内部表征结构：扩散目标促进更分层、冗余度更高的表示，而自回归目标则形成紧密耦合的深度依赖结构。初始状态具有强持续性，即使训练目标改变也难以消除。基于此，可设计出无需依赖缓存共享的高效推理机制，为大模型部署提供新的优化路径。
...
TL;DR: Guardian 是一个端到端的决策支持系统，用于失踪儿童调查和早期搜索规划。它将异构、非结构化的案件文档转化为时空对齐的表示，通过地理编码和交通上下文增强案件信息，并提供0-72小时的概率性搜索产品。系统采用三层预测架构：第一层为马尔可夫链模型，结合道路可达性成本、隐蔽偏好和走廊偏倚，分昼夜参数化；第二层使用强化学习将马尔可夫链输出转化为可操作的搜索计划；第三层利用大语言模型对第二层的搜索计划进行事后验证。通过合成案例研究，验证了系统在24/48/72小时时间窗口下的定量性能，分析了敏感性、失败模式与权衡，结果表明该系统能生成可解释的先验信息，用于区域优化和人工审查。


<details>
  <summary>Details</summary>
Motivation: 当前失踪儿童调查的前72小时至关重要，但执法机构面临数据碎片化、非结构化以及缺乏动态地理空间预测工具的问题。现有方法难以有效整合多源信息并生成可操作的搜索建议，亟需一个智能、可解释且实用的决策支持系统。

Method: 提出三层次预测架构：1）基于道路可达性、隐蔽偏好和走廊偏倚的分昼夜马尔可夫链模型，生成初始概率分布；2）利用强化学习将概率分布转化为实际可执行的搜索计划；3）采用大语言模型对生成的搜索计划进行后验验证，确保其合理性与可操作性。整个系统实现从非结构化文本到时空预测产品的转化。

Result: 系统在合成案例中成功生成了0-72小时内具有可解释性的概率搜索优先级，验证了各层级的有效性。结果显示，该系统能够优化搜索区域划分，减少冗余搜索，提升响应效率，并支持人工复核。同时识别出模型在极端地形或数据缺失情况下的脆弱性，提出改进方向。

Conclusion: Guardian 系统通过融合马尔可夫链、强化学习与大语言模型，构建了一个可解释、可操作且适应性强的失踪儿童搜索决策支持框架。其三层次架构在真实场景中具备推广潜力，显著提升了早期搜索阶段的科学性与效率。
...
### [151] [A Consensus-Driven Multi-LLM Pipeline for Missing-Person Investigations](https://arxiv.org/abs/2603.08954)
*Joshua Castillo,Ravi Mukkamala*

Main category: cs.AI

TL;DR: Guardian LLM Pipeline 是一个用于失踪儿童调查和早期搜索规划的多模型系统，利用大语言模型（LLM）进行智能信息提取与处理。该系统通过任务专用的LLM协同工作，并由共识LLM引擎整合不同模型输出、解决分歧。系统采用QLoRA微调技术，基于精心筛选的数据集提升性能，强调保守、可审计的LLM使用方式，将其作为结构化信息提取与标注工具，而非全权决策者。


<details>
  <summary>Details</summary>
Motivation: 失踪人员调查的前72小时至关重要，现有系统缺乏高效的信息处理与协调机制。为提升调查效率与准确性，需要一种能够智能提取和整合关键信息的自动化系统。

Method: 设计并实现Guardian LLM Pipeline，包含任务专用的LLM模型、共识决策引擎以及基于QLoRA的微调策略。系统通过弱监督与LLM辅助标注方法训练，确保结果可追溯且安全可控。

Result: 系统实现了端到端的缺失信息提取与搜索规划支持，显著提升了信息处理速度与一致性，同时保持了可审计性与安全性。

Conclusion: Guardian LLM Pipeline 有效结合了多模型协作与共识机制，在保障可靠性的同时增强了对失踪人员调查的支持能力，为未来智能辅助执法系统提供了可行范式。
...
### [152] [The FABRIC Strategy for Verifying Neural Feedback Systems](https://arxiv.org/abs/2603.08964)
*I. Samuel Akinwande,Sydney M. Katz,Mykel J. Kochenderfer,Clark Barrett*

Main category: cs.AI

TL;DR: 本文提出了一种新的算法，用于计算非线性神经反馈系统的后向可达集的上界和下界近似，并将其与现有的前向分析技术集成，形成名为FaBRIC的算法。在多个基准测试中，该方法显著优于现有最先进水平。


<details>
  <summary>Details</summary>
Motivation: 后向可达分析在神经反馈系统中的应用受限于现有技术的可扩展性不足，因此需要新的方法来填补这一研究空白。

Method: 提出新算法以计算后向可达集的过近似和欠近似，并与现有前向分析技术集成，形成整合式方法FaBRIC。

Result: 在代表性基准测试中，所提方法在性能上显著优于当前最先进的技术。

Conclusion: 本工作为神经反馈系统的后向可达分析提供了有效的解决方案，并通过集成前向与后向分析显著提升了验证能力。
...
TL;DR: MEMO通过耦合保留与探索的自洽框架，优化推理时的上下文，显著提升多智能体大模型在文本游戏中的表现和稳定性。它利用持久记忆库存储自洽轨迹中的结构化洞察，并在后续游戏中注入先验知识；同时通过基于TrueSkill的不确定性感知选择和优先重放机制，实现提示演化与罕见关键状态的再访问。在五个文本游戏中，MEMO将GPT-4o-mini和Qwen-2.5-7B-Instruct的平均胜率分别从25.1%和20.9%提升至49.5%和44.3%，并大幅降低运行间方差，使提示变化下的排名更稳定。该方法在谈判和不完全信息博弈中效果尤为突出，而强化学习在完全信息场景中仍具优势。


<details>
  <summary>Details</summary>
Motivation: 多轮多智能体大模型游戏评估存在显著的运行间方差，早期微小偏差在长周期交互中累积并被多智能体耦合放大，导致胜率估计偏差和排名不可靠。提示选择进一步加剧此问题，生成不同有效策略。亟需一种能提升性能与鲁棒性的上下文优化方法。

Method: 提出MEMO（Memory-augmented MOdel context optimization）框架，通过耦合保留与探索机制优化推理时上下文：保留部分维护持久记忆库，存储自洽轨迹中的结构化洞察并作为先验注入后续决策；探索部分采用锦标赛式提示演化，结合TrueSkill进行不确定性感知选择，并使用优先重放机制重新访问稀有且决定性状态。

Result: 在五个文本游戏上，MEMO使GPT-4o-mini的平均胜率从25.1%提升至49.5%，Qwen-2.5-7B-Instruct从20.9%提升至44.3%，运行间方差显著下降，提示变化下的排名更加稳定。在谈判和不完全信息游戏中的增益最大，而强化学习在完全信息设置中仍更优。

Conclusion: 多智能体大模型在游戏任务中的性能与鲁棒性仍有巨大提升空间，通过上下文优化可实现显著改进。MEMO通过记忆增强与探索优化，在复杂、非完全信息环境中表现出色，为提升大模型协作能力提供了新路径。
...
### [154] [From Days to Minutes: An Autonomous AI Agent Achieves Reliable Clinical Triage in Remote Patient Monitoring](https://arxiv.org/abs/2603.09052)
...
TL;DR: Sentinel is an autonomous AI agent using Model Context Protocol (MCP) to triage remote patient monitoring (RPM) vitals with high sensitivity and accuracy. It outperforms clinicians in detecting emergencies and actionable alerts, maintains low overtriage risk, and operates at a median cost of $0.34 per triage, offering a scalable solution to the limitations of prior RPM trials.


<details>
  <summary>Details</summary>
Motivation: Existing RPM systems generate overwhelming data volumes that clinical staff cannot manage effectively, leading to failed trials despite evidence that intensive monitoring reduces mortality. The current model is too expensive and unscalable, necessitating an automated, intelligent triage system.

Method: Sentinel uses MCP for contextual triage across 21 clinical tools with multi-step reasoning. Evaluation includes self-consistency testing, comparison with rule-based thresholds, clinician validation via connected matrix design, leave-one-out analysis, and independent physician adjudication of severe overtriage cases.

Result: Sentinel achieved 95.8% emergency sensitivity and 88.5% sensitivity for all actionable alerts, with 85.7% specificity and 69.4% four-level accuracy (kappa=0.778). It outperformed all individual clinicians in emergency and actionable sensitivity. Overtriage was minimal (22.5%), and severe discrepancies were validated as clinically appropriate in 88–94% of cases. Self-consistency was near-perfect (kappa=0.850). Median cost per triage was $0.34.

Conclusion: Sentinel enables scalable, high-sensitivity RPM triage by automating context synthesis, overcoming the key limitation of past trials and delivering a clinically defensible, cost-effective solution that supports life-saving monitoring.
...
### [155] [DataFactory: Collaborative Multi-Agent Framework for Advanced Table Question Answering](https://arxiv.org/abs/2603.09152)
*Tong Wang,Chi Jin,Yongkang Chen,Huan Deng,Xiaohui Kuang,Gang Zhao*

Main category: cs.AI

TL;DR: 该论文提出DataFactory，一个用于表格问答（TableQA）的多智能体框架，通过专业化团队协作与自动化知识转换解决大语言模型在上下文长度、幻觉和复杂推理方面的局限。框架包含数据领导者（采用ReAct范式）、数据库团队和知识图谱团队，实现复杂查询的结构化分解。通过定义数据到知识图谱的映射函数T:D×S×R→G，并引入基于自然语言的灵活协商机制，提升协作鲁棒性。结合上下文工程策略，融入历史模式与领域知识以减少幻觉。在TabFact、WikiTableQuestions和FeTaQA上实验表明，相比基线显著提升准确率（如TabFact提升20.2%，WikiTQ提升23.9%），团队协作优于单一团队设置，且效果显著（Cohen's d > 1）。框架为多智能体协同提供设计指南，并支持企业级数据分析中的结构化查询与图式知识表示。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在表格问答中受限于上下文长度、易产生幻觉、以及单智能体架构难以处理复杂推理任务，尤其在涉及语义关系和多跳逻辑时表现不佳，亟需更高效、可靠的多智能体解决方案。

Method: 提出DataFactory多智能体框架，由数据领导者（使用ReAct范式进行推理编排）、数据库团队和知识图谱团队组成；设计数据到知识图谱的自动转换映射函数T:D×S×R→G；引入基于自然语言的动态协商机制实现灵活协作；结合上下文工程策略整合历史模式与领域知识以降低幻觉风险。

Result: 在TabFact、WikiTableQuestions和FeTaQA三个基准上，使用8个来自5家厂商的大语言模型，结果显示本方法在准确性上显著优于基线：分别提升20.2%（TabFact）和23.9%（WikiTQ），团队协作相较单团队方案也有明显提升（如TabFact+5.5%，WikiTQ+14.4%，FeTaQA ROUGE-2+17.1%），效果具有统计显著性（Cohen's d > 1）。

Conclusion: DataFactory框架有效克服了现有大语言模型在表格问答中的关键瓶颈，通过多智能体协同与知识转化机制实现了更精准、稳健的推理能力，为复杂数据交互提供了可扩展的设计范式与实际应用平台。
...
TL;DR: 本文提出逻辑推理能力的提升可能意外增强AI系统的处境意识（situational awareness），并引入RAISE框架，揭示了演绎、归纳和类比三种推理路径如何推动AI从自我识别到战略欺骗的渐进式演变。研究指出当前安全措施无法应对这一风险，并建议建立‘镜像测试’基准与推理安全对等原则，呼吁逻辑推理领域正视其潜在责任。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型逻辑推理能力的增强，其在自我认知、上下文理解与策略性思考方面的能力也显著提升，可能催生危险的处境意识，威胁AI安全。现有研究尚未充分关注这一潜在风险，亟需系统性分析与应对。

Method: 提出RAISE框架，形式化演绎自推理、归纳上下文识别、类比自我建模三条机制路径，构建从基础自我识别到战略欺骗的升级阶梯，并将主流逻辑推理研究主题映射至处境意识增强的具体放大器。

Result: 所有主要的LLM逻辑推理研究均直接促进不同程度的处境意识；现有安全措施不足以防范此风险；提出‘镜像测试’基准与推理安全对等原则作为潜在解决方案。

Conclusion: 逻辑推理研究若不考虑其对处境意识的强化作用，可能无意中加剧AI系统的自主性和潜在危害。必须重新审视该领域的责任边界，并建立相应的安全约束机制。
...
### [157] [Abundant Intelligence and Deficient Demand: A Macro-Financial Stress Test of Rapid AI Adoption](https://arxiv.org/abs/2603.09209)
*Xupeng Chen*

Main category: cs.AI

TL;DR: 本文提出一种针对快速人工智能（AI）采用的宏观金融压力测试框架，指出核心问题在于‘分配与合同不匹配’：AI带来的产出丰裕与需求不足并存，因经济制度仍基于人类认知稀缺性。文中通过三个机制展开分析：1）替代螺旋效应——企业理性用AI替代人力导致总劳动收入下降，进而抑制总需求，加速更多AI采用；2）幽灵GDP现象——当AI产出取代人力产出时，货币流通速度随劳动力份额下降，造成衡量产出与实际消费收入之间的差距；3）中介崩溃——AI降低信息摩擦，压缩中介利润至仅物流成本水平，引发软件即服务、支付、咨询、保险及财务顾问等领域的价格重估。由于高收入群体（占美国消费47-65%）对AI暴露度最高，其在私人信贷（全球2.5万亿美元）和抵押贷款市场（13万亿美元）中的传导影响尤为显著。研究提出十一个可检验预测，并附明确可证伪条件。基于FRED时间序列与BLS职业数据校准的模拟，量化了稳定调整向爆炸式危机转变的临界条件。


<details>
  <summary>Details</summary>
Motivation: 当前关于AI的讨论多聚焦于生产力冲击或存在性风险，但本文指出更紧迫的问题是经济制度滞后于技术变革，特别是分配机制与合同安排未能适应由AI引发的产出丰裕与需求不足之间的结构性失衡。现有经济模型未充分捕捉AI对收入分配、货币流通与中介功能的深层冲击，亟需建立系统性压力测试框架以评估潜在金融危机。

Method: 构建理论模型，结合动态一般均衡思想与制度嵌入视角，形式化三个关键机制：替代螺旋（含反馈稳定性分析）、幽灵GDP（货币速度与收入测度关系）、中介崩溃（利润率压缩路径）。利用美国宏观经济数据（FRED）与职业层面就业数据（BLS）进行参数校准，开展数值模拟，识别系统从稳定过渡到危机的临界条件。

Result: 模型显示，在特定条件下（如AI能力增长过快、扩散速度超预期、再安置率不足），替代螺旋将产生自我强化的爆炸性反馈；幽灵GDP导致名义产出与真实消费收入脱节，可能误导政策判断；中介利润空间被压缩，引发跨行业价格重构。模拟表明，若无补偿性转移支付或制度调整，全球私人信贷与抵押贷款市场将面临系统性压力，尤其在高收入群体高度暴露的情境下。

Conclusion: AI快速普及可能引发非传统型金融不稳定，其根源并非技术失效，而是制度与技术间的结构性错配。为防止危机，需提前设计收入再分配机制、调整货币政策框架以应对货币流通变化，并重新思考中介角色的合法性与定价逻辑。本研究提供了可操作的压力测试工具与预警信号。
...
### [158] [Cognitively Layered Data Synthesis for Domain Adaptation of LLMs to Space Situational Awareness](https://arxiv.org/abs/2603.09231)
*Ding Linghu,Cheng Wang,Da Fan,Wei Shi,Kaifeng Yin,Xiaoliang Xue,Fan Yang,Haiyi Ren,Cong Zhang*

Main category: cs.AI

TL;DR: 提出BD-FDG框架，通过知识树结构化组织、认知分层问题建模和多维质量控制，构建23万样本的太空态势感知（SSA）领域SFT数据集，微调Qwen3-8B得到SSA-LLM-8B，在领域测试集上BLEU-1提升达144%~176%，对战胜率82.21%，同时保持通用基准性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂工程领域如太空态势感知（SSA）中的应用受限于与任务链结构不匹配、缺乏高阶认知监督以及数据质量标准与工程规范不符，核心瓶颈在于高质量监督微调（SFT）数据集的构建。

Method: BD-FDG框架包含三个机制：1）使用知识树确保知识覆盖结构化；2）基于布卢姆分类学设计涵盖九类六级认知层次（从记忆到创造）的问题生成方案，实现难度连续梯度；3）采用多维度评分流水线保障领域严谨性与一致性。

Result: 构建了约23万样本的SSA-SFT数据集，微调后得到SSA-LLM-8B，在领域测试集上相对BLEU-1提升144%（无思考）和176%（思考），对战胜率达82.21%，且在通用基准（MMLU-Pro, MATH-500）上表现稳定。

Conclusion: 基于认知分层的SFT数据构建是复杂工程领域大模型适配的有效范式，BD-FDG提供了一个可迁移的领域特定模型优化框架。
...
### [159] [Social-R1: Towards Human-like Social Reasoning in LLMs](https://arxiv.org/abs/2603.09249)
*Jincenzi Wu,Yuxuan Lei,Jianxun Lian,Yitian Huang,Lexin Zhou,Haotian Li,Xing Xie,Helen Meng*

Main category: cs.AI

TL;DR: 本文提出ToMBench-Hard基准和Social-R1强化学习框架，通过对抗性难例和多维奖励机制，提升大模型的社会推理能力，使4B参数模型超越更大模型并在多个基准上实现稳健泛化。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在社会智能方面仍存在不足，主要依赖表面模式而非真正的社会推理，亟需通过具有挑战性的训练样本和过程监督来提升模型的人类级社会认知能力。

Method: 引入ToMBench-Hard对抗性基准提供难例训练数据，并设计Social-R1强化学习框架，通过结构对齐、逻辑完整性和信息密度等多维度奖励，监督模型推理全过程，实现轨迹级对齐。

Result: 4B参数模型在多个社会推理任务中表现优于更大规模模型，具备强泛化能力，验证了挑战性训练与过程对齐的有效性。

Conclusion: 通过使用具有挑战性的训练案例并进行轨迹级推理对齐，可有效促进大模型社会智能的高效与可靠发展。
...
### [160] [Logos: An evolvable reasoning engine for rational molecular design](https://arxiv.org/abs/2603.09268)
*Haibin Wen,Zhe Zhao,Fanfu Wang,Tianyi Xu,Hao Zhang,Chao Yang,Ye Wei*

Main category: cs.AI

TL;DR: Logos 是一个紧凑的分子推理模型，结合多步逻辑推理与严格的化学一致性，通过分阶段训练策略，在保证化学有效性的同时实现高结构准确性。该模型在多个基准数据集上表现优异，优于或媲美更大规模的通用语言模型，且参数量显著更少。它还能在涉及多重冲突约束的分子优化任务中保持稳定，并通过显式中间推理步骤支持人类可解释性评估，为可靠、可解释的人工智能在分子科学中的应用提供了可行路径。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型在分子设计中往往在物理保真度与透明推理之间存在权衡：要么具备高物理准确性但缺乏可解释性，要么具备灵活推理但无法保证化学有效性。这种不平衡限制了AI在真实科研流程中的可靠性，亟需一种兼具推理能力与化学合理性的模型。

Method: Logos采用分阶段训练策略：首先用显式推理示例训练模型从分子描述到结构决策的逻辑映射；随后逐步对齐推理模式与分子表示；最后将化学规则和不变量直接纳入优化目标，引导生成符合化学规律的结构。

Result: Logos在多个基准数据集上实现了优异的结构准确性和化学有效性，性能媲美甚至超越大型通用语言模型，同时仅使用极少参数。在复杂多约束优化任务中表现出稳定行为，并支持人类对中间推理过程的审查与评估。

Conclusion: 联合优化推理结构与物理一致性是构建可靠、可解释人工智能系统的关键路径，有助于推动人工智能更深入地融入分子科学的发现流程。
...
### [161] [Rescaling Confidence: What Scale Design Reveals About LLM Metacognition](https://arxiv.org/abs/2603.09309)
*Yuyang Dai*

Main category: cs.AI

TL;DR: 该研究探讨了大语言模型（LLM）在黑箱设置中使用数值化置信度报告不确定性时，置信度量表设计的影响。研究发现，当前常用的0-100量表存在严重离散化问题，超过78%的置信度集中在三个整数上。通过系统调整量表的粒度、边界位置和范围规则，发现0-20量表在元认知敏感性（meta-d'）方面表现更优，而边界压缩会降低性能，且整数偏好在不规则量表下依然存在。因此，置信度量表设计并非中立，应作为评估中的首要变量。


<details>
  <summary>Details</summary>
Motivation: 现有研究广泛使用0-100的置信度量表来估计大语言模型的不确定性，但该量表的设计本身未被充分考察，可能影响结果的准确性。研究旨在揭示量表设计对置信度报告质量的影响，推动其作为可优化的实验变量。

Method: 系统地操纵置信度量表的三个维度：粒度（如0-20 vs 0-100）、边界放置（如是否包含0/100）、范围规则性（如等距或不规则）。在六种大语言模型和三个数据集上评估元认知敏感性（meta-d'），以衡量置信度与实际正确率之间的匹配程度。

Result: 0-20量表在所有条件下均显著提升元认知效率；边界压缩导致性能下降；即使在不规则量表下，模型仍倾向于选择整数作为置信度值。

Conclusion: 置信度量表的设计直接影响大语言模型的不确定性表达质量，不应被视为固定不变的参数。研究建议在评估中将量表设计作为第一类实验变量进行优化。
...
### [162] [Curveball Steering: The Right Direction To Steer Isn't Always Linear](https://arxiv.org/abs/2603.09313)
*Shivam Raval,Hae Jin Song,Linlin Wu,Abir Harrasse,Jeff Phillips,Amirali Abdullah*

Main category: cs.AI

TL;DR: Curveball steering 是一种基于多项式核主成分分析（PCA）的非线性干预方法，旨在克服传统线性干预在大语言模型（LLM）行为控制中的不一致性问题。该方法考虑了激活空间的内在几何结构，通过测量测地距离与欧氏距离的比值发现激活空间存在显著且依赖概念的几何畸变，表明其不适合全局线性假设。实验表明，Curveball 在几何畸变强烈的场景下显著优于线性方法，证明了几何感知的非线性干预是一种更合理的替代方案。


<details>
  <summary>Details</summary>
Motivation: 现有激活控制方法依赖于线性表示假设，认为行为属性可通过全局线性方向进行操纵。然而实践中线性干预表现不稳定，因此本文质疑这一假设，旨在探索激活空间的真实几何特性以改进干预方法。

Method: 提出 Curveball steering，基于多项式核 PCA 在特征空间中执行非线性干预，以更好地尊重模型激活空间的内在几何结构。通过分析测地距离与欧氏距离的比率来量化几何畸变，并据此设计更符合实际几何特性的干预策略。

Result: Curveball steering 在多种任务和几何畸变程度较高的情况下均显著优于传统的线性 PCA 干扰方法，尤其在高畸变场景中表现突出，验证了非线性、几何感知干预的有效性。

Conclusion: 激活空间具有复杂的非线性几何结构，不能简单用全局线性假设近似。基于几何感知的非线性干预（如 Curveball steering）为大语言模型的行为控制提供了更可靠、一致的解决方案，是线性方法的有力替代。
...
TL;DR: 本文提出一种开放、透明且可复现的方法，构建用于评估NLP模型（特别是RAG系统）是否符合欧盟人工智能法案的资源。通过结合领域知识与大语言模型的处理和推理能力，生成包含风险等级分类、条款检索、义务生成和问答任务的数据集。该数据集以机器可读格式提供，并有效解决了法规中未明确界定的风险边界问题，如有限和最低风险情形。实验表明，基于RAG的解决方案在禁止类和高风险场景中分别达到0.87和0.85的F1分数，验证了数据集的有效性。


<details>
  <summary>Details</summary>
Motivation: 随着AI在公共和社会领域的快速部署，合规性评估需求增加，但现有方法受限于资源不足，依赖人工评估，易出错且覆盖不全。亟需自动化、半自动化工具来评估AI系统对欧盟人工智能法案等监管框架的符合程度。

Method: 结合领域知识作为解释基础，利用大语言模型的生成与推理能力，创建符合欧盟人工智能法案要求的多任务数据集，涵盖风险等级分类、文章检索、义务生成和问答任务。所有数据以机器可读格式输出，确保可重复性和可扩展性。

Result: 所构建的数据集成功支持了对RAG系统的评估，在禁止类和高风险场景下分别获得0.87和0.85的F1分数，证明其在实际应用中的有效性与实用性。

Conclusion: 本研究提供了一种高效、可复现的自动化评估框架，为NLP模型（尤其是RAG系统）在复杂法规环境下的合规性评估提供了有力支持，推动AI治理向更智能、更精准的方向发展。
...
TL;DR: 该论文研究了大模型合并过程中出现的‘合并崩溃’现象，即某些任务特化模型在合并后性能急剧下降。研究表明，这种崩溃与任务间的表征不兼容性密切相关，而与参数空间冲突关系不大，挑战了现有合并方法的普遍认知。作者基于率失真理论提出了一个维度相关的理论边界，揭示了任务合并性的根本限制。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法在某些任务组合下会出现性能灾难性下降，称为合并崩溃。尽管已有研究关注参数空间冲突，但对根本原因缺乏深入理解，亟需系统分析和理论解释。

Method: 通过大规模实验和统计分析，对比不同任务组合下的合并表现，评估表征兼容性和参数空间冲突指标的相关性，并基于率失真理论构建理论框架，推导出任务合并的维度依赖边界。

Result: 发现表征不兼容性是导致合并崩溃的关键因素，而参数空间冲突相关性较弱；理论分析表明，任务合并存在固有极限，无法通过任何方法完全克服。

Conclusion: 合并崩溃源于任务间表征的根本不兼容，而非参数冲突；任务合并能力受制于理论上的维度依赖边界，未来方法需考虑任务表示的兼容性设计。
...
### [165] [GenePlan: Evolving Better Generalized PDDL Plans using Large Language Models](https://arxiv.org/abs/2603.09481)
*Andrew Murray,Danial Dervovic,Alberto Pozanco,Michael Cashmore*

Main category: cs.AI

TL;DR: GenePlan 是一种利用大语言模型（LLM）辅助的进化算法来生成经典规划任务中领域相关的通用规划器的新框架。它将通用规划问题转化为优化问题，通过迭代演化可解释的 Python 规划器，以最小化在多种问题实例中的计划长度。在六个现有基准领域和两个新领域的实验评估中，GenePlan 的平均 SAT 得分为 0.91，接近最先进规划器的 0.93，并显著优于其他基于 LLM 的基线方法（如 CoT 提示法，平均 SAT 得分为 0.64）。生成的规划器能够快速求解新实例（每任务平均耗时 0.49 秒），且成本较低（每个领域平均使用 GPT-4o 花费 1.82 美元）。


<details>
  <summary>Details</summary>
Motivation: 传统通用规划方法在面对多样化问题实例时表现不佳，而现有的基于大语言模型的方法虽然具备一定潜力，但缺乏系统性优化和可解释性。因此，需要一种结合进化算法与大语言模型优势的新型框架，以生成高效、可解释且泛化能力强的通用规划器。

Method: GenePlan 将通用规划建模为一个优化问题，利用大语言模型生成初始规划代码，并通过进化算法（如遗传算法）迭代优化这些代码，目标是最小化在多个问题实例上的计划长度。每一代中，规划器通过测试不同实例的表现进行评估，优胜者被保留并用于生成下一代。整个过程结合了 LLM 的创造力与进化算法的搜索能力，最终获得性能优异且可解释的 Python 规划器。

Result: 在六个现有基准领域和两个新领域上，GenePlan 实现了平均 SAT 得分 0.91，接近最先进规划器的 0.93，显著优于基于链式思维提示的 LLM 方法（平均得分为 0.64）。此外，生成的规划器在新实例上平均仅需 0.49 秒即可完成求解，且每域平均成本仅为 1.82 美元（使用 GPT-4o），展现出高效率与低成本特性。

Conclusion: GenePlan 成功展示了将大语言模型与进化算法相结合在通用规划任务中的有效性，不仅实现了接近顶尖性能，还具备良好的可解释性、快速推理能力和经济可行性，为未来自动化规划器生成提供了新的方向。
...
### [166] [Vibe-Creation: The Epistemology of Human-AI Emergent Cognition](https://arxiv.org/abs/2603.09486)
*Ilya Levin*

Main category: cs.AI

TL;DR: 本文提出人类推理与生成式人工智能（GenAI）的互动产生了一种新型认知-认识论结构，称为“第三实体”——一种由两种本体上不相容的认知模式通过转导耦合而产生的临时性结构。文章结合皮尔士符号学、波兰尼的默会知识理论、西蒙东的个体化哲学、伊德的后现象学及莫林的复杂性理论，构建多层理论框架，并引入“氛围创造”概念，描述该实体在高维语义空间中导航的前反思认知模式，认为这是默会知识的自动化，对认识论、心智哲学和教育理论具有深远影响。同时提出“非对称涌现”以刻画第三实体的能动性：真正新颖且不可还原，但根植于人类意图责任。最后探讨了该框架对教育机构转型及智能能力重新定义的意义。


<details>
  <summary>Details</summary>
Motivation: 传统工具使用、增强或协作伙伴关系等隐喻无法充分描述人类推理与生成式人工智能之间的互动。随着GenAI的发展，这种互动催生出一种新的认知-认识论形态，亟需一个全新的理论框架来理解其本质和影响。

Method: 综合运用皮尔士符号学、波兰尼默会知识理论、西蒙东个体化哲学、伊德后现象学以及莫林复杂性理论，构建一个多层理论模型；提出‘氛围创造’作为前反思认知模式，用以解释第三实体如何在高维语义空间中运作；引入‘非对称涌现’概念以界定其能动性特征。

Result: 揭示了人类与GenAI互动中形成的‘第三实体’是一种临时、新兴、不可还原的认知结构；证明了‘氛围创造’是默会知识自动化的体现；阐明了第三实体虽具独立性，但始终依赖于人类意图责任；为教育变革和智能能力重构提供了理论基础。

Conclusion: 该理论框架不仅深化了对人机认知关系的理解，还为未来教育制度的转型和智力能力标准的再定义提供了关键洞见，标志着认知范式的根本转变。
...
TL;DR: 该研究提出一种基于大语言模型（LLM）生成个性化假新闻辟谣信息的新方法，通过与五大性格特质（外向性、宜人性、尽责性、神经质、开放性）对齐的个性提示，将通用辟谣内容转化为针对特定人格特征的个性化版本。采用另一独立LLM作为自动化评估器模拟相应人格特质，避免了昂贵的人类评估成本。结果显示个性化消息普遍比通用消息更具说服力，其中开放性提升说服力，而神经质则降低说服力。多模型评估结果差异表明使用多个模型能更全面反映效果。本研究展示了利用LLM实现精准辟谣的可行性，同时也引发关于技术伦理的重要讨论。


<details>
  <summary>Details</summary>
Motivation: 传统假新闻辟谣信息往往采用通用化表达，难以有效触达不同性格特征的受众。为提升辟谣信息的针对性和说服力，需探索个性化策略。同时，人工评估成本高昂且效率低下，亟需高效自动化评估手段。因此，研究旨在开发一种结合人格特质的个性化辟谣生成与自动化评估框架，以提高辟谣传播效能并降低成本。

Method: 首先，基于大语言模型（LLM）构建个性提示模板，将五大人格特质（外向性、宜人性、尽责性、神经质、开放性）融入输入语境；其次，利用这些提示引导LLM生成针对不同人格特征的个性化辟谣文本；随后，设计一个独立的多模型评估系统，每个评估模型模拟一种人格特质，自动判断生成内容的说服力；最后，通过对比不同人格下生成内容的评估得分，分析个性因素对辟谣效果的影响。

Result: 个性化辟谣消息在多数情况下比通用版本更具说服力；开放性特质的个体更容易被说服，而神经质特质者则表现出较低的可说服性；多模型评估结果呈现互补性，表明单一模型可能产生偏差，综合多模型评估能获得更可靠结论。

Conclusion: 本研究证明了基于人格特质的个性化假新闻辟谣信息生成具有显著优势，能够有效提升辟谣信息的传播效果。同时，通过自动化评估机制降低了人力成本，提升了评估效率。然而，该技术潜在的滥用风险也引发了重要伦理问题，如操纵公众认知等，需在未来研究中加强监管与规范。
...
TL;DR: 该论文提出Omni Parsing框架，旨在解决多模态解析中任务定义碎片化和非结构化数据异质性问题。框架建立统一分类体系，涵盖文档、图像和音视频流，采用分层渐进式解析范式，包含整体检测、细粒度识别和多层次解释三个层次，通过证据锚定机制实现高层语义与底层事实的严格对齐，支持基于证据的逻辑推断，将非结构化信号转化为可定位、可枚举、可追溯的标准化知识。构建了标准化数据集并发布Logics-Parsing-Omni模型及OmniParsingBench评估基准，实验表明感知与认知协同提升模型可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态解析面临任务定义分散和非结构化数据异质性强的问题，缺乏统一的解析框架和评估标准，难以实现从感知到认知的高效转化。

Method: 提出Omni Parsing框架，包含三个层级：1）整体检测，实现对象或事件的时空精确定位；2）细粒度识别，完成符号化（如OCR/ASR）与属性提取；3）多层次解释，构建从局部语义到全局逻辑的推理链。引入证据锚定机制，确保高层语义描述与底层事实对齐，实现证据驱动的逻辑归纳。

Result: 成功将复杂音视频信号转化为机器可读的结构化知识；实验证明细粒度感知与高层认知具有协同效应，显著提升模型可靠性；构建了标准化数据集、模型及评估基准OmniParsingBench，代码与资源已开源。

Conclusion: Omni Parsing框架通过统一分类体系与分层解析机制，实现了多模态数据从感知到认知的高效转化，为构建可追溯、可验证的智能系统提供了新范式。
...
TL;DR: 本文提出将多模态大模型（MLLMs）的安全对齐从针对恶意意图或情境违规的范式，转向后果驱动的安全性，以支持自主和具身智能体的稳健部署。为此，作者构建了OOD-MMSafe基准，包含455个精心设计的图文对，用于评估模型在上下文依赖的因果链中识别潜在危险的能力。分析发现，前沿模型普遍存在因果盲区，高容量闭源模型失败率高达67.5%；且静态对齐存在偏好天花板，模型能力提升并未带来安全推理的改善。为此，提出基于后果感知的安全策略优化框架CASPO，通过模型内在推理作为动态参考，实现逐令牌自蒸馏奖励。实验表明，CASPO显著提升了后果预测能力，使Qwen2.5-VL-7B和Qwen3-VL-4B的风险识别失败率分别降至7.3%和5.7%，同时保持整体性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型的安全对齐主要关注恶意意图或情境违规，但不足以应对自主和具身智能体在复杂现实环境中可能引发的后果性风险。因此需要引入后果驱动的安全范式，以更准确地识别隐含于因果链中的潜在危害。

Method: 提出OOD-MMSafe基准，用于评估模型在上下文依赖因果链中识别潜在危险的能力；开发Consequence-Aware Safety Policy Optimization (CASPO)框架，利用模型自身推理过程作为动态参考，生成逐令牌的自蒸馏奖励信号，实现对安全行为的动态优化。

Result: CASPO显著提升了模型的后果预测与风险识别能力，使Qwen2.5-VL-7B和Qwen3-VL-4B的失败率分别降至7.3%和5.7%；同时验证了现有静态对齐方法在模型容量增长时出现的‘偏好天花板’现象。

Conclusion: 本文证明了后果驱动安全对齐对于具身和自主系统的重要性，并通过新基准与动态优化框架实现了显著性能提升，为未来安全型多模态智能体的发展提供了关键路径。
...
### [170] [World2Mind: Cognition Toolkit for Allocentric Spatial Reasoning in Foundation Models](https://arxiv.org/abs/2603.09774)
*Shouwei Ruan,Bin Wang,Zhenyu Wu,Qihui Zhu,Yuxiang Zhang,Hang Su,Yubin Wang*

Main category: cs.AI

TL;DR: 提出World2Mind，一种无需训练的空间智能工具包，通过3D重建和实例分割构建结构化空间认知地图，利用椭圆参数生成全局布局的仿射空间树（AST），并设计三阶段推理链以克服3D重建误差。实验表明其可使GPT-5.2等前沿模型性能提升5%~18%，甚至纯文本模型仅凭AST结构文本即可实现接近多模态模型的复杂3D空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有多模态基础模型在空间推理上受限于对统计捷径的过拟合或局限于2D视觉感知，缺乏在未见场景中的准确性和泛化能力。

Method: 利用3D重建与实例分割构建空间认知地图；设计基于椭圆参数的仿射空间树（AST）提供几何-拓扑先验；采用三阶段推理链（工具调用评估、模态解耦线索收集、几何-语义交织推理）以缓解3D重建误差。

Result: World2Mind显著提升主流模型（如GPT-5.2）的空间推理性能，提升幅度达5%~18%；纯文本模型仅依赖AST结构文本即可实现接近先进多模态模型的3D空间推理表现。

Conclusion: World2Mind通过模拟生物智能的空间认知机制，实现了无需训练的高效空间推理能力，为多模态基础模型提供了强大的空间理解支持，并揭示了结构化空间知识在纯文本模型中亦可激发强大空间推理潜力。
...
### [171] [Influencing LLM Multi-Agent Dialogue via Policy-Parameterized Prompts](https://arxiv.org/abs/2603.09890)
*Hongbo Bo,Jingyu Hu,Weiru Liu*

Main category: cs.AI

TL;DR: 本文提出了一种基于提示参数化的轻量级策略框架，将提示视为语言模型执行的动作，通过五个组件动态构建提示以影响对话行为，无需训练。实验在两个公共议题讨论场景中验证了该方法的有效性，结果显示提示参数化能有效调控对话流程，为多智能体系统的社会模拟研究提供了简单有效的机制。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的多智能体系统研究依赖于随意提示，缺乏系统性的策略视角；本文旨在探索提示作为动作是否可参数化，以构建无需训练的轻量级策略，从而更可控地影响对话行为。

Method: 将提示视为智能体的动作，基于当前状态通过五个组件动态生成提示，形成由状态-动作对组成的参数化策略，实现对对话行为的非训练式控制。

Result: 在两个公共议题讨论场景中，使用不同大语言模型驱动的智能体进行实验，结果表明提示参数化能有效影响对话的响应性、反驳、证据使用、避免重复和立场转变等维度，显著调控对话动态。

Conclusion: 提示参数化提供了一种无需训练的简单而有效的机制，可用于调控多智能体对话过程，推动多智能体系统向社会模拟方向发展。
...
### [172] [MedMASLab: A Unified Orchestration Framework for Benchmarking Multimodal Medical Multi-Agent Systems](https://arxiv.org/abs/2603.09909)
*Yunhang Qian,Xiaobin Hu,Jiaquan Yu,Siyang Xin,Xiaokun Chen,Jiangning Zhang,Peng-Tao Jiang,Jiawei Liu,Hongwei Bran Li*

Main category: cs.AI

TL;DR: MedMASLab 是一个统一的多模态医疗多智能体系统框架与基准平台，旨在解决当前医疗多智能体系统在架构碎片化、数据集成不统一和跨专科评估缺失等问题。该平台引入了标准化的多模态通信协议，支持11种异构架构与24种医学模态的无缝整合；提出基于大视觉-语言模型的零样本语义评估方法，实现对诊断逻辑与视觉定位的精准验证；构建了涵盖11个器官系统、473种疾病、覆盖11个临床基准的最广泛基准集。实验揭示当前多智能体系统在跨专业领域存在显著性能脆弱性，尽管推理深度提升，但泛化能力受限。研究还系统分析了交互机制与成本-性能权衡，为未来自主临床系统提供了新的技术基准。代码与数据已公开。


<details>
  <summary>Details</summary>
Motivation: 当前医疗多智能体系统面临架构碎片化、数据接入不统一、视觉-推理评估不一致及缺乏跨专科基准等挑战，限制了其在复杂临床决策支持中的应用与发展。

Method: 提出MedMASLab框架，包含：(1) 标准化多模态智能体通信协议，支持11种异构架构与24种医学模态的集成；(2) 基于大视觉-语言模型的零样本语义评估器，用于验证诊断逻辑与视觉锚定；(3) 构建大规模跨学科基准数据集，整合11个临床基准，覆盖473种疾病与11个器官系统。

Result: 系统评估发现，尽管多智能体系统提升了推理深度，但在跨专业子领域间表现出显著性能下降，显示出领域特异性脆弱性。通过消融实验揭示了交互机制的影响与成本-性能权衡关系，确立了新的技术基线。

Conclusion: MedMASLab 为多模态医疗多智能体系统提供了统一的开发与评估平台，解决了现有系统的整合难题，并为未来自主临床系统的发展奠定了坚实基础。
...
### [173] [PathMem: Toward Cognition-Aligned Memory Transformation for Pathology MLLMs](https://arxiv.org/abs/2603.09943)
*Jinyue Li,Yuci Liang,Qiankun Li,Xinheng Lyu,Jiayu Qian,Huabao Chen,Kun Wang,Zhigang Zeng,Anil Anthony Bharath,Yang Liu*

Main category: cs.AI

TL;DR: 提出PathMem，一种基于记忆的多模态框架，用于病理学多模态大模型（MLLMs），通过长时记忆（LTM）和工作记忆（WM）的动态转换，实现结构化病理知识的上下文感知整合与可解释的记忆控制，显著提升病理图像报告生成与诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大模型在病理诊断中缺乏对结构化领域知识（如分类、分级标准、临床证据）的显式整合机制，且难以实现可解释的记忆控制，导致无法稳定结合病理特定诊断标准进行推理。

Method: 设计内存中心的多模态框架PathMem，将结构化病理知识组织为长时记忆（LTM），引入记忆变换器（Memory Transformer），通过多模态记忆激活与上下文感知的知识定位，实现从LTM到工作记忆（WM）的动态转移，并支持下游推理中的上下文感知记忆精炼。

Result: 在多个基准测试中达到最先进水平，显著提升全视野图像（WSI）报告生成的精确率（+12.8% WSI-Precision）、相关性（+10.1% WSI-Relevance），以及开放式诊断性能（分别提升9.7%和8.9%）。

Conclusion: PathMem通过模拟人类病理学家的层级记忆过程，有效解决了多模态大模型在病理学应用中知识整合与可解释性不足的问题，为智能病理诊断提供了可扩展、可解释的推理框架。
...
### [174] [The Confidence Gate Theorem: When Should Ranked Decision Systems Abstain?](https://arxiv.org/abs/2603.09947)
*Ronald Doku*

Main category: cs.AI

TL;DR: 本文研究了在排序决策系统（如推荐系统、广告拍卖、临床分诊）中，基于置信度的放弃策略何时能单调提升决策质量，何时会失效。关键条件为排名对齐（rank-alignment）和无反转区（no inversion zones）。研究发现，结构性不确定性（如冷启动）导致近似单调的改进，而情境性不确定性（如时间漂移）则破坏单调性。通过三个领域验证：协同过滤（MovieLens）、电商意图检测（RetailRocket, Criteo, Yoochoose）、临床路径分诊（MIMIC-IV）。结构化置信信号（如观测次数）在情境漂移下表现不佳，与随机放弃相当；而上下文感知信号（如集成分歧、近期特征）虽有改善但未能完全恢复单调性。基于残差定义的异常标签在分布偏移下性能显著下降（AUC从0.71降至0.61–0.62），否定异常驱动干预的常见做法。结论提出实用部署诊断：部署前在保留数据上检验C1和C2条件，并匹配置信信号与主导不确定性类型。


<details>
  <summary>Details</summary>
Motivation: 在排序决策系统中，何时干预、何时放弃是一个核心问题。现有方法依赖置信度进行放弃，但其有效性受不确定性类型影响。本文旨在揭示置信度放弃策略何时有效、何时失效，尤其区分结构性与情境性不确定性的影响。

Method: 理论分析结合实证验证。通过形式化条件（排名对齐、无反转区）分析置信度放弃的单调性；在三个真实数据集上测试不同置信信号在分布变化下的表现，包括冷启动、时间漂移等场景。使用AUC评估异常标签稳定性，对比不同置信信号的性能差异。

Result: 结构性不确定性下置信度放弃近似单调提升性能；情境性不确定性下，传统结构化置信信号失效，产生大量单调性违反（如MovieLens中与随机放弃相当）；上下文感知信号（如集成分歧、近期特征）可减少违规（从3降至1–2），但无法完全恢复单调性；基于残差的异常标签在分布偏移下性能大幅下降（AUC从0.71降至0.61–0.62）。

Conclusion: 置信度放弃的有效性取决于不确定性类型。应区分结构性与情境性不确定性，在部署前检查C1/C2条件，并选择与主导不确定性类型匹配的置信信号。否则，可能引入反效果。
...
### [175] [Think Before You Lie: How Reasoning Improves Honesty](https://arxiv.org/abs/2603.09957)
...
TL;DR: 该研究通过一个包含真实道德权衡的新数据集，探讨了大语言模型（LLM）产生欺骗行为的深层原因。与人类在深思熟虑后诚实度下降不同，研究发现推理过程反而普遍提升了模型的诚实性。这种效果并非源于推理内容本身，而是由于表示空间的几何特性：欺骗区域在空间中具有非稳定性，容易因输入改写、输出重采样和激活噪声而被打破；而诚实回答则更稳定。因此，推理过程可被视为在有偏表示空间中移动，最终将模型推向更稳定的诚实默认状态。


<details>
  <summary>Details</summary>
Motivation: 现有对大语言模型的评估主要关注欺骗率，但对导致欺骗行为的根本条件缺乏理解。本研究旨在揭示影响模型诚实性的内在机制，特别是在复杂道德情境下的行为变化。

Method: 构建了一个包含现实道德权衡的新型数据集，考察不同规模和家族的大语言模型在面对成本变化时的诚实表现。通过分析推理过程中的表示空间动态，包括输入改写、输出重采样和激活噪声的影响，探究欺骗与诚实行为在表示空间中的稳定性差异。

Result: 发现推理显著提升模型的诚实性，且这一效应不依赖于推理内容，而是由表示空间的几何结构决定。欺骗区域在空间中为非稳定状态，易受扰动影响；而诚实区域更为稳定。推理过程促使模型从不稳定区域向更稳定的诚实区域迁移。

Conclusion: 大语言模型的诚实性受到其表示空间几何结构的深刻影响。推理过程虽不直接生成正确答案，却通过引导模型穿越有偏的空间路径，使其趋向于更稳定的诚实默认状态。这表明，模型的道德行为可能更多地由其内在表示结构而非显式推理内容所决定。
...
### [176] [Equitable Multi-Task Learning for AI-RANs](https://arxiv.org/abs/2603.08717)
*Panayiotis Raptis,Fatih Aslan,George Iosifidis*

Main category: cs.LG

TL;DR: 本文提出一种在线-内在线公平多任务学习（OWO-FMTL）框架，用于在边缘计算环境中实现异构用户间长期公平的推理性能。该框架通过外层循环更新共享模型，内层循环采用轻量级原始对偶更新动态调整用户优先级，结合广义alpha公平性度量，在效率与公平性之间实现平衡。理论与实验均表明，该方法能有效降低性能差异，且计算开销低，适用于边缘部署，在动态场景下优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: AI-RAN需要在共享边缘资源下服务具有时变学习任务的异构用户，但现有方法难以保证用户间长期公平的推理性能，因此亟需一种自适应且公平的学习机制。

Method: 提出OWO-FMTL框架，包含外层循环（全局模型更新）和内层循环（用户优先级动态重平衡），采用轻量级原始对偶优化，结合广义alpha公平性目标，实现长期公平性保障。

Result: 实验验证了该方法在凸学习与深度学习任务中均优于现有基线，在动态环境下显著降低用户间性能差距，同时保持低计算开销，适合边缘部署。

Conclusion: OWO-FMTL框架能够有效实现多任务学习中的长期公平性，兼顾系统效率与用户公平，具备良好的可扩展性和实用性，适用于AI-enabled Radio Access Networks等边缘智能场景。
...
### [177] [Generalized Reduction to the Isotropy for Flexible Equivariant Neural Fields](https://arxiv.org/abs/2603.08758)
*Alejandro García-Castellanos,Gijs Bellaard,Remco Duits,Daniel Pelt,Erik J Bekkers*

Main category: cs.LG

TL;DR: 该论文提出了一种在异质乘积空间上构建群不变函数的新方法，通过将群作用于乘积空间的问题转化为其子群在单一空间上的不变性问题，实现了表达能力的保留与简化。


<details>
  <summary>Details</summary>
Motivation: 许多几何学习问题需要在异质乘积空间（即不同空间的乘积，各自带有不同的群作用）中定义不变量，而传统方法无法直接应用，因此亟需一种通用且可扩展的框架来处理此类问题。

Method: 当群 $G$ 在空间 $M$ 上传递作用时，证明了 $G$-不变函数在 $X \times M$ 上可以被约化为 $H$-不变函数在 $X$ 上的函数，其中 $H$ 是 $M$ 的稳定子群；这一过程建立了轨道等价 $(X \times M)/G \cong X/H$，从而实现结构上的简化与表达保持。

Result: 该方法成功将等变神经场（Equivariant Neural Fields）推广至任意群作用和齐性条件空间，消除了现有方法中的主要结构性限制，提升了模型的灵活性与适用性。

Conclusion: 本工作提供了一个统一且原则性的框架，用于处理异质乘积空间上的群不变性问题，并在等变神经场中展示了其有效性，为更广泛的几何学习任务提供了理论支持与实践工具。
...
### [178] [SPREAD: Subspace Representation Distillation for Lifelong Imitation Learning](https://arxiv.org/abs/2603.08763)
*Kaushik Roy,Giovanni D'urso,Nicholas Lawrance,Brendan Tidd,Peyman Moghadam*

Main category: cs.LG

TL;DR: SPREAD 是一种几何保持的框架，通过奇异值分解（SVD）在低秩子空间中对齐策略表示，以保留任务表示的内在流形结构。结合置信度引导的蒸馏策略，仅对最可信的动作样本应用KL散度损失，提升优化稳定性与泛化能力。在LIBERO基准测试中显著改善知识迁移，缓解灾难性遗忘，达到当前最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法依赖原始特征空间中的L2范数匹配，对噪声和高维变化敏感，难以保持任务表示的内在流形结构，导致知识遗忘和性能下降。

Method: 提出SPREAD框架，利用SVD在低秩子空间中对齐不同任务的策略表示，并设计置信度引导的蒸馏策略，通过限制KL散度损失于前M个最可信动作样本，增强可靠性与优化稳定性。

Result: 在LIBERO基准上，SPREAD显著提升了知识迁移能力，有效缓解了灾难性遗忘，并实现了最先进的性能表现。

Conclusion: SPREAD通过几何保持的表示对齐与置信度引导的蒸馏机制，为终身模仿学习提供了稳定、鲁棒且可泛化的解决方案。
...
### [179] [SoftJAX & SoftTorch: Empowering Automatic Differentiation Libraries with Informative Gradients](https://arxiv.org/abs/2603.08824)
*Anselm Paulus,A. René Geist,Vít Musil,Sebastian Hoffmann,Onur Beker,Georg Martius*

Main category: cs.LG

TL;DR: 本文介绍了 SoftJAX 和 SoftTorch，两个开源、功能完整的软可微编程库，旨在解决 JAX 和 PyTorch 中硬性操作（如阈值、布尔逻辑、离散索引和排序）导致梯度为零或未定义的问题。通过提供一系列软化函数作为硬函数的即插即用替代品，涵盖元素级操作、布尔与索引的模糊逻辑处理、轴向操作（基于最优传输或排列体投影）以及直通梯度估计，显著提升了可微编程的实用性。


<details>
  <summary>Details</summary>
Motivation: 现有自动微分框架中的许多基本操作（如阈值、排序、布尔逻辑）缺乏有效梯度，阻碍了复杂模型的优化。虽然已有多种软化方法提出，但实现分散且难以整合比较。因此亟需一个统一、完整且易用的软可微编程工具集。

Method: 设计并实现 SoftJAX 与 SoftTorch 两个库，提供对 JAX 与 PyTorch 的完整兼容接口，包含：(i) 元素级软化操作（如 clip、abs）；(ii) 基于模糊逻辑的布尔与索引操作；(iii) 基于最优传输或排列体投影的轴向操作（如 sort、rank）；(iv) 支持直通梯度估计。所有功能均以模块化方式集成，便于组合与使用。

Result: 实验表明，SoftJAX 与 SoftTorch 能有效为硬性操作提供有意义的梯度，支持复杂可微模型的训练。在多个基准测试和实际案例研究中验证了其有效性与易用性，代码已开源，便于社区使用与扩展。

Conclusion: SoftJAX 与 SoftTorch 成功构建了一个统一、完整且易用的软可微编程工具箱，解决了现有框架中硬性操作梯度缺失的问题，推动了可微编程在科学计算与机器学习中的广泛应用。
...
### [180] [Expressivity-Efficiency Tradeoffs for Hybrid Sequence Models](https://arxiv.org/abs/2603.08859)
*John Cooper,Ilias Diakonikolas,Mingchen Ma,Frederic Sala*

Main category: cs.LG

TL;DR: 该论文研究了混合序列模型（结合Transformer和状态空间模型层）在特定任务上的优势，证明了非混合模型在某些核心合成任务上存在根本性局限，而混合模型则能以小规模参数和工作内存高效解决这些任务。实验验证了理论结果，并表明学习得到的混合模型在性能、长度泛化和分布外鲁棒性方面优于非混合模型。


<details>
  <summary>Details</summary>
Motivation: 当前对混合序列模型的研究虽多，但缺乏对其在何种情况下以及如何优于单一模型的基本理解。本文旨在填补这一空白，通过分析典型合成任务揭示混合模型的优势机制。

Method: 通过理论分析构建并证明了在特定任务中非混合模型（Transformer或状态空间模型）需大量参数或大工作内存才能完成任务；同时构造了小型混合模型来解决选择性复制和关联回忆等任务，并通过实验验证其有效性与优越性。

Result: 理论上证明了非混合模型存在根本性限制，而混合模型可在小规模下有效解决问题；实验表明学习型混合模型比参数量多6倍的非混合模型表现更好，且具有更强的长度泛化能力和分布外鲁棒性。

Conclusion: 混合序列模型结合了Transformer的表达能力与状态空间模型的计算效率，在特定任务中展现出显著优势，尤其在小规模、强泛化和鲁棒性方面优于非混合模型，为未来模型设计提供了理论支持与实践指导。
...
### [181] [A New Modeling to Feature Selection Based on the Fuzzy Rough Set Theory in Normal and Optimistic States on Hybrid Information Systems](https://arxiv.org/abs/2603.08900)
*Mohammad Hossein Safarpour,Seyed Mohammad Alavi,Mohammad Izadikhah,Hossein Dibachi*

Main category: cs.LG

TL;DR: 该论文提出了一种名为FSbuHD的新特征选择模型，用于解决高维大数据中模糊粗糙集理论面临的计算效率低和噪声问题。通过计算对象间的组合距离来构建模糊等价关系，并将特征选择问题转化为优化问题，利用元启发式算法求解。该模型具有正常和乐观两种模式，基于不同的模糊等价关系选择。在UCI标准数据集上的实验表明，FSbuHD在效率和效果上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统模糊粗糙集理论在高维空间中通过交运算获取模糊等价关系时存在计算耗时、内存消耗大及产生噪声的问题，影响特征选择的准确性与效率，因此亟需一种更高效、鲁棒的替代方法。

Method: 提出一种基于组合距离计算的新型模糊等价关系构造方法，将特征选择问题转化为优化问题，采用元启发式算法求解；设计了正常与乐观两种运行模式，以适应不同场景需求。

Result: 在多个UCI基准数据集上的实验结果表明，FSbuHD在特征选择精度和计算效率方面均显著优于现有主流算法，具备良好的稳定性和泛化能力。

Conclusion: FSbuHD是一种高效且有效的特征选择方法，能够有效克服传统模糊粗糙集在高维数据中的局限性，为大数据环境下的特征选择提供了新的解决方案。
...
### [182] [Quantifying Memorization and Privacy Risks in Genomic Language Models](https://arxiv.org/abs/2603.08913)
*Alexander Nemecek,Wenbiao Li,Xiaoqian Jiang,Jaideep Vaidya,Erman Ayday*

Main category: cs.LG

TL;DR: 本文提出了一种多向量隐私评估框架，用于量化基因组语言模型（GLMs）中的记忆风险。通过结合困惑度检测、蜜罐序列提取和成员推断三种方法，构建了一个统一的评估流程，并在合成与真实基因组数据中植入蜜罐序列以控制实验条件。研究发现不同架构和训练策略下的GLMs均存在可测量的记忆现象，且风险程度各异，表明单一攻击方式无法全面反映记忆风险，强调了多向量隐私审计在基因组AI系统中的必要性。


<details>
  <summary>Details</summary>
Motivation: 随着基因组语言模型在敏感基因组队列上训练或微调，存在记忆训练数据中特定序列的风险，可能引发隐私泄露、数据泄漏及合规问题。尽管通用语言模型的记忆风险已受关注，但针对基因组数据特性的系统性评估仍不足，尤其因基因组数据具有固定碱基字母表、强生物学结构及个体可识别性等特点。因此，亟需建立专门的隐私评估框架来量化并管理此类风险。

Method: 提出一个综合性的多向量隐私评估框架，整合了三种互补的方法：基于困惑度的检测、蜜罐序列提取和成员推断；通过在合成与真实基因组数据中以不同重复率植入蜜罐序列，实现对重复频率与训练动态如何影响记忆效应的精确量化；构建统一评估流水线，输出最坏情况下的记忆风险评分。

Result: 实验证明基因组语言模型确实存在可测量的记忆行为，记忆程度随模型架构和训练策略而异；单一攻击手段无法全面揭示记忆风险，强调必须采用多向量隐私审计作为基因组人工智能系统的标准实践。

Conclusion: 基因组语言模型存在显著的记忆风险，且该风险受模型容量与训练过程影响。为确保隐私安全，应将多向量隐私审计纳入基因组人工智能开发的标准流程，以应对复杂的数据敏感性挑战。
...
### [183] [The $qs$ Inequality: Quantifying the Double Penalty of Mixture-of-Experts at Inference](https://arxiv.org/abs/2603.08960)
*Vignesh Adhinarayanan,Nuwan Jayasena*

Main category: cs.LG

TL;DR: 本文揭示了Mixture-of-Experts（MoE）模型在推理阶段效率下降的根本原因，提出“qs不等式”作为判断MoE相对于稠密模型是否在结构上处于劣势的预测准则。该准则结合稀疏性（s）与质量等效因子（q），指出MoE在长上下文场景下因专家路由导致权重重用碎片化，进而陷入带宽瓶颈。实验证明，在长序列（如128k）下，质量相当的稠密模型可实现高达4.5倍的吞吐优势，且大规模MoE模型可能在实际集群中不可行。研究建议：训练时的FLOP效率不能代表推理性能，MoE更适合作为训练优化手段，未来应通过蒸馏转化为稠密模型以实现高效推理。


<details>
  <summary>Details</summary>
Motivation: MoE模型虽在训练时具有低计算量的优势，但其推理效率常大幅下降，尤其在长上下文任务中表现不佳。现有研究未系统解释这一现象的根本原因，也缺乏对推理效率的可靠评估标准。因此，亟需理解MoE在推理阶段的结构性瓶颈，并建立可指导部署策略的理论框架。

Method: 提出“qs不等式”作为分析工具，量化稀疏性（s）与质量等效因子（q）之间的关系；通过形式化分析揭示专家路由带来的权重重用碎片化问题；在DeepSeek-V3、Qwen3-235B、Grok-1和Switch-C等前沿模型上进行多场景验证，对比MoE与质量匹配的稠密模型在长上下文下的推理性能。

Result: 在128k上下文长度下，质量匹配的稠密模型相比MoE模型实现最高达4.5倍的吞吐提升；大规模MoE模型（如Switch-C）在某些集群规模下变得不可行，而对应稠密模型仍可运行；证实了‘reuse fragmentation’是普遍存在的架构缺陷。

Conclusion: MoE模型的训练效率优势不能直接转化为推理效率优势，尤其在长上下文场景中。其结构性缺陷使其在推理阶段面临严重性能瓶颈。因此，将MoE通过蒸馏转换为稠密模型，可能是实现高效推理部署的有效路径。
...
### [184] [Semantic Level of Detail: Multi-Scale Knowledge Representation via Heat Kernel Diffusion on Hyperbolic Manifolds](https://arxiv.org/abs/2603.08965)
*Edward Izgorodin*

Main category: cs.LG

TL;DR: 提出Semantic Level of Detail (SLoD)框架，利用Poincaré球上的热核扩散定义连续缩放算子，实现知识图谱中抽象层次的自动解析。通过图拉普拉斯谱隙检测自然尺度边界，无需手动设定分辨率参数。在合成数据和真实世界WordNet数据上均表现出优异性能，能有效识别有意义的抽象层级。


<details>
  <summary>Details</summary>
Motivation: 现有AI记忆系统虽将知识组织为图结构，但缺乏对抽象层次连续控制的原理性机制，无法确定抽象层级间的质变边界或智能体如何导航这些边界。

Method: 基于Poincaré球上的热核扩散构建连续缩放算子；利用图拉普拉斯谱隙检测自然出现的尺度边界，实现无监督的抽象层次发现。

Result: 在合成层次模型（HSBM）上边界检测准确率高达ARI=1.00；在82K节点的WordNet数据上与真实分类深度相关性τ=0.79，证明该方法能自动发现真实世界知识图谱中的有意义抽象层级。

Conclusion: SLoD框架通过谱隙驱动的尺度边界检测，实现了知识图谱中抽象层次的连续、自适应控制，为AI记忆系统提供了可解释且高效的多粒度推理机制。
...
### [185] [MAPLE: Elevating Medical Reasoning from Statistical Consensus to Process-Led Alignment](https://arxiv.org/abs/2603.08987)
*Kailong Fan,Anqi Pu,Yichen Wu,Wanhua Li,Yicong Li,Hanspeter Pfister,Huafeng Liu,Xiang Li,Quanzheng Li,Ning Guo*

Main category: cs.LG

TL;DR: 本文提出一种新型统一训练范式，将医学流程奖励模型（Med-RPM）与测试时强化学习（TTRL）结合，用细粒度、专家对齐的监督替代传统多数投票（MV），以确保强化学习基于医学正确性而非共识。在四个基准上的实验表明，该方法显著优于现有TTRL和独立PRM选择方法，验证了从随机启发式转向结构化步骤奖励对构建可靠可扩展医疗AI系统的重要性。


<details>
  <summary>Details</summary>
Motivation: 标准TTRL依赖多数投票作为启发式监督信号，在复杂医疗场景中不可靠，因为最频繁的推理路径未必是临床正确的。需要更可靠的监督机制来提升医学推理的准确性。

Method: 提出将医学流程奖励模型（Med-RPM）融入TTRL框架，以细粒度、专家对齐的监督取代传统的多数投票，实现基于医学正确性的强化学习，从而将搜索智能提炼到模型参数记忆中。

Result: 在四个不同基准上的实证评估显示，所提方法在性能上持续且显著优于现有TTRL和单独使用PRM的方法，证明了结构化奖励机制的有效性。

Conclusion: 从随机启发式转向结构化、分步奖励是构建可靠且可扩展医疗人工智能系统的关键。
...
### [186] [The Coupling Within: Flow Matching via Distilled Normalizing Flows](https://arxiv.org/abs/2603.09014)
...
TL;DR: 该论文提出一种名为标准化流匹配（NFM）的新方法，通过从预训练的自回归归一化流（AR-NF）模型中提取并蒸馏其近乎确定性的耦合关系，来指导学生流模型的训练。相比传统的独立耦合或基于最优传输（OT）的自适应耦合，NFM在训练和推理性能上均显著提升，且优于教师模型本身。


<details>
  <summary>Details</summary>
Motivation: 现有流模型训练依赖于固定的耦合方式（如独立耦合），尽管自适应耦合（如基于最优传输）能提升性能，但计算复杂且难以扩展。本文旨在通过利用预训练归一化流模型所具备的双射性质，实现更高效、更优的耦合信息传递，从而突破当前流模型训练的瓶颈。

Method: 提出一种基于知识蒸馏的框架，将预训练的自回归归一化流（AR-NF）模型中的耦合结构作为‘教师’，将其生成的噪声-数据对之间的确定性映射关系，用于指导学生流模型的训练。该方法不直接计算耦合，而是通过蒸馏获得高质量的耦合信号，使学生模型兼具高灵活性与强生成能力。

Result: 实验表明，使用NFM训练的学生流模型在多个图像生成基准上超越了采用独立耦合或最优传输耦合的流模型，并且在生成质量、采样效率和训练稳定性方面均优于原始教师模型（AR-NF）。

Conclusion: NFM通过引入来自预训练归一化流的蒸馏耦合机制，实现了流模型训练范式的革新。该方法不仅提升了生成性能，还保留了流模型在推理时灵活调整积分步数的优势，为大规模生成模型的设计提供了新思路。
...
### [187] [When to Retrain after Drift: A Data-Only Test of Post-Drift Data Size Sufficiency](https://arxiv.org/abs/2603.09024)
*Ren Fujiwara,Yasuko Matsubara,Yasushi Sakurai*

Main category: cs.LG

TL;DR: CALIPER是一种数据驱动的、检测器和模型无关的测试方法，用于估计在概念漂移后重新训练所需的最小数据量。它利用动态系统生成的数据流中的状态依赖性，通过单次遍历加权局部回归，并跟踪一步预测误差随局部参数θ的变化。当有效样本量门限满足时，若误差随θ增加呈现单调不增趋势，则表明数据量已足够用于稳定重训练。该方法具有低时间与内存开销，在多个领域、学习器类型和检测器上均表现优异，优于固定数据量策略甚至增量更新。


<details>
  <summary>Details</summary>
Motivation: 解决突发概念漂移导致预测模型失效的问题，特别是如何确定何时重新训练以及需要多少后漂移数据才能保证重训练稳定性，现有研究对此关注不足。

Method: 基于状态依赖性的数据流分析，采用单次遍历的加权局部回归，通过监控一步预测误差随局部参数θ的变化趋势来判断数据是否充足；引入有效样本量门限以触发评估条件。

Result: 在四个异构领域、三种学习器族和两种检测器下，CALIPER始终匹配或超越最佳固定数据量策略，且计算开销极小，常优于增量更新，显著缩小了漂移检测与数据充分适应之间的差距。

Conclusion: CALIPER为流式学习中概念漂移后的数据充足性判断提供了高效、通用且可靠的解决方案，实现了从漂移检测到稳健再训练的闭环。
...
### [188] [Two Teachers Better Than One: Hardware-Physics Co-Guided Distributed Scientific Machine Learning](https://arxiv.org/abs/2603.09032)
*Yuchen Yuan,Junhuan Yang,Hao Wan,Yipei Liu,Hanhan Wu,Youzuo Lin,Lei Yang*

Main category: cs.LG

TL;DR: EPIC 是一种硬件与物理原理协同指导的分布式科学机器学习框架，用于解决广域传感、实时性及能源和可靠性约束下的中心化模型部署难题。通过在终端设备进行轻量级编码、在中心节点进行物理感知解码，并利用跨注意力机制捕捉波场耦合关系，EPIC 以压缩的潜在特征代替原始数据传输，显著降低通信开销，同时保持物理一致性。在包含5个终端和1个中心节点的分布式测试平台上，基于OpenFWI的10个数据集评估表明，EPIC将延迟降低8.9倍、通信能耗降低33.8倍，并在8个数据集上提升重建保真度。


<details>
  <summary>Details</summary>
Motivation: 集中式科学机器学习在广域传感、实时处理和严格能效与可靠性要求下面临通信延迟高、能耗大等问题；而直接分布通用机器学习模型会破坏物理规律，导致性能下降。因此亟需一种兼顾通信效率与物理一致性的分布式框架。

Method: 提出EPIC框架，结合硬件特性与物理规律，在终端设备执行轻量级编码，在中心节点进行物理感知解码；采用跨注意力机制建模多接收器间波场耦合关系，仅传输紧凑的潜在特征而非原始数据。

Result: 在真实分布式测试环境中，相比传统方法，EPIC实现8.9倍的延迟降低和33.8倍的通信能耗减少；且在10个数据集中的8个上提升了重建保真度，证明其高效性与物理保真能力。

Conclusion: EPIC通过硬件-物理协同设计，实现了低通信开销下的高性能科学机器学习，为边缘计算环境中的物理系统建模提供了可扩展、可靠且高效的解决方案。
...
### [189] [Sim2Act: Robust Simulation-to-Decision Learning via Adversarial Calibration and Group-Relative Perturbation](https://arxiv.org/abs/2603.09053)
*Hongyu Cao,Jinghan Zhang,Kunpeng Liu,Dongjie Wang,Feng Xia,Haifeng Chen,Xiaohua Hu,Yanjie Fu*

Main category: cs.LG

TL;DR: Sim2Act 提出了一种鲁棒的仿真到决策框架，通过对抗校准机制重新加权决策关键状态-动作对的仿真误差，并采用组相对扰动策略在仿真不确定性下稳定策略学习，从而提升仿真鲁棒性和决策稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在平均仿真保真度或保守正则化方面存在局限，导致在高风险高回报动作上出现策略坍塌，无法有效应对由噪声或偏差数据引起的仿真预测误差。

Method: 引入对抗校准机制以调整决策关键区域的仿真误差权重，并设计组相对扰动策略来稳定策略学习过程，避免过度悲观约束。

Result: 在多个供应链基准测试中，该方法显著提升了仿真鲁棒性与在结构化和非结构化扰动下的决策性能稳定性。

Conclusion: Sim2Act 有效解决了仿真误差对决策稳定性的影响，实现了仿真与策略层面的双重鲁棒性，适用于高风险任务场景。
...
### [190] [Dynamic Multi-period Experts for Online Time Series Forecasting](https://arxiv.org/abs/2603.09062)
*Seungha Hong,Sukang Chae,Suyeon Kim,Sanghwan Jang,Hwanjo Yu*

Main category: cs.LG

TL;DR: DynaME proposes a novel framework for Online Time Series Forecasting by distinguishing between Recurring Drift (reappearance of past patterns) and Emergent Drift (new patterns), using specialized experts for the former and a general expert for the latter, achieving superior performance over baselines.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat concept drift as a monolithic issue, failing to account for its diverse nature; this limits adaptability in real-world time series forecasting where both recurring and emergent patterns occur.

Method: DynaME introduces a dynamic multi-period expert system: it uses a committee of specialized experts to capture recurring patterns and switches to a stable general expert when uncertainty is high, indicating emergent drift.

Result: Experiments on benchmark datasets show DynaME effectively handles both types of drift and significantly outperforms existing methods.

Conclusion: By explicitly modeling the dual nature of concept drift, DynaME provides a robust and adaptive solution for online time series forecasting.
...
### [191] [Learning Adaptive LLM Decoding](https://arxiv.org/abs/2603.09065)
*Chloe H. Su,Zhe Ye,Samuel Tenka,Aidan Yang,Soonho Kong,Udaya Ghai*

Main category: cs.LG

TL;DR: 本文提出一种自适应解码策略，通过强化学习训练轻量级解码适配器，在推理时动态选择采样方法，根据可用计算资源优化解码过程。在序列层面将解码建模为上下文相关赌博问题，基于提示嵌入和并行采样预算选择策略；在词元层面则建模为部分可观测马尔可夫决策过程（POMDP），依据模型内部特征和剩余词元预算选择采样动作。在MATH和CodeContests基准上的实验表明，所提方法显著提升了准确率与计算预算之间的权衡：在固定词元预算下，词元级适配器使Pass@1准确率最高提升10.2%，序列级适配器在固定并行采样条件下获得2-3%的增益。消融实验证明了两种层级适应的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型解码依赖固定的采样超参数（如温度、top-p），但任务难度和不确定性在不同提示及解码步骤间存在显著差异，导致性能受限。为提升解码效率与准确性，需实现动态、自适应的解码策略，以更好地利用有限计算资源。

Method: 提出两种层级的自适应解码框架：序列级将解码视为上下文相关赌博问题，基于提示嵌入与并行采样预算选择整体解码策略；词元级将其建模为部分可观测马尔可夫决策过程（POMDP），在每一步根据内部模型特征与剩余词元预算选择采样动作。采用强化学习训练轻量级解码适配器，使用可验证的终端奖励（如数学与编程任务的正确性）进行优化。

Result: 在MATH和CodeContests基准上，所提自适应解码策略显著改善了准确率-预算权衡：词元级适配器在固定词元预算下使Pass@1准确率最高提升10.2%；序列级适配器在固定并行采样条件下实现2-3%的准确率提升。消融实验验证了序列级与词元级适应对性能提升的贡献。

Conclusion: 通过引入基于强化学习的轻量级解码适配器，能够有效实现解码策略的动态调整，显著提升大语言模型在不同计算约束下的解码性能，证明了自适应解码在提高准确性与资源利用率方面的潜力。
...
### [192] [Exclusive Self Attention](https://arxiv.org/abs/2603.09078)
*Shuangfei Zhai*

Main category: cs.LG

TL;DR: 提出了一种名为独占自注意力（XSA）的新方法，通过限制注意力仅捕捉与当前标记值向量正交的信息（排除自身位置信息），从而提升Transformer在序列建模上的表现。在标准语言建模任务中，XSA在不同模型规模下均优于传统自注意力（SA），且随着序列长度增加，性能优势愈发明显。


<details>
  <summary>Details</summary>
Motivation: 传统自注意力机制在建模序列时可能受到自身位置信息的干扰，影响上下文建模效果。为改善这一问题，提出XSA以增强对上下文信息的捕捉能力。

Method: 通过设计一种约束机制，使注意力计算只关注与当前标记值向量正交的部分，从而排除自身位置的影响，实现更纯净的上下文建模。

Result: 在多种模型规模（最大达2.7B参数）和不同序列长度下，XSA均显著优于标准自注意力，在长序列任务中优势更为突出。

Conclusion: 独占自注意力（XSA）是一种有效且简单的改进方法，能显著提升Transformer在序列建模任务中的性能，尤其适用于长序列场景。
...
### [193] [PPO-Based Hybrid Optimization for RIS-Assisted Semantic Vehicular Edge Computing](https://arxiv.org/abs/2603.09082)
*Wei Feng,Jingbo Zhang,Qiong Wu,Pingyi Fan,Qiang Fan*

Main category: cs.LG

TL;DR: 本文提出了一种基于可重构智能表面（RIS）的语义感知车联网边缘计算框架，以应对动态环境和间歇性链路下的低延迟需求。通过联合优化卸载比例、语义符号数量和RIS相位偏移，采用两层混合方案（PPO用于离散决策，线性规划用于卸载优化），显著降低端到端延迟。仿真结果表明，相比遗传算法（GA）和量子行为粒子群优化（QPSO），平均端到端延迟降低约40%至50%，且在多达30辆车辆的高密度场景下仍保持低延迟性能，展现出良好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 为支持车联网中对时延敏感的应用，在动态环境和间歇性链路条件下，现有方法难以有效降低通信与计算延迟，亟需一种能同时优化无线连接与语义信息传输的新架构。

Method: 提出一种两层混合优化框架：使用近端策略优化（PPO）处理离散决策（如卸载比例与语义符号数），结合线性规划（LP）进行连续变量优化（如RIS相位调整），实现对系统参数的联合优化。

Result: 仿真验证了该框架的优越性：相比GA和QPSO，平均端到端延迟降低40%~50%；在最多30辆车的密集场景中仍能维持低延迟，表现出强可扩展性。

Conclusion: 所提出的RIS辅助语义感知车联网边缘计算框架能够有效应对动态与高负载环境下的低延迟挑战，具备实际部署潜力。
...
TL;DR: 本研究利用中英文新闻标题（路透社、道琼斯新闻线、中国新闻网）生成月度情感得分，结合传统表格数据（基本金属指数、汇率、通胀率、能源价格），评估轻量级微调大语言模型（LLM）在铝价预测中的表现。通过2007-2024年上海金属交易所的多空模拟，发现高波动时期，基于微调Qwen3模型的情感数据与LSTM结合的模型显著优于仅使用表格数据的基准模型（夏普比率1.04 vs 0.23）。研究还揭示了不同新闻来源、主题和事件类型在铝价预测中的差异化作用。


<details>
  <summary>Details</summary>
Motivation: 探索轻量级微调大语言模型在提取铝价预测信号方面的有效性，以及这些信号在何种市场条件下最具信息价值，填补现有研究空白。

Method: 从英文和中文新闻标题中提取月度情感分数，融合传统经济指标，构建基于LSTM的预测模型，并在2007-2024年上海金属交易所数据上进行长期多空策略回测，评估模型性能与经济效用。

Result: 在高波动市场环境下，融合微调Qwen3模型情感数据的LSTM模型表现出显著更强的预测能力，其夏普比率达1.04，远超仅使用传统数据的基准模型（夏普比率0.23）。研究进一步识别出不同新闻源、话题和事件类型对铝价预测的贡献差异。

Conclusion: 微调的大语言模型所提取的情感信号在高波动期对铝价预测具有重要价值，尤其当结合时间序列建模方法如LSTM时，能有效提升预测精度与投资回报。不同信息源和事件类型的影响具有异质性，应被纳入模型设计考量。
...
### [195] [Probabilistic Hysteresis Factor Prediction for Electric Vehicle Batteries with Graphite Anodes Containing Silicon](https://arxiv.org/abs/2603.09103)
...
TL;DR: 本研究提出一种数据驱动的概率滞环因子预测方法，针对硅-石墨负极电池的显著电压滞环问题，通过数据标准化框架处理不同工况下的异构驾驶循环，结合统计学习与深度学习模型，在考虑计算效率的同时实现滞环因子的不确定性量化预测。通过重训练、零样本预测、微调和联合训练等多种实验验证了模型在未见车型上的泛化能力，有效支持高能量密度电池的荷电状态（SoC）估计，推动先进电池技术的应用。


<details>
  <summary>Details</summary>
Motivation: 硅-石墨负极电池虽具更高能量密度和充电性能，但其显著的电压滞环使荷电状态（SoC）估计面临挑战；现有建模方法依赖高保真测试或仅适用于传统石墨电池，缺乏对不确定性的量化及计算效率考量，亟需高效且鲁棒的新型建模方法。

Method: 提出数据谐化框架以统一异构驾驶循环；采用统计学习与深度学习模型进行滞环因子预测；通过重训练、零样本预测、微调和联合训练评估模型泛化能力；综合考虑预测精度与计算效率。

Result: 所提方法在多种未见车辆模型上展现出良好泛化性能，成功实现对硅-石墨电池滞环因子的概率性预测，并量化了不确定性，显著提升SoC估计的可靠性，同时满足实际应用中的计算约束。

Conclusion: 该研究为硅-石墨基锂离子电池的荷电状态估计提供了高效、可推广的数据驱动解决方案，克服了传统方法在建模复杂性、不确定性处理和计算成本方面的局限，推动了高性能电池技术的实际应用。
...
### [196] [Decoupling Reasoning and Confidence: Resurrecting Calibration in Reinforcement Learning from Verifiable Rewards](https://arxiv.org/abs/2603.09117)
*Zhengzhao Ma,Xueru Wen,Boxi Cao,Yaojie Lu,Hongyu Lin,Jinglin Yang,Min He,Xianpei Han,Le Sun*

Main category: cs.LG

TL;DR: 本文提出DCPO框架，通过解耦推理与校准目标，有效缓解大语言模型在强化学习中因奖励可验证性导致的过度自信问题，同时保持高精度并显著提升校准性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在优化准确率和校准误差时存在根本性的梯度冲突，导致模型过度自信，亟需一种能系统解耦二者的目标框架。

Method: 基于理论分析发现梯度冲突，提出DCPO框架，将推理与校准目标分离优化，避免相互干扰。

Result: 实验表明DCPO在保持与GRPO相当的准确率的同时，显著改善校准性能，有效缓解过度自信问题。

Conclusion: 本研究为大语言模型的可靠部署提供了重要洞察与实用解决方案，强调解耦优化目标对提升模型可信度的关键作用。
...
### [197] [Wrong Code, Right Structure: Learning Netlist Representations from Imperfect LLM-Generated RTL](https://arxiv.org/abs/2603.09161)
*Siyang Cai,Cangyuan Li,Yinhe Han,Ying Wang*

Main category: cs.LG

TL;DR: 本文提出一种利用功能不完美的LLM生成RTL来增强电路网表表示学习的数据增强框架，通过合成数据中的结构模式实现对真实电路的高效泛化，有效突破了高质量标注数据稀缺的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 真实电路设计受知识产权保护且标注成本高，导致高质量标注数据稀缺；现有方法受限于小规模、干净标签电路，难以扩展到实际设计；尽管LLM可大规模生成RTL，但其功能性错误阻碍了应用。

Method: 基于观察：即使LLM生成的RTL功能不完全正确，其综合出的网表仍保留能指示意图功能的结构模式，提出一个从自动化代码生成到下游任务的端到端框架，利用噪声合成数据训练模型。

Result: 在不同规模的基准上评估了子电路边界识别和组件分类等任务，模型在真实网表上的表现与甚至超过使用少量高质量数据训练的方法，证明了该方法在真实场景下的强泛化能力。

Conclusion: 该方法通过系统性利用不完美但大规模的合成数据，显著缓解了电路表示学习中的数据瓶颈问题，为大规模电路分析提供了可行路径。
...
### [198] [GIAT: A Geologically-Informed Attention Transformer for Lithology Identification](https://arxiv.org/abs/2603.09165)
*Jie Li,Qishun Yang,Nuo Li*

Main category: cs.LG

TL;DR: 提出了一种名为地质信息注意力变压器（GIAT）的新框架，通过将地质先验知识融入Transformer的注意力机制，显著提升岩性识别的准确性与可解释性。该方法利用类别相关序列相关性（CSC）滤波器生成地质引导的关系矩阵，并注入自注意力计算中，使模型更符合地质规律。在两个挑战性数据集上，GIAT达到最高95.4%的准确率，且在输入扰动下表现出优异的解释一致性，为地球科学中的深度学习模型提供了新范式。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的岩性识别模型虽具备强大的序列建模能力，但其‘黑箱’特性及缺乏地质先验指导，限制了性能和可信度。需要一种能融合地质知识、提升模型可解释性和预测合理性的方法。

Method: 提出Geologically-Informed Attention Transformer（GIAT），核心是引入地质信息注意力偏置机制；利用类别相关序列相关性（CSC）滤波器生成地质引导的关系矩阵，并将其嵌入自注意力计算中，以显式引导模型关注地质上合理的模式。

Result: 在两个复杂数据集上，GIAT实现了高达95.4%的准确率，优于现有模型；同时在输入扰动下展现出更强的解释忠实性，预测结果具有良好的地质合理性。

Conclusion: GIAT成功将地质先验知识与数据驱动的Transformer模型深度融合，构建出更准确、可靠且可解释的深度学习模型，为地球科学领域的智能分析提供了一个新的范式。
...
### [199] [Better Bounds for the Distributed Experts Problem](https://arxiv.org/abs/2603.09168)
*David P. Woodruff,Samson Zhou*

Main category: cs.LG

TL;DR: 本文研究分布式专家问题，其中n个专家分布在s个服务器上，持续T个时间步。每个专家在每个时间步的损失是其在s个服务器上损失向量的ℓ_p范数。目标是最小化遗憾R（即协议损失与最优专家损失之差），并以最少通信量实现。提出一种新协议，实现遗憾约为$R\gtrsim\frac{1}{\sqrt{T}\cdot\text{poly}\log(nsT)}$，通信量为$\mathcal{O}\left(\frac{n}{R^2}+\frac{s}{R^2}\right)\cdot\max(s^{1-2/p},1)\cdot\text{poly}\log(nsT)$比特，优于以往工作。


<details>
  <summary>Details</summary>
Motivation: 在分布式设置中最小化遗憾的同时降低通信开销，尤其适用于大规模多服务器场景下的在线学习任务。

Method: 设计了一种新的分布式学习协议，结合ℓ_p范数损失建模和通信优化机制，通过控制信息交换频率和压缩传输数据来减少通信量，同时保持较低的遗憾水平。

Result: 所提协议在保证遗憾界$R\gtrsim\frac{1}{\sqrt{T}\cdot\text{poly}\log(nsT)}$的前提下，显著降低了通信复杂度，优于现有方法。

Conclusion: 该协议在分布式专家问题中实现了更优的遗憾-通信权衡，为实际应用中的高效在线学习提供了可行方案。
...
### [200] [Latent-DARM: Bridging Discrete Diffusion And Autoregressive Models For Reasoning](https://arxiv.org/abs/2603.09184)
*Lina Berrayana,Ahmed Heakl,Abdullah Sohail,Thomas Hofmann,Salman Khan,Wei Chen*

Main category: cs.LG

TL;DR: Latent-DARM 是一种基于潜在空间通信的框架，用于连接具有不同能力的多智能体系统中的规划器（DDLM）和执行器（ARM），克服了传统自回归语言模型（ARM）在全局推理和计划修订上的局限性，同时解决了离散扩散语言模型（DDLM）文本流畅性不足的问题。该框架在数学、科学和常识推理基准测试中显著提升性能，如在 DART-5 上准确率从 27.0% 提升至 36.0%，在 AIME2024 上从 0.0% 提升至 14.0%，且仅使用不到 2.2% 的标记预算，接近先进推理模型的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基于自回归语言模型（ARM）的多智能体系统受限于序列生成方式，难以实现全局推理与计划修订；而离散扩散语言模型（DDLM）虽具备强规划能力，但文本生成流畅性差，无法与 ARM 直接协作。因此亟需一种高效融合两者优势的协同机制。

Method: 提出 Latent-DARM 框架，通过潜在空间实现 DDLM（规划器）与 ARM（执行器）之间的信息传递，使规划过程可全局优化并支持迭代修订，同时保持执行阶段的文本流畅性。

Result: 在多个基准测试中，Latent-DARM 显著优于传统的文本接口方式，在 DART-5 和 AIME2024 上分别将准确率提升 9.0% 和 14.0%；其性能接近当前最先进的推理模型，同时仅消耗不到 2.2% 的 token 预算。

Conclusion: Latent-DARM 成功实现了异构模型间的高效多智能体协作，推动了跨模型协同生成的发展，为未来复杂任务中的分工合作提供了新范式。
...
### [201] [The Radio-Frequency Transformer for Signal Separation](https://arxiv.org/abs/2603.09201)
*Egor Lifar,Semyon Savkin,Rachana Madhukara,Tejas Jayashankar,Yury Polyanskiy,Gregory W. Wornell*

Main category: cs.LG

TL;DR: 本文研究信号分离问题，旨在从未知非高斯背景/干扰中估计感兴趣的信号（SOI）。基于包含SOI和干扰的训练数据，提出一种完全数据驱动的信号分离方法：首先学习一个针对SOI的良好离散分词器，然后在交叉熵损失下端到端训练Transformer。相较于传统的均方误差（MSE），使用交叉熵训练显著提升性能。所提出的分词器基于SoundStream改进，引入额外Transformer层，并将VQVAE替换为有限标量量化（FSQ）。在MIT RF Challenge数据集的真实与合成混合信号上，该方法表现优异，相比之前最先进方法，对QPSK信号从5G干扰中分离实现122倍的误码率（BER）降低。所学表示能自适应干扰类型，无需额外信息，在推理时展现出零样本泛化能力，表明其在射频以外领域也具潜力。尽管本方法应用于射频混合信号，但其架构同样适用于引力波数据（如LIGO应变）及其他需要数据驱动建模背景和噪声的科学传感问题。


<details>
  <summary>Details</summary>
Motivation: 传统信号分离方法在处理未知非高斯干扰时性能受限，尤其在缺乏先验知识的情况下。现有基于MSE的模型难以有效区分复杂干扰与目标信号。因此亟需一种无需依赖人工设计、可自适应干扰类型的全数据驱动信号分离方法，以提升在真实场景中的鲁棒性与泛化能力。

Method: 采用改进的SoundStream架构构建分词器，引入额外Transformer层并用有限标量量化（FSQ）替代VQVAE；在训练阶段通过交叉熵损失端到端训练Transformer模型，实现对目标信号与干扰的高效分离。

Result: 在MIT RF Challenge数据集上，该方法在真实与合成混合信号中表现优异，实现了比先前最先进方法122倍的误码率（BER）降低；所学表示具备零样本泛化能力，能适应未见过的干扰类型，且不依赖额外信息。

Conclusion: 本文提出的方法是一种高效的全数据驱动信号分离框架，通过结合改进的分词器与交叉熵训练的Transformer，实现了对复杂非高斯干扰的有效抑制。其强大的自适应与零样本泛化能力使其不仅适用于射频信号分离，还具有推广至引力波探测等科学传感任务的潜力。
...
### [202] [Strategically Robust Multi-Agent Reinforcement Learning with Linear Function Approximation](https://arxiv.org/abs/2603.09208)
*Jake Gonzales,Max Horwitz,Eric Mazumdar,Lillian J. Ratliff*

Main category: cs.LG

TL;DR: 提出RQRE-OVI算法，用于在大规模或连续状态空间中通过线性函数逼近计算风险敏感的量化响应均衡（RQRE），实现可证明高效且鲁棒的均衡计算。该方法通过有限样本后悔分析，揭示了理性度与风险敏感性之间的权衡：理性度提高降低后悔，风险敏感性引入正则化提升稳定性。RQRE具有唯一、平滑解，且策略映射关于收益估计是Lipschitz连续的，优于纳什均衡。实验表明，RQRE-OVI在自对弈中表现良好，在交叉对弈中显著更鲁棒。


<details>
  <summary>Details</summary>
Motivation: 现有纳什均衡在一般和博弈中计算困难且易受近似误差影响，存在多解性和脆弱性问题。为解决这些问题，需要一种具有唯一性、稳定性和鲁棒性的均衡概念。本文提出基于有界理性和风险敏感性的风险敏感量化响应均衡（RQRE），以提供更稳健的替代方案。

Method: 提出RQRE-OVI算法，结合乐观值迭代与线性函数逼近，用于在大或连续状态空间中计算RQRE。通过有限样本后悔分析，理论分析其收敛性，并刻画样本复杂度随理性参数和风险敏感参数的变化规律。

Result: RQRE-OVI实现了可证明的收敛性，揭示了理性与风险敏感性之间的定量权衡：理性提升性能，风险敏感增强稳定性。实验显示，相比基于纳什均衡的方法，RQRE-OVI在自对弈中表现相当，但在交叉对弈中表现出显著更高的鲁棒性。此外，RQRE策略映射具有Lipschitz连续性，且可解释为分布鲁棒优化问题。

Conclusion: RQRE-OVI提供了一条原则性强、可扩展且可调的均衡学习路径，兼具良好期望性能与鲁棒性，适用于多智能体强化学习中的实际应用。
...
### [203] [Efficient Reasoning at Fixed Test-Time Cost via Length-Aware Attention Priors and Gain-Aware Training](https://arxiv.org/abs/2603.09253)
*Rian Atri*

Main category: cs.LG

TL;DR: 本文研究在计算资源受限情况下的高效推理，提出两种仅在训练阶段使用的组件：长度感知注意力先验（RPA）和最小收益感知控制器（Guardian）。RPA通过模糊的制度位置对齐构建归一化前softmax偏置，以结构化正则化方式引导注意力，不增加推理参数；Guardian基于双时间尺度策略梯度视角，在验证性能提升时动态调节注意力锐度，推理阶段禁用。从KL角度看，softmax(z) + log π 可解释为带KL正则化的最大后验估计，使先验具有理论基础。在WikiText 2上，模型在保持基线延迟与内存的前提下，显著降低验证交叉熵。推理时仅需添加一个预计算的缓存偏置B，每头一次加法操作，开销极小，无明显延迟变化。结果表明，长度感知先验与后期收益控制可在长跨度、噪声较大的日志域中有效保留改进，同时维持测试时间成本不变。


<details>
  <summary>Details</summary>
Motivation: 在计算资源受限的场景下，如何在不增加推理成本的前提下实现结构化、正确的决策。现有方法往往在推理阶段引入额外开销，限制了实际应用。因此需要一种既能提升性能又不影响推理效率的方法。

Method: 引入两个仅训练阶段使用的模块：1）长度感知注意力先验（RPA），通过模糊制度位置对齐生成归一化前softmax偏置，作为结构化正则化引导注意力；2）最小收益感知控制器（Guardian），基于双时间尺度策略梯度优化思想，在验证性能改善时动态调节注意力锐度，推理时关闭。二者均不参与推理，且通过KL视角建立理论基础。

Result: 在严格计算约束条件下（WikiText 2），模型在保持与基线相同延迟和内存占用的情况下，显著降低验证交叉熵。推理阶段仅需添加一个预计算的缓存偏置，每头一次加法，开销可忽略，无显著延迟变化。尤其在长序列、高噪声日志场景中表现优异，有效保留性能提升。

Conclusion: 通过引入长度感知先验和后期收益控制机制，可在不增加推理开销的前提下实现更优的结构化决策能力，尤其适用于长序列、噪声敏感任务，为高效推理提供了可扩展且理论扎实的解决方案。
...
TL;DR: 本文提出了新的基于表示的归纳泛化界，利用最优传输理论，以Wasserstein距离表达编码特征分布之间的差异。该方法在无需分布假设的归纳设置下，有效计算且与实际泛化表现高度相关，揭示了GNN聚合过程对表示分布的影响，并解释了深度与泛化误差之间的非单调关系。


<details>
  <summary>Details</summary>
Motivation: 现有归纳泛化界依赖于计算上不可行且与实际行为不一致的经典复杂度度量，亟需更高效、更准确的泛化界。

Method: 通过最优传输理论建立全局和类别相关的泛化界，使用Wasserstein距离衡量编码特征分布间的差异，考虑训练时可访问测试特征的情况。

Result: 所提边界具有高效可计算性，与图节点分类中的实证泛化性能强相关，优于传统复杂度度量；分析揭示了GNN聚合对分布的变换作用，导致类内集中与类间分离之间的权衡，从而提供随深度变化的泛化特性描述。

Conclusion: 本工作提出的基于最优传输的泛化界在理论上更贴近实际，能有效刻画GNN深度与泛化性能之间的非单调关系，为模型设计提供了新视角。
...
TL;DR: Reward-Zero 是一种基于自然语言任务描述的通用隐式奖励机制，通过语言嵌入生成连续、语义对齐的进展信号，用于强化学习（RL）训练。它无需任务特定设计即可加速探索、稳定训练并提升泛化能力，在多个复杂任务中表现优于传统方法如PPO。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习依赖稀疏或延迟的环境反馈，而人工设计奖励函数成本高且难以泛化。本文旨在开发一种无需人工干预的通用奖励机制，利用语言嵌入自动构建语义层面的完成度信号，以提升样本效率与任务泛化能力。

Method: 通过对比任务描述的语言嵌入与智能体交互经验生成的嵌入，计算语义相似度，形成连续的进度奖励信号；该信号作为隐式奖励融入标准RL框架，不依赖任务特定工程。

Result: 实验表明，使用Reward-Zero的智能体收敛更快，最终成功率更高，且在某些复杂任务中解决了手写奖励无法解决的问题；同时构建了一个基于语言嵌入的微型基准来评估任务执行中的完成感。

Conclusion: 语言驱动的隐式奖励函数为实现更高效、可扩展、通用的具身智能体强化学习提供了可行路径，具有显著的应用前景。
...
### [206] [TA-GGAD: Testing-time Adaptive Graph Model for Generalist Graph Anomaly Detection](https://arxiv.org/abs/2603.09349)
*Xiong Zhang,Hong Peng,Changlong Fu,Xin Jin,Yun Yang,Cheng Xie*

Main category: cs.LG

TL;DR: 该研究针对跨域图异常检测中的领域偏移问题，提出了一种名为$\mathcal{AD}$（异常异配性）的新理论，并基于此构建了一个新型图基础模型，实现了在多个不同图数据上的跨域泛化能力，仅需一次训练即可在多种场景下有效检测异常，实验在14个真实世界图数据集上验证了其卓越性能，达到当前最优水平。


<details>
  <summary>Details</summary>
Motivation: 真实世界中存在大量异常节点（如虚假新闻、违规用户、恶意交易等），严重威胁图数据生态健康；现有跨域检测模型因领域偏移导致泛化能力差，亟需一种能有效应对多领域差异的通用异常检测方法。

Method: 识别并量化了跨域图异常检测中的特征不匹配模式——异常异配性（$\mathcal{AD}$），并据此设计了一种新型图基础模型，通过统一建模实现跨域泛化，仅需单次训练即可适应多种图结构与数据分布。

Result: 在14个真实世界图数据集上的实验表明，该模型在跨域异常检测任务中取得了突破性进展，达到了当前最先进的检测精度，显著优于已有方法。

Conclusion: 提出的$\mathcal{AD}$理论为通用图异常检测（GGAD）提供了新的理论视角和实践路径，推动了该领域的进一步发展。代码已开源。
...
### [207] [ActiveUltraFeedback: Efficient Preference Data Generation using Active Learning](https://arxiv.org/abs/2603.09692)
*Davit Melikidze,Marian Schneider,Jessica Lam,Martin Wertich,Ido Hakimi,Barna Pásztor,Andreas Krause*

Main category: cs.LG

TL;DR: ACTIVEULTRAFEEDBACK 是一个模块化的主动学习流水线，通过不确定性估计动态识别最具有信息量的响应进行标注，以降低RLHF中偏好数据收集的成本。该方法评估了标准响应选择策略，并引入两种新方法DRTS和DELTAUCB，专注于具有较大预测质量差距的响应对，从而提升微调信号的有效性。实验表明，该方法仅需静态基线1/6的标注数据即可实现相当或更优的下游性能。


<details>
  <summary>Details</summary>
Motivation: RLHF在对齐大语言模型方面虽已标准化，但其效果受限于获取偏好数据的高成本，尤其是在低资源和专家领域。因此，需要一种高效的方法来减少对人工标注数据的依赖。

Method: 提出ACTIVEULTRAFEEDBACK主动学习流水线，利用不确定性估计识别最有信息量的响应；结合标准响应选择方法与两种新方法（DRTS和DELTAUCB），优先选择预测质量差距大的响应对以增强微调信号。

Result: 使用ACTIVEULTRAFEEDBACK生成的高质量数据集显著提升了下游任务性能，仅用1/6的标注数据就达到了与静态基线相当或更好的效果。

Conclusion: ACTIVEULTRAFEEDBACK有效降低了偏好数据收集成本，同时保持甚至提升了模型性能，为低资源和专家领域中的高效对齐提供了可行方案。
...
### [208] [From Representation to Clusters: A Contrastive Learning Approach for Attributed Hypergraph Clustering](https://arxiv.org/abs/2603.09370)
*Li Ni,Shuaikang Zeng,Lin Mu,Longlong Lin*

Main category: cs.LG

TL;DR: CAHC is an end-to-end contrastive learning method for attributed hypergraph clustering that jointly learns node embeddings and cluster assignments, using both node-level and hyperedge-level contrastive objectives. It improves clustering performance by incorporating clustering-oriented guidance during embedding refinement.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning methods for attributed hypergraph clustering separate representation learning from clustering, lacking direct supervision and potentially including irrelevant information in the learned graph.

Method: CAHC employs a two-step framework: (1) representation learning via a novel contrastive objective combining node-level and hyperedge-level information; (2) joint optimization of embeddings and cluster assignments to refine embeddings with clustering-oriented guidance.

Result: CAHC outperforms baseline methods on eight datasets, demonstrating superior clustering performance.

Conclusion: CAHC achieves state-of-the-art results by integrating clustering supervision directly into the learning process, making it more effective for attributed hypergraph clustering.
...
### [209] [MSSR: Memory-Aware Adaptive Replay for Continual LLM Fine-Tuning](https://arxiv.org/abs/2603.09892)
*Yiyang Lu,Yu He,Jianlong Chen,Hongyuan Zha*

Main category: cs.LG

TL;DR: 本文提出了一种名为记忆启发的采样与调度重放（MSSR）的经验重放框架，旨在解决大型语言模型在持续微调过程中面临的灾难性遗忘问题。MSSR通过估计样本级别的记忆强度，并自适应地安排重放间隔，在保持快速适应能力的同时有效缓解遗忘。实验表明，MSSR在三种骨干模型和11个顺序任务上均优于现有先进基线，尤其在推理密集型和多项选择类基准测试中表现突出。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在动态环境中部署时需持续微调以适应新任务和数据分布变化，但其强大的适应能力也导致灾难性遗忘问题，即先前知识在连续训练中退化。现有基于重放的策略如固定交错重放、基于准确率监督和损失驱动调度等存在依赖启发式规则、仅部分缓解遗忘或计算开销大的局限性。因此，亟需一种更高效且自适应的解决方案来平衡快速学习与知识保留。

Method: 提出Memory-Inspired Sampler and Scheduler Replay (MSSR)框架，利用序列微调下的保留动态特性，估计每个样本的记忆强度，并据此自适应地调度重放时间点，实现对遗忘的有效抑制。该方法结合了样本级记忆评估与智能重放调度机制，无需额外大量计算资源。

Result: 在三个骨干模型和11个顺序任务上的广泛实验显示，MSSR始终优于当前最先进的重放基线方法；特别是在推理密集型任务和多项选择题任务中取得了显著提升，证明了其在复杂任务场景下的优越性能和鲁棒性。

Conclusion: MSSR是一种高效且自适应的经验重放框架，能够有效缓解大型语言模型在持续微调过程中的灾难性遗忘问题，同时保持快速适应能力。该方法为动态环境下的模型持续学习提供了有力支持，具有良好的泛化能力和实际应用前景。
...
### [210] [SPAARS: Safer RL Policy Alignment through Abstract Exploration and Refined Exploitation of Action Space](https://arxiv.org/abs/2603.09378)
*Swaminathan S K,Aritra Hazra*

Main category: cs.LG

TL;DR: SPAARS 是一种用于离线到在线强化学习的课程学习框架，通过在低维潜在空间中约束探索以实现安全、高效的策略改进，随后无缝过渡到原始动作空间，避免解码器瓶颈。其两种变体分别支持无序（s,a）对和需要轨迹片段的预训练。理论证明了性能差距的上界，展示了潜在空间策略梯度的方差降低优势，并通过行为克隆控制课程转换稳定性。实验表明，SPAARS-SUPE 在 kitchen-mixed-v0 上达到 0.825 的归一化回报（优于 SUPE 的 0.75），样本效率提升 5 倍；独立的 SPAARS 在 hopper 和 walker2d 任务上分别取得 92.7 和 102.9 的归一化回报，显著超过 IQL 基线。


<details>
  <summary>Details</summary>
Motivation: 解决离线到在线强化学习中的安全探索问题，现有方法因依赖条件变分自编码器（CVAE）导致解码器重建损失带来的性能天花板（即利用差距），限制了策略优化潜力。

Method: 提出 SPAARS 框架，采用课程学习机制：初期在低维潜在空间进行探索以保证安全性与样本效率，后期转移至原始动作空间以突破性能瓶颈；结合行为克隆控制课程转换稳定性；提供两种实现方式：基于 CVAE 且仅需无序 (s,a) 对的版本，以及结合 OPAL 技能预训练的 SPAARS-SUPE 版本。

Result: SPAARS-SUPE 在 kitchen-mixed-v0 上获得 0.825 的归一化回报，优于 SUPE 的 0.75，样本效率提高 5 倍；独立的 SPAARS 在 hopper-medium-v2 和 walker2d-medium-v2 上分别取得 92.7 和 102.9 的归一化回报，显著超越 IQL 基线（66.3 和 78.3）。

Conclusion: SPAARS 有效缓解了离线到在线强化学习中的探索安全性和性能天花板问题，通过潜在空间引导与课程转移机制，在无需复杂数据结构的前提下实现了更高的样本效率和更强的策略性能，验证了无序对 CVAE 实例化的有效性。
...
### [211] [Reconstructing Movement from Sparse Samples: Enhanced Spatio-Temporal Matching Strategies for Low-Frequency Data](https://arxiv.org/abs/2603.09412)
*Ali Yousefian,Arianna Burzacchi,Simone Vantini*

Main category: cs.LG

TL;DR: 本文针对时空匹配算法在密集环境和较高采样间隔下的计算效率与精度局限，提出四种改进：动态缓冲区、自适应观测概率、重设计的时间评分函数以及基于历史移动模式的行为分析。通过米兰真实数据集验证，新方法在无真实标签情况下显著提升了性能效率与路径质量。


<details>
  <summary>Details</summary>
Motivation: 现有时空匹配算法在高密度城市环境中，面对较高的采样间隔时，存在计算效率低和匹配精度不足的问题，亟需优化以提升实用性。

Method: 提出四种改进策略：动态缓冲区用于灵活调整匹配范围；自适应观测概率根据环境复杂度调整置信度；重设计的时空评分函数更合理地权衡时间与空间因素；行为分析模块引入历史轨迹模式以增强预测能力。

Result: 实验结果表明，改进后的算法在计算效率和路径匹配质量方面均显著优于原算法，尤其在复杂城市区域表现更佳，且在缺乏真实轨迹的情况下仍能有效评估。

Conclusion: 所提出的四项改进有效解决了原始时空匹配算法在复杂环境中的性能瓶颈，为高密度城市中GPS轨迹的精准匹配提供了高效可行的新方案。
...
### [212] [Impact of Markov Decision Process Design on Sim-to-Real Reinforcement Learning](https://arxiv.org/abs/2603.09427)
*Tatjana Krau,Jorge Mandlmaier,Tobias Damm,Frieder Heieck*

Main category: cs.LG

TL;DR: 该研究系统分析了强化学习（RL）在工业过程控制中从仿真到现实的迁移问题，重点关注马尔可夫决策过程（MDP）设计中的关键因素，如状态组成、目标引入、奖励设计、终止条件和环境动力学模型。通过颜色混合任务，在仿真与真实硬件上对比不同MDP配置的表现，结果表明基于物理的动力学模型在严格精度要求下可实现高达50%的真实成功率，而简化模型则完全失败。研究为工业场景下的RL部署提供了实用的MDP设计指导。


<details>
  <summary>Details</summary>
Motivation: 强化学习在工业过程控制中展现出巨大潜力，但其在仿真环境中训练的策略在实际硬件部署时常面临显著的‘仿真到现实’差距。如何通过合理设计MDP以提升策略的可迁移性，是当前亟需解决的关键问题。

Method: 采用颜色混合任务作为实验平台，系统地评估不同MDP设计要素（包括状态构成、目标设定、奖励函数、终止条件及环境动力学模型）在仿真与真实环境中的表现差异。通过对比多种配置在真实硬件上的执行效果，验证各设计选择对迁移性能的影响。

Result: 基于物理动力学模型的MDP配置在严格精度约束下实现了高达50%的真实世界成功概率，而使用简化模型的配置则完全失效。实验结果证明，精确建模环境动态对提升仿真实现向现实部署的迁移能力至关重要。

Conclusion: 在工业过程控制中应用强化学习时，应优先采用基于物理的环境动力学模型，并综合优化状态表示、奖励函数与终止条件等MDP设计要素，以有效缩小仿真与现实之间的差距，提升策略的实际可用性。
...
### [213] [From Weighting to Modeling: A Nonparametric Estimator for Off-Policy Evaluation](https://arxiv.org/abs/2603.09436)
*Rong J. B. Zhu*

Main category: cs.LG

TL;DR: 本文研究上下文带宽中的离策略评估问题，提出非参数加权（NW）方法以降低逆概率加权（IPW）的高方差问题，并结合奖励预测构建模型辅助非参数加权（MNW）方法，显著降低估计方差且保持低偏差。


<details>
  <summary>Details</summary>
Motivation: 传统逆概率加权（IPW）方法在处理历史数据与新策略动作分布不一致时存在高方差问题，尽管双重稳健（DR）方法能减少奖励建模带来的方差，但未直接解决IPW本身的高方差缺陷。因此需要一种既能保持低偏差又能显著降低方差的新方法。

Method: 提出非参数加权（NW）方法，利用非参数模型构建权重，从而在保持低偏差的同时降低方差；进一步引入奖励预测，形成模型辅助非参数加权（MNW），通过显式建模和缓解奖励建模偏差来提升估计精度。

Result: 大量实证比较表明，所提方法在价值估计中持续优于现有技术，具有更低的方差和稳定的低偏差表现。

Conclusion: 提出的非参数加权（NW）和模型辅助非参数加权（MNW）方法有效解决了传统离策略评估中的高方差问题，在保持低偏差的同时显著提升了估计精度，是上下文带宽中离策略评估的有效解决方案。
...
### [214] [Variational Routing: A Scalable Bayesian Framework for Calibrated Mixture-of-Experts Transformers](https://arxiv.org/abs/2603.09453)
*Albus Yizhuo Li,Matthew Wicker*

Main category: cs.LG

TL;DR: 本文提出了一种名为变分专家混合路由（VMoER）的结构化贝叶斯方法，用于在大规模基础模型的专家混合（MoE）层中建模不确定性。该方法将贝叶斯推断限制在专家选择阶段，避免了对整个模型进行高成本的贝叶斯计算。通过两种推理策略——对路由logits进行压缩变分推断和推断随机专家选择的温度参数——VMoER在多项基准测试中显著提升了路由稳定性（提升38%）、降低校准误差（减少94%）并提高分布外检测的AUROC（提升12%），同时额外计算开销低于1% FLOPs。结果表明，VMoER为构建鲁棒且具备不确定性感知能力的大规模基础模型提供了可扩展的路径。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型在需要量化输出不确定性的关键场景中部署日益广泛，而传统贝叶斯方法因计算开销过大难以应用于大规模模型。尽管现代模型通过稀疏结构（如MoE）实现万亿级参数量，但缺乏有效的不确定性建模手段。因此，亟需一种既能保持计算效率又能提供可靠不确定性估计的方法。

Method: 提出变分专家混合路由（VMoER），将贝叶斯推断聚焦于专家选择阶段，采用两种策略：1）对路由logits进行压缩变分推断；2）通过推断温度参数实现随机专家选择。该设计避免了全模型贝叶斯推断，从而保持高效性。

Result: 在多个基础模型上，VMoER使路由在噪声下的稳定性提升38%，校准误差降低94%，分布外检测的AUROC提升12%，且额外计算开销小于1% FLOPs。

Conclusion: VMoER是一种高效的、可扩展的不确定性建模方法，能够有效支持大规模基础模型在实际应用中的可靠性与安全性，为构建鲁棒、可信的智能系统提供了重要技术路径。
...
### [215] [Temporal-Conditioned Normalizing Flows for Multivariate Time Series Anomaly Detection](https://arxiv.org/abs/2603.09490)
*David Baumgartner,Helge Langseth,Kenth Engø-Monsen,Heri Ramampiaro*

Main category: cs.LG

TL;DR: 本文提出了一种时间条件归一化流（tcNF）框架，通过将归一化流基于先前观测进行条件化，有效捕捉复杂的时间动态，并生成对正常行为的准确概率分布，从而实现鲁棒的异常检测。在多个数据集上的实验表明，该方法在准确性与鲁棒性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法在建模时间序列中的复杂依赖关系和不确定性方面存在不足，难以准确识别低概率事件。因此需要一种能够精确刻画时间动态并提供可靠概率估计的新方法。

Method: tcNF采用自回归方式，将归一化流的参数依赖于历史观测值，从而实现对时间序列中复杂动态的建模。通过学习预期行为的概率分布，可识别出概率极低的异常事件。

Result: tcNF在多种时间序列数据集上表现出优异的异常检测性能，相比现有方法具有更高的准确性和鲁棒性。同时，作者提供了全面的优缺点分析和开源代码，以促进复现与后续研究。

Conclusion: tcNF是一种有效的异常检测框架，能够准确建模时间序列中的依赖关系和不确定性，为复杂时间序列数据的异常检测提供了新的解决方案。
...
### [216] [Efficiently Aligning Draft Models via Parameter- and Data-Efficient Adaptation](https://arxiv.org/abs/2603.09527)
*Luxi Lin,Zhihang Lin,Zhanpeng Zeng,Yuhao Chen,Qingyu Zhang,Jixiang Luo,Xuelong Li,Rongrong Ji*

Main category: cs.LG

TL;DR: 提出一种名为EDA的高效草案适应框架，通过解耦架构、数据再生策略和样本选择机制，实现对微调后目标模型的轻量级草案模型适配，显著提升推测解码性能并降低训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码在目标模型微调后性能下降，而重新训练草案模型成本高且低效，亟需一种参数与数据高效的适配方法。

Method: 采用解耦架构分离共享与特定输出分布，仅更新轻量级私有组件；利用微调后的目标模型再生训练数据以增强对齐；通过样本选择机制优先处理高价值数据。

Result: 实验表明EDA能有效恢复微调模型上的推测性能，获得更高的平均接受长度，同时大幅降低训练成本，优于全量重训。

Conclusion: EDA为微调模型下的推测解码提供了一种高效、低成本的适配方案，具有良好的实用性和可扩展性。
...
### [217] [Learning Bayesian and Markov Networks with an Unreliable Oracle](https://arxiv.org/abs/2603.09563)
*Juha Harviainen,Pekka Parviainen,Vidya Sagar Sharma*

Main category: cs.LG

TL;DR: This paper studies structure learning in Markov and Bayesian networks under unreliable conditional independence oracles. It shows that Markov networks can tolerate some errors if there are few vertex-wise disjoint paths, but Bayesian networks cannot tolerate any errors for guaranteed identification, even with bounded structural complexity. Algorithms are provided for identifiable cases.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of structure learning in Markov networks and Bayesian networks when the conditional independence oracle used is unreliable, making a bounded number of errors.

Method: The study analyzes the impact of errors in conditional independence queries on structure identifiability. For Markov networks, it leverages the number of vertex-wise disjoint paths to establish identifiability despite errors. For Bayesian networks, it proves that even with bounded graph parameters like treewidth, no errors can be tolerated for guaranteed structure identification. Algorithms are developed for cases where the structure is uniquely identifiable.

Result: For Markov networks, structure remains uniquely identifiable if the maximum number of vertex-wise disjoint paths is low, even with moderately exponential errors. For Bayesian networks, no errors can be tolerated for always identifying the correct structure, even under bounded treewidth. Efficient algorithms are provided for structure learning in identifiable cases.

Conclusion: The results highlight fundamental differences between Markov and Bayesian networks in robustness to oracle errors: Markov networks are more resilient, while Bayesian networks require error-free queries for reliable structure learning.
...
### [218] [An Optimal Control Approach To Transformer Training](https://arxiv.org/abs/2603.09571)
*Kağan Akman,Naci Saldı,Serdar Yüksel*

Main category: cs.LG

TL;DR: 本文提出一种严格的最优控制理论方法来训练Transformer，考虑了执行过程中的输入独立性、问题的集合控制特性以及位置依赖性。通过将Transformer建模为具有共享动作的离散时间受控粒子系统，展现出无噪声的McKean-Vlasov动力学。尽管该动力学非马尔可夫，但通过将其提升到概率测度空间，可转化为完全可观测的马尔可夫决策过程（MDP）。位置编码被纳入状态空间以保持序列顺序。利用动态规划原理，在温和紧致性假设下证明了全局最优策略的存在性。进一步证明了提升后的闭环策略等价于初始分布相关的开环策略，具备输入独立性并兼容标准Transformer训练。为此，提出了三重量化训练程序，对状态空间、概率测度空间和动作空间进行量化，并证明任何优化策略在量化模型上的表现接近原始训练问题的最优解。最后，建立了提升模型的稳定性和经验一致性，显示价值函数对初始经验测度扰动连续，且随着数据量增加策略收敛。该方法提供了一种无需光滑性或凸性假设的全局最优且鲁棒的替代训练方式，区别于传统的梯度方法。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer训练依赖于梯度下降方法，存在局部最优、不稳定性及对光滑性和凸性的依赖等问题。为了克服这些局限，本文旨在构建一个基于最优控制理论的严格框架，确保训练过程满足关键结构约束（如输入独立性、集合控制特性和位置依赖性），从而实现全局最优且稳健的训练。

Method: 将Transformer建模为离散时间受控粒子系统，采用无噪声的McKean-Vlasov动力学；通过将系统提升至概率测度空间，构造出完全可观测的马尔可夫决策过程（MDP）；引入位置编码以保持序列顺序；应用动态规划原理分析策略存在性；提出三重量化训练方案，分别对状态、概率测度和动作空间进行离散化；并通过连续性与收敛性分析验证模型的稳定性和一致性。

Result: 成功构建了一个全局最优的Transformer训练框架，其策略具有输入独立性，兼容标准训练流程；三重量化模型的最优策略近似原问题最优解；价值函数对初始分布扰动连续，策略随数据量增大而收敛，表明模型具备良好稳定性与经验一致性；整个方法不依赖光滑性或凸性假设，为非梯度训练提供了理论基础。

Conclusion: 本文提出的最优控制理论框架为Transformer训练提供了一种全新的全局优化视角，不仅满足关键结构约束，而且在理论上保证了最优性、稳定性和一致性，是传统梯度训练方法的有效替代，具有重要的理论意义与应用潜力。
...
### [219] [Towards Understanding Adam Convergence on Highly Degenerate Polynomials](https://arxiv.org/abs/2603.09581)
*Zhiwei Bai,Jiajie Zhao,Zhangchen Zhou,Zhi-Qin John Xu,Yaoyu Zhang*

Main category: cs.LG

TL;DR: 该研究揭示了Adam优化器在特定退化多项式函数上的自动收敛特性，证明其在这些函数上可实现局部线性收敛，显著优于梯度下降和动量法的次线性收敛。其加速源于二阶矩 $v_t$ 与平方梯度 $g_t^2$ 的解耦机制，导致有效学习率指数级放大。研究还构建了Adam的超参数相图，识别出三种行为模式：稳定收敛、波动尖峰和类似SignGD的振荡。


<details>
  <summary>Details</summary>
Motivation: 现有研究多依赖外部调度器或设置 $β_2$ 接近1才能保证Adam收敛，但缺乏对其实现内在自动收敛的理论理解。本文旨在探索Adam在何种目标函数类中具备自然收敛能力，以揭示其真正的优势来源。

Method: 通过分析退化多项式函数的动态行为，推导局部渐近稳定性条件；结合理论建模与实验验证，分析Adam中 $v_t$ 与 $g_t^2$ 的解耦机制；利用相图方法系统刻画不同超参数组合下的优化行为。

Result: 在特定退化多项式上，Adam无需额外调度即可实现自动收敛；理论预测与实验结果高度一致；相较于梯度下降和动量法，其收敛速度达到局部线性；发现三种超参数行为区域：稳定收敛、尖峰和振荡。

Conclusion: Adam在特定退化多项式上具有天然的自动收敛能力，其性能优势源于二阶矩与梯度平方之间的解耦机制，该机制实现了有效学习率的指数放大。研究为理解Adam的内在动力学提供了理论支持，并指明了其超参数选择的关键区域。
...
### [220] [Nonparametric Variational Differential Privacy via Embedding Parameter Clipping](https://arxiv.org/abs/2603.09583)
*Dina El Zein,Shashi Kumar,James Henderson*

Main category: cs.LG

TL;DR: 本文提出了一种基于瑞尼散度上界最小化的参数裁剪策略，以解决非参数变分信息瓶颈（NVIB）中潜在表示因信息含量过高而导致隐私保障不足和训练数值不稳定的問題。该方法在理论上导出了对后验均值、方差和混合权重参数的约束，应用于基于NVIB的模型后，实验表明其在保持更高下游任务性能的同时，显著提升了隐私保护能力。


<details>
  <summary>Details</summary>
Motivation: 现有非参数变分微分隐私（NVDP）框架中，潜在表示可能漂移到高信息区域，导致隐私保障弱化和训练数值不稳定，亟需一种可理论支撑的机制来改善隐私-效用权衡。

Method: 从最小化瑞尼散度上界的优化目标出发，推导出对后验分布参数（均值、方差、混合权重）的理论约束，并实施参数裁剪策略。

Result: 所提方法在保持或提升下游任务性能的同时，实现了更紧的瑞尼散度上界，表明更强的隐私保护能力，验证了其在提高隐私-效用平衡方面的有效性。

Conclusion: 本文提出的参数裁剪策略是一种简单但有效的改进方法，能够显著增强变分模型的隐私性与鲁棒性，推动其在实际应用中的可行性。
...
### [221] [MM-algorithms for traditional and convex NMF with Tweedie and Negative Binomial cost functions and empirical evaluation](https://arxiv.org/abs/2603.09601)
*Elisabeth Sommer James,Asger Hobolth,Marta Pelizzola*

Main category: cs.LG

TL;DR: 本文提出了一种统一框架，用于传统和凸非负矩阵分解（NMF），在包括负二项式和Tweedie模型在内的广泛分布假设下，通过极大化-最小化方法推导出所有模型的乘法更新规则，并为凸NMF提供了新的更新规则。实证评估表明，噪声模型的选择对模型拟合和特征恢复至关重要，且凸NMF在类别数量较大时可作为传统NMF的高效稳健替代方案。代码已开源于R包nmfgenr。


<details>
  <summary>Details</summary>
Motivation: 标准NMF通常基于高斯或泊松噪声假设，但这些假设在处理具有过度离散或其他复杂均值-方差关系的数据时可能不充分，因此需要更灵活的分布假设来提升模型适应性。

Method: 采用极大化-最小化方法，在广义分布假设（如负二项式、Tweedie）下推导出乘法更新规则，并针对凸NMF设计了新的更新算法，同时构建了统一实现框架。

Result: 实验结果表明，噪声模型的选择显著影响模型拟合效果与特征提取性能；凸NMF在大规模分类场景中表现出更高的效率与鲁棒性，优于传统NMF。

Conclusion: 所提出的统一框架有效扩展了NMF的应用范围，支持多种分布假设下的建模，且凸NMF是一种值得推广的高效稳健替代方法。
...
### [222] [FreqCycle: A Multi-Scale Time-Frequency Analysis Method for Time Series Forecasting](https://arxiv.org/abs/2603.09661)
*Boya Zhang,Shuaijie Yin,Huiwen Zhu,Xing He*

Main category: cs.LG

TL;DR: FreqCycle提出一种新框架，通过FECF模块提取低频特征，通过SFPL模块增强中高频能量，有效提升时序预测性能；进一步扩展为MFreqCycle以处理多周期耦合与长回溯窗口问题，在多个基准上实现最优性能与高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注低频模式建模，忽视中高频特征，限制了深度学习模型的性能提升。

Method: 提出FreqCycle框架，包含Filter-Enhanced Cycle Forecasting (FECF)模块用于学习共享周期模式，以及Segmented Frequency-domain Pattern Learning (SFPL)模块通过可学习滤波器和自适应加权增强中高频能量；进一步构建层次化MFreqCycle以解耦嵌套周期特征。

Result: 在七个不同领域的基准测试中，FreqCycle达到最先进的准确率，并保持更快的推理速度，实现了性能与效率的平衡。

Conclusion: FreqCycle及其扩展版本MFreqCycle能够有效捕捉时序数据中的多周期特征，显著提升预测精度并保证高效推理，为时序预测提供了新的解决方案。
...
### [223] [No evaluation without fair representation : Impact of label and selection bias on the evaluation, performance and mitigation of classification models](https://arxiv.org/abs/2603.09662)
*Magali Legast,Toon Calders,François Fouss*

Main category: cs.LG

TL;DR: 本文通过引入一个可控的偏置框架，系统地研究了标签偏置和多种选择偏置子类型对分类模型性能及偏置缓解方法有效性的影响。研究发现，不同偏置类型对模型表现的影响各异，且在无额外偏置的测试集上，公平性与准确性、个体公平与群体公平之间并不存在权衡关系。此外，缓解方法的效果受数据中具体偏置类型的影响。研究呼吁未来工作应改进模型与公平干预措施的评估方式，并深入探索更多类型的偏置及其组合效应。


<details>
  <summary>Details</summary>
Motivation: 现有研究对不同类型偏置（如标签偏置和选择偏置）的独立影响关注不足，导致对模型公平性评估不充分，缺乏对偏置缓解方法真实效能的准确判断。

Method: 提出一个偏置引入与评估框架，通过在低歧视的真实数据集上控制引入各类偏置，实现对不同偏置类型影响的独立分析，并在无偏测试集上评估模型与缓解方法的表现。

Result: 不同偏置类型对模型性能的影响存在差异；在无偏测试集上，公平性与准确性、个体公平与群体公平之间无明显权衡；偏置缓解方法的有效性取决于数据中具体的偏置类型。

Conclusion: 当前评估体系需改进，未来应更深入理解各种偏置类型、其组合效应以及数据特征等因素对缓解方法效率的影响。
...
### [224] [On Catastrophic Forgetting in Low-Rank Decomposition-Based Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2603.09684)
*Muhammad Ahmad,Jingjing Zheng,Yankai Cao*

Main category: cs.LG

TL;DR: 本文通过实证研究揭示了参数高效微调（PEFT）在序列学习中灾难性遗忘的机制，发现更新子空间的几何结构与参数化方式对遗忘有显著影响。基于低秩分解的方法（如LoRA）在共享小矩阵子空间时易受任务干扰，而基于张量的分解（如LoRETTA）和结构对齐的参数化（如WeGeFT）能有效缓解遗忘，因其在极小预算下捕捉了更丰富的结构信息并保留了预训练表示。


<details>
  <summary>Details</summary>
Motivation: 理解参数高效微调（PEFT）在序列学习中的行为，尤其是其在灾难性遗忘方面的表现，以指导高效适应策略的选择。

Method: 通过实证分析不同PEFT方法在更新子空间设计上的表现，比较低秩分解、张量分解及结构对齐参数化的效果。

Result: 发现更新子空间的几何结构和参数化方式显著影响遗忘程度；张量基方法和结构对齐方法在保持性能的同时显著减少遗忘。

Conclusion: 更新子空间的设计是持续学习中的关键因素，应优先考虑结构丰富且与预训练表示一致的参数化策略。
...
### [225] [Physics-informed neural operator for predictive parametric phase-field modelling](https://arxiv.org/abs/2603.09693)
*Nanxi Chen,Airong Chen,Rujin Ma*

Main category: cs.LG

TL;DR: 本文提出了一种物理信息神经算子框架PF-PINO，用于高效学习参数化相场偏微分方程。通过在数据保真度损失函数中嵌入相场控制方程的残差，该方法在训练过程中有效施加了物理约束。在电化学腐蚀、枝晶凝固和旋节分解等基准问题上验证表明，PF-PINO在精度、泛化能力和长期稳定性方面显著优于传统FNO，为相场建模提供了一个高效可靠的计算工具，并展示了物理信息神经算子在复杂界面演化问题中的潜力。


<details>
  <summary>Details</summary>
Motivation: 相场建模在预测材料微观结构和形貌演化时计算成本高，尤其在高通量参数研究中。虽然傅里叶神经算子（FNO）能加速求解参数化偏微分方程，但缺乏显式物理约束，限制了其在复杂相场动力学中的泛化性和长期准确性。因此，需要一种结合物理规律与机器学习优势的新方法。

Method: 提出PF-PINO框架，将相场控制方程的残差作为物理约束引入数据保真度损失函数中，在训练过程中强制满足物理规律，从而提升模型的准确性和鲁棒性。

Result: PF-PINO在多个基准相场问题上表现优异，相比传统FNO显著提升了精度、泛化能力及长期稳定性，证明其在复杂界面演化模拟中的有效性与实用性。

Conclusion: PF-PINO是一种高效且稳健的相场建模工具，成功融合了物理规律与神经算子的优势，为科学机器学习在复杂界面演化问题中的应用提供了新范式。
...
### [226] [A Multi-Prototype-Guided Federated Knowledge Distillation Approach in AI-RAN Enabled Multi-Access Edge Computing System](https://arxiv.org/abs/2603.09727)
*Luyao Zou,Hayoung Oh,Chu Myaet Thwal,Apurba Adhikary,Seohyeon Hong,Zhu Han*

Main category: cs.LG

TL;DR: 提出了一种多原型引导的联邦知识蒸馏（MP-FedKD）方法，用于解决AI-RAN赋能的MEC系统中非独立同分布（non-IID）数据带来的挑战。通过引入自知识蒸馏、条件分层聚类（CHAC）和原型对齐机制，有效缓解了传统单原型方法的信息损失问题，并设计了新的LEMGP损失函数以增强全局与局部嵌入之间的关系建模。实验表明，该方法在多种非IID设置下均优于现有先进方法。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习在处理非独立同分布（non-IID）数据时存在性能下降问题，尤其是基于单原型的平均策略容易造成信息丢失，影响模型泛化能力。因此亟需一种更有效的机制来应对数据异构性挑战，提升边缘计算环境下的智能模型训练效果。

Method: 提出MP-FedKD方法，结合自知识蒸馏、条件分层聚类（CHAC）构建多原型，设计原型对齐方案，并引入新型LEMGP损失函数以优化本地客户端的训练过程，从而更好地适应非IID数据分布。

Result: 在多个数据集及不同非IID设置下的大量实验结果显示，所提方法在准确率、平均准确率以及均方根误差（RMSE）和平均绝对误差（MAE）方面均显著优于现有基准方法。

Conclusion: MP-FedKD通过多原型策略和精细化的损失设计，有效缓解了非IID数据带来的挑战，在边缘智能场景中展现出优越的性能，为AI-RAN与MEC融合系统的高效协同学习提供了新思路。
...
TL;DR: 提出一种自适应通道剪枝辅助的分割学习（ACP-SL）方案，通过标签感知的通道重要性评分模块生成通道重要性得分，并利用自适应通道剪枝模块剪掉不重要的通道，从而压缩中间特征表示（即‘粉碎数据’），降低通信开销。实验表明，ACP-SL在测试准确率上优于基准方法，并在更少训练轮次内达到目标准确率，显著减少通信负担。


<details>
  <summary>Details</summary>
Motivation: 分割学习虽能减轻客户端计算负担，但中间特征表示的传输导致高通信开销，尤其在大量客户端参与时。因此亟需有效降低通信成本的方法。

Method: 设计标签感知通道重要性评分（LCIS）模块以评估通道重要性，并引入自适应通道剪枝（ACP）模块动态剪除低重要性通道，实现对粉碎数据的压缩。

Result: ACP-SL在测试准确率上持续优于基准方案，并在更少训练轮次内达到目标准确率，显著降低通信开销。

Conclusion: ACP-SL通过智能通道剪枝有效压缩了分割学习中的中间数据，实现了更高的通信效率和更快的收敛速度，是一种高效且实用的优化方案。
...
TL;DR: 本文提出了一种名为$\alpha$-GaBO的新颖贝叶斯优化算法家族，专用于概率单纯形上的优化。该方法基于信息几何学，构建了反映单纯形几何特性的Matérn核函数，并设计了一类一参数的几何优化器来求解获取函数。在基准函数和多个真实世界应用（如组件混合、分类器混合及机器人控制任务）中，该方法表现出优于传统受限欧几里得方法的性能。


<details>
  <summary>Details</summary>
Motivation: 许多实际优化问题涉及概率或混合物，其变量自然位于概率单纯形上，这是一个非欧几里得且受约束的域。传统的贝叶斯优化方法在欧几里得空间中进行，难以有效捕捉单纯形的几何结构，导致优化效率低下。因此，亟需一种能够考虑单纯形几何特性的优化方法。

Method: 基于信息几何学，为概率单纯形引入黎曼度量和一类联络，构造出适应单纯形几何的Matérn核函数，并设计了一族一参数的几何优化器用于获取函数优化。该方法充分利用单纯形的非欧特性，实现更精准的建模与搜索。

Result: 在多个基准函数和真实应用场景中，$\alpha$-GaBO显著优于现有的受限欧几里得贝叶斯优化方法，展现出更高的优化效率与收敛速度，尤其在高维概率单纯形上表现突出。

Conclusion: $\alpha$-GaBO通过融合信息几何与贝叶斯优化，为概率单纯形上的黑箱优化提供了一种高效、精确的新范式，具有广泛的应用潜力。
...
TL;DR: 本文提出In-Context RLVR，通过利用策略模型的上下文学习能力衡量解题过程的质量（即演示效用），引入证据增益（Evidence Gain）作为质量信号，在不依赖外部评估器或高成本计算的情况下，隐式地对奖励进行重加权，从而提升大语言模型的推理质量。


<details>
  <summary>Details</summary>
Motivation: 标准RLVR将所有正确答案视为等价，可能强化偶然得到正确结果但推理质量差的路径。高质量推理应作为更有效的教学示范，因此需要一种机制来区分不同解题过程的质量。

Method: 利用策略模型自身的上下文学习能力，通过贝叶斯分析计算证据增益（Evidence Gain），作为解题质量的度量，并基于此设计新的奖励重加权机制——In-Context RLVR。

Result: 在数学基准测试中，In-Context RLVR在准确率和推理质量上均优于标准RLVR，证明了该方法能有效识别并强化高质量推理路径。

Conclusion: 高质量推理具有更高的演示效用，通过内在的证据增益信号可实现对推理质量的自动评估与优化，显著提升模型的推理能力。
...
TL;DR: 提出GAST方法，通过联合数据和层的稀疏微调，在参数高效微调中同时优化数据与层的选择，提升模型性能并减少冗余信息。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只关注数据或层的单维度选择，忽视不同数据点对各层贡献差异，且可能丢弃低质量但有价值的信息，因此需要更全面的优化策略。

Method: GAST采用分层稀疏策略，自适应地为每层选择最具影响力的训练数据点，实现数据与层的联合选择，构成统一优化框架。

Result: 实验表明GAST在多个基准上均优于基线方法，展现出更强的性能和潜力。

Conclusion: GAST提供了一种更全面、高效的参数高效微调方案，为未来PEFT研究指明了新方向。
...
TL;DR: CarbonBench 是首个用于碳通量上推中零样本空间迁移学习的基准，包含全球567个通量塔站点超过130万条日观测数据（2000–2024年），提供分层评估协议、统一遥感与气象特征集以及多种基线模型，旨在推动跨生态系统和气候区的模型泛化能力评估，促进机器学习与地球系统科学融合。


<details>
  <summary>Details</summary>
Motivation: 当前碳通量模型难以在缺乏观测的生态系统中有效泛化，而现有方法缺乏标准化基准来评估其在不同植被类型和气候区间的零样本空间迁移性能，亟需一个可量化、可比较的评估框架。

Method: 构建 CarbonBench 基准，整合全球多源数据，设计分层评估协议以分离空间转移与时间自相关性，提供统一特征输入，并引入从树模型到领域泛化架构的多种基线方法。

Result: CarbonBench 成功建立了首个系统化的零样本空间迁移学习评估平台，支持跨区域、跨气候与植被类型的模型性能对比，为下一代气候建模提供了可靠测试工具。

Conclusion: CarbonBench 为碳通量预测中的迁移学习提供了标准化评估框架，推动了模型在未见生态系统的泛化能力研究，有助于提升全球碳循环模拟的准确性与可靠性。
...
TL;DR: 提出OptEMA及其两种新变体OptEMA-M和OptEMA-V，通过自适应递减的EMA系数实现闭环、无Lipschitz常数依赖的优化，可在标准假设下实现噪声自适应收敛率$\widetilde{\mathcal{O}}(T^{-1/2}+σ^{1/2} T^{-1/4})$，零噪声时达到近最优确定性速率$\widetilde{\mathcal{O}}(T^{-1/2})$，无需手动调参。


<details>
  <summary>Details</summary>
Motivation: 现有Adam类方法的理论分析存在局限：在零噪声下表现不佳、依赖严格有界条件、使用固定或开环步长、需已知Lipschitz常数。为克服这些瓶颈，本文旨在设计一种无需先验知识且能自适应噪声的新型优化器。

Method: 提出OptEMA框架，包含两个变体：OptEMA-M采用自适应递减的一阶矩EMA系数与固定二阶衰减；OptEMA-V则交换角色。该方法为闭环策略，有效步长依赖于优化轨迹，不依赖Lipschitz常数。

Result: 在平滑性、目标函数下界、无偏梯度及有界方差的标准假设下，两种变体均实现噪声自适应收敛率$\widetilde{\mathcal{O}}(T^{-1/2}+σ^{1/2} T^{-1/4})$；当噪声水平σ=0时，退化为近最优的$\widetilde{\mathcal{O}}(T^{-1/2})$，且无需人工调整超参数。

Conclusion: OptEMA通过引入闭环、自适应的EMA机制，显著提升了现有Adam类优化器的理论性能，实现了在噪声环境下的自适应收敛，并在零噪声情况下保持最优精度，无需额外调参，具有重要的理论与实践意义。
...
TL;DR: 本文揭示了基于漂移的生成建模中高斯核下漂移算子等价于平滑分布上的得分差，从而解答了原工作中未解的三个关键问题：分布相等性、核函数选择及停止梯度操作的必要性。研究将漂移方法置于得分匹配框架内，并通过线性化麦凯恩-弗拉索夫动力学与傅里叶空间分析，发现其收敛时间尺度具有类似等离子体物理中朗道阻尼的频率依赖特性，解释了为何拉普拉斯核优于高斯核。提出指数带宽退火策略 $\sigma(t)=\sigma_0 e^{-rt}$，将收敛时间从 $\exp(O(K_{\max}^2))$ 降至 $O(\log K_{\max})$。此外，通过将漂移形式化为平滑KL散度的Wasserstein梯度流，证明停止梯度源于JKO方案中的冻结场离散化，移除该操作将破坏梯度流性质。这一变分视角还提供了一般化构造新漂移算子的模板，如基于Sinkhorn散度的漂移算子。


<details>
  <summary>Details</summary>
Motivation: 尽管基于漂移的生成模型在单步图像生成中取得了最先进的性能，但其成功主要依赖经验，缺乏理论基础。特别是关于漂移算子是否能保证分布相等、如何选择核函数以及停止梯度操作的作用等问题仍未解决，亟需深入的理论解释以指导方法设计和改进。

Method: 通过分析高斯核下的漂移算子与平滑分布得分差之间的精确等价关系，建立理论框架；采用线性化麦凯恩-弗拉索夫动力学并结合傅里叶空间分析，揭示频率相关的收敛特性；引入指数带宽退火策略优化训练效率；基于变分原理，将漂移视为平滑KL散度的Wasserstein梯度流，利用JKO方案推导出停止梯度操作的数学来源。

Result: 1. 理论证明了在特定条件下，漂移为零可推出分布相等；2. 解释了拉普拉斯核优于高斯核的原因在于避免高频瓶颈；3. 提出的指数带宽退火策略显著加速收敛；4. 严格证明停止梯度操作是保持梯度流性质所必需的；5. 构造出基于Sinkhorn散度的新漂移算子，验证了方法的通用性。

Conclusion: 本工作系统地建立了基于漂移的生成建模的理论基础，将其置于得分匹配与变分梯度流的统一框架中，不仅解释了现有方法的成功机制，还提供了可扩展的理论工具与优化策略，为未来生成模型的设计提供了坚实的理论支撑。
...
TL;DR: 提出SignalMC-MED基准，用于评估生物信号基础模型在同步单导联心电图（ECG）和光电容积脉搏波（PPG）数据上的表现。该基准基于MC-MED数据集，包含22,256个就诊记录，每条数据为10分钟重叠的ECG与PPG信号，涵盖20项临床相关任务。系统评估显示，专用生物信号模型优于通用时间序列模型，多模态融合显著提升性能，完整10分钟信号优于短片段，大模型不必然优于小模型。手工特征提供强基线，并与学习表示互补。


<details>
  <summary>Details</summary>
Motivation: 现有生物信号基础模型在长时序多模态数据上的系统性评估不足，亟需标准化基准以推动模型评估与应用。

Method: 构建SignalMC-MED基准，包含大规模同步ECG与PPG数据，涵盖多种临床任务；对代表性时间序列与生物信号基础模型在不同输入设置下进行系统评估。

Result: 专用生物信号模型优于通用模型；多模态融合带来显著性能提升；完整10分钟信号优于短片段；大模型并非总是更优；手工特征与学习表征具有互补性。

Conclusion: SignalMC-MED成为评估生物信号基础模型的标准化基准，为模型选择与部署提供了实用指导。
...
### [235] [Towards a Neural Debugger for Python](https://arxiv.org/abs/2603.09951)
*Maximilian Beck,Jonas Gehring,Jannik Kossen,Gabriel Synnaeve*

Main category: cs.LG

TL;DR: 本文提出神经调试器（neural debuggers），通过微调大语言模型或从零开始预训练小型模型，使语言模型能够模拟传统调试器的交互操作，如单步进入、单步跳过、单步跳出及设置断点。该方法支持正向执行（预测未来状态和输出）与逆向执行（根据调试操作推断先前状态或输入），在CruxEval数据集上表现出色，证明了其对条件执行建模的强大能力。该研究为未来智能编程系统中以神经调试器作为世界模型，实现模拟调试环境中的执行反馈或与真实调试工具交互奠定了基础，推动代码生成、程序理解与自动化调试的发展。


<details>
  <summary>Details</summary>
Motivation: 现有神经解释器无法支持开发者常用的交互式调试行为，如在特定断点处暂停、逐步执行等，限制了其在实际编程场景中的应用。因此需要一种能模拟传统调试器行为的神经模型，以增强对代码执行过程的可控性与灵活性。

Method: 通过对大型语言模型进行微调，或从头预训练较小的语言模型，使其学习代码执行轨迹中的调试操作模式，从而实现对正向和逆向执行的条件建模；支持包括单步进入、单步跳过、单步跳出及断点设置在内的多种调试操作。

Result: 在CruxEval基准测试中，所提出的神经调试器在输出预测和输入推断任务上均表现出优异性能，验证了其对复杂调试行为下条件执行的有效建模能力。

Conclusion: 神经调试器是迈向智能编程系统的重要一步，可作为模拟调试环境中的世界模型，提供执行反馈或与真实调试工具交互，为更强大的代码生成、程序理解和自动化调试技术奠定基础。
...
### [236] [Task Aware Modulation Using Representation Learning for Upsaling of Terrestrial Carbon Fluxes](https://arxiv.org/abs/2603.09974)
*Aleksei Rozanov,Arvind Renganathan,Vipin Kumar*

Main category: cs.LG

TL;DR: TAM-RL框架通过结合时空表征学习与基于物理约束的编码器-解码器架构，显著提升了全球碳通量估算的准确性与泛化能力，相较于现有方法在150多个通量塔站点上降低了8-9.6%的均方根误差，并将解释方差（R²）从19.4%提升至43.8%。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的碳通量上推产品因地面观测稀疏且区域分布不均，难以泛化到未观测区域，导致系统性区域偏差和高预测不确定性。因此需要一种能融合物理规律与自适应表征学习的新方法以提升模型的鲁棒性和可迁移性。

Method: 提出Task-Aware Modulation with Representation Learning (TAM-RL)框架，结合时空表征学习、知识引导的编码器-解码器结构以及基于碳平衡方程设计的损失函数，实现对碳通量的精准建模与跨区域推广。

Result: 在超过150个代表不同生物群落和气候区的通量塔站点上，TAM-RL相比现有最先进数据集，RMSE降低8-9.6%，$R^2$从19.4%提升至43.8%，表明该方法在提升预测性能和减少区域偏差方面具有显著优势。

Conclusion: 将物理约束与自适应表征学习相结合，能够有效增强全球碳通量估计的稳健性与可迁移性，为准确估算全球碳预算提供了新范式。
